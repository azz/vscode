...
suite('Extfs', () => {
	...
	test('delSync - recursive folder structure', function(done: () => void) {
		...
		extfs.mkdirp(newDir, 493, error => {
			...
			fs.writeFileSync(
				path.join(newDir, 'somefolder', 'somefile.txt'),
				'Contents'
			);
	...
	test('copy, move and delete', function(done: () => void) {
		...
		extfs.copy(sourceDir, targetDir, error => {
			...
			extfs.mv(targetDir, targetDir2, error => {
				...
				assert.ok(
					fs.existsSync(path.join(targetDir2, 'examples', 'small.jxs'))
				);

				extfs.mv(
					path.join(targetDir2, 'index.html'),
					path.join(targetDir2, 'index_moved.html'),
					error => {
						if (error) {
							return onError(error, done);
						}
	...
	test('readdir', function(done: () => void) {
		if (
			strings.canNormalize &&
			typeof process.versions['electron'] !== 'undefined' /* needs electron */
		) {
			const id = uuid.generateUuid();
			const parentDir = path.join(os.tmpdir(), 'vsctests', id);
