...
function assertLocation(
	...
	expectedCompleteProperty: boolean
): void {
	var offset = input.indexOf('|');
	input =
		input.substring(0, offset) + input.substring(offset + 1, input.length);
	var actual = getLocation(input, offset);
	assert(actual);
	assert.deepEqual(actual.path, expectedSegments, input);
	assert.equal(
		actual.previousNode && actual.previousNode.type,
		expectedNodeType,
		input
	);
...
function assertMatchesLocation(
	input: string,
...
suite('JSON', () => {
	...
	test('location: matches', () => {
		...
		assertMatchesLocation('{ "dependencies": { "fo|": 1 } }', ['dependencies']);
		assertMatchesLocation('{ "dependencies": { "fo": | } }', [
			'dependencies',
			'*'
		]);
	});
