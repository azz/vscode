...
suite('IPC performance', () => {
		...
		const result = measure(service, 10, 10, 250) // warm-up
			.then(() => {
				return (function nextRun() {
					...
					const run = runs[i++];
					return measure(service, run.batches, 1, run.dataSize).then(() => {
						return nextRun();
					});
				})();
			});
	...
	function measure(
		...
		let hits = 0;
		let count = 0;
		return service.batchPerf(batches, size, dataSize).then(
			() => {
				console.log(
					`Batches: ${batches}, size: ${size}, dataSize: ${dataSize}, n: ${batches *
						size *
						dataSize}, duration: ${Date.now() - start}`
				);
