...
export interface ITestChannel extends IChannel {
	...
	call(
		command: 'batchPerf',
		args: { batches: number; size: number; dataSize: number }
	): PPromise<any, any[]>;
...
export class TestChannel implements ITestChannel {
	...
	call(command: string, ...args: any[]): TPromise<any> {
		switch (command) {
			...
			case 'event:marco':
				return eventToCall(this.testService.onMarco);
			case 'batchPerf':
				return this.testService.batchPerf(
					args[0].batches,
					args[0].size,
					args[0].dataSize
				);
			default:
				return TPromise.wrapError(new Error('command not found'));
		}
	}
}
