...
function isDirectory(
	path: string,
	callback: (error: Error, isDirectory?: boolean) => void
): void {
...
export function copy(
	...
	target: string,
	callback: (error: Error) => void,
	copiedSources?: { [path: string]: boolean }
): void {
	if (!copiedSources) {
	...
	fs.stat(source, (error, stat) => {
		...
		mkdirp(target, stat.mode & 511, err => {
			readdir(source, (err, files) => {
				loop(
					files,
					(file: string, clb: (error: Error, _result) => void) => {
						copy(
							paths.join(source, file),
							paths.join(target, file),
							(error: Error) => clb(error, undefined),
							copiedSources
						);
					},
					callback
...
function pipeFs(
	...
	readStream.on('end', () => {
		(<any>writeStream).end(() => {
			// In this case the write stream is known to have an end signature with callback
			if (!callbackHandled) {
				callbackHandled = true;
		...
		if (!exists) {
			...
			fs.lstat(path, (err, stat) => {
				if (err || !stat) {
					...
					!stat.isDirectory() ||
					stat.isSymbolicLink() /* !!! never recurse into links when deleting !!! */
				) {
					let mode = stat.mode;
					if (!(mode & 128)) {
						// 128 === 0200
						fs.chmod(path, mode | 128, (err: Error) => {
							// 128 === 0200
							if (err) {
								callback(err);
							} else {
...
let canFlush = true;
export function writeFileAndFlush(
	path: string,
	data: string | NodeBuffer,
