 ...
 */
export function readExactlyByStream(
 ...
 */
export function readExactlyByFile(
	file: string,
	totalBytes: number
): TPromise<ReadResult> {
	return new TPromise<ReadResult>((complete, error) => {
 ...
 */
export function readToMatchingString(
	file: string,
				...
				fs.read(
					...
					(err, moreBytesRead) => {
						...
						if (bytesRead >= maximumBytesToRead) {
							return end(
								new Error(
									`Could not find ${matchingString} in first ${maximumBytesToRead} bytes of ${file}`
								),
								null
							);
						}
