...
export type ParsedExpression = (
	path: string,
	basename?: string,
	siblingsFn?: () => string[] | TPromise<string[]>
) => string | TPromise<string> /* the matching pattern */;
...
interface ParsedStringPattern {
	(path: string, basename: string):
		| string
		| TPromise<string> /* the matching pattern */;
	basenames?: string[];
	patterns?: string[];
	allBasenames?: string[];
	allPaths?: string[];
...
interface ParsedExpressionPattern {
	(path: string, basename: string, siblingsPatternFn: () =>
		| SiblingsPattern
		| TPromise<SiblingsPattern>):
		| string
		| TPromise<string> /* the matching pattern */;
...
function trimForExclusions(pattern: string, options: IGlobOptions): string {
	return options.trimForExclusions && strings.endsWith(pattern, '/**')
...
function trivia2(base: string, originalPattern: string): ParsedStringPattern {
	...
	const parsedPattern: ParsedStringPattern = function(path, basename) {
		...
		return path === base ||
			strings.endsWith(path, slashBase) ||
			strings.endsWith(path, backslashBase)
			? originalPattern
			: null;
	};
...
function trivia3(pattern: string, options: IGlobOptions): ParsedStringPattern {
	...
	const withBasenames = arrays.first(
		parsedPatterns,
		pattern => !!(<ParsedStringPattern>pattern).allBasenames
	);
	if (withBasenames) {
...
function trivia4and5(
	...
	const nativePath = paths.nativeSep !== paths.sep
		? path.replace(ALL_FORWARD_SLASHES, paths.nativeSep)
		: path;
	const nativePathEnd = paths.nativeSep + nativePath;
	const parsedPattern: ParsedStringPattern = matchPathEnds
		? function(path, basename) {
				return path &&
					(path === nativePath || strings.endsWith(path, nativePathEnd))
					? pattern
					: null;
			}
		: function(path, basename) {
...
export function parse(
	...
	if (typeof arg1 === 'string') {
		...
		if (parsedPattern.allBasenames) {
			(<ParsedStringPattern>(<any>resultPattern)).allBasenames =
				parsedPattern.allBasenames;
		}
		if (parsedPattern.allPaths) {
			(<ParsedStringPattern>(<any>resultPattern)).allPaths =
	...
	const resultExpression: ParsedStringPattern = function(
		...
		function siblingsToSiblingsPattern(siblings: string[]) {
			if (siblings && siblings.length) {
				...
				const name = basename.substr(
					0,
					basename.length - paths.extname(path).length
				);
		...
		for (let i = 0, n = parsedPatterns.length; i < n; i++) {
			// Pattern matches path
			const result = (<ParsedExpressionPattern>parsedPatterns[i])(
				path,
				basename,
				siblingsPatternFn
			);
...
function parseExpressionPattern(
	...
	if (value) {
		...
		if (typeof when === 'string') {
			const siblingsPatternToMatchingPattern = (
				siblingsPattern: SiblingsPattern
			): string => {
			...
			const result: ParsedExpressionPattern = (
				path: string,
				basename: string,
				siblingsPatternFn: () => SiblingsPattern | TPromise<SiblingsPattern>
			) => {
...
function aggregateBasenameMatches(
	parsedPatterns: (ParsedStringPattern | ParsedExpressionPattern)[],
	result?: string
): (ParsedStringPattern | ParsedExpressionPattern)[] {
	...
	const basenames = basenamePatterns.reduce<string[]>(
		(all, current) => all.concat((<ParsedStringPattern>current).basenames),
		[]
	);
	let patterns: string[];
	if (result) {
		...
		patterns = basenamePatterns.reduce(
			(all, current) => all.concat((<ParsedStringPattern>current).patterns),
			[]
		);
	}
