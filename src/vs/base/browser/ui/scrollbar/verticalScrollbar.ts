...
export class VerticalScrollbar extends AbstractScrollbar {
	constructor(
		...
		if (options.verticalHasArrows) {
			...
			this._createArrow({
				...
				onActivate: () =>
					this._host.onMouseWheel(new StandardMouseWheelEvent(null, 0, 1))
			});
	...
	public getVerticalSliderVerticalCenter(): number {
		return (
			...
			this._scrollbarState.getSliderCenter()
		);
	}
	...
	protected _updateSlider(sliderSize: number, sliderPosition: number): void {
		...
		if (this._canUseTranslate3d) {
			this.slider.setTransform(
				'translate3d(0px, ' + sliderPosition + 'px, 0px)'
			);
	...
	protected _mouseDownRelativePosition(
		e: ISimplifiedMouseEvent,
		domNodePosition: IDomNodePagePosition
	): number {
