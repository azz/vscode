...
export const editorWordHighlight = registerColor(
	...
	{ dark: '#575757B8', light: '#57575740', hc: null },
	nls.localize(
		'wordHighlight',
		'Background color of a symbol during read-access, like reading a variable.'
	)
);
export const editorWordHighlightStrong = registerColor(
	'editor.wordHighlightStrongBackground',
	{ dark: '#004972B8', light: '#0e639c40', hc: null },
	nls.localize(
		'wordHighlightStrong',
...
class WordHighlighter {
	...
	constructor(editor: editorCommon.ICommonCodeEditor) {
		...
		this.toUnhook.push(
			editor.onDidChangeConfiguration(e => {
				let newValue = this.editor.getConfiguration().contribInfo
	...
	private _onPositionChanged(e: ICursorPositionChangedEvent): void {
		...
		var currentWordRange = new Range(
			lineNumber,
			word.startColumn,
			lineNumber,
			word.endColumn
		...
		var workerRequestIsValid =
			this._lastWordRange && this._lastWordRange.equalsRange(currentWordRange);

		// Even if we are on a different word, if that word is in the decorations ranges, the request is still valid
		// (Same symbol)
		for (
			var i = 0, len = this._decorationIds.length;
			!workerRequestIsValid && i < len;
			i++
		) {
		...
		if (workerRequestIsValid) {
			...
			this.workerRequest = getOccurrencesAtPosition(
				this.model,
	...
	private static _getDecorationOptions(
...
registerThemingParticipant((theme, collector) => {
	...
	if (selectionHighlight) {
		collector.addRule(
			`.monaco-editor .focused .selectionHighlight { background-color: ${selectionHighlight}; }`
		);
		collector.addRule(
	...
	if (wordHighlight) {
		collector.addRule(
			`.monaco-editor .wordHighlight { background-color: ${wordHighlight}; }`
		);
	}
	let wordHighlightStrong = theme.getColor(editorWordHighlightStrong);
	if (wordHighlightStrong) {
		collector.addRule(
			`.monaco-editor .wordHighlightStrong { background-color: ${wordHighlightStrong}; }`
		);
	}
	let hcOutline = theme.getColor(activeContrastBorder);
	if (hcOutline) {
		collector.addRule(
			`.monaco-editor .selectionHighlight { border: 1px dotted ${hcOutline}; box-sizing: border-box; }`
		);
		collector.addRule(
			`.monaco-editor .wordHighlight { border: 1px dashed ${hcOutline}; box-sizing: border-box; }`
		);
		collector.addRule(
			`.monaco-editor .wordHighlightStrong { border: 1px dashed ${hcOutline}; box-sizing: border-box; }`
		);
	}
});
