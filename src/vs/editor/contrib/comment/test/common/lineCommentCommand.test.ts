...
import {
	...
	LineCommentCommand,
	Type
} from 'vs/editor/contrib/comment/common/lineCommentCommand';
...
suite('Editor Contrib - Line Comment Command', () => {
	function testLineCommentCommand(
		lines: string[],
		selection: Selection,
		expectedLines: string[],
		expectedSelection: Selection
	): void {
		let mode = new CommentMode({
			lineComment: '!@#',
			blockComment: ['<!@#', '#@!>']
		});
		testCommand(
	...
	function testAddLineCommentCommand(
		lines: string[],
		selection: Selection,
		expectedLines: string[],
		expectedSelection: Selection
		...
		testCommand(
			...
			mode.getLanguageIdentifier(),
			selection,
			sel => new LineCommentCommand(sel, 4, Type.ForceAdd),
			expectedLines,
			expectedSelection
		);
		mode.dispose();
	...
	function createBasicLinePreflightData(
...
suite('Editor Contrib - Line Comment in mixed modes', () => {
	...
	class OuterMode extends MockMode {
		constructor(commentsConfig: CommentRule) {
			...
			this._register(
				modes.TokenizationRegistry.register(
					this.getLanguageIdentifier().language,
					{
						...
						tokenize2: (
							...
							tokens[0 << 1] = 0;
							tokens[(0 << 1) + 1] =
								(modes.ColorId.DefaultForeground <<
									modes.MetadataConsts.FOREGROUND_OFFSET) |
