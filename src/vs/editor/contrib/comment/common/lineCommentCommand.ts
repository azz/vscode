...
export class LineCommentCommand implements editorCommon.ICommand {
	 ...
	 */
	public static _gatherPreflightCommentStrings(
		...
		for (i = 0, lineCount = lines.length; i < lineCount; i++) {
			...
			if (shouldRemoveComments) {
				...
				if (
					...
					lineContent.charCodeAt(commentStrEndOffset) === CharCode.Space
				) {
	...
	public static _gatherPreflightData(
		...
		var lines = LineCommentCommand._gatherPreflightCommentStrings(
			model,
			startLineNumber,
			endLineNumber
		);
	...
	private _executeLineComments(
		...
		if (data.shouldRemoveComments) {
			...
			ops = LineCommentCommand._createAddLineCommentsOperations(
				data.lines,
				s.startLineNumber
			);
		}
		...
		for (var i = 0, len = ops.length; i < len; i++) {
			...
			if (
				ops[i].range.isEmpty() &&
				ops[i].range.getStartPosition().equals(cursorPosition)
			) {
	...
	private _attemptRemoveBlockComment(
		...
		let startTokenAllowedBeforeColumn =
			endToken.length +
			Math.max(
	...
	private _executeBlockComment(
		...
		if (!ops) {
			if (s.isEmpty()) {
				...
				var firstNonWhitespaceIndex = strings.firstNonWhitespaceIndex(
					lineContent
				);
	...
	public getEditOperations(
		...
		var data = LineCommentCommand._gatherPreflightData(
			this._type,
			model,
			s.startLineNumber,
			s.endLineNumber
	 ...
	 */
	public static _createAddLineCommentsOperations(
		lines: ILinePreflightData[],
		startLineNumber: number
	): editorCommon.IIdentifiedSingleEditOperation[] {
		var i: number,
