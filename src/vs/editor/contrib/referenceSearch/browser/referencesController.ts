...
export class ReferencesController implements editorCommon.IEditorContribution {
	...
	public constructor(
		...
		this._editor = editor;
		this._referenceSearchVisible = ctxReferenceSearchVisible.bindTo(
			contextKeyService
		);
	}
	...
	public toggleWidget(
		range: Range,
		modelPromise: TPromise<ReferencesModel>,
		...
		const data = <LayoutData>JSON.parse(
			this._storageService.get(storageKey, undefined, '{}')
		);
		this._widget = new ReferenceWidget(
			this._editor,
			data,
		...
		const promise = modelPromise.then(
			model => {
		...
		onDone(duration =>
			this._telemetryService.publicLog('findReferences', {
				duration,
