...
@editorContribution
class GotoDefinitionWithMouseEditorContribution
	...
	private startFindDefinition(
		...
		if (
			...
			this.currentWordUnderMouse.startColumn === word.startColumn &&
			this.currentWordUnderMouse.endColumn === word.endColumn &&
			this.currentWordUnderMouse.word === word.word
		) {
			return;
		}
				...
				if (results.length > 1) {
					...
					this.textModelResolverService
						...
						.then(ref => {
							...
							const startIndent = textEditorModel.getLineFirstNonWhitespaceColumn(
								startLineNumber
							);
							...
							this.addDecoration(
								...
								{
									language: this.modeService.getModeIdByFilenameOrFirstLine(
										textEditorModel.uri.fsPath
									),
									value
								}
							);
	...
	private gotoDefinition(
		...
		const action = new DefinitionAction(
			new DefinitionActionConfig(sideBySide, false, true, false),
			{
				alias: undefined,
				label: undefined,
				id: undefined,
		...
		return this.editor.invokeWithinContext(accessor =>
			action.run(accessor, this.editor)
		);
	}
...
registerThemingParticipant((theme, collector) => {
	...
	if (activeLinkForeground) {
		collector.addRule(
			`.monaco-editor .goto-definition-link { color: ${activeLinkForeground} !important; }`
		);
	}
});
