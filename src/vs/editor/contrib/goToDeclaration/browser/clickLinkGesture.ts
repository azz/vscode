...
export class ClickLinkOptions {
	...
	public readonly triggerModifier:
		| 'ctrlKey'
		| 'shiftKey'
		| 'altKey'
		| 'metaKey';
	public readonly triggerSideBySideKey: KeyCode;
	public readonly triggerSideBySideModifier:
		| 'ctrlKey'
		| 'shiftKey'
		| 'altKey'
		| 'metaKey';
...
export class ClickLinkGesture extends Disposable {
	private readonly _onMouseMoveOrRelevantKeyDown: Emitter<
		[ClickLinkMouseEvent, ClickLinkKeyboardEvent]
	> = this._register(
		new Emitter<[ClickLinkMouseEvent, ClickLinkKeyboardEvent]>()
	);
	...
	constructor(editor: ICodeEditor) {
		...
		this._opts = createOptions(
			this._editor.getConfiguration().multiCursorModifier
		);
		...
		this._register(
			this._editor.onMouseMove((e: IEditorMouseEvent) =>
				this.onEditorMouseMove(new ClickLinkMouseEvent(e, this._opts))
			)
		);
		this._register(
			this._editor.onMouseDown((e: IEditorMouseEvent) =>
				this.onEditorMouseDown(new ClickLinkMouseEvent(e, this._opts))
			)
		);
		this._register(
			this._editor.onMouseUp((e: IEditorMouseEvent) =>
				this.onEditorMouseUp(new ClickLinkMouseEvent(e, this._opts))
			)
		);
		this._register(
			this._editor.onKeyDown((e: IKeyboardEvent) =>
				this.onEditorKeyDown(new ClickLinkKeyboardEvent(e, this._opts))
			)
		);
		this._register(
			this._editor.onKeyUp((e: IKeyboardEvent) =>
				this.onEditorKeyUp(new ClickLinkKeyboardEvent(e, this._opts))
			)
		);
		...
		this._register(
			this._editor.onDidChangeCursorSelection(e =>
				this.onDidChangeCursorSelection(e)
			)
		);
		this._register(this._editor.onDidChangeModel(e => this.resetHandler()));
		this._register(
			this._editor.onDidChangeModelContent(() => this.resetHandler())
		);
		this._register(
