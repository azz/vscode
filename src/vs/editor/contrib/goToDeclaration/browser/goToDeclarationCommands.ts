...
export class DefinitionAction extends EditorAction {
	...
	public run(
		...
		return this._getDeclarationsAtPosition(model, pos).then(
			references => {
				...
				for (let i = 0; i < references.length; i++) {
					...
					if (
						this._configuration.filterCurrent &&
	...
	protected _getDeclarationsAtPosition(
	...
	protected _getNoResultFoundMessage(
		info?: editorCommon.IWordAtPosition
	): string {
	...
	protected _getMetaTitle(model: ReferencesModel): string {
		return (
			...
			nls.localize('meta.title', ' – {0} definitions', model.references.length)
		);
	}
	...
	private _onResult(
		...
		if (this._configuration.openInPeek) {
			...
			let next = model.nearestReference(
				editor.getModel().uri,
				editor.getPosition()
			);
			this._openReference(
				editorService,
				next,
				this._configuration.openToSide
			).then(editor => {
	...
	private _openReference(
		...
		reference: Location,
		sideBySide: boolean
	): TPromise<editorCommon.ICommonCodeEditor> {
		let { uri, range } = reference;
	...
	private _openInPeek(
		editorService: IEditorService,
		target: editorCommon.ICommonCodeEditor,
		model: ReferencesModel
	) {
