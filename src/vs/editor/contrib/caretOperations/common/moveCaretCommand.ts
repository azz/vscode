...
export class MoveCaretCommand implements ICommand {
	...
	public getEditOperations(
		...
		if (this._isMovingLeft && s.startColumn === 0) {
			...
			!this._isMovingLeft &&
			s.endColumn === model.getLineMaxColumn(s.startLineNumber)
		) {
			return;
		}
		...
		if (this._isMovingLeft) {
			...
			right =
				lineContent.substring(s.startColumn - 2, s.startColumn - 1) +
				lineContent.substring(s.endColumn - 1);
		} else {
			left =
				lineContent.substring(0, s.startColumn - 1) +
				lineContent.substring(s.endColumn - 1, s.endColumn);
			middle = lineContent.substring(s.startColumn - 1, s.endColumn - 1);
		...
		builder.addEditOperation(
			new Range(lineNumber, 1, lineNumber, model.getLineMaxColumn(lineNumber)),
			null
		);
		builder.addEditOperation(
	...
	public computeCursorState(
		...
		if (this._moved) {
			result = result.setStartPosition(
