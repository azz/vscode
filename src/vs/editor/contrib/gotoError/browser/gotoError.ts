...
class MarkerModel {
	...
	public setMarkers(markers: IMarker[]): void {
		...
		this._markers.sort(
			(left, right) =>
				Severity.compare(left.severity, right.severity) ||
				Range.compareRangesUsingStarts(left, right)
		);
	...
	public reveal(): void {
		...
		this.withoutWatchingEditorPosition(() => {
			const pos = new Position(
				this._markers[this._nextIdx].startLineNumber,
				this._markers[this._nextIdx].startColumn
			);
...
class MarkerNavigationWidget extends ZoneWidget {
	...
	public show(where: Position, heightInLines: number): void {
		...
		if (
			this.editor.getConfiguration().accessibilitySupport !==
	...
	private _wireModelAndView(): void {
		// listen to events
		this._model.onCurrentMarkerChanged(
			this.showAtMarker,
			this,
			this._callOnDispose
		);
		this._model.onMarkerSetChanged(
	...
	public showAtMarker(marker: IMarker): void {
		...
		this._title.innerHTML = nls.localize(
			'title.wo_source',
			'({0}/{1})',
			this._model.indexOf(marker),
			this._model.total
		);
...
class MarkerNavigationAction extends EditorAction {
	...
	public run(
		...
		telemetryService.publicLog('zoneWidgetShown', {
			mode: 'go to error',
			...editor.getTelemetryData()
		});
		if (model) {
			if (this._isNext) {
				model.next();
...
class MarkerController implements editorCommon.IEditorContribution {
	...
	constructor(
		...
		this._editor = editor;
		this._markersNavigationVisible = CONTEXT_MARKERS_NAVIGATION_VISIBLE.bindTo(
	...
	public getOrCreateModel(): MarkerModel {
		...
		this._zone = new MarkerNavigationWidget(
			this._editor,
			this._model,
			this._themeService
		);
		...
		this._callOnClose.push(
			this._editor.onDidChangeModel(() => this._cleanUp())
		);
		this._model.onCurrentMarkerChanged(
			marker => !marker && this._cleanUp(),
			undefined,
			this._callOnClose
		);
	...
	private _onMarkerChanged(changedResources: URI[]): void {
		if (
			!changedResources.some(
				r => this._editor.getModel().uri.toString() === r.toString()
			)
		) {
			return;
		}
...
export const editorMarkerNavigationError = registerColor(
	'editorMarkerNavigationError.background',
	{ dark: errorDefault, light: errorDefault, hc: errorDefault },
	nls.localize(
		'editorMarkerNavigationError',
		'Editor marker navigation widget error color.'
	)
);
export const editorMarkerNavigationWarning = registerColor(
	'editorMarkerNavigationWarning.background',
	{ dark: warningDefault, light: warningDefault, hc: warningDefault },
	nls.localize(
		'editorMarkerNavigationWarning',
		'Editor marker navigation widget warning color.'
	)
);
export const editorMarkerNavigationBackground = registerColor(
	'editorMarkerNavigation.background',
	{ dark: '#2D2D30', light: Color.white, hc: '#0C141F' },
	nls.localize(
