...
import {
	...
	IndentLinesAction
} from 'vs/editor/contrib/linesOperations/common/linesOperations';
...
suite('Editor Contrib - Line Operations', () => {
	suite('DeleteAllLeftAction', () => {
		test('should delete to the left of the cursor', function() {
			withMockCodeEditor(['one', 'two', 'three'], {}, (editor, cursor) => {
			...
			withMockCodeEditor(
				...
				(editor, cursor) => {
					...
					assert.deepEqual(
						...
						new Selection(1, 6, 1, 6).toString(),
						'002'
					);
					...
					assert.deepEqual(
						editor.getSelection().toString(),
						new Selection(2, 7, 2, 7).toString(),
						'004'
					);
					...
					assert.deepEqual(
						editor.getSelection().toString(),
						new Selection(3, 7, 3, 7).toString(),
						'006'
					);
					...
					assert.deepEqual(
						editor.getSelection().toString(),
						new Selection(4, 2, 4, 8).toString(),
						'008'
					);
					...
					assert.deepEqual(
						editor.getSelection().toString(),
						new Selection(5, 1, 5, 3).toString(),
						'010'
					);
				}
			);
		...
		test('should work in multi cursor mode', function() {
			withMockCodeEditor(
				...
				(editor, cursor) => {
					...
					assert.deepEqual(
						editor.getSelection().toString(),
						new Selection(3, 4, 3, 8).toString(),
						'003'
					);
				}
			);
	...
	test('transpose', function() {
		withMockCodeEditor(['hello world', '', '', '   '], {}, (editor, cursor) => {
			let model = editor.getModel();
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(1, 2, 1, 2).toString(),
				'002'
			);
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(1, 7, 1, 7).toString(),
				'004'
			);
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(2, 2, 2, 2).toString(),
				'006'
			);
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(4, 1, 4, 1).toString(),
				'008'
			);
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(4, 3, 4, 3).toString(),
				'010'
			);
		});
		...
		withMockCodeEditor(
			...
			(editor, cursor) => {
				...
				assert.deepEqual(
					editor.getSelection().toString(),
					new Selection(2, 1, 2, 1).toString(),
					'012'
				);
				...
				assert.deepEqual(
					editor.getSelection().toString(),
					new Selection(4, 2, 4, 2).toString(),
					'014'
				);
				...
				assert.deepEqual(
					editor.getSelection().toString(),
					new Selection(7, 2, 7, 2).toString(),
					'016'
				);
				...
				assert.deepEqual(
					editor.getSelection().toString(),
					new Selection(8, 12, 8, 12).toString(),
					'020'
				);
			}
		);
	...
	test('toggle case', function() {
		withMockCodeEditor(['hello world', 'öçşğü'], {}, (editor, cursor) => {
			let model = editor.getModel();
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(1, 1, 1, 12).toString(),
				'002'
			);
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(1, 3, 1, 3).toString(),
				'006'
			);
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(1, 4, 1, 4).toString(),
				'008'
			);
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(2, 1, 2, 6).toString(),
				'010'
			);
		...
		withMockCodeEditor(['', '   '], {}, (editor, cursor) => {
			...
			assert.deepEqual(
				editor.getSelection().toString(),
				new Selection(1, 1, 1, 1).toString(),
				'014'
			);
	...
	suite('DeleteAllRightAction', () => {
		...
		test('should work with multiple cursors', () => {
			withMockCodeEditor(['hello', 'there', 'world'], {}, (editor, cursor) => {
				const model = editor.getModel();
				const action = new DeleteAllRightAction();
	...
	test('InsertLineBeforeAction', function() {
		function testInsertLineBefore(
			lineNumber: number,
			column: number,
			callback: (model: IModel, cursor: Cursor) => void
		): void {
	...
	test('Bug 18276:[editor] Indentation broken when selection is empty', () => {
		let model = Model.createFromString(['function baz() {'].join('\n'), {
