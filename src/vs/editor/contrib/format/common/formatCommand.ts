...
export class EditOperationsCommand implements editorCommon.ICommand {
	...
	static fixLineTerminators(
		edit: editorCommon.ISingleEditOperation,
		model: editorCommon.ITokenizedModel
	): void {
	...
	static trimEdit(
		...
		return this._trimEdit(
			model.validateRange(edit.range),
			edit.text,
	...
	static _trimEdit(
		...
		if (commonPrefixLength > 0) {
			...
			let newStartPosition = model.modifyPosition(
		...
		if (commonSuffixLength > 0) {
			// Apply rear trimming
			let newEndPosition = model.modifyPosition(
			...
			currentText = currentText.substring(
				0,
				currentText.length - commonSuffixLength
			);
		}
