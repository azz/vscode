...
class AcceptOnCharacterOracle {
	...
	constructor(
		...
		accept: (item: ICompletionItem) => any
	) {
		...
		this._disposables.push(
			editor.onWillType(text => {
				if (this._activeItem) {
					...
					if (
						this._activeAcceptCharacters.has(ch) &&
...
export class SuggestController implements IEditorContribution {
	...
	constructor(
		...
		this._toDispose.push(
			this._model.onDidTrigger(e => this._widget.showTriggered(e.auto))
		);
		this._toDispose.push(
		...
		this._toDispose.push(
			this._model.onDidCancel(e => !e.retrigger && this._widget.hideWidget())
		);

		// Manage the acceptSuggestionsOnEnter context key
		let acceptSuggestionsOnEnter = SuggestContext.AcceptSuggestionsOnEnter.bindTo(
			_contextKeyService
		);
		...
		this._toDispose.push(
			this._editor.onDidChangeConfiguration(e => updateFromConfig())
		);
		updateFromConfig();

		this._widget = _instantiationService.createInstance(
			SuggestWidget,
			this._editor
		);
		...
		const autoAcceptOracle = new AcceptOnCharacterOracle(
			_editor,
			this._widget,
			item => this._onDidSelectItem(item)
		);
		this._toDispose.push(
			autoAcceptOracle,
			this._model.onDidSuggest(e => {
		...
		this._toDispose.push(
			this._widget.onDidFocus(item => {
				const position = this._editor.getPosition();
				const startColumn =
					item.position.column - item.suggestion.overwriteBefore;
		...
		if (item) {
			...
			if (Array.isArray(suggestion.additionalTextEdits)) {
				...
				this._editor.executeEdits(
					...
					suggestion.additionalTextEdits.map(edit =>
			...
			this._telemetryService.publicLog('suggestSnippetInsert', {
				...this._editor.getTelemetryData(),
