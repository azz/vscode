...
@editorContribution
class InspectTokensController extends Disposable
...
function getSafeTokenizationSupport(
	...
	let tokenizationSupport = TokenizationRegistry.get(
	...
	return {
		...
		tokenize: (line: string, state: IState, deltaOffset: number) =>
			nullTokenize(languageIdentifier.language, line, state, deltaOffset),
		tokenize2: (line: string, state: IState, deltaOffset: number) =>
			nullTokenize2(languageIdentifier.id, line, state, deltaOffset)
	};
}
...
class InspectTokensWidget extends Disposable implements IContentWidget {
	...
	private _compute(position: Position): void {
		...
		if (token1Index < data.tokens1.length) {
			...
			let tokenEndIndex = token1Index + 1 < data.tokens1.length
		...
		if (token1Index < data.tokens1.length) {
			result += `<span class="tm-token-type">${escape(
	...
	private _getTokensAtLine(lineNumber: number): ICompleteLineTokenization {
		...
		let tokenizationResult1 = this._tokenizationSupport.tokenize(
			this._model.getLineContent(lineNumber),
			stateBeforeLine,
			0
		);
		let tokenizationResult2 = this._tokenizationSupport.tokenize2(
			this._model.getLineContent(lineNumber),
			stateBeforeLine,
			0
		);
	...
	private _getStateBeforeLine(lineNumber: number): IState {
		...
		for (let i = 1; i < lineNumber; i++) {
			let tokenizationResult = this._tokenizationSupport.tokenize(
...
registerThemingParticipant((theme, collector) => {
	...
	if (border) {
		...
		collector.addRule(
			`.monaco-editor .tokens-inspect-widget { border: ${borderWidth}px solid ${border}; }`
		);
		collector.addRule(
			`.monaco-editor .tokens-inspect-widget .tokens-inspect-separator { background-color: ${border}; }`
		);
	}
	let background = theme.getColor(editorHoverBackground);
	if (background) {
		collector.addRule(
			`.monaco-editor .tokens-inspect-widget { background-color: ${background}; }`
		);
	}
});
