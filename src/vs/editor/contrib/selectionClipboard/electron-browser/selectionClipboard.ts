...
export class SelectionClipboard extends Disposable
	...
	constructor(
		...
		if (platform.isLinux) {
			...
			this._register(
				editor.onDidChangeConfiguration((e: IConfigurationChangedEvent) => {
					if (e.contribInfo) {
						isEnabled = editor.getConfiguration().contribInfo
							.selectionClipboard;
					}
				})
			);
			...
			let setSelectionToClipboard = this._register(
				new RunOnceScheduler(() => {
					...
					for (let i = 0; i < selections.length; i++) {
						...
						result.push(
							model.getValueInRange(sel, EndOfLinePreference.TextDefined)
						);
					}
