...
export class ModesHoverController implements editorCommon.IEditorContribution {
	...
	constructor(
		...
		if (editor.getConfiguration().contribInfo.hover) {
			...
			this._toUnhook.push(
				this._editor.onMouseLeave((e: IEditorMouseEvent) => this._hideWidgets())
			);
			this._toUnhook.push(
				this._editor.onKeyDown((e: IKeyboardEvent) => this._onKeyDown(e))
			);
			this._toUnhook.push(
				this._editor.onDidChangeModel(() => this._hideWidgets())
			);
			this._toUnhook.push(
			...
			this._contentWidget = new ModesContentHoverWidget(
				editor,
				openerService,
				modeService
			);
			this._glyphWidget = new ModesGlyphHoverWidget(
				editor,
				openerService,
				modeService
			);
	...
	private _onEditorMouseDown(mouseEvent: IEditorMouseEvent): void {
		...
		if (
			targetType === MouseTargetType.OVERLAY_WIDGET &&
	...
	private _onEditorMouseMove(mouseEvent: IEditorMouseEvent): void {
		...
		if (
			this._editor.getConfiguration().contribInfo.hover &&
...
registerThemingParticipant((theme, collector) => {
	...
	if (editorHoverHighlightColor) {
		collector.addRule(
			`.monaco-editor .hoverHighlight { background-color: ${editorHoverHighlightColor}; }`
		);
	}
	let hoverBackground = theme.getColor(editorHoverBackground);
	if (hoverBackground) {
		collector.addRule(
			`.monaco-editor .monaco-editor-hover { background-color: ${hoverBackground}; }`
		);
	}
	let hoverBorder = theme.getColor(editorHoverBorder);
	if (hoverBorder) {
		collector.addRule(
			`.monaco-editor .monaco-editor-hover { border: 1px solid ${hoverBorder}; }`
		);
		collector.addRule(
			`.monaco-editor .monaco-editor-hover .hover-row:not(:first-child):not(:empty) { border-top: 1px solid ${hoverBorder.transparent(
				0.5
			)}; }`
		);
	}
	...
	if (link) {
		collector.addRule(
			`.monaco-editor .monaco-editor-hover a { color: ${link}; }`
		);
	}
	let codeBackground = theme.getColor(textCodeBlockBackground);
	if (codeBackground) {
		collector.addRule(
			`.monaco-editor .monaco-editor-hover code { background-color: ${codeBackground}; }`
		);
	}
});
