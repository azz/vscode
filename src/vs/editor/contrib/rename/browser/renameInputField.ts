...
export default class RenameInputField implements IContentWidget, IDisposable {
	...
	constructor(
		...
		this._disposables.push(
			themeService.onThemeChange(theme => this.onThemeChange(theme))
		);
	}
	...
	public getDomNode(): HTMLElement {
		if (!this._domNode) {
			...
			this._domNode.style.height = `${this._editor.getConfiguration()
				.lineHeight}px`;
	...
	private updateStyles(theme: ITheme): void {
		...
		this._inputField.style.backgroundColor = background
			? background.toString()
			: null;
	...
	public getInput(
		...
		this._inputField.size = Math.max(
		...
		return new TPromise<string>((c, e) => {
			...
			this._currentAcceptInput = () => {
				if (
					this._inputField.value.trim().length === 0 ||
					this._inputField.value === value
				) {
			...
			disposeOnDone.push(
				this._editor.onDidBlurEditor(() => this.cancelInput())
			);

			this._show();
		}, this._currentCancelInput).then(
			newValue => {
				always();
				return newValue;
			},
