...
export class OneSnippet {
	...
	dispose(): void {
		if (this._placeholderDecorations) {
			this._editor.changeDecorations(accessor =>
				this._placeholderDecorations.forEach(handle =>
					accessor.removeDecoration(handle)
				)
			);
		}
	...
	move(fwd: boolean | undefined): Selection[] {
		...
		if (
			fwd === true &&
			this._placeholderGroupsIdx < this._placeholderGroups.length - 1
		) {
			this._placeholderGroupsIdx += 1;
	...
	get isAtFirstPlaceholder() {
		return (
			this._placeholderGroupsIdx <= 0 || this._placeholderGroups.length === 0
		);
	}
				...
				for (const nestedPlaceholder of nested._snippet.placeholders) {
					if (nestedPlaceholder.isFinalTabstop) {
						nestedPlaceholder.index =
							placeholder.index + nested._snippet.placeholders.length / 10;
					} else {
						nestedPlaceholder.index =
							placeholder.index + nestedPlaceholder.index / 10;
					}
				}
...
export class SnippetSession {
		...
		for (let i = 1; i < templateLines.length; i++) {
			...
			templateLines[i] =
				model.normalizeIndentation(
					lineLeadingWhitespace + templateLeadingWhitespace
				) + templateLines[i].substr(templateLeadingWhitespace.length);
		}
	...
	static createEditsAndSnippets(
		...
		let firstBeforeText = model.getValueInRange(
		...
		let firstAfterText = model.getValueInRange(
			SnippetSession.adjustSelection(
				model,
				editor.getSelection(),
				0,
		...
		for (const { selection, idx } of indexedSelection) {
			...
			const adjustedTemplate = SnippetSession.adjustWhitespace(
				model,
				start,
				template
			);

			const snippet = SnippetParser.parse(
				adjustedTemplate,
				enforceFinalTabstop
			...
			delta +=
				snippet.text.length - model.getValueLengthInRange(snippetSelection);
	...
	insert(): void {
		...
		const { edits, snippets } = SnippetSession.createEditsAndSnippets(
			this._editor,
			this._template,
			this._overwriteBefore,
			this._overwriteAfter,
			false
		);
			...
			model.pushEditOperations(
				...
				undoEdits => {
					if (this._snippets[0].hasPlaceholder) {
						...
						return undoEdits.map(edit =>
							Selection.fromPositions(edit.range.getEndPosition())
						);
					}
				}
			)
	...
	merge(
		...
		this._editor.setSelections(
			this._editor
				.getModel()
				.pushEditOperations(this._editor.getSelections(), edits, undoEdits => {
