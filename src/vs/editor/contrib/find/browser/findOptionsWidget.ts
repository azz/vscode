...
export class FindOptionsWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		let inputActiveOptionBorderColor = themeService
			.getTheme()
			.getColor(inputActiveOptionBorder);
	...
	private _hideSoon = this._register(
		new RunOnceScheduler(() => this._hide(), 1000)
	);
...
registerThemingParticipant((theme, collector) => {
	...
	if (widgetBackground) {
		collector.addRule(
			`.monaco-editor .findOptionsWidget { background-color: ${widgetBackground}; }`
		);
	}
	...
	if (widgetShadowColor) {
		collector.addRule(
			`.monaco-editor .findOptionsWidget { box-shadow: 0 2px 8px ${widgetShadowColor}; }`
		);
	}
	...
	if (hcBorder) {
		collector.addRule(
			`.monaco-editor .findOptionsWidget { border: 2px solid ${hcBorder}; }`
		);
	}
});
