...
suite('FindModel', () => {
	function findTest(
		testName: string,
		callback: (editor: ICommonCodeEditor, cursor: Cursor) => void
	): void {
		test(testName, () => {
			withMockCodeEditor(
				[
	...
	function assertFindState(
		editor: ICommonCodeEditor,
		cursor: number[],
		highlighted: number[],
		findDecorations: number[][]
	): void {
	...
	findTest('incremental find from beginning of file', (editor, cursor) => {
		...
		assertFindState(
			...
			[6, 14, 6, 19],
			[6, 14, 6, 19],
			[[6, 14, 6, 19], [6, 27, 6, 32], [7, 14, 7, 19], [8, 14, 8, 19]]
		);
	...
	findTest('find next ^$', (editor, cursor) => {
		...
		assertFindState(editor, [1, 1, 1, 1], null, [[4, 1, 4, 1], [12, 1, 12, 1]]);
	...
	findTest('replace bla', (editor, cursor) => {
		...
		assertFindState(
			...
			[11, 4, 11, 7],
			[11, 4, 11, 7],
			[[11, 4, 11, 7], [11, 7, 11, 10], [11, 10, 11, 13]]
		);
	...
	findTest(
		...
		(editor, cursor) => {
			...
			assertFindState(editor, [1, 1, 1, 1], null, [[2, 2, 2, 9], [3, 2, 3, 9]]);
	...
	findTest(
		'finds only in editable range if replace is shown',
		(editor, cursor) => {
