...
suite('FindController', () => {
		...
		withMockCodeEditor(
			...
			(editor, cursor) => {
				...
				findController
					.getState()
					.change(
						{ searchString: 'some+thing', isRegex: true, isRevealed: true },
						false
					);
	...
	test('show next find term do not update history', () => {
		withMockCodeEditor(
			...
			(editor, cursor) => {
				...
				assert.deepEqual(
					['1', '2', '3', '4'],
					toArray(findController.getHistory())
				);
			}
		);
	});
	...
	test('issue #6661: AddSelectionToNextFindMatchAction can work with touching ranges', () => {
		withMockCodeEditor(
			...
			(editor, cursor) => {
				...
				assert.equal(editor.getValue(), ['zz', 'z', 'zz'].join('\n'));
		...
		withMockCodeEditor(
			...
			(editor, cursor) => {
				...
				findController
					.getState()
					.change(
						{ searchString: '\\b\\s{3}\\b', replaceString: ' ', isRegex: true },
						false
					);
		...
		withMockCodeEditor(
			...
			(editor, cursor) => {
				...
				findController
					.getState()
					.change(
						{ searchString: '^', replaceString: 'x', isRegex: true },
						false
					);
				findController.moveToNextMatch();

				assert.deepEqual(editor.getSelections().map(fromRange), [[2, 1, 2, 1]]);
	...
	function testAddSelectionToNextFindMatchAction(
		...
		callback: (
			...
			action: AddSelectionToNextFindMatchAction,
			findController: TestFindController
		) => void
	): void {
		withMockCodeEditor(
			text,
	...
	test('AddSelectionToNextFindMatchAction starting with single collapsed selection', () => {
		...
		testAddSelectionToNextFindMatchAction(
			...
			(editor, action, findController) => {
				...
				assert.deepEqual(editor.getSelections(), [new Selection(1, 1, 1, 4)]);
