...
export class TextAreaInput extends Disposable {
	...
	public onCompositionUpdate: Event<ICompositionData> = this
	...
	constructor(
		...
		this._register(
			dom.addStandardDisposableListener(
				...
				(e: IKeyboardEvent) => {
					if (
						this._isDoingComposition &&
						e.equals(KeyCode.KEY_IN_COMPOSITION)
					) {
			...
			dom.addDisposableListener(
				...
				(e: CompositionEvent) => {
					...
					if (browser.isEdgeOrIE || browser.isChrome) {
						this._textAreaState = this._textAreaState.readFromTextArea(
							this._textArea
						);
					}
		...
		this._register(
			dom.addDisposableListener(textArea.domNode, 'input', () => {
				if (this._isDoingComposition) {
					// See https://github.com/Microsoft/monaco-editor/issues/320
					if (browser.isChromev56) {
						const [newState, typeInput] = deduceComposition(
							this._textArea.getValue()
						);
						this._textAreaState = newState;
				...
				const [newState, typeInput] = deduceInputFromTextAreaValue(
					/*couldBeEmojiInput*/ platform.isMacintosh
				);
				if (
					typeInput.replaceCharCnt === 0 &&
			...
			dom.addDisposableListener(
				...
				(e: ClipboardEvent) => {
					if (ClipboardEventUtils.canUseTextData(e)) {
						...
						if (
							this._textArea.getSelectionStart() !==
							this._textArea.getSelectionEnd()
						) {
		...
		this._register(
			dom.addDisposableListener(textArea.domNode, 'focus', () =>
				this._setHasFocus(true)
			)
		);
		this._register(
			dom.addDisposableListener(textArea.domNode, 'blur', () =>
	...
	public writeScreenReaderContent(reason: string): void {
		...
		this._setAndWriteTextAreaState(
			reason,
			this._host.getScreenReaderContent(this._textAreaState)
		);
	}
	...
	private _ensureClipboardGetsEditorSelection(e: ClipboardEvent): void {
		...
		if (!ClipboardEventUtils.canUseTextData(e)) {
			...
			this._setAndWriteTextAreaState(
				'copy or cut',
				TextAreaState.selectedText(copyPlainText)
			);
			return;
		}
