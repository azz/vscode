...
export function setLanguageConfiguration(
	...
	let languageIdentifier = StaticServices.modeService
		.get()
	...
	return LanguageConfigurationRegistry.register(
		languageIdentifier,
		configuration
	);
}
...
export function setTokensProvider(
	...
	let adapter = new TokenizationSupport2Adapter(
...
export function setMonarchTokensProvider(
	...
	let adapter = createTokenizationSupport(
...
export function registerReferenceProvider(
	languageId: string,
	provider: modes.ReferenceProvider
): IDisposable {
...
export function registerSignatureHelpProvider(
	languageId: string,
	provider: modes.SignatureHelpProvider
): IDisposable {
...
export function registerDocumentSymbolProvider(
	languageId: string,
	provider: modes.DocumentSymbolProvider
): IDisposable {
...
export function registerDocumentHighlightProvider(
	languageId: string,
	provider: modes.DocumentHighlightProvider
): IDisposable {
...
export function registerDefinitionProvider(
	languageId: string,
	provider: modes.DefinitionProvider
): IDisposable {
...
export function registerImplementationProvider(
	languageId: string,
	provider: modes.ImplementationProvider
): IDisposable {
...
export function registerTypeDefinitionProvider(
	languageId: string,
	provider: modes.TypeDefinitionProvider
): IDisposable {
...
export function registerCodeLensProvider(
	languageId: string,
	provider: modes.CodeLensProvider
): IDisposable {
...
export function registerCodeActionProvider(
	...
	return modes.CodeActionProviderRegistry.register(languageId, {
		provideCodeActions: (
			...
			range: Range,
			token: CancellationToken
		): modes.CodeAction[] | Thenable<modes.CodeAction[]> => {
			let markers = StaticServices.markerService
				.get()
...
export function registerDocumentFormattingEditProvider(
	languageId: string,
	provider: modes.DocumentFormattingEditProvider
): IDisposable {
	return modes.DocumentFormattingEditProviderRegistry.register(
		languageId,
		provider
	);
}
...
export function registerDocumentRangeFormattingEditProvider(
	languageId: string,
	provider: modes.DocumentRangeFormattingEditProvider
): IDisposable {
	return modes.DocumentRangeFormattingEditProviderRegistry.register(
		languageId,
		provider
	);
}
...
export function registerOnTypeFormattingEditProvider(
	languageId: string,
	provider: modes.OnTypeFormattingEditProvider
): IDisposable {
	return modes.OnTypeFormattingEditProviderRegistry.register(
		languageId,
		provider
	);
}
...
class SuggestAdapter {
	...
	provideCompletionItems(
		...
		return toThenable<CompletionItem[] | CompletionList>(
			...
			if (Array.isArray(value)) {
				...
				console.warn(
					'INVALID result from completion provider. expected CompletionItem-array or CompletionList but got:',
					value
				);
			}
