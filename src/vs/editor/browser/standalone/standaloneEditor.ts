...
} from 'vs/editor/browser/standalone/standaloneCodeEditor';
import { ScrollbarVisibility } from 'vs/base/common/scrollable';
import {
	IEditorOverrideServices,
	DynamicStandaloneServices,
	StaticServices
} from 'vs/editor/browser/standalone/standaloneServices';
...
import {
	...
	IColorizerOptions
} from 'vs/editor/browser/standalone/colorizer';
import {
	SimpleEditorService,
...
} from 'vs/editor/common/services/standaloneThemeService';
...
function withAllStandaloneServices<T extends editorCommon.IEditor>(
	domElement: HTMLElement,
	override: IEditorOverrideServices,
	callback: (services: DynamicStandaloneServices) => T
): T {
...
export function createModel(
	...
	if (!language) {
		...
		return doCreateModel(
			value,
			StaticServices.modeService
				.get()
				.getOrCreateModeByFilenameOrFirstLine(path, firstLine),
			uri
		);
	}
	return doCreateModel(
...
export function setModelLanguage(
	...
	StaticServices.modelService
		.get()
		.setMode(model, StaticServices.modeService.get().getOrCreateMode(language));
}
...
export function onDidCreateModel(
	listener: (model: editorCommon.IModel) => void
): IDisposable {
...
export function onDidChangeModelLanguage(
	listener: (
		e: { readonly model: editorCommon.IModel; readonly oldLanguage: string }
	) => void
): IDisposable {
...
function getSafeTokenizationSupport(
	...
	return {
		...
		tokenize: (line: string, state: modes.IState, deltaOffset: number) =>
			nullTokenize(languageId, line, state, deltaOffset),
		tokenize2: undefined
	};
}
