...
export class RenderedLinesCollection<T extends ILine> {
	...
	public onLinesDeleted(
		...
		for (
			...
			if (
				deleteFromLineNumber <= lineNumber &&
				lineNumber <= deleteToLineNumber
			) {
	...
	public onLinesChanged(
		...
		for (
			...
			if (
				changedLineNumber >= startLineNumber &&
				changedLineNumber <= endLineNumber
			) {
				// Notify the line
				this._lines[
	...
	public onLinesInserted(
		...
		if (insertCnt + insertFromLineNumber > endLineNumber) {
			...
			let deleted = this._lines.splice(
		...
		let afterLines = this._lines.slice(
			insertIndex,
			this._lines.length - insertCnt
		);
		let deletedLines = this._lines.slice(
			this._lines.length - insertCnt,
			this._lines.length
		);
	...
	public onTokensChanged(
		...
		for (let i = 0, len = ranges.length; i < len; i++) {
			...
			if (
				rng.toLineNumber < startLineNumber ||
				rng.fromLineNumber > endLineNumber
			) {
...
export class VisibleLinesCollection<T extends IVisibleLine> {
	...
	constructor(host: IVisibleLinesHost<T>) {
		...
		this._linesCollection = new RenderedLinesCollection<T>(() =>
			this._host.createVisibleLine()
		);
	}
	...
	public onLinesChanged(e: viewEvents.ViewLinesChangedEvent): boolean {
		return this._linesCollection.onLinesChanged(
			e.fromLineNumber,
			e.toLineNumber
		);
	}
	...
	public onLinesDeleted(e: viewEvents.ViewLinesDeletedEvent): boolean {
		let deleted = this._linesCollection.onLinesDeleted(
			e.fromLineNumber,
			e.toLineNumber
		);
		if (deleted) {
	...
	public onLinesInserted(e: viewEvents.ViewLinesInsertedEvent): boolean {
		let deleted = this._linesCollection.onLinesInserted(
			e.fromLineNumber,
			e.toLineNumber
		);
		if (deleted) {
	...
	public renderLines(viewportData: ViewportData): void {
		...
		let renderer = new ViewLayerRenderer<T>(
...
class ViewLayerRenderer<T extends IVisibleLine> {
	...
	private _finishRendering(
		...
		for (let i = 0, len = ctx.linesLength; i < len; i++) {
			...
			let renderResult = line.renderLine(
				lineNumber,
				deltaTop[i],
				this.viewportData
			);
