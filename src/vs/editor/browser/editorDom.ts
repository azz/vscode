...
export class PageCoordinates {
	...
	public toClientCoordinates(): ClientCoordinates {
		return new ClientCoordinates(
			this.x - dom.StandardWindow.scrollX,
			this.y - dom.StandardWindow.scrollY
		);
	}
}
...
export class ClientCoordinates {
	...
	public toPageCoordinates(): PageCoordinates {
		return new PageCoordinates(
			this.clientX + dom.StandardWindow.scrollX,
			this.clientY + dom.StandardWindow.scrollY
		);
	}
}
...
export class EditorMouseEventFactory {
	...
	public onMouseLeave(
		...
		return dom.addDisposableNonBubblingMouseOutListener(
			target,
			(e: MouseEvent) => {
				callback(this._create(e));
			}
...
export class GlobalEditorMouseMoveMonitor extends Disposable {
	...
	public startMonitoring(
		...
		let myMerger: dom.IEventMerger<EditorMouseEvent> = (
			...
			return merger(
				lastEvent,
				new EditorMouseEvent(currentEvent, this._editorViewDomNode)
			);
		};

		this._globalMouseMoveMonitor.startMonitoring(
			myMerger,
			mouseMoveCallback,
			() => {
