...
export class ViewLineOptions {
	...
	constructor(config: IConfiguration) {
		...
		this.renderControlCharacters =
			config.editor.viewInfo.renderControlCharacters;
...
export class ViewLine implements IVisibleLine {
	...
	public renderLine(
		...
		return `<div style="top:${deltaTop}px;height:${this._options
			.lineHeight}px;" class="${ViewLine.CLASS_NAME}">${output.html}</div>`;
	}
	...
	public getVisibleRangesForRange(
		...
		return this._renderedViewLine.getVisibleRangesForRange(
			startColumn,
			endColumn,
			context
		);
...
class FastRenderedViewLine implements IRenderedViewLine {
	...
	private static _createCharOffset(
		characterMapping: CharacterMapping
	): Uint32Array {
	...
	private _getOrCreateCharOffset(): Uint32Array {
		if (this._charOffset === null) {
			this._charOffset = FastRenderedViewLine._createCharOffset(
	...
	public getColumnOfNodeOffset(
		...
		let charOffset = this._characterMapping.partDataToCharOffset(
			spanIndex,
...
class RenderedViewLine {
	...
	constructor(
		...
		characterMapping: CharacterMapping,
		containsRTL: boolean,
		containsForeignElements: boolean
	) {
		this.domNode = domNode;
		this.input = renderLineInput;
		this._characterMapping = characterMapping;
		...
		if (!containsRTL) {
			this._pixelOffsetCache = new Int32Array(
				this._characterMapping.length + 1
			);
			for (
	...
	private _actualReadPixelOffset(
		...
		let r = RangeUtil.readHorizontalRanges(
			this._getReadingTarget(),
			partIndex,
			charOffsetInPart,
			partIndex,
			charOffsetInPart,
	...
	private _readRawVisibleRangesForRange(
		...
		let startPartData = this._characterMapping.charOffsetToPartData(
			startColumn - 1
		);
		...
		let endPartData = this._characterMapping.charOffsetToPartData(
			endColumn - 1
		);
...
class WebKitRenderedViewLine extends RenderedViewLine {
	protected _readVisibleRangesForRange(
		...
		let output = super._readVisibleRangesForRange(
			startColumn,
			endColumn,
			context
		);
