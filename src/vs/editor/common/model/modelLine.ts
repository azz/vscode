...
export class LineMarker {
	...
	public static compareMarkers(a: LineMarker, b: LineMarker): number {
		if (a.position.column === b.position.column) {
			return (
				(a.stickToPreviousCharacter ? 0 : 1) -
				(b.stickToPreviousCharacter ? 0 : 1)
			);
		}
...
export class ModelLine {
	...
	private _setPlusOneIndentLevel(value: number): void {
		this._metadata =
			(this._metadata & 0x00000001) | ((value & 0xefffffff) << 1);
	}
	...
	public updateTabSize(tabSize: number): void {
		if (tabSize === 0) {
			...
			this._setPlusOneIndentLevel(
				computePlusOneIndentLevel(this._text, tabSize)
			);
		}
	}
	...
	public setTokens(topLevelLanguageId: LanguageId, tokens: Uint32Array): void {
		...
		if (tokens.length === 2) {
			...
			if (
				tokens[0] === 0 &&
				tokens[1] === ModelLine._getDefaultMetadata(topLevelLanguageId)
			) {
				this._lineTokens = null;
				return;
			}
	...
	private _createTokensAdjuster(): ITokensAdjuster {
		...
		let adjust = (
			...
			while (tokenStartOffset < toColumn && tokenIndex < tokensLength) {
				if (tokenStartOffset > 0 && delta !== 0) {
					...
					let newTokenStartOffset = Math.max(
						minimumAllowedIndex,
						tokenStartOffset + delta
					);
		...
		let finish = (delta: number, lineTextLength: number) => {
			...
			this._deleteMarkedTokens(
				this._markOverflowingTokensForDeletion(
					removeTokensCount,
					lineTextLength
				)
			);
	...
	private _markOverflowingTokensForDeletion(
		removeTokensCount: number,
	...
	private _deleteMarkedTokens(removeTokensCount: number): void {
		...
		let lineTokens = new Uint32Array(this._lineTokens);
		let tokensLength = lineTokens.length >>> 1;
		let newTokens = new Uint32Array((tokensLength - removeTokensCount) << 1),
			newTokenIdx = 0;
	...
	private _createMarkersAdjuster(
		markersTracker: MarkersTracker
	): IMarkersAdjuster {
		if (!this._markers) {
		...
		let adjustDelta = (
			...
			while (
				...
				adjustMarkerBeforeColumn(toColumn, moveSemantics)
			) {
				if (delta !== 0) {
					let newColumn = Math.max(
						minimumAllowedColumn,
						marker.position.column + delta
					);
	...
	public applyEdits(
		...
		for (let i = 0, len = edits.length; i < len; i++) {
			...
			if (commonLength > 0) {
				...
				tokensAdjuster.adjust(
					edit.startColumn - 1 + commonLength,
					deltaColumn,
					startColumn
				);
			...
			resultText =
				resultText.substring(0, startColumn - 1) +
				edit.text +
				resultText.substring(endColumn - 1);
			...
			markersAdjuster.adjustSet(
				edit.endColumn,
				startColumn + insertingCnt,
				edit.forceMoveMarkers
					? MarkerMoveSemantics.ForceMove
					: MarkerMoveSemantics.MarkerDefined
			);
	...
	public split(
		...
		if (this._markers) {
			...
			for (let i = 0, len = this._markers.length; i < len; i++) {
				...
				if (
					...
					(marker.position.column === splitColumn &&
						(forceMoveMarkers || !marker.stickToPreviousCharacter))
				) {
			...
			if (otherMarkers) {
				for (let i = 0, len = otherMarkers.length; i < len; i++) {
					...
					marker.updateColumn(
						markersTracker,
						marker.position.column - (splitColumn - 1)
					);
				}
			}
		}
		...
		// Mark overflowing tokens for deletion & delete marked tokens
		this._deleteMarkedTokens(
			this._markOverflowingTokensForDeletion(0, this._text.length)
		);
		...
		if (other._markers) {
			...
			for (let i = 0, len = otherMarkers.length; i < len; i++) {
				...
				marker.updatePosition(
					...
					new Position(
						this._lineNumber,
						marker.position.column + thisTextLength
					)
				);
			}
