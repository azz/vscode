...
} from 'vs/editor/common/model/textModelWithMarkers';
...
export class InternalDecoration implements editorCommon.IModelDecoration {
	...
	public setRange(
		multiLineDecorationsMap: { [key: string]: InternalDecoration },
		range: Range
	): void {
		...
		let rangeWasMultiLine =
			this.range.startLineNumber !== this.range.endLineNumber;
		this.range = range;
		let rangeIsMultiline =
			this.range.startLineNumber !== this.range.endLineNumber;
...
export class TextModelWithDecorations extends TextModelWithMarkers
	implements editorCommon.ITextModelWithDecorations {
	/**
	...
	private _internalDecorations: {
		[internalDecorationId: number]: InternalDecoration;
	};
	...
	private static _shouldStartMarkerSticksToPreviousCharacter(
		...
		if (
			...
			stickiness ===
				editorCommon.TrackedRangeStickiness.GrowsOnlyWhenTypingBefore
		) {
			return true;
		}
		return false;
	...
	private static _shouldEndMarkerSticksToPreviousCharacter(
		stickiness: editorCommon.TrackedRangeStickiness
	): boolean {
		if (
			stickiness ===
	...
	private _changeDecorations<T>(
		decorationsTracker: DecorationsTracker,
		ownerId: number,
		callback: (
			changeAccessor: editorCommon.IModelDecorationsChangeAccessor
		) => T
		...
		let changeAccessor: editorCommon.IModelDecorationsChangeAccessor = {
			addDecoration: (
				...
				return this._addDecorationImpl(
					decorationsTracker,
					ownerId,
			...
			deltaDecorations: (
				...
				return this._deltaDecorationsImpl(
					...
					ownerId,
					oldDecorations,
					this._normalizeDeltaDecorations(newDecorations)
				);
			}
		};
		let result: T = null;
	 ...
	 */
	private _getMultiLineDecorations(
		filterRange: Range,
		...
		for (let decorationId in this._multiLineDecorationsMap) {
			...
			if (
				filterOwnerId &&
				decoration.ownerId &&
				decoration.ownerId !== filterOwnerId
			) {
				continue;
			}
			...
			if (
				range.startLineNumber === filterEndLineNumber &&
				range.startColumn > filterEndColumn
			) {
				continue;
			}
			...
			if (
				range.endLineNumber === filterStartLineNumber &&
				range.endColumn < filterStartColumn
			) {
				continue;
			}
	...
	private _getDecorationsInRange(
		...
		let result = this._getMultiLineDecorations(
			filterRange,
			filterOwnerId,
			filterOutValidation
		);
		...
		for (
			let lineNumber = filterStartLineNumber;
			lineNumber <= filterEndLineNumber;
			lineNumber++
		) {
	...
	public getLinesDecorations(
		...
		return this._getDecorationsInRange(
			new Range(startLineNumber, 1, endLineNumber, endColumn),
			ownerId,
			filterOutValidation
		);
	}
	...
	private _handleTrackedMarkers(markersTracker: MarkersTracker): void {
		...
		let uniqueChangedDecorations: string[] = [],
			uniqueChangedDecorationsLen = 0;
		...
		for (let i = 0, len = changedInternalDecorationIds.length; i < len; i++) {
			...
			let range = TextModelWithDecorations._createRangeFromMarkers(
				startMarker,
				endMarker
			);
	...
	private static _createRangeFromMarkers(
	...
	private _handleTrackedDecorations(
		decorationsTracker: DecorationsTracker
	): void {
		if (
	...
	private emitModelDecorationsChangedEvent(
		e: textModelEvents.IModelDecorationsChangedEvent
	): void {
	...
	private _normalizeDeltaDecorations(
		deltaDecorations: editorCommon.IModelDeltaDecoration[]
		...
		for (let i = 0, len = deltaDecorations.length; i < len; i++) {
			...
			result.push(
				new ModelDeltaDecoration(
					i,
					this.validateRange(deltaDecoration.range),
					_normalizeOptions(deltaDecoration.options)
				)
			);
		}
		return result;
	...
	private _addDecorationImpl(
		...
		let markers = this._addMarkers([
			{
				...
				stickToPreviousCharacter: TextModelWithDecorations._shouldStartMarkerSticksToPreviousCharacter(
					options.stickiness
				)
			},
			{
		...
		let decoration = new InternalDecoration(
			decorationId,
	...
	private _addDecorationsImpl(
		decorationsTracker: DecorationsTracker,
		ownerId: number,
		newDecorations: ModelDeltaDecoration[]
	): string[] {
	...
	private _changeDecorationImpl(
		...
		if (newRange.startLineNumber !== startMarker.position.lineNumber) {
			...
			this._lines[startMarker.position.lineNumber - 1].removeMarker(
				startMarker
			);
			this._lines[newRange.startLineNumber - 1].addMarker(startMarker);
		}
		startMarker.setPosition(
			new Position(newRange.startLineNumber, newRange.startColumn)
		);
		...
		endMarker.setPosition(
			new Position(newRange.endLineNumber, newRange.endColumn)
		);
	...
	private _changeDecorationOptionsImpl(
		...
		if (decoration.options.stickiness !== options.stickiness) {
			decoration.startMarker.stickToPreviousCharacter = TextModelWithDecorations._shouldStartMarkerSticksToPreviousCharacter(
				options.stickiness
			);
			decoration.endMarker.stickToPreviousCharacter = TextModelWithDecorations._shouldEndMarkerSticksToPreviousCharacter(
				options.stickiness
			);
		}
	...
	private _removeDecorationsImpl(
		decorationsTracker: DecorationsTracker,
		decorationIds: string[]
	): void {
		let removeMarkers: LineMarker[] = [],
			removeMarkersLen = 0;
	...
	private _resolveOldDecorations(
		oldDecorations: string[]
	): InternalDecoration[] {
		let result: InternalDecoration[] = [];
	...
	private _deltaDecorationsImpl(
		...
		if (oldDecorationsIds.length === 0) {
			// Nothing to remove
			return this._addDecorationsImpl(
				decorationsTracker,
				ownerId,
				newDecorations
			);
		...
		while (
			oldDecorationsIndex < oldDecorationsLength &&
			newDecorationsIndex < newDecorationsLength
		) {
			let oldDecoration = oldDecorations[oldDecorationsIndex];
			let newDecoration = newDecorations[newDecorationsIndex];
			let comparison = Range.compareRangesUsingStarts(
				oldDecoration.range,
				newDecoration.range
			);
		...
		if (decorationsToAdd.length > 0) {
			let newIds = this._addDecorationsImpl(
				decorationsTracker,
				ownerId,
				decorationsToAdd
			);
...
export class ModelDecorationOptions
	implements editorCommon.IModelDecorationOptions {
	...
	private constructor(
		...
		this.glyphMarginHoverMessage =
			options.glyphMarginHoverMessage || strings.empty;
		this.isWholeLine = options.isWholeLine || false;
		this.showIfCollapsed = options.showIfCollapsed || false;
		this.overviewRuler = new ModelDecorationOverviewRulerOptions(
