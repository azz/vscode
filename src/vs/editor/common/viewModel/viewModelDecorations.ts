...
export class ViewModelDecorations implements IDisposable {
	...
	public onModelDecorationsChanged(e: IModelDecorationsChangedEvent): void {
		...
		if (
			this._decorationsCache !== null &&
			this._decorationsCache !== undefined
		) {
	...
	private _getOrCreateViewModelDecoration(
		modelDecoration: editorCommon.IModelDecoration
	): ViewModelDecoration {
		...
		if (r.range === null) {
			...
			if (modelDecoration.options.isWholeLine) {
				let start = this._coordinatesConverter.convertModelPositionToViewPosition(
					new Position(modelRange.startLineNumber, 1)
				);
				let end = this._coordinatesConverter.convertModelPositionToViewPosition(
					new Position(
				...
				r.range = this._coordinatesConverter.convertModelRangeToViewRange(
					modelRange
				);
			}
		}
	...
	public getAllOverviewRulerDecorations(): ViewModelDecoration[] {
		let modelDecorations = this.model.getAllDecorations(
			this.editorId,
			this.configuration.editor.readOnly
		);
		...
		for (let i = 0, len = modelDecorations.length; i < len; i++) {
			...
			let viewModelDecoration = this._getOrCreateViewModelDecoration(
				modelDecoration
			);
	...
	public getDecorationsViewportData(
		...
		cacheIsValid =
			cacheIsValid && this._cachedModelDecorationsResolver !== null;
		cacheIsValid =
			cacheIsValid &&
			viewRange.equalsRange(this._cachedModelDecorationsResolverViewRange);
		if (!cacheIsValid) {
			this._cachedModelDecorationsResolver = this._getDecorationsViewportData(
				viewRange
			);
	...
	private _getDecorationsViewportData(
		...
		let viewportModelRange = this._coordinatesConverter.convertViewRangeToModelRange(
			viewportRange
		);
		let startLineNumber = viewportRange.startLineNumber;
		let endLineNumber = viewportRange.endLineNumber;
		let modelDecorations = this.model.getDecorationsInRange(
			viewportModelRange,
			this.editorId,
			this.configuration.editor.readOnly
		);

		let decorationsInViewport: ViewModelDecoration[] = [],
		...
		for (let i = 0, len = modelDecorations.length; i < len; i++) {
			...
			if (decorationOptions.inlineClassName) {
				...
				let intersectedStartLineNumber = Math.max(
					startLineNumber,
					viewRange.startLineNumber
				);
				let intersectedEndLineNumber = Math.min(
					endLineNumber,
