...
export const editorCursor = registerColor(
...
export const editorLineNumbers = registerColor(
	...
	{ dark: '#5A5A5A', light: '#2B91AF', hc: Color.white },
...
export const editorErrorForeground = registerColor(
	...
	{ dark: '#FF0000', light: '#FF0000', hc: null },
	nls.localize(
		'errorForeground',
...
export const editorErrorBorder = registerColor(
	...
	nls.localize('errorBorder', 'Border color of error squigglies in the editor.')
);

export const editorWarningForeground = registerColor(
	'editorWarning.foreground',
	{ dark: '#008000', light: '#008000', hc: null },
	nls.localize(
		'warningForeground',
...
registerThemingParticipant((theme, collector) => {
	...
	if (background) {
		collector.addRule(
			`.monaco-editor, .monaco-editor .monaco-editor-background, .monaco-editor .inputarea.ime-input { background-color: ${background}; }`
		);
	}
	let foreground = theme.getColor(editorForeground);
	if (foreground) {
		collector.addRule(
			`.monaco-editor, .monaco-editor .inputarea.ime-input { color: ${foreground}; }`
		);
	}
	...
	if (gutter) {
		collector.addRule(
			`.monaco-editor .margin { background-color: ${gutter}; }`
		);
	}
	...
	if (rangeHighlight) {
		collector.addRule(
			`.monaco-editor .rangeHighlight { background-color: ${rangeHighlight}; }`
		);
	}
	let outline = theme.getColor(activeContrastBorder);
	if (outline) {
		collector.addRule(
			`.monaco-editor .rangeHighlight { border: 1px dotted ${outline}; }; }`
		);
	}
