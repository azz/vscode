...
export class ColumnSelection {
	private static _columnSelect(
		...
		for (let i = 0; i < lineCount; i++) {
			...
			let startColumn = CursorColumns.columnFromVisibleColumn2(
				config,
				model,
				lineNumber,
				fromVisibleColumn
			);
			let endColumn = CursorColumns.columnFromVisibleColumn2(
				config,
				model,
				lineNumber,
				toVisibleColumn
			);
			let visibleStartColumn = CursorColumns.visibleColumnFromColumn2(
				config,
				model,
				new Position(lineNumber, startColumn)
			);
			let visibleEndColumn = CursorColumns.visibleColumnFromColumn2(
				config,
				model,
	...
	public static columnSelect(
		config: CursorConfiguration,
		model: ICursorSimpleModel,
		fromViewSelection: Selection,
		toViewLineNumber: number,
		toViewVisualColumn: number
	): IColumnSelectResult {
		const fromViewPosition = new Position(
			fromViewSelection.selectionStartLineNumber,
			fromViewSelection.selectionStartColumn
		);
		const fromViewVisibleColumn = CursorColumns.visibleColumnFromColumn2(
			config,
			model,
			fromViewPosition
		);
		return ColumnSelection._columnSelect(
			config,
			model,
			fromViewPosition.lineNumber,
			fromViewVisibleColumn,
			toViewLineNumber,
			toViewVisualColumn
		);
	}
	...
	public static columnSelectRight(
		...
		let minViewLineNumber = Math.min(
			cursor.position.lineNumber,
			toViewLineNumber
		);
		let maxViewLineNumber = Math.max(
			cursor.position.lineNumber,
			toViewLineNumber
		);
		for (
			...
			let lineMaxVisualViewColumn = CursorColumns.visibleColumnFromColumn2(
				config,
				model,
