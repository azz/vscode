...
export class CursorMoveCommands {
	...
	private static _moveToLineStart(
		...
		const firstNonBlankColumn = context.viewModel.getLineFirstNonWhitespaceColumn(
	...
	private static _moveToLineEnd(
		...
		const viewModelMaxColumn = context.viewModel.getLineMaxColumn(
			viewStatePosition.lineNumber
		);
		const isEndOfViewLine = viewStatePosition.column === viewModelMaxColumn;

		const modelStatePosition = cursor.modelState.position;
		const modelMaxColumn = context.model.getLineMaxColumn(
		...
		if (context.model.hasEditableRange()) {
			...
			if (!selection.equalsRange(editableRange)) {
				...
				return CursorState.fromModelState(
					new SingleCursorState(
						new Range(
							editableRange.startLineNumber,
							editableRange.startColumn,
							editableRange.startLineNumber,
							editableRange.startColumn
						),
						0,
	...
	public static line(
		...
		inSelectionMode: boolean,
		_position: IPosition,
		_viewPosition: IPosition
	): CursorState {
		const position = context.model.validatePosition(_position);
		const viewPosition = _viewPosition
			? context.validateViewPosition(
					new Position(_viewPosition.lineNumber, _viewPosition.column),
					position
				)
		...
		const enteringLineNumber = cursor.modelState.selectionStart.getStartPosition()
			.lineNumber;

		if (position.lineNumber < enteringLineNumber) {
			return CursorState.fromViewState(
				cursor.viewState.move(
					cursor.modelState.hasSelection(),
			...
			if (selectToViewLineNumber > lineCount) {
				...
				selectToViewColumn = context.viewModel.getLineMaxColumn(
	...
	public static move(
		context: CursorContext,
		cursors: CursorState[],
		args: CursorMove.ParsedArguments
	): CursorState[] {
		...
		switch (args.direction) {
			...
			case CursorMove.Direction.ViewPortTop: {
				...
				const modelColumn = context.model.getLineFirstNonWhitespaceColumn(
					modelLineNumber
				);
				return [
	...
	public static findPositionInViewportIfOutside(
		...
		if (
			...
			const viewColumn = context.viewModel.getLineFirstNonWhitespaceColumn(
				viewLineNumber
			);
			return this._moveToViewPosition(
				context,
				cursor,
				inSelectionMode,
				viewLineNumber,
	...
	private static _moveHalfLineLeft(
		...
		for (let i = 0, len = cursors.length; i < len; i++) {
			...
			const halfLine = Math.round(
				context.viewModel.getLineContent(viewLineNumber).length / 2
			);
			result[i] = CursorState.fromViewState(
				MoveOperations.moveLeft(
					context.config,
	...
	private static _moveToViewPosition(
		...
		return CursorState.fromViewState(
			cursor.viewState.move(inSelectionMode, toViewLineNumber, toViewColumn, 0)
		);
	}
	...
	private static _moveToViewCenterColumn(
		...
		for (let i = 0, len = cursors.length; i < len; i++) {
			...
			const viewColumn = Math.round(
				(context.viewModel.getLineMaxColumn(viewLineNumber) +
					context.viewModel.getLineMinColumn(viewLineNumber)) /
					2
			);
			result[i] = this._moveToViewPosition(
				context,
	...
	private static _moveToViewLastNonWhitespaceColumn(
		...
		for (let i = 0, len = cursors.length; i < len; i++) {
			const cursor = cursors[i];
			const viewLineNumber = cursor.viewState.position.lineNumber;
			const viewColumn = context.viewModel.getLineLastNonWhitespaceColumn(
				viewLineNumber
			);
			result[i] = this._moveToViewPosition(
				context,
