...
export class TypeOperations {
	...
	public static shiftIndent(
		...
		count = count || 1;
		let desiredIndentCount = ShiftCommand.shiftIndentCount(
			indentation,
	...
	public static unshiftIndent(
		...
		count = count || 1;
		let desiredIndentCount = ShiftCommand.unshiftIndentCount(
			indentation,
			indentation.length + count,
			config.tabSize
		);
		...
		for (let i = 0, len = selections.length; i < len; i++) {
			...
			if (
				...
				selection.startColumn ===
					model.getLineMinColumn(selection.startLineNumber) &&
				selection.endColumn ===
	...
	private static _goodIndentForLine(
		...
		let expectedIndentAction = LanguageConfigurationRegistry.getGoodIndentActionForLine(
			model,
			lineNumber
		);
	...
	private static _replaceJumpToNextIndent(
		...
		if (config.insertSpaces) {
			let visibleColumnFromColumn = CursorColumns.visibleColumnFromColumn2(
				config,
				model,
				position
			);
	...
	public static replacePreviousChar(
		config: CursorConfiguration,
		model: ITokenizedModel,
		selections: Selection[],
		txt: string,
	...
	private static _enter(
		...
		if (!r.ignoreCurrentLine) {
			...
			if (goodIndent !== null && goodIndent === r.indentation) {
				...
				if (
					...
					beforeText =
						config.normalizeIndentation(goodIndent) +
						lineText.substring(indentation.length, range.startColumn - 1);
		...
		if (enterAction.removeText) {
			indentation = indentation.substring(
				0,
				indentation.length - enterAction.removeText
			);
		}
		...
		if (enterAction.indentAction === IndentAction.None) {
			...
			return TypeOperations._typeCommand(
				...
				beforeText +
					'\n' +
					config.normalizeIndentation(indentation + enterAction.appendText),
				keepPosition
			);
			...
			let increasedIndent = config.normalizeIndentation(
				indentation + enterAction.appendText
			);
			...
			return TypeOperations._typeCommand(
				...
				beforeText +
					...
					config.normalizeIndentation(
						actualIndentation + enterAction.appendText
					),
				keepPosition
			);
		}
	...
	private static _isAutoClosingCloseCharType(
		config: CursorConfiguration,
		model: ITokenizedModel,
		selections: Selection[],
		ch: string
		...
		if (
			...
			!config.autoClosingPairsClose.hasOwnProperty(ch)
		) {
			return false;
		}
	...
	private static _runAutoClosingCloseCharType(
		config: CursorConfiguration,
		model: ITokenizedModel,
		selections: Selection[],
		ch: string
	...
	private static _isAutoClosingOpenCharType(
		config: CursorConfiguration,
		model: ITokenizedModel,
		selections: Selection[],
		ch: string
		...
		if (
			!config.autoClosingBrackets ||
			!config.autoClosingPairsOpen.hasOwnProperty(ch)
		) {
			return false;
		}
		...
		for (let i = 0, len = selections.length; i < len; i++) {
			...
			if (characterAfter) {
				...
				for (let otherCloseBrace in config.autoClosingPairsClose) {
					const otherBraceIsSymmetric =
						config.autoClosingPairsOpen[otherCloseBrace] === otherCloseBrace;
			...
			try {
				shouldAutoClosePair = LanguageConfigurationRegistry.shouldAutoClosePair(
					ch,
					lineTokens,
					position.column
				);
	...
	private static _runAutoClosingOpenCharType(
		...
		for (let i = 0, len = selections.length; i < len; i++) {
			...
			commands[i] = new ReplaceCommandWithOffsetCursorState(
				selection,
				ch + closeCharacter,
				0,
	...
	private static _isSurroundSelectionType(
		...
		for (let i = 0, len = selections.length; i < len; i++) {
			...
			for (
				...
				const startIndex = lineNumber === selection.startLineNumber
					? selection.startColumn - 1
					: 0;
				const endIndex = lineNumber === selection.endLineNumber
	...
	private static _typeInterceptorElectricChar(
		...
		try {
			electricAction = LanguageConfigurationRegistry.onElectricCharacter(
				ch,
				lineTokens,
				position.column
			);
		...
		if (electricAction.appendText) {
			const command = new ReplaceCommandWithOffsetCursorState(
				selections,
				ch + electricAction.appendText,
				0,
				-electricAction.appendText.length
			);
