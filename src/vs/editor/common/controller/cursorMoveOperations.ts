...
export class MoveOperations {
	...
	public static moveLeft(
		...
		if (cursor.hasSelection() && !inSelectionMode) {
			...
			let r = MoveOperations.left(
				config,
				model,
				cursor.position.lineNumber,
				cursor.position.column - (noOfColumns - 1)
			);
	...
	public static down(
		...
		if (lineNumber > lineCount) {
			...
			column = CursorColumns.columnFromVisibleColumn2(
				config,
				model,
				lineNumber,
	...
	public static translateDown(
		...
		let selectionStart = MoveOperations.down(
			config,
			model,
		...
		let position = MoveOperations.down(
			config,
			model,
	...
	public static translateUp(
		...
		let selectionStart = MoveOperations.up(
			config,
			model,
			selection.selectionStartLineNumber,
			selection.selectionStartColumn,
		...
		let position = MoveOperations.up(
			config,
			model,
			selection.positionLineNumber,
			selection.positionColumn,
