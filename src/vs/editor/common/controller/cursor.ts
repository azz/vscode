...
export class Cursor extends viewEvents.ViewEventEmitter implements ICursors {
	...
	public readonly onDidChange: Event<CursorStateChangedEvent> = this
		._onDidChange.event;
	...
	constructor(
		...
		this._register(
			this._model.addBulkListener(events => {
				if (this._isHandling) {
					return;
				}
				...
				for (let i = 0, len = events.length; i < len; i++) {
					...
					if (eventType === TextModelEventType.ModelRawContentChanged2) {
						...
						hadFlushEvent =
							hadFlushEvent ||
							rawChangeEvent.containsEvent(RawContentChangedType.Flush);
					}
				}
		...
		this._register(
			viewModel.addEventListener((events: viewEvents.ViewEvent[]) => {
	...
	public getColumnSelectData(): IColumnSelectData {
		...
		return {
			...
			toViewVisualColumn: CursorColumns.visibleColumnFromColumn2(
				this.context.config,
	...
	private _emitStateChangedIfNecessary(
		...
		if (this._model.hasEditableRange()) {
			...
			if (
				!editableRange.containsPosition(
					newState.cursorState[0].modelState.position
				)
			) {
		...
		this._emit([
			new viewEvents.ViewCursorStateChangedEvent(
				viewSelections,
				isInEditableRange,
				screenReaderMessage
			)
		...
		this._onDidChange.fire(
			new CursorStateChangedEvent(selections, source || 'keyboard', reason)
		);
	...
	public trigger(source: string, handlerId: string, payload: any): void {
		...
		try {
			switch (handlerId) {
				...
				case H.ReplacePreviousChar:
					this._replacePreviousChar(
						<string>payload.text,
						<number>payload.replaceCharCnt
					);
					break;
		...
		if (
			this._emitStateChangedIfNecessary(source, cursorChangeReason, oldState)
		) {
			this._revealRange(
				RevealTarget.Primary,
	...
	private _replacePreviousChar(text: string, replaceCharCnt: number): void {
		this._executeEditOperation(
			TypeOperations.replacePreviousChar(
				this.context.config,
				this.context.model,
				this.getSelections(),
				text,
	...
	private _externalExecuteCommand(command: editorCommon.ICommand): void {
		...
		this._executeEditOperation(
			new EditOperationResult([command], {
	...
	private _externalExecuteCommands(commands: editorCommon.ICommand[]): void {
		this._executeEditOperation(
			new EditOperationResult(commands, {
				shouldPushStackElementBefore: false,
				shouldPushStackElementAfter: false
			})
		);
...
class CommandExecutor {
	...
	private static _innerExecuteCommands(
		...
		const selectionsAfter = ctx.model.pushEditOperations(
			...
			(
				...
				const minorBasedSorter = (
					a: editorCommon.IIdentifiedSingleEditOperation,
					...
					if (groupedInverseEditOperations[i].length > 0) {
						...
						cursorSelections[i] = commands[i].computeCursorState(ctx.model, {
							...
							getTrackedSelection: (id: string) => {
								...
								const selectionStartMarker = ctx.model._getMarker(
									ctx.selectionStartMarkers[idx]
								);
								const positionMarker = ctx.model._getMarker(
									ctx.positionMarkers[idx]
								);
								return new Selection(
									selectionStartMarker.lineNumber,
									selectionStartMarker.column,
									positionMarker.lineNumber,
									positionMarker.column
								);
							}
						});
					} else {
	...
	private static _getEditOperationsFromCommand(
		...
		const trackSelection = (
			...
			ctx.selectionStartMarkers[l] = ctx.model._addMarker(
				0,
				selection.selectionStartLineNumber,
				selection.selectionStartColumn,
				selectionMarkerStickToPreviousCharacter
			);
			ctx.positionMarkers[l] = ctx.model._addMarker(
				0,
		...
		try {
			...
			e.friendlyMessage = nls.localize(
				'corrupt.commands',
	...
	private static _getLoserCursorMap(
		...
		for (let i = 1; i < operations.length; i++) {
			...
			if (
				previousOp.range
					.getStartPosition()
					.isBefore(currentOp.range.getEndPosition())
			) {
