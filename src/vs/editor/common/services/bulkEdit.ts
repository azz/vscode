...
class EditTask implements IDisposable {
	...
	public apply(): void {
		if (this._edits.length > 0) {
			this._edits = this._edits
				...
				.sort((a, b) => {
					let ret = Range.compareRangesUsingStarts(
						a.value.range,
						b.value.range
					);
					if (ret === 0) {
			...
			this._model.pushEditOperations(
				...
				this._edits,
				edits => this._getEndCursorSelections(edits)
			);
		}
	...
	private _getEndCursorSelections(
...
class SourceModelEditTask extends EditTask {
	...
	constructor(
		modelReference: IReference<ITextEditorModel>,
	...
	public prepare(): TPromise<BulkEditModel> {
		...
		forEach(this._edits, entry => {
			const promise = this._textModelResolverService
				.createModelReference(URI.parse(entry.key))
				.then(ref => {
					...
					if (
						this._sourceModel &&
						textEditorModel.uri.toString() === this._sourceModel.toString()
					) {
						this._sourceModelTask = new SourceModelEditTask(
							ref,
...
export function createBulkEdit(
	textModelResolverService: ITextModelResolverService,
	editor?: ICommonCodeEditor,
	fileService?: IFileService
): BulkEdit {
