...
export abstract class CommonEditorConfiguration extends Disposable
	implements editorCommon.IConfiguration {
	...
	private _onDidChange = this._register(
		new Emitter<editorOptions.IConfigurationChangedEvent>()
	);
	public onDidChange: Event<editorOptions.IConfigurationChangedEvent> = this
		._onDidChange.event;
	...
	constructor(options: editorOptions.IEditorOptions) {
		...
		this._rawOptions.scrollbar = objects.mixin(
			{},
			this._rawOptions.scrollbar || {}
		);
		this._rawOptions.minimap = objects.mixin(
			{},
			this._rawOptions.minimap || {}
		);
		this._rawOptions.find = objects.mixin({}, this._rawOptions.find || {});

		this._validatedOptions = editorOptions.EditorOptionsValidator.validate(
			this._rawOptions,
			EDITOR_DEFAULTS
		);
		this.editor = null;
		...
		this._register(
			EditorZoom.onDidChangeZoomLevel(_ => this._recomputeOptions())
		);
	...
	private _computeInternalOptions(): editorOptions.InternalEditorOptions {
		...
		return editorOptions.InternalEditorOptionsFactory.createInternalEditorOptions(
			env,
			opts
		);
	}
...
const editorConfiguration: IConfigurationNode = {
	...
	properties: {
		...
		'editor.cursorBlinking': {
			...
			default: editorOptions.blinkingStyleToString(
				EDITOR_DEFAULTS.viewInfo.cursorBlinking
			),
			description: nls.localize(
				'cursorBlinking',
		...
		'editor.cursorStyle': {
			...
			default: editorOptions.cursorStyleToString(
				EDITOR_DEFAULTS.viewInfo.cursorStyle
			),
			description: nls.localize(
				'cursorStyle',
