 ...
 * @internal
 */
export function blinkingStyleToString(
...
function _cursorStyleFromString(
	cursorStyle: string,
	defaultValue: TextEditorCursorStyle
...
export class InternalEditorOptions {
	...
	public equals(other: InternalEditorOptions): boolean {
		return (
			...
			InternalEditorOptions._equalsLayoutInfo(
				this.layoutInfo,
				other.layoutInfo
			) &&
			this.fontInfo.equals(other.fontInfo) &&
			InternalEditorOptions._equalsViewOptions(this.viewInfo, other.viewInfo) &&
			InternalEditorOptions._equalsWrappingInfo(
				this.wrappingInfo,
				other.wrappingInfo
			) &&
			InternalEditorOptions._equalsContribOptions(
				this.contribInfo,
				other.contribInfo
			)
		);
	...
	public createChangeEvent(
		...
		return {
			...
			accessibilitySupport:
				this.accessibilitySupport !== newOpts.accessibilitySupport,
			multiCursorModifier:
				this.multiCursorModifier !== newOpts.multiCursorModifier,
			...
			emptySelectionClipboard:
				this.emptySelectionClipboard !== newOpts.emptySelectionClipboard,
			layoutInfo: !InternalEditorOptions._equalsLayoutInfo(
				this.layoutInfo,
				newOpts.layoutInfo
			),
			fontInfo: !this.fontInfo.equals(newOpts.fontInfo),
			viewInfo: !InternalEditorOptions._equalsViewOptions(
				this.viewInfo,
				newOpts.viewInfo
			),
			wrappingInfo: !InternalEditorOptions._equalsWrappingInfo(
				this.wrappingInfo,
				newOpts.wrappingInfo
			),
			contribInfo: !InternalEditorOptions._equalsContribOptions(
				this.contribInfo,
				newOpts.contribInfo
			)
		};
	 ...
	 * @internal
	 */
	private static _equalsLayoutInfo(
		a: EditorLayoutInfo,
		b: EditorLayoutInfo
	): boolean {
		return (
	 ...
	 * @internal
	 */
	private static _equalsOverviewRuler(
		a: OverviewRulerPosition,
		b: OverviewRulerPosition
	): boolean {
		return (
			a.width === b.width &&
			a.height === b.height &&
	 ...
	 * @internal
	 */
	private static _equalsViewOptions(
		a: InternalEditorViewOptions,
		b: InternalEditorViewOptions
	): boolean {
		return (
	 ...
	 * @internal
	 */
	private static _equalsScrollbarOptions(
		a: InternalEditorScrollbarOptions,
	 ...
	 * @internal
	 */
	private static _equalsMinimapOptions(
		a: InternalEditorMinimapOptions,
		b: InternalEditorMinimapOptions
	): boolean {
		return (
			a.enabled === b.enabled &&
	...
	private static _equalFindOptions(
		a: InternalEditorFindOptions,
		b: InternalEditorFindOptions
	): boolean {
		return (
	 ...
	 * @internal
	 */
	private static _equalsWrappingInfo(
		a: EditorWrappingInfo,
		b: EditorWrappingInfo
	): boolean {
		return (
	 ...
	 * @internal
	 */
	private static _equalsContribOptions(
		a: EditorContribOptions,
		b: EditorContribOptions
	): boolean {
		return (
			a.selectionClipboard === b.selectionClipboard &&
			a.hover === b.hover &&
			a.contextmenu === b.contextmenu &&
			InternalEditorOptions._equalsQuickSuggestions(
				a.quickSuggestions,
				b.quickSuggestions
			) &&
			...
			a.acceptSuggestionOnCommitCharacter ===
				b.acceptSuggestionOnCommitCharacter &&
	...
	private static _equalsQuickSuggestions(
		a: boolean | { other: boolean; comments: boolean; strings: boolean },
		b: boolean | { other: boolean; comments: boolean; strings: boolean }
	): boolean {
		if (typeof a === 'boolean') {
			if (typeof b !== 'boolean') {
				return false;
...
function _wrappingIndentFromString(
	wrappingIndent: string,
...
function _cursorBlinkingStyleFromString(
	...
	defaultValue: TextEditorCursorBlinkingStyle
...
export class EditorOptionsValidator {
	...
	public static validate(
		opts: IEditorOptions,
		defaults: IValidatedEditorOptions
		...
		if (typeof opts.multiCursorModifier === 'string') {
			if (opts.multiCursorModifier === 'ctrlCmd') {
				configuredMulticursorModifier = platform.isMacintosh
					? 'metaKey'
					: 'ctrlKey';
			} else {
		...
		const multiCursorModifier = _stringSet<
			'altKey' | 'metaKey' | 'ctrlKey'
		>(configuredMulticursorModifier, defaults.multiCursorModifier, [
			'altKey',
			'metaKey',
			'ctrlKey'
		]);
		...
		return {
			...
			lineNumbersMinChars: _clampedInt(
				opts.lineNumbersMinChars,
				defaults.lineNumbersMinChars,
				1,
				10
			),
			lineDecorationsWidth: typeof opts.lineDecorationsWidth === 'undefined'
				? defaults.lineDecorationsWidth
				: opts.lineDecorationsWidth,
			readOnly: _boolean(opts.readOnly, defaults.readOnly),
			mouseStyle: _stringSet<
				'text' | 'default' | 'copy'
			>(opts.mouseStyle, defaults.mouseStyle, ['text', 'default', 'copy']),
			disableTranslate3d: _boolean(
				opts.disableTranslate3d,
				defaults.disableTranslate3d
			),
			automaticLayout: _boolean(opts.automaticLayout, defaults.automaticLayout),
			wordWrap: wordWrap,
			wordWrapColumn: _clampedInt(
				opts.wordWrapColumn,
				defaults.wordWrapColumn,
				1,
				Constants.MAX_SAFE_SMALL_INTEGER
			),
			wordWrapMinified: _boolean(
				opts.wordWrapMinified,
				defaults.wordWrapMinified
			),
			wrappingIndent: _wrappingIndentFromString(
				opts.wrappingIndent,
				defaults.wrappingIndent
			),
			wordWrapBreakBeforeCharacters: _string(
				opts.wordWrapBreakBeforeCharacters,
				defaults.wordWrapBreakBeforeCharacters
			),
			wordWrapBreakAfterCharacters: _string(
				opts.wordWrapBreakAfterCharacters,
				defaults.wordWrapBreakAfterCharacters
			),
			wordWrapBreakObtrusiveCharacters: _string(
				opts.wordWrapBreakObtrusiveCharacters,
				defaults.wordWrapBreakObtrusiveCharacters
			),
			autoClosingBrackets: _boolean(
				opts.autoClosingBrackets,
				defaults.autoClosingBrackets
			),
			dragAndDrop: _boolean(opts.dragAndDrop, defaults.dragAndDrop),
			emptySelectionClipboard: _boolean(
				opts.emptySelectionClipboard,
				defaults.emptySelectionClipboard
			),
			useTabStops: _boolean(opts.useTabStops, defaults.useTabStops),
			multiCursorModifier: multiCursorModifier,
			accessibilitySupport: _stringSet<
				'auto' | 'on' | 'off'
			>(opts.accessibilitySupport, defaults.accessibilitySupport, [
				'auto',
				'on',
				'off'
			]),
	...
	private static _sanitizeScrollbarOpts(
		opts: IEditorScrollbarOptions,
		defaults: InternalEditorScrollbarOptions,
		mouseWheelScrollSensitivity: number
	): InternalEditorScrollbarOptions {
		if (typeof opts !== 'object') {
			return defaults;
		}
		const horizontalScrollbarSize = _clampedInt(
			opts.horizontalScrollbarSize,
			defaults.horizontalScrollbarSize,
			0,
			1000
		);
		const verticalScrollbarSize = _clampedInt(
			opts.verticalScrollbarSize,
			defaults.verticalScrollbarSize,
			0,
			1000
		);
		return {
			...
			verticalHasArrows: _boolean(
				opts.verticalHasArrows,
				defaults.verticalHasArrows
			),
			horizontalHasArrows: _boolean(
				opts.horizontalHasArrows,
				defaults.horizontalHasArrows
			),

			horizontalScrollbarSize: horizontalScrollbarSize,
			horizontalSliderSize: _clampedInt(
			...
			verticalSliderSize: _clampedInt(
			...
			handleMouseWheel: _boolean(
				opts.handleMouseWheel,
				defaults.handleMouseWheel
			),
			mouseWheelScrollSensitivity: mouseWheelScrollSensitivity
		};
	}

	private static _sanitizeMinimapOpts(
		opts: IEditorMinimapOptions,
		defaults: InternalEditorMinimapOptions
		...
		return {
			...
			showSlider: _stringSet<
				'always' | 'mouseover'
			>(opts.showSlider, defaults.showSlider, ['always', 'mouseover']),
			renderCharacters: _boolean(
				opts.renderCharacters,
				defaults.renderCharacters
			),
			maxColumn: _clampedInt(opts.maxColumn, defaults.maxColumn, 1, 10000)
		};
	}

	private static _santizeFindOpts(
		opts: IEditorFindOptions,
		defaults: InternalEditorFindOptions
		...
		return {
			seedSearchStringFromSelection: _boolean(
				opts.seedSearchStringFromSelection,
				defaults.seedSearchStringFromSelection
			),
			autoFindInSelection: _boolean(
				opts.autoFindInSelection,
				defaults.autoFindInSelection
			)
		};
	}

	private static _sanitizeViewInfo(
		opts: IEditorOptions,
		defaults: InternalEditorViewOptions
		...
		let renderCustomLineNumbers: (lineNumber: number) => string =
			defaults.renderCustomLineNumbers;
		...
		const disableMonospaceOptimizations =
			_boolean(
				opts.disableMonospaceOptimizations,
				defaults.disableMonospaceOptimizations
			) || fontLigatures;
		...
		let renderWhitespace = opts.renderWhitespace;
		{
			...
			renderWhitespace = _stringSet<
				'none' | 'boundary' | 'all'
			>(opts.renderWhitespace, defaults.renderWhitespace, [
				'none',
				'boundary',
				'all'
			]);
		...
		let renderLineHighlight = opts.renderLineHighlight;
		{
			...
			renderLineHighlight = _stringSet<
				'none' | 'gutter' | 'line' | 'all'
			>(opts.renderLineHighlight, defaults.renderLineHighlight, [
				'none',
				'gutter',
				'line',
				'all'
		...
		const mouseWheelScrollSensitivity = _float(
		...
		return {
			...
			selectOnLineNumbers: _boolean(
				opts.selectOnLineNumbers,
				defaults.selectOnLineNumbers
			),
			glyphMargin: _boolean(opts.glyphMargin, defaults.glyphMargin),
			revealHorizontalRightPadding: _clampedInt(
				opts.revealHorizontalRightPadding,
				defaults.revealHorizontalRightPadding,
				0,
				1000
			),
			roundedSelection: _boolean(
				opts.roundedSelection,
				defaults.roundedSelection
			),
			overviewRulerLanes: _clampedInt(
				opts.overviewRulerLanes,
				defaults.overviewRulerLanes,
				0,
				3
			),
			overviewRulerBorder: _boolean(
				opts.overviewRulerBorder,
				defaults.overviewRulerBorder
			),
			cursorBlinking: _cursorBlinkingStyleFromString(
				opts.cursorBlinking,
				defaults.cursorBlinking
			),
			mouseWheelZoom: _boolean(opts.mouseWheelZoom, defaults.mouseWheelZoom),
			cursorStyle: _cursorStyleFromString(
				opts.cursorStyle,
				defaults.cursorStyle
			),
			hideCursorInOverviewRuler: _boolean(
				opts.hideCursorInOverviewRuler,
				defaults.hideCursorInOverviewRuler
			),
			scrollBeyondLastLine: _boolean(
				opts.scrollBeyondLastLine,
				defaults.scrollBeyondLastLine
			),
			stopRenderingLineAfter: _clampedInt(
				opts.stopRenderingLineAfter,
				defaults.stopRenderingLineAfter,
				-1,
				Constants.MAX_SAFE_SMALL_INTEGER
			),
			renderWhitespace: renderWhitespace,
			renderControlCharacters: _boolean(
				opts.renderControlCharacters,
				defaults.renderControlCharacters
			),
			fontLigatures: fontLigatures,
			renderIndentGuides: _boolean(
				opts.renderIndentGuides,
				defaults.renderIndentGuides
			),
			renderLineHighlight: renderLineHighlight,
			scrollbar: scrollbar,
			minimap: minimap,
			fixedOverflowWidgets: _boolean(
				opts.fixedOverflowWidgets,
				defaults.fixedOverflowWidgets
			)
		};
	}

	private static _sanitizeContribInfo(
		opts: IEditorOptions,
		defaults: EditorContribOptions
		...
		if (typeof opts.quickSuggestions === 'object') {
			...
			quickSuggestions = _boolean(
				opts.quickSuggestions,
				defaults.quickSuggestions
			);
		}
		const find = this._santizeFindOpts(opts.find, defaults.find);
		return {
			selectionClipboard: _boolean(
				opts.selectionClipboard,
				defaults.selectionClipboard
			),
			hover: _boolean(opts.hover, defaults.hover),
			contextmenu: _boolean(opts.contextmenu, defaults.contextmenu),
			quickSuggestions: quickSuggestions,
			quickSuggestionsDelay: _clampedInt(
				opts.quickSuggestionsDelay,
				defaults.quickSuggestionsDelay,
				Constants.MIN_SAFE_SMALL_INTEGER,
				Constants.MAX_SAFE_SMALL_INTEGER
			),
			parameterHints: _boolean(opts.parameterHints, defaults.parameterHints),
			iconsInSuggestions: _boolean(
				opts.iconsInSuggestions,
				defaults.iconsInSuggestions
			),
			...
			suggestOnTriggerCharacters: _boolean(
				opts.suggestOnTriggerCharacters,
				defaults.suggestOnTriggerCharacters
			),
			acceptSuggestionOnEnter: _stringSet<
				'on' | 'smart' | 'off'
			>(opts.acceptSuggestionOnEnter, defaults.acceptSuggestionOnEnter, [
				'on',
				'smart',
				'off'
			]),
			acceptSuggestionOnCommitCharacter: _boolean(
				opts.acceptSuggestionOnCommitCharacter,
				defaults.acceptSuggestionOnCommitCharacter
			),
			snippetSuggestions: _stringSet<
				'top' | 'bottom' | 'inline' | 'none'
			>(opts.snippetSuggestions, defaults.snippetSuggestions, [
				'top',
				'bottom',
				'inline',
				'none'
			]),
			wordBasedSuggestions: _boolean(
				opts.wordBasedSuggestions,
				defaults.wordBasedSuggestions
			),
			suggestFontSize: _clampedInt(
				opts.suggestFontSize,
				defaults.suggestFontSize,
				0,
				1000
			),
			suggestLineHeight: _clampedInt(
				opts.suggestLineHeight,
				defaults.suggestLineHeight,
				0,
				1000
			),
			selectionHighlight: _boolean(
				opts.selectionHighlight,
				defaults.selectionHighlight
			),
			occurrencesHighlight: _boolean(
				opts.occurrencesHighlight,
				defaults.occurrencesHighlight
			),
			codeLens:
				_boolean(opts.codeLens, defaults.codeLens) &&
					_boolean(opts.referenceInfos, true),
			folding: _boolean(opts.folding, defaults.folding),
			showFoldingControls: _stringSet<
				'always' | 'mouseover'
			>(opts.showFoldingControls, defaults.showFoldingControls, [
				'always',
				'mouseover'
			]),
...
export class InternalEditorOptionsFactory {
	private static _tweakValidatedOptions(
		opts: IValidatedEditorOptions,
		accessibilitySupport: platform.AccessibilitySupport
	): IValidatedEditorOptions {
		const accessibilityIsOn =
			accessibilitySupport === platform.AccessibilitySupport.Enabled;
		const accessibilityIsOff =
		...
		return {
			...
			viewInfo: {
				...
				revealHorizontalRightPadding:
					opts.viewInfo.revealHorizontalRightPadding,
				roundedSelection: accessibilityIsOn
					? false
					: opts.viewInfo.roundedSelection, // DISABLED WHEN SCREEN READER IS ATTACHED
				...
				renderWhitespace: accessibilityIsOn
					? 'none'
					: opts.viewInfo.renderWhitespace, // DISABLED WHEN SCREEN READER IS ATTACHED
				renderControlCharacters: accessibilityIsOn
					? false
					: opts.viewInfo.renderControlCharacters, // DISABLED WHEN SCREEN READER IS ATTACHED
				fontLigatures: accessibilityIsOn ? false : opts.viewInfo.fontLigatures, // DISABLED WHEN SCREEN READER IS ATTACHED
				renderIndentGuides: accessibilityIsOn
					? false
					: opts.viewInfo.renderIndentGuides, // DISABLED WHEN SCREEN READER IS ATTACHED
				renderLineHighlight: accessibilityIsOn
					? 'none'
					: opts.viewInfo.renderLineHighlight, // DISABLED WHEN SCREEN READER IS ATTACHED
			...
			contribInfo: {
				...
				acceptSuggestionOnCommitCharacter:
					opts.contribInfo.acceptSuggestionOnCommitCharacter,
				...
				selectionHighlight: accessibilityIsOn
					? false
					: opts.contribInfo.selectionHighlight, // DISABLED WHEN SCREEN READER IS ATTACHED
				occurrencesHighlight: accessibilityIsOn
					? false
					: opts.contribInfo.occurrencesHighlight, // DISABLED WHEN SCREEN READER IS ATTACHED
				...
				matchBrackets: accessibilityIsOn
					? false
					: opts.contribInfo.matchBrackets, // DISABLED WHEN SCREEN READER IS ATTACHED
	...
	public static createInternalEditorOptions(
		...
		if (
			typeof opts.lineDecorationsWidth === 'string' &&
			/^\d+(\.\d+)?ch$/.test(opts.lineDecorationsWidth)
		) {
			const multiple = parseFloat(
				opts.lineDecorationsWidth.substr(
					0,
					opts.lineDecorationsWidth.length - 2
				)
			);
			lineDecorationsWidth =
				multiple * env.fontInfo.typicalHalfwidthCharacterWidth;
		} else {
		...
		} = null;
		{
			...
			if (accessibilitySupport === platform.AccessibilitySupport.Enabled) {
				...
				bareWrappingInfo = {
					isWordWrapMinified: false,
					isViewportWrapping: true,
					wrappingColumn: Math.min(
						Math.max(1, layoutInfo.viewportColumn),
						wordWrapColumn
					)
				};
		...
		return new InternalEditorOptions({
			canUseTranslate3d: opts.disableTranslate3d
				? false
				: env.canUseTranslate3d,
			pixelRatio: env.pixelRatio,
			...
			dragAndDrop: opts.dragAndDrop,
			emptySelectionClipboard:
				opts.emptySelectionClipboard && env.emptySelectionClipboard,
...
export class EditorLayoutProvider {
	public static compute(_opts: IEditorLayoutProviderOpts): EditorLayoutInfo {
		...
		const remainingWidth =
			outerWidth - glyphMarginWidth - lineNumbersWidth - lineDecorationsWidth;
		...
		if (!minimap) {
			...
			if (pixelRatio >= 2) {
				renderMinimap = minimapRenderCharacters
					? RenderMinimap.Large
					: RenderMinimap.LargeBlocks;
			...
			minimapWidth = Math.max(
				0,
				Math.floor(
					(remainingWidth - verticalScrollbarWidth) *
						minimapCharWidth /
		...
		const viewportColumn = Math.max(
			1,
			Math.floor(
				(contentWidth - verticalScrollbarWidth) / typicalHalfwidthCharacterWidth
			)
		);
