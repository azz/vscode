...
} from 'vs/editor/common/modes/monarch/monarchTypes';
...
function createGuard(
	...
	if ((op === '~' || op === '!~') && /^(\w|\|)*$/.test(pat)) {
		var inWords = objects.createKeywordMatcher(
			pat.split('|'),
			lexer.ignoreCase
		);
		tester = function(s) {
		...
		if (pat.indexOf('$') < 0) {
			...
			tester = function(s, id, matches, state) {
				var re = compileRegExp(
					lexer,
					'^' +
						monarchCommon.substituteMatches(lexer, pat, id, matches, state) +
						'$'
				);
				return re.test(s);
			};
		...
		if (pat.indexOf('$') < 0) {
			...
			tester = function(s, id, matches, state, eos) {
				var patexp = monarchCommon.substituteMatches(
					lexer,
					patx,
					id,
					matches,
						...
						if (next.indexOf('$') < 0) {
							...
							if (
								!monarchCommon.stateExists(
									lexer,
									monarchCommon.substituteMatches(lexer, next, '', [], '')
								)
							) {
								monarchCommon.throwError(
									lexer,
			...
			test: function(id, matches, state, eos) {
				...
				for (idx in cases) {
					if (cases.hasOwnProperty(idx)) {
						var didmatch =
							!cases[idx].test || cases[idx].test(id, matches, state, eos);
						if (didmatch) {
...
export function compile(
	...
	lexer.defaultToken = string(json.defaultToken, 'source', function() {
		monarchCommon.throwError(lexer, "the 'defaultToken' must be a string");
	});
