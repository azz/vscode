...
var getReversedRegexForBracketPair = once<ISimpleInternalBracket, RegExp>(
	input => `${input.open};${input.close}`,
	input => {
		return createOrRegex([
			toReversedString(input.open),
			toReversedString(input.close)
		]);
	}
);
...
export class BracketsUtils {
	...
	public static findPrevBracketInToken(
		reversedBracketRegex: RegExp,
		lineNumber: number,
		lineText: string,
		...
		let reversedTokenText = reversedLineText.substring(
			lineText.length - currentTokenEnd,
			lineText.length - currentTokenStart
		);
	...
	public static findNextBracketInToken(
		bracketRegex: RegExp,
		lineNumber: number,
		lineText: string,
		currentTokenStart: number,
		...
		let currentTokenText = lineText.substring(
			currentTokenStart,
			currentTokenEnd
		);
