...
import {
	...
	IGrammar,
	Registry,
	IEmbeddedLanguagesMap as IEmbeddedLanguagesMap2
...
export class TMScopeRegistry {
	...
	private _onDidEncounterLanguage: Emitter<LanguageId> = new Emitter<
		LanguageId
	>();
	public onDidEncounterLanguage: Event<LanguageId> = this
		._onDidEncounterLanguage.event;
	...
	public register(
		...
		this._scopeNameToLanguageRegistration[
			scopeName
		] = new TMLanguageRegistration(scopeName, filePath, embeddedLanguages);
	}
...
export class MainProcessTextMateSyntax implements ITextMateService {
			...
			for (let i = 0; i < extensions.length; i++) {
				...
				for (let j = 0; j < grammars.length; j++) {
					this._handleGrammarExtensionPointUser(
						extensions[i].description.extensionFolderPath,
						grammars[j],
						extensions[i].collector
					);
				}
			}
		});
	...
	private _updateTheme(): void {
		...
		this._grammarRegistry.setTheme({
			name: colorTheme.label,
			settings: colorTheme.tokenColors
		});
		let colorMap = MainProcessTextMateSyntax._toColorMap(
			this._grammarRegistry.getColorMap()
		);
	...
	private _handleGrammarExtensionPointUser(
		...
		if (
			...
			(typeof syntax.language !== 'string' ||
				!this._modeService.isRegisteredMode(syntax.language))
		) {
			collector.error(
				nls.localize(
					'invalid.language',
		...
		let normalizedAbsolutePath = normalize(
			join(extensionFolderPath, syntax.path)
		);
	...
	private _createGrammar(modeId: string): TPromise<ICreateGrammarResult> {
		...
		let languageRegistration = this._scopeRegistry.getLanguageRegistration(
			scopeName
		);
		if (!languageRegistration) {
			// No TM grammar defined
			return TPromise.wrapError<ICreateGrammarResult>(
				new Error(
					nls.localize(
						'no-tm-grammar',
		...
		return new TPromise<ICreateGrammarResult>((c, e, p) => {
			this._grammarRegistry.loadGrammarWithEmbeddedLanguages(
				scopeName,
				languageId,
				embeddedLanguages,
				(err, grammar) => {
