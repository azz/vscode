...
suite('Decoration Render Options', () => {
	var options: IDecorationRenderOptions = {
		gutterIconPath: URI.parse(
			'https://github.com/Microsoft/vscode/blob/master/resources/linux/code.png'
		),
	...
	function readStyleSheet(styleSheet: HTMLStyleElement): string {
		if ((<any>styleSheet.sheet).rules) {
			return Array.prototype.map
				.call(
					(<any>styleSheet.sheet).rules,
					(r: { cssText: string }) => r.cssText
				)
				.join('\n');
		}
	...
	test('css properties', () => {
		...
		assert(
			sheet.indexOf(
				"background: url('https://github.com/Microsoft/vscode/blob/master/resources/linux/code.png') center center no-repeat;"
			) > 0 ||
				sheet.indexOf(
					'background: url("https://github.com/Microsoft/vscode/blob/master/resources/linux/code.png") center center / contain no-repeat;'
				) > 0
		);
	...
	test('theme color', () => {
		...
		assert.equal(
			sheet,
			'.monaco-editor .ced-example-0 { background-color: rgb(255, 0, 0); }'
		);
		...
		assert.equal(
			sheet,
			'.monaco-editor .ced-example-0 { background-color: rgb(238, 0, 0); border-color: rgb(0, 255, 255); box-sizing: border-box; }'
		);
	...
	test('css properties, gutterIconPaths', () => {
		...
		var s = new CodeEditorServiceImpl(themeServiceMock, styleSheet);
		s.registerDecorationType('example', {
			gutterIconPath: '/Users/foo/bar.png'
		});
		var sheet = readStyleSheet(styleSheet); //.innerHTML || styleSheet.sheet.toString();
		assert(
			sheet.indexOf(
				"background: url('file:///Users/foo/bar.png') center center no-repeat;"
			) > 0 ||
				sheet.indexOf(
					'background: url("file:///Users/foo/bar.png") center center no-repeat;'
				) > 0
		...
		s.registerDecorationType('example', {
			gutterIconPath: 'c:\\files\\miles\\more.png'
		});
		sheet = readStyleSheet(styleSheet);
		...
		s = new CodeEditorServiceImpl(themeServiceMock, styleSheet);
		s.registerDecorationType('example', {
			gutterIconPath: "/Users/foo/b'ar.png"
		});
		sheet = readStyleSheet(styleSheet);
		assert(
			sheet.indexOf(
				"background: url('file:///Users/foo/b%27ar.png') center center no-repeat;"
			) > 0 ||
				sheet.indexOf(
					'background: url("file:///Users/foo/b%27ar.png") center center no-repeat;'
				) > 0
