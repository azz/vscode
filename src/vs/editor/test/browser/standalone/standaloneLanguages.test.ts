...
import {
	...
	ILineTokens,
	IToken
} from 'vs/editor/browser/standalone/standaloneLanguages';
import {
...
suite('TokenizationSupport2Adapter', () => {
	...
	const tokenMetadata =
		languageIdentifier.id << MetadataConsts.LANGUAGEID_OFFSET;
	...
	function testBadTokensProvider(
		...
		const actualModernTokens = adapter.tokenize2(
			'whatever',
			MockState.INSTANCE,
			offsetDelta
		);
	...
	test('tokens always start at index 0 (no offset delta)', () => {
		testBadTokensProvider(
			[{ startIndex: 7, scopes: 'foo' }, { startIndex: 0, scopes: 'bar' }],
			0,
			[
			...
			[
				0,
				tokenMetadata | (0 << MetadataConsts.FOREGROUND_OFFSET),
				0,
				tokenMetadata | (1 << MetadataConsts.FOREGROUND_OFFSET)
			]
		);
	});
	...
	test('tokens always start after each other (no offset delta)', () => {
		testBadTokensProvider(
			...
			[
				...
				tokenMetadata | (1 << MetadataConsts.FOREGROUND_OFFSET),
				5,
				tokenMetadata | (2 << MetadataConsts.FOREGROUND_OFFSET)
			]
		);
	});
