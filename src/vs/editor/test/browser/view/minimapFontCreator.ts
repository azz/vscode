...
for (
	let chCode = Constants.START_CH_CODE;
	chCode <= Constants.END_CH_CODE;
	chCode++
) {
	ctx.fillText(
...
let sampleData = ctx.getImageData(
	0,
	4,
	Constants.SAMPLED_CHAR_WIDTH * Constants.CHAR_COUNT,
	Constants.SAMPLED_CHAR_HEIGHT
);
...
function renderMinimapCharRenderer(
...

	{
		let x2 = createFakeImageData(
			Constants.x2_CHAR_WIDTH * Constants.CHAR_COUNT,
			Constants.x2_CHAR_HEIGHT
		);
	...
	{
		let x1 = createFakeImageData(
			Constants.x1_CHAR_WIDTH * Constants.CHAR_COUNT,
			Constants.x1_CHAR_HEIGHT
		);
		// set the background color
...
(function() {
	...
	for (let charIndex = 0; charIndex < Constants.CHAR_COUNT; charIndex++) {
		...
		for (
			let i = 0;
			i < Constants.x2_CHAR_HEIGHT * Constants.x2_CHAR_WIDTH;
			i++
		) {
			if (i % 2 === 0) {
				r += '\n';
...
(function() {
	...
	for (let charIndex = 0; charIndex < Constants.CHAR_COUNT; charIndex++) {
		...
		for (
			let i = 0;
			i < Constants.x1_CHAR_HEIGHT * Constants.x1_CHAR_WIDTH;
			i++
		) {
			r += '\n';
...
function renderImageData(
	...
	for (var i = 0; i < imageData.height; i++) {
		for (var j = 0; j < imageData.width; j++) {
			...
			output += `<div style="position:absolute;top:${PX_SIZE *
				i}px;left:${PX_SIZE *
				j}px;width:${PX_SIZE}px;height:${PX_SIZE}px;background:rgba(${R},${G},${B},${A /
				256})"></div>`;
		}
	}
