...
suite('LanguageSelector', function() {
	...
	test('score, default schemes', function() {
		...
		assert.equal(
			score({ language: 'farboo', scheme: 'git' }, uri, language),
			10
		);
		assert.equal(score({ language: 'farboo', scheme: '*' }, uri, language), 10);
	...
	test('score, filter', function() {
		...
		assert.equal(
			score({ language: 'farboo', scheme: 'file' }, model.uri, model.language),
			10
		);
		assert.equal(
			score({ language: 'farboo', scheme: 'http' }, model.uri, model.language),
			0
		);

		assert.equal(score({ pattern: '**/*.fb' }, model.uri, model.language), 10);
		assert.equal(
			score({ pattern: '**/*.fb', scheme: 'file' }, model.uri, model.language),
			10
		);
		assert.equal(
			score({ pattern: '**/*.fb' }, URI.parse('foo:bar'), model.language),
			0
		);
		assert.equal(
			score(
		...
		assert.equal(
			score({ language: 'javascript', scheme: 'git' }, doc.uri, doc.langId),
			10
		); // 10;
