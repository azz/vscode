...
import {
	tokenizeToString,
	tokenizeLineToHTML
} from 'vs/editor/common/modes/textToHtmlTokenizer';
...
suite('Editor Modes - textToHtmlTokenizer', () => {
	...
	test('TextToHtmlTokenizer 1', () => {
		...
		let expectedStr = `<div class="monaco-tokenized-source">${toStr(
			expected
		)}</div>`;
	...
	test('tokenizeLineToHTML', () => {
		...
		const lineTokens = [
			...
			new ViewLineToken(5, 1 << MetadataConsts.FOREGROUND_OFFSET >>> 0),
			new ViewLineToken(10, 4 << MetadataConsts.FOREGROUND_OFFSET >>> 0),
			new ViewLineToken(11, 1 << MetadataConsts.FOREGROUND_OFFSET >>> 0),
			new ViewLineToken(
				17,
		...
		assert.equal(
			...
			['<div>', '<span style="color: #00ff00;">hello</span>', '</div>'].join('')
		);

		assert.equal(
			tokenizeLineToHTML(text, lineTokens, colorMap, 6, 9, 4),
			['<div>', '<span style="color: #00ff00;">ell</span>', '</div>'].join('')
		);
	});
});
