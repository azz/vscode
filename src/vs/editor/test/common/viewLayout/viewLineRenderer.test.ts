...
suite('viewLineRenderer.renderLine', () => {
	function createPart(endIndex: number, foreground: number): ViewLineToken {
		return new ViewLineToken(
			endIndex,
			foreground << MetadataConsts.FOREGROUND_OFFSET >>> 0
		);
	}

	function assertCharacterReplacement(
		lineContent: string,
		tabSize: number,
		expected: string,
		expectedCharOffsetInPart: number[][],
		expectedPartLengts: number[]
	): void {
		let _actual = renderViewLine(
			new RenderLineInput(
				false,
				lineContent,
				false,
	...
	test('typical line', () => {
		...
		assertPartLengths(_actual.characterMapping, [
			4,
			4,
			6,
			1,
	...
	test('issue #6885: Splits large tokens', () => {
		...
		let _lineText =
			'This is just a long line that contains very interesting text. This is just a long line that contains very interesting text.';

		function assertSplitsTokens(
			message: string,
			lineText: string,
			expectedOutput: string[]
		): void {
			let lineParts = [createPart(lineText.length, 1)];
			let actual = renderViewLine(
				new RenderLineInput(
					false,
					lineText,
					false,
	...
	function assertCharacterMapping(
		...
		for (let partIndex = 0; partIndex < expected.length; partIndex++) {
			...
			for (let i = 0; i < part.length; i++) {
				...
				// here
				let actualOffset = actual.partDataToCharOffset(
					partIndex,
...
suite('viewLineRenderer.renderLine 2', () => {
	...
	test('createLineParts simple', () => {
		testCreateLineParts(
			...
			0,
			'none',
			['<span>', '<span class="mtk1">Hello&nbsp;world!</span>', '</span>'].join(
				''
			)
		);
	});
	...
	function createTestGetColumnOfLinePartOffset(
		lineContent: string,
		tabSize: number,
		parts: ViewLineToken[],
		expectedPartLengths: number[]
	): (
		partIndex: number,
		partLength: number,
		offset: number,
		...
		let renderLineOutput = renderViewLine(
			new RenderLineInput(
				false,
				lineContent,
				false,
		...
		return (
			...
			let charOffset = renderLineOutput.characterMapping.partDataToCharOffset(
				partIndex,
				partLength,
				offset
			);
			...
			assert.equal(
				actual,
				expected,
				'getColumnOfLinePartOffset for ' + partIndex + ' @ ' + offset
			);
		};
	}
