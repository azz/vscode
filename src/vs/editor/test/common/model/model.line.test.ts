...
function assertLineTokens(_actual: LineTokens, _expected: TestToken[]): void {
	let expected = ViewLineTokenFactory.inflateArr(
		TestToken.toTokens(_expected),
		_actual.getLineLength()
	);
...
suite('Editor Model - modelLine.applyEdits text', () => {
	...
	test('single insert 1', () => {
		testEdits('', [editOp(1, 1, 'Hello world')], 'Hello world');
	});

	test('single insert 2', () => {
		testEdits('Hworld', [editOp(2, 2, 'ello ')], 'Hello world');
	});

	test('multiple inserts 1', () => {
		testEdits(
			'Hw',
			[editOp(2, 2, 'ello '), editOp(3, 3, 'orld')],
			'Hello world'
		);
	});
	...
	test('single delete 1', () => {
		testEdits('Hello world', [editOp(1, 12, '')], '');
	});

	test('single delete 2', () => {
		testEdits('Hello world', [editOp(2, 7, '')], 'Hworld');
	});

	test('multiple deletes 1', () => {
		testEdits('Hello world', [editOp(2, 7, ''), editOp(8, 12, '')], 'Hw');
	});

	test('multiple deletes 2', () => {
		testEdits(
			'Hello world, this is H.A.L.',
			[editOp(2, 7, ''), editOp(8, 12, ''), editOp(13, 28, '')],
			'Hw,'
		);
	});
	...
	test('single replace 2', () => {
		testEdits('H1234world', [editOp(2, 6, 'ello ')], 'Hello world');
	});

	test('multiple replace 1', () => {
		testEdits(
			'H123w321',
			[editOp(2, 5, 'ello '), editOp(6, 9, 'orld')],
			'Hello world'
		);
	});
...
class TestToken {
	...
	public static toTokens(tokens: TestToken[]): Uint32Array {
		...
		for (let i = 0; i < tokensLen; i++) {
			...
			result[(i << 1) + 1] =
				token.color << MetadataConsts.FOREGROUND_OFFSET >>> 0;
		}
		return result;
	}
}
...
suite('Editor Model - modelLine.applyEdits text & tokens', () => {
	...
	test('insertion on empty line', () => {
		...
		line.applyEdits(
			new MarkersTracker(),
			[{ startColumn: 1, endColumn: 10, text: '', forceMoveMarkers: false }],
			NO_TAB_SIZE
		);
		...
		line.applyEdits(
			new MarkersTracker(),
			[{ startColumn: 1, endColumn: 1, text: 'a', forceMoveMarkers: false }],
			NO_TAB_SIZE
		);
...
suite('Editor Model - modelLine.applyEdits text & markers', () => {
	...
	function testLineEditMarkers(
		initialText: string,
		initialMarkers: LineMarker[],
		edits: ILineEdit[],
		expectedText: string,
		expectedChangedMarkers: number[],
		_expectedMarkers: LineMarker[]
	): void {
		let line = new ModelLine(1, initialText, NO_TAB_SIZE);
	...
	test('insertion: two markers at column 1', () => {
		testLineEditMarkers(
			'abcd efgh',
			[marker(1, 1, true), marker(2, 1, false)],
			[
				{
					startColumn: 1,
					endColumn: 1,
			...
			'aabcd efgh',
			[2],
			[marker(1, 1, true), marker(2, 2, false)]
		);
	});
	...
	test('insertion: two markers at column 1 unsorted', () => {
		testLineEditMarkers(
			'abcd efgh',
			[marker(2, 1, false), marker(1, 1, true)],
			[
				{
					startColumn: 1,
					endColumn: 1,
	...
	test('delete near markers', () => {
		testLineEditMarkers(
			'abcd',
			[marker(1, 3, true), marker(2, 3, false)],
			[
				{
					startColumn: 3,
					endColumn: 4,
	...
	test('replacing the entire line with more text', () => {
		testLineEditMarkers(
			...
			[marker(1, 1, false), marker(2, 16, true)],
			[
				{
					startColumn: 1,
					endColumn: 21,
	...
	test('replacing the entire line with less text', () => {
		testLineEditMarkers(
			...
			'ttt',
			[2],
			[marker(1, 1, false), marker(2, 4, true)]
		);
	});
	...
	test('replace selection', () => {
		testLineEditMarkers(
			'first',
			[marker(1, 1, true), marker(2, 6, false)],
			[
				{
					startColumn: 1,
					endColumn: 6,
			...
			'something',
			[2],
			[marker(1, 1, true), marker(2, 10, false)]
		);
	});
});
...
suite('Editor Model - modelLine.split text & markers', () => {
	...
	function testLineSplitMarkers(
		...
		expectedText2: string,
		expectedChangedMarkers: number[],
		_expectedMarkers1: LineMarker[],
		_expectedMarkers2: LineMarker[]
	): void {
		let line = new ModelLine(1, initialText, NO_TAB_SIZE);
		line.addMarkers(initialMarkers);

		let changedMarkers = new MarkersTracker();
		let otherLine = line.split(
			changedMarkers,
			splitColumn,
			forceMoveMarkers,
			NO_TAB_SIZE
	...
	test('split it the middle 1', () => {
		testLineSplitMarkers(
			...
			'bcd efgh',
			[4, 5, 6, 7, 8],
			[marker(1, 1, true), marker(2, 1, false), marker(3, 2, true)],
			[
				marker(4, 1, false),
				marker(5, 4, true),
				marker(6, 4, false),
	...
	test('split it the middle 3', () => {
		testLineSplitMarkers(
			...
			[marker(6, 1, false), marker(7, 6, true), marker(8, 6, false)]
		);
	});
	...
	test('split it the middle 4', () => {
		testLineSplitMarkers(
			...
			[marker(7, 5, true), marker(8, 5, false)]
		);
	});
});

suite('Editor Model - modelLine.append text & markers', () => {
	function markerOnFirstLine(
		id: number,
		column: number,
		stickToPreviousCharacter: boolean
	): LineMarker {
	...
	function testLinePrependMarkers(
		aText: string,
		aMarkers: LineMarker[],
		bText: string,
		bMarkers: LineMarker[],
	...
	test('append 2', () => {
		testLinePrependMarkers(
			...
			'fgh',
			[markerOnSecondLine(7, 4, true), markerOnSecondLine(8, 4, false)],
			'abcd efgh',
			[7, 8],
			[
				markerOnFirstLine(1, 1, true),
