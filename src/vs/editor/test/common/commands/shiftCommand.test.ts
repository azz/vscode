...
import {
	...
	getEditOperation,
	testCommand
} from 'vs/editor/test/common/commands/commandTestUtils';
...
function withDockBlockCommentMode(
	callback: (mode: DocBlockCommentMode) => void
): void {
...
suite('Editor Commands - ShiftCommand', () => {
	...
	test("bug #16815:Shift+Tab doesn't go back to tabstop", () => {
		...
		var testOutdent = (
			...
			if (lineText.length > 0) {
				_assertUnshiftCommand(
					...
					oneIndent,
					[lineText + 'aaa'],
					[createSingleEditOp(expectedIndent, 1, 1, 1, lineText.length + 1)]
				);
			} else {
		...
		var testIndentation = (
			tabSize: number,
			lineText: string,
			expectedOnOutdent: number,
			expectedOnIndent: number
		) => {
			var spaceIndent = '';
		...
		function _assertUnshiftCommand(
			tabSize: number,
			oneIndent: string,
			text: string[],
			expected: IIdentifiedSingleEditOperation[]
		): void {
