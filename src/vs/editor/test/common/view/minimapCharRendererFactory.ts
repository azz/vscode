...
export class MinimapCharRendererFactory {
	public static create(source: Uint8ClampedArray): MinimapCharRenderer {
		...
		let x2CharData = this.toGrayscale(
			MinimapCharRendererFactory._downsample2x(source)
		);
		let x1CharData = this.toGrayscale(
			MinimapCharRendererFactory._downsample1x(source)
		);
	...
	private static _extractSampledChar(
	...
	private static _downsample2xChar(
		...
		const resultLen =
			...
			Constants.x2_CHAR_WIDTH *
			InternalConstants.CA_CHANNELS_CNT;
	...
	private static _downsample2x(data: Uint8ClampedArray): Uint8ClampedArray {
		...
		const sampledChar = new Uint8ClampedArray(
			Constants.SAMPLED_CHAR_HEIGHT *
				Constants.SAMPLED_CHAR_WIDTH *
				Constants.RGBA_CHANNELS_CNT
		);
		const downsampledChar = new Uint8ClampedArray(
			Constants.x2_CHAR_HEIGHT *
				Constants.x2_CHAR_WIDTH *
				InternalConstants.CA_CHANNELS_CNT
		);
	...
	private static _downsample1xChar(
		source: Uint8ClampedArray,
		dest: Uint8ClampedArray
	): void {
