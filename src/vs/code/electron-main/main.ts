...
import {
	IStorageService,
	StorageService
} from 'vs/platform/storage/node/storage';
...
function createServices(args: ParsedArgs): IInstantiationService {
	...
	services.set(
		IEnvironmentService,
		new SyncDescriptor(EnvironmentService, args, process.execPath)
	);
...
function setupIPC(accessor: ServicesAccessor): TPromise<Server> {
	...
	function allowSetForegroundWindow(
		service: LaunchChannelClient
	): TPromise<void> {
		let promise = TPromise.as<void>(void 0);
		if (platform.isWindows) {
			promise = service.getMainProcessId().then(processId => {
				logService.log(
				...
				try {
					const { allowSetForegroundWindow } = <any>require.__$__nodeRequire(
	...
	function setup(retry: boolean): TPromise<Server> {
		return serve(environmentService.mainIPCHandle).then(
			server => {
	...
	return instantiationService
		.invokeFunction(accessor => {
			...
			return instantiationService
				...
				.then(mainIpcServer => {
					const app = instantiationService.createInstance(
						CodeApplication,
						mainIpcServer,
						instanceEnv
					);
