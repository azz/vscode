...
export class CodeMenu {
	...
	constructor(
		...
		this.keybindingsResolver = instantiationService.createInstance(
			KeybindingsResolver
		);

		this.onConfigurationUpdated(
			this.configurationService.getConfiguration<IConfiguration>()
		);
	...
	private registerListeners(): void {
		...
		this.windowsService.onWindowClose(_ =>
			this.onClose(this.windowsService.getWindowCount())
		);
		...
		this.configurationService.onDidUpdateConfiguration(e =>
			this.onConfigurationUpdated(e.config, true /* update menu if changed */)
		);
	...
	private onConfigurationUpdated(
		...
		const newMultiCursorModifierSetting =
			config && config.editor && config.editor.multiCursorModifier;
		if (
			newMultiCursorModifierSetting !== this.currentMultiCursorModifierSetting
		) {
			this.currentMultiCursorModifierSetting = newMultiCursorModifierSetting;
			updateMenu = true;
		}

		const newSidebarLocation =
			(config &&
				config.workbench &&
				config.workbench.sideBar &&
		...
		let newStatusbarVisible =
			config &&
			config.workbench &&
			config.workbench.statusBar &&
		...
		let newActivityBarVisible =
			config &&
			config.workbench &&
			config.workbench.activityBar &&
			config.workbench.activityBar.visible;
		...
		let newEnableMenuBarMnemonics =
			config && config.window && config.window.enableMenuBarMnemonics;
	...
	private install(): void {
		...
		const debugMenuItem = new MenuItem({
			label: this.mnemonicLabel(
				nls.localize(
					{ key: 'mDebug', comment: ['&& denotes a mnemonic'] },
					'&&Debug'
				)
			),
			submenu: debugMenu
		...
		if (isMacintosh && !this.appMenuInstalled) {
			...
			dockMenu.append(
				new MenuItem({
					...
					click: () => this.windowsService.openNewWindow(OpenContext.DOCK)
				})
			);
	...
	private setMacApplicationMenu(macApplicationMenu: Electron.Menu): void {
		const about = new MenuItem({
			label: nls.localize('mAbout', 'About {0}', product.nameLong),
			role: 'about'
		});
		...
		const hide = new MenuItem({
			label: nls.localize('mHide', 'Hide {0}', product.nameLong),
			role: 'hide',
		...
		const quit = new MenuItem(
			this.likeAction('workbench.action.quit', {
				...
				click: () => this.windowsService.quit()
			})
		);
	...
	private setFileMenu(fileMenu: Electron.Menu): void {
		...
		if (hasNoWindows) {
			newFile = new MenuItem(
				this.likeAction('workbench.action.files.newUntitledFile', {
					label: this.mnemonicLabel(
						nls.localize(
					...
					click: () => this.windowsService.openNewWindow(OpenContext.MENU)
				})
			);
		} else {
			newFile = this.createMenuItem(
				nls.localize(
					{ key: 'miNewFile', comment: ['&& denotes a mnemonic'] },
					'&&New File'
				),
		...
		const open = new MenuItem(
			this.likeAction('workbench.action.files.openFileFolder', {
				label: this.mnemonicLabel(
					nls.localize(
				...
				click: (menuItem, win, event) =>
					this.windowsService.openFileFolderPicker(this.isOptionClick(event), {
						from: telemetryFrom
					})
			})
		);
		const openFolder = new MenuItem(
			this.likeAction('workbench.action.files.openFolder', {
				label: this.mnemonicLabel(
					nls.localize(
				...
				click: (menuItem, win, event) =>
					this.windowsService.openFolderPicker(
						this.isOptionClick(event),
						undefined,
						{ from: telemetryFrom }
					)
			})
		...
		if (hasNoWindows) {
			openFile = new MenuItem(
				this.likeAction('workbench.action.files.openFile', {
					label: this.mnemonicLabel(
						nls.localize(
					...
					click: (menuItem, win, event) =>
						this.windowsService.openFilePicker(
							this.isOptionClick(event),
							undefined,
							undefined,
							{ from: telemetryFrom }
		...
		const openRecent = new MenuItem({
			...
			enabled: openRecentMenu.items.length > 0
		});

		const saveFile = this.createMenuItem(
			nls.localize(
				{ key: 'miSave', comment: ['&& denotes a mnemonic'] },
				'&&Save'
			),
			'workbench.action.files.save',
			this.windowsService.getWindowCount() > 0
		);
		const saveFileAs = this.createMenuItem(
			nls.localize(
		...
		const saveAllFiles = this.createMenuItem(
			nls.localize(
		...
		].some(s => this.currentAutoSaveSetting === s);
		const autoSave = new MenuItem(
			this.likeAction(
				'vscode.toggleAutoSave',
				{
					label: this.mnemonicLabel(nls.localize('miAutoSave', 'Auto Save')),
					type: 'checkbox',
					checked: autoSaveEnabled,
					enabled: this.windowsService.getWindowCount() > 0,
					click: () =>
						this.windowsService.sendToFocused('vscode.toggleAutoSave')
				},
				false
			)
		);
		...
		const newWindow = new MenuItem(
			this.likeAction('workbench.action.newWindow', {
				label: this.mnemonicLabel(
					nls.localize(
						{ key: 'miNewWindow', comment: ['&& denotes a mnemonic'] },
						'New &&Window'
		...
		const revertFile = this.createMenuItem(
			nls.localize(
		...
		const closeWindow = new MenuItem(
			this.likeAction('workbench.action.closeWindow', {
				label: this.mnemonicLabel(
					nls.localize(
				...
				click: () => this.windowsService.getLastActiveWindow().win.close(),
				enabled: this.windowsService.getWindowCount() > 0
			})
		);

		const closeFolder = this.createMenuItem(
			nls.localize(
		...
		const closeEditor = this.createMenuItem(
			nls.localize(
	...
	private getPreferencesMenu(): Electron.MenuItem {
		const settings = this.createMenuItem(
			nls.localize(
		...
		const kebindingSettings = this.createMenuItem(
			nls.localize(
		...
		const keymapExtensions = this.createMenuItem(
			nls.localize(
		...
		const snippetsSettings = this.createMenuItem(
			nls.localize(
		...
		const colorThemeSelection = this.createMenuItem(
			nls.localize(
				{ key: 'miSelectColorTheme', comment: ['&& denotes a mnemonic'] },
				'&&Color Theme'
			),
			'workbench.action.selectTheme'
		);
		const iconThemeSelection = this.createMenuItem(
			nls.localize(
				{ key: 'miSelectIconTheme', comment: ['&& denotes a mnemonic'] },
				'File &&Icon Theme'
			),
	...
	private setOpenRecentMenu(openRecentMenu: Electron.Menu): void {
		...
		if (folders.length > 0) {
			...
			for (
				let i = 0;
				i < CodeMenu.MAX_MENU_RECENT_ENTRIES && i < folders.length;
				i++
			) {
				openRecentMenu.append(
					this.createOpenRecentMenuItem(folders[i], 'openRecentFolder')
				);
			}
		}
		...
		if (files.length > 0) {
			...
			for (
				let i = 0;
				i < CodeMenu.MAX_MENU_RECENT_ENTRIES && i < files.length;
				i++
			) {
				openRecentMenu.append(
					this.createOpenRecentMenuItem(files[i], 'openRecentFile')
				);
			}
		}
	...
	private createOpenRecentMenuItem(
		...
		return new MenuItem(
			this.likeAction(
				commandId,
				{
					label: this.unmnemonicLabel(
						tildify(path, this.environmentService.userHome)
					),
					click: (menuItem, win, event) => {
	...
	private setEditMenu(winLinuxEditMenu: Electron.Menu): void {
		...
		if (isMacintosh) {
			undo = this.createDevToolsAwareMenuItem(
				nls.localize(
					{ key: 'miUndo', comment: ['&& denotes a mnemonic'] },
					'&&Undo'
				),
				'undo',
				devTools => devTools.undo()
			);
			redo = this.createDevToolsAwareMenuItem(
				nls.localize(
					{ key: 'miRedo', comment: ['&& denotes a mnemonic'] },
					'&&Redo'
				),
				'redo',
				devTools => devTools.redo()
			);
			cut = this.createRoleMenuItem(
				nls.localize(
		...
		const find = this.createMenuItem(
			nls.localize(
		...
		const replace = this.createMenuItem(
			nls.localize(
		...
		const findInFiles = this.createMenuItem(
			nls.localize(
		...
		const replaceInFiles = this.createMenuItem(
			nls.localize(
		...
		const emmetExpandAbbreviation = this.createMenuItem(
			nls.localize(
				{
		...
		const showEmmetCommands = this.createMenuItem(
			nls.localize(
				{ key: 'miShowEmmetCommands', comment: ['&& denotes a mnemonic'] },
				'E&&mmet...'
			),
			'workbench.action.showEmmetCommands'
		);
		const toggleLineComment = this.createMenuItem(
			nls.localize(
		...
		const toggleBlockComment = this.createMenuItem(
			nls.localize(
	...
	private setSelectionMenu(winLinuxEditMenu: Electron.Menu): void {
		...
		if (this.currentMultiCursorModifierSetting === 'ctrlCmd') {
			...
			multiCursorModifierLabel = nls.localize(
				'miMultiCursorAlt',
				'Use Alt+Click for Multi-Cursor'
			);
		} else {
			multiCursorModifierLabel = isMacintosh
		...
		const multicursorModifier = this.createMenuItem(
			multiCursorModifierLabel,
			'workbench.action.toggleMultiCursorModifier'
		);
		const insertCursorAbove = this.createMenuItem(
			nls.localize(
				{ key: 'miInsertCursorAbove', comment: ['&& denotes a mnemonic'] },
				'&&Add Cursor Above'
			),
			'editor.action.insertCursorAbove'
		);
		const insertCursorBelow = this.createMenuItem(
			nls.localize(
				{ key: 'miInsertCursorBelow', comment: ['&& denotes a mnemonic'] },
				'A&&dd Cursor Below'
			),
			'editor.action.insertCursorBelow'
		);
		const insertCursorAtEndOfEachLineSelected = this.createMenuItem(
			nls.localize(
				{
		...
		const addSelectionToNextFindMatch = this.createMenuItem(
			nls.localize(
				{
		...
		const addSelectionToPreviousFindMatch = this.createMenuItem(
			nls.localize(
				{
		...
		const selectHighlights = this.createMenuItem(
			nls.localize(
				{ key: 'miSelectHighlights', comment: ['&& denotes a mnemonic'] },
				'Select All &&Occurrences'
			),
			'editor.action.selectHighlights'
		);

		const copyLinesUp = this.createMenuItem(
			nls.localize(
		...
		const copyLinesDown = this.createMenuItem(
			nls.localize(
		...
		const moveLinesUp = this.createMenuItem(
			nls.localize(
		...
		const moveLinesDown = this.createMenuItem(
			nls.localize(
		...
		if (isMacintosh) {
			selectAll = this.createDevToolsAwareMenuItem(
				nls.localize(
					{ key: 'miSelectAll', comment: ['&& denotes a mnemonic'] },
					'&&Select All'
				),
				'editor.action.selectAll',
				devTools => devTools.selectAll()
			);
		} else {
			selectAll = this.createMenuItem(
				nls.localize(
					{ key: 'miSelectAll', comment: ['&& denotes a mnemonic'] },
					'&&Select All'
				),
		...
		const smartSelectGrow = this.createMenuItem(
			nls.localize(
				{ key: 'miSmartSelectGrow', comment: ['&& denotes a mnemonic'] },
				'&&Expand Selection'
			),
			'editor.action.smartSelect.grow'
		);
		const smartSelectshrink = this.createMenuItem(
			nls.localize(
				{ key: 'miSmartSelectShrink', comment: ['&& denotes a mnemonic'] },
				'&&Shrink Selection'
			),
	...
	private setViewMenu(viewMenu: Electron.Menu): void {
		const explorer = this.createMenuItem(
			nls.localize(
		...
		const search = this.createMenuItem(
			nls.localize(
		...
		const debug = this.createMenuItem(
			nls.localize(
		...
		const extensions = this.createMenuItem(
			nls.localize(
		...
		const output = this.createMenuItem(
			nls.localize(
		...
		const debugConsole = this.createMenuItem(
			nls.localize(
		...
		const integratedTerminal = this.createMenuItem(
			nls.localize(
				{
		...
		const problems = this.createMenuItem(
			nls.localize(
		...
		if (this.extensionViewlets.length) {
			...
			this.extensionViewlets.forEach(viewlet => {
				additionalViewletsMenu.append(
					this.createMenuItem(viewlet.label, viewlet.id)
				);
			});

			additionalViewlets = new MenuItem({
				label: this.mnemonicLabel(
					nls.localize(
						{ key: 'miAdditionalViews', comment: ['&& denotes a mnemonic'] },
						'Additional &&Views'
					)
				),
				submenu: additionalViewletsMenu,
				enabled: true
			});
		}

		const commands = this.createMenuItem(
			nls.localize(
		...
		const fullscreen = new MenuItem(
			this.withKeybinding('workbench.action.toggleFullScreen', {
				label: this.mnemonicLabel(
					nls.localize(
				...
				click: () =>
					this.windowsService.getLastActiveWindow().toggleFullScreen(),
				enabled: this.windowsService.getWindowCount() > 0
			})
		);
		const toggleZenMode = this.createMenuItem(
			nls.localize('miToggleZenMode', 'Toggle Zen Mode'),
			'workbench.action.toggleZenMode',
			this.windowsService.getWindowCount() > 0
		);
		const toggleMenuBar = this.createMenuItem(
			nls.localize(
		...
		const splitEditor = this.createMenuItem(
			nls.localize(
		...
		const toggleEditorLayout = this.createMenuItem(
			nls.localize(
		...
		const toggleSidebar = this.createMenuItem(
			nls.localize(
		...
		const moveSidebar = this.createMenuItem(
			moveSideBarLabel,
			'workbench.action.toggleSidebarPosition'
		);

		const togglePanel = this.createMenuItem(
			nls.localize(
		...
		if (this.currentStatusbarVisible) {
			statusBarLabel = nls.localize(
				{ key: 'miHideStatusbar', comment: ['&& denotes a mnemonic'] },
				'&&Hide Status Bar'
			);
		} else {
			statusBarLabel = nls.localize(
		...
		const toggleStatusbar = this.createMenuItem(
			statusBarLabel,
		...
		const toggleActivtyBar = this.createMenuItem(
			activityBarLabel,
			'workbench.action.toggleActivityBarVisibility'
		);

		const toggleWordWrap = this.createMenuItem(
			nls.localize(
		...
		const toggleRenderWhitespace = this.createMenuItem(
			nls.localize(
		...
		const toggleRenderControlCharacters = this.createMenuItem(
			nls.localize(
				{
		...
		const zoomIn = this.createMenuItem(
			nls.localize(
		...
		const zoomOut = this.createMenuItem(
			nls.localize(
		...
		const resetZoom = this.createMenuItem(
			nls.localize(
	...
	private setGotoMenu(gotoMenu: Electron.Menu): void {
		const back = this.createMenuItem(
			nls.localize(
		...
		const forward = this.createMenuItem(
			nls.localize(
		...
		const nextEditor = this.createMenuItem(
			nls.localize(
		...
		const previousEditor = this.createMenuItem(
			nls.localize(
		...
		const nextEditorInGroup = this.createMenuItem(
			nls.localize(
		...
		const previousEditorInGroup = this.createMenuItem(
			nls.localize(
		...
		const switchEditor = new MenuItem({
			label: this.mnemonicLabel(
				nls.localize(
			...
			submenu: switchEditorMenu,
			enabled: true
		});

		const switchGroupMenu = new Menu();

		const focusFirstGroup = this.createMenuItem(
			nls.localize(
		...
		const focusSecondGroup = this.createMenuItem(
			nls.localize(
		...
		const focusThirdGroup = this.createMenuItem(
			nls.localize(
		...
		const nextGroup = this.createMenuItem(
			nls.localize(
		...
		const previousGroup = this.createMenuItem(
			nls.localize(
		...
		const switchGroup = new MenuItem({
			label: this.mnemonicLabel(
				nls.localize(
			...
			submenu: switchGroupMenu,
			enabled: true
		});

		const gotoFile = this.createMenuItem(
			nls.localize(
		...
		const gotoSymbolInFile = this.createMenuItem(
			nls.localize(
				{ key: 'miGotoSymbolInFile', comment: ['&& denotes a mnemonic'] },
				'Go to &&Symbol in File...'
			),
			'workbench.action.gotoSymbol'
		);
		const gotoSymbolInWorkspace = this.createMenuItem(
			nls.localize(
		...
		const gotoDefinition = this.createMenuItem(
			nls.localize(
		...
		const gotoTypeDefinition = this.createMenuItem(
			nls.localize(
		...
		const goToImplementation = this.createMenuItem(
			nls.localize(
		...
		const gotoLine = this.createMenuItem(
			nls.localize(
	...
	private setDebugMenu(debugMenu: Electron.Menu): void {
		const start = this.createMenuItem(
			nls.localize(
		...
		const startWithoutDebugging = this.createMenuItem(
			nls.localize(
				{ key: 'miStartWithoutDebugging', comment: ['&& denotes a mnemonic'] },
				'Start &&Without Debugging'
			),
			'workbench.action.debug.run'
		);
		const stop = this.createMenuItem(
			nls.localize(
		...
		const restart = this.createMenuItem(
			nls.localize(
		...
		const openConfigurations = this.createMenuItem(
			nls.localize(
		...
		const addConfiguration = this.createMenuItem(
			nls.localize(
		...
		const stepOver = this.createMenuItem(
			nls.localize(
		...
		const stepInto = this.createMenuItem(
			nls.localize(
		...
		const stepOut = this.createMenuItem(
			nls.localize(
		...
		const continueAction = this.createMenuItem(
			nls.localize(
		...
		const toggleBreakpoint = this.createMenuItem(
			nls.localize(
		...
		breakpointsMenu.append(
			this.createMenuItem(
				nls.localize(
					{ key: 'miColumnBreakpoint', comment: ['&& denotes a mnemonic'] },
					'C&&olumn Breakpoint'
				),
		...
		const newBreakpoints = new MenuItem({
			label: this.mnemonicLabel(
				nls.localize(
			...
			submenu: breakpointsMenu
		});
		const enableAllBreakpoints = this.createMenuItem(
			nls.localize(
		...
		const disableAllBreakpoints = this.createMenuItem(
			nls.localize(
		...
		const removeAllBreakpoints = this.createMenuItem(
			nls.localize(
		...
		const installAdditionalDebuggers = this.createMenuItem(
			nls.localize(
				{
	...
	private setHelpMenu(helpMenu: Electron.Menu): void {
		const toggleDevToolsItem = new MenuItem(
			this.likeAction('workbench.action.toggleDevTools', {
				label: this.mnemonicLabel(
					nls.localize(
		...
		if (product.reportIssueUrl) {
			...
			if (this.windowsService.getWindowCount() > 0) {
				reportIssuesItem = this.createMenuItem(
					label,
					'workbench.action.reportIssues'
				);
			} else {
				reportIssuesItem = new MenuItem({
					label: this.mnemonicLabel(label),
					click: () => this.openUrl(product.reportIssueUrl, 'openReportIssues')
				});
			}
		}

		const keyboardShortcutsUrl = isLinux
		...
		arrays
			.coalesce([
				new MenuItem({
					...
					click: () =>
						this.windowsService.sendToFocused(
							'vscode:runAction',
				...
				product.introductoryVideosUrl
					...
					: null,
				product.introductoryVideosUrl || keyboardShortcutsUrl
					? __separator__()
					: null,
				product.twitterUrl
					? new MenuItem({
							...
							click: () => this.openUrl(product.twitterUrl, 'openTwitterUrl')
						})
					: null,
				product.requestFeatureUrl
					? new MenuItem({
							label: this.mnemonicLabel(
								nls.localize(
							...
							click: () =>
								this.openUrl(product.requestFeatureUrl, 'openUserVoiceUrl')
						})
					: null,
				reportIssuesItem,
				product.twitterUrl ||
							...
							click: () => {
								if (language) {
									const queryArgChar = product.licenseUrl.indexOf('?') > 0
										...
										: '?';
									this.openUrl(
										`${product.licenseUrl}${queryArgChar}lang=${language}`,
										'openLicenseUrl'
									);
								} else {
							...
							click: () => {
								if (language) {
									const queryArgChar = product.licenseUrl.indexOf('?') > 0
										...
										: '?';
									this.openUrl(
										`${product.privacyStatementUrl}${queryArgChar}lang=${language}`,
				...
				product.licenseUrl || product.privacyStatementUrl
					...
					: null,
				toggleDevToolsItem,
				isWindows && product.quality !== 'stable'
	...
	private getUpdateMenuItems(): Electron.MenuItem[] {
		switch (this.updateService.state) {
			...
			case UpdateState.UpdateDownloaded:
				return [
					new MenuItem({
						label: nls.localize('miRestartToUpdate', 'Restart To Update...'),
						click: () => {
			...
			default:
				const result = [
					new MenuItem({
						label: nls.localize('miCheckForUpdates', 'Check for Updates...'),
						click: () =>
							setTimeout(() => {
	...
	private createMenuItem(
		...
		const click: () => void = typeof arg2 === 'function'
			? arg2
			: (menuItem, win, event) => {
					let commandId = arg2;
		...
		const enabled = typeof arg3 === 'boolean'
			? arg3
			: this.windowsService.getWindowCount() > 0;
	...
	private createDevToolsAwareMenuItem(
		label: string,
		commandId: string,
		devToolsFocusedFn: (contents: Electron.WebContents) => void
	): Electron.MenuItem {
		return new MenuItem(
			this.withKeybinding(commandId, {
				label: this.mnemonicLabel(label),
				enabled: this.windowsService.getWindowCount() > 0,
				click: () => {
					...
					if (windowInFocus.win.webContents.isDevToolsFocused()) {
						devToolsFocusedFn(
							windowInFocus.win.webContents.devToolsWebContents
						);
					} else {
						this.windowsService.sendToFocused('vscode:runAction', commandId);
					}
	...
	private withKeybinding(
		commandId: string,
		options: Electron.MenuItemOptions
		...
		if (binding && binding.label) {
			...
			if (binding.isNative) {
				...
				if (bindingIndex >= 0) {
					options.label = `${options.label.substr(
						0,
						bindingIndex
					)} [${binding.label}]`;
				} else {
	...
	private openAboutDialog(): void {
		const lastActiveWindow =
			this.windowsService.getFocusedWindow() ||
			this.windowsService.getLastActiveWindow();

		dialog.showMessageBox(
			lastActiveWindow && lastActiveWindow.win,
			{
