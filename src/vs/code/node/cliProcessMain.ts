...
class Main {
	...
	private installExtension(extensions: string[]): TPromise<any> {
		const vsixTasks: Task[] = extensions
			...
			.map(id => () => {
				const extension = path.isAbsolute(id)
					? id
			...
			.map(id => () => {
				return this.extensionManagementService
					...
					.then(installed => {
						...
						return this.extensionGalleryService
							.query({ names: [id] })
...
export function main(argv: ParsedArgs): TPromise<void> {
	...
	services.set(
		IEnvironmentService,
		new SyncDescriptor(EnvironmentService, argv, process.execPath)
	);
	...
	return instantiationService.invokeFunction(accessor => {
		...
		return TPromise.join(
			[envService.appSettingsHome, envService.extensionsPath].map(p =>
				mkdirp(p)
			)
		).then(() => {
			const {
			...
			if (isBuilt && !extensionDevelopmentPath && product.enableTelemetry) {
				...
				if (product.aiConfig && product.aiConfig.asimovKey) {
					appenders.push(
						new AppInsightsAppender(
							eventPrefix,
							null,
							product.aiConfig.asimovKey
						)
					);
				}
