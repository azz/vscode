...
export class InstantiationService implements IInstantiationService {
	...
	invokeFunction<R>(
		signature: (accessor: ServicesAccessor, ...more: any[]) => R,
		...args: any[]
	): R {
	...
	private _createInstanceAsync<T>(
		descriptor: AsyncDescriptor<T>,
		args: any[]
	): TPromise<T> {
		...
		return new TPromise(
			(c, e, p) => {
				require([descriptor.moduleName], (_module?: any) => {
					...
					if (!_module) {
						return e(
							illegalArgument('module not found: ' + descriptor.moduleName)
						);
					}
					...
					if (typeof ctor !== 'function') {
						return e(
							illegalArgument(
								'not a function: ' + descriptor.ctorName ||
	...
	private _createInstance<T>(desc: SyncDescriptor<T>, args: any[]): T {
		...
		let serviceDependencies = _util
			.getServiceDependencies(desc.ctor)
		...
		for (const dependency of serviceDependencies) {
			...
			if (!service && this._strict && !dependency.optional) {
				throw new Error(
					`[createInstance] ${desc.ctor
						.name} depends on UNKNOWN service ${dependency.id}.`
				);
			}
			serviceArgs.push(service);
		}

		let firstServiceArgPos = serviceDependencies.length > 0
		...
		if (staticArgs.length !== firstServiceArgPos) {
			console.warn(
				`[createInstance] First service dependency of ${desc.ctor
					.name} at position ${firstServiceArgPos +
					1} conflicts with ${staticArgs.length} static arguments`
			);
