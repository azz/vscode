...
suite('EnvironmentService', () => {
	test('parseExtensionHostPort when built', () => {
		...
		assert.deepEqual(parse(['--debugPluginHost']), {
			port: null,
			break: false
		});
		assert.deepEqual(parse(['--debugPluginHost=1234']), {
			port: 1234,
			break: false
		});
		assert.deepEqual(parse(['--debugBrkPluginHost']), {
			port: null,
			break: false
		});
		assert.deepEqual(parse(['--debugBrkPluginHost=5678']), {
			port: 5678,
			break: true
		});
		assert.deepEqual(
			parse(['--debugPluginHost=1234', '--debugBrkPluginHost=5678']),
			{ port: 5678, break: true }
		);
	});
	...
	test('userDataPath', () => {
		const parse = (
			a,
			b: { cwd: () => string; env: { [key: string]: string } }
		) => parseUserDataDir(parseArgs(a), <any>b);

		assert.equal(
			parse(['--user-data-dir', './dir'], { cwd: () => '/foo', env: {} }),
			path.resolve('/foo/dir'),
			'should use cwd when --user-data-dir is specified'
		);
		assert.equal(
			parse(['--user-data-dir', './dir'], {
				cwd: () => '/foo',
				env: { VSCODE_CWD: '/bar' }
			}),
			path.resolve('/bar/dir'),
