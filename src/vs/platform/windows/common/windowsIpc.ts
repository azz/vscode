...
export interface IWindowsChannel extends IChannel {
	...
	call(
		command: 'openFileFolderPicker',
		arg: [number, boolean, ITelemetryData]
	): TPromise<void>;
	call(
		command: 'openFilePicker',
		arg: [number, boolean, string, ITelemetryData]
	): TPromise<void>;
	call(
	...
	call(
		command: 'setRepresentedFilename',
		arg: [number, string]
	): TPromise<void>;
	call(
		command: 'addToRecentlyOpen',
		arg: { path: string; isFile?: boolean }[]
	): TPromise<void>;
	call(command: 'removeFromRecentlyOpen', arg: string[]): TPromise<void>;
	call(command: 'clearRecentPathsList'): TPromise<void>;
	call(
		command: 'getRecentlyOpen',
		arg: number
	): TPromise<{ files: string[]; folders: string[] }>;
	...
	call(
		command: 'openWindow',
		arg: [string[], { forceNewWindow?: boolean; forceReuseWindow?: boolean }]
	): TPromise<void>;
	call(command: 'openNewWindow'): TPromise<void>;
	call(command: 'showWindow', arg: number): TPromise<void>;
	call(
		command: 'getWindows'
	): TPromise<{ id: number; path: string; title: string }[]>;
	call(command: 'getWindowCount'): TPromise<number>;
	call(
		command: 'relaunch',
		arg: { addArgs?: string[]; removeArgs?: string[] }
	): TPromise<number>;
	...
	call(
		command: 'startCrashReporter',
		arg: Electron.CrashReporterStartOptions
	): TPromise<void>;
	...
	call(command: string, arg?: any): TPromise<any> {
		switch (command) {
			case 'event:onWindowOpen':
				return eventToCall(this.onWindowOpen);
			case 'event:onWindowFocus':
				return eventToCall(this.onWindowFocus);
			case 'openFileFolderPicker':
				return this.service.openFileFolderPicker(arg[0], arg[1], arg[2]);
			case 'openFilePicker':
				return this.service.openFilePicker(arg[0], arg[1], arg[2], arg[3]);
			case 'openFolderPicker':
				return this.service.openFolderPicker(arg[0], arg[1], arg[2]);
			case 'reloadWindow':
				return this.service.reloadWindow(arg);
			case 'openDevTools':
				return this.service.openDevTools(arg);
			case 'toggleDevTools':
				return this.service.toggleDevTools(arg);
			case 'closeFolder':
				return this.service.closeFolder(arg);
			case 'toggleFullScreen':
				return this.service.toggleFullScreen(arg);
			case 'setRepresentedFilename':
				return this.service.setRepresentedFilename(arg[0], arg[1]);
			case 'addToRecentlyOpen':
				return this.service.addToRecentlyOpen(arg);
			case 'removeFromRecentlyOpen':
				return this.service.removeFromRecentlyOpen(arg);
			case 'clearRecentPathsList':
				return this.service.clearRecentPathsList();
			case 'getRecentlyOpen':
				return this.service.getRecentlyOpen(arg);
			case 'focusWindow':
				return this.service.focusWindow(arg);
			case 'isFocused':
				return this.service.isFocused(arg);
			case 'isMaximized':
				return this.service.isMaximized(arg);
			case 'maximizeWindow':
				return this.service.maximizeWindow(arg);
			case 'unmaximizeWindow':
				return this.service.unmaximizeWindow(arg);
			case 'onWindowTitleDoubleClick':
				return this.service.onWindowTitleDoubleClick(arg);
			case 'setDocumentEdited':
				return this.service.setDocumentEdited(arg[0], arg[1]);
			case 'openWindow':
				return this.service.openWindow(arg[0], arg[1]);
			case 'openNewWindow':
				return this.service.openNewWindow();
			case 'showWindow':
				return this.service.showWindow(arg);
			case 'getWindows':
				return this.service.getWindows();
			case 'getWindowCount':
				return this.service.getWindowCount();
			case 'relaunch':
				return this.service.relaunch(arg[0]);
			...
			case 'log':
				return this.service.log(arg[0], arg[1]);
			case 'closeExtensionHostWindow':
				return this.service.closeExtensionHostWindow(arg);
			case 'showItemInFolder':
				return this.service.showItemInFolder(arg);
			case 'openExternal':
				return this.service.openExternal(arg);
			case 'startCrashReporter':
				return this.service.startCrashReporter(arg);
		}
		return undefined;
	}
}
...
export class WindowsChannelClient implements IWindowsService {
	...
	private _onWindowOpen: Event<number> = eventFromCall<number>(
		this.channel,
		'event:onWindowOpen'
	);
	get onWindowOpen(): Event<number> {
		return this._onWindowOpen;
	}

	private _onWindowFocus: Event<number> = eventFromCall<number>(
		this.channel,
		'event:onWindowFocus'
	);
	...
	openFileFolderPicker(
		...
		return this.channel.call('openFileFolderPicker', [
			windowId,
			forceNewWindow,
			data
		]);
	...
	openFilePicker(
		...
		return this.channel.call('openFilePicker', [
			windowId,
			forceNewWindow,
			path,
			data
	...
	openFolderPicker(
		...
		return this.channel.call('openFolderPicker', [
			windowId,
			forceNewWindow,
			data
		]);
	...
	startCrashReporter(
		config: Electron.CrashReporterStartOptions
	): TPromise<void> {
