...
import {
	KeybindingResolver,
	IResolveResult
} from 'vs/platform/keybinding/common/keybindingResolver';
import {
	IKeybindingEvent,
	IKeybindingService,
	IKeyboardEvent
...
export abstract class AbstractKeybindingService implements IKeybindingService {
	...
	public softDispatch(
		...
		const currentChord = this._currentChord
			? this._currentChord.keypress
			: null;
	...
	protected _dispatch(
		...
		if (resolveResult && resolveResult.enterChord) {
			...
			if (this._statusService) {
				this._currentChordStatusMessage = this._statusService.setStatusMessage(
					nls.localize(
						'first.chord',
		...
		if (this._statusService && this._currentChord) {
			if (!resolveResult || !resolveResult.commandId) {
				this._statusService.setStatusMessage(
					nls.localize(
						'missing.chord',
