...
export class KeybindingResolver {
	...
	constructor(
		...
		this._keybindings = KeybindingResolver.combine(
			defaultKeybindings,
			overrides
		);
	...
	private static _isTargetedForRemoval(
		...
		keypressFirstPart: string,
		keypressChordPart: string,
		command: string,
		when: ContextKeyExpr
	): boolean {
		...
		if (
			keypressFirstPart &&
			defaultKb.keypressFirstPart !== keypressFirstPart
		) {
			return false;
		}
		if (
			keypressChordPart &&
			defaultKb.keypressChordPart !== keypressChordPart
		) {
			return false;
		}
		if (when) {
	...
	private _addKeyPress(keypress: string, item: ResolvedKeybindingItem): void {
		...
		for (let i = conflicts.length - 1; i >= 0; i--) {
			...
			if (
				conflictIsChord &&
				itemIsChord &&
				conflict.keypressChordPart !== item.keypressChordPart
			) {
	...
	public static getAllUnboundCommands(
		...
		for (let id in commands) {
			...
			if (
				typeof commands[id].description === 'object' &&
				!isFalsyOrEmpty(
					(<ICommandHandlerDescription>commands[id].description).args
				)
			) {
				// command with args
				continue;
