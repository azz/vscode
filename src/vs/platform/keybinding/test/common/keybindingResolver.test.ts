...
suite('KeybindingResolver', () => {
	function kbItem(
		...
		const resolvedKeybinding = keybinding !== 0
			? new USLayoutResolvedKeybinding(createKeybinding(keybinding, OS), OS)
			: null;
	...
	test('resolve key', function() {
		...
		assert.equal(
			KeybindingResolver.contextMatchesRules(
		...
		assert.equal(
			resolver.resolve(
				createContext({ bar: 'baz' }),
				null,
				getDispatchStr(<SimpleKeybinding>runtimeKeybinding)
			).commandId,
			'yes'
		);
		assert.equal(
	...
	test('KeybindingResolver.combine removal with unspecified when', function() {
		...
		let overrides = [kbItem(KeyCode.KEY_A, '-yes1', null, null, false)];
		let actual = KeybindingResolver.combine(defaults, overrides);
		assert.deepEqual(actual, [
			kbItem(KeyCode.KEY_B, 'yes2', null, ContextKeyExpr.equals('2', 'b'), true)
		]);
	...
	test('KeybindingResolver.combine removal with unspecified when and unspecified keybinding', function() {
		...
		let overrides = [kbItem(0, '-yes1', null, null, false)];
		let actual = KeybindingResolver.combine(defaults, overrides);
		assert.deepEqual(actual, [
			kbItem(KeyCode.KEY_B, 'yes2', null, ContextKeyExpr.equals('2', 'b'), true)
		]);
		...
		let assertIsIncluded = (a: ContextKeyExpr[], b: ContextKeyExpr[]) => {
			assert.equal(
				KeybindingResolver.whenIsEntirelyIncluded(
					false,
					new ContextKeyAndExpr(a),
					new ContextKeyAndExpr(b)
				),
				true
			);
		};
	...
	test('resolve command', function() {
		...
		let items = [
			...
			_kbItem(KeyCode.KEY_X, 'second', ContextKeyExpr.equals('key2', true)),
			// This one is a secondary mapping for `second`
			_kbItem(KeyCode.KEY_Z, 'second', null),
			// This one sometimes overwrites first
			_kbItem(KeyCode.KEY_X, 'third', ContextKeyExpr.equals('key3', true)),
			...
			_kbItem(KeyMod.CtrlCmd | KeyCode.KEY_G, 'eleven', null)
		];
		...
		let testKey = (commandId: string, expectedKeys: number[]) => {
			...
			assert.equal(
				lookupResult.length,
				expectedKeys.length,
				'Length mismatch @ commandId ' +
					commandId +
					'; GOT: ' +
					JSON.stringify(lookupResult, null, '\t')
			);
			for (let i = 0, len = lookupResult.length; i < len; i++) {
				const expected = new USLayoutResolvedKeybinding(
					createKeybinding(expectedKeys[i], OS),
					OS
				);
		...
		let testResolve = (
			...
			if (expectedKey.type === KeybindingType.Chord) {
				...
				assert.equal(
					result.commandId,
					commandId,
					'Finds chorded command ' + commandId
				);
				assert.equal(
					result.enterChord,
					false,
