...
let configurationExtPoint = ExtensionsRegistry.registerExtensionPoint<
	...
	constructor() {
		configurationExtPoint.setHandler(extensions => {
			for (var i = 0; i < extensions.length; i++) {
				var extensionValue = <IJSONValidationExtensionPoint[]>extensions[i]
					.value;
				...
				extensionValue.forEach(extension => {
					...
					if (strings.startsWith(uri, './')) {
						try {
							uri = URI.file(
								paths.normalize(paths.join(extensionPath, uri))
							).toString();
						} catch (e) {
							collector.error(
								nls.localize(
