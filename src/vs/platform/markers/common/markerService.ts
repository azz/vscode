...
class MarkerStats implements MarkerStatistics {
	...
	private _resourceStats(resource: URI): MarkerStatistics {
		...
		if (
			...
			resource.scheme === Schemas.walkThrough ||
			resource.scheme === Schemas.walkThroughSnippet
		) {
			return result;
		}
...
export class MarkerService implements IMarkerService {
	...
	private _onMarkerChangedEvent: Event<URI[]> = debounceEvent(
		this._onMarkerChanged.event,
		MarkerService._debouncer,
		0
	);
	...
	read(
		filter: { owner?: string; resource?: URI; take?: number } = Object.create(
			null
		)
	): IMarker[] {
