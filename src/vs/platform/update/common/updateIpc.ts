...
export class UpdateChannel implements IUpdateChannel {
	...
	call(command: string, arg?: any): TPromise<any> {
		switch (command) {
			case 'event:onError':
				return eventToCall(this.service.onError);
			case 'event:onUpdateAvailable':
				return eventToCall(this.service.onUpdateAvailable);
			case 'event:onUpdateNotAvailable':
				return eventToCall(this.service.onUpdateNotAvailable);
			case 'event:onUpdateReady':
				return eventToCall(this.service.onUpdateReady);
			case 'event:onStateChange':
				return eventToCall(this.service.onStateChange);
			case 'checkForUpdates':
				return this.service.checkForUpdates(arg);
			case 'quitAndInstall':
				return this.service.quitAndInstall();
			case '_getInitialState':
				return TPromise.as(this.service.state);
		}
		return undefined;
	}
}
...
export class UpdateChannelClient implements IUpdateService {
	...
	private _onUpdateAvailable = eventFromCall<{ url: string; version: string }>(
		this.channel,
	...
	private _onUpdateNotAvailable = eventFromCall<boolean>(
		this.channel,
	...
	private _onUpdateReady = eventFromCall<IRawUpdate>(
		this.channel,
		'event:onUpdateReady'
	);
	...
	private _onRemoteStateChange = eventFromCall<State>(
		this.channel,
		'event:onStateChange'
	);
