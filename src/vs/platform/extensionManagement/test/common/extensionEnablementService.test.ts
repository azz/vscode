		...
		super(
			...
			instantiationService.get(IExtensionManagementService) ||
				instantiationService.stub(IExtensionManagementService, {
					onDidUninstallExtension: new Emitter()
				})
		);
	}

	public reset(): void {
		this.getGloballyDisabledExtensions().forEach(d =>
			this.setEnablement(d, true)
		);
		this.getWorkspaceDisabledExtensions().forEach(d =>
			this.setEnablement(d, true, true)
		);
	}
}
	...
	test('test disable an extension for workspace and then globally', done => {
		testObject
			...
			.then(() => {
				assert.deepEqual(
					['pub.a'],
					testObject.getWorkspaceDisabledExtensions()
				);
				assert.deepEqual(['pub.a'], testObject.getGloballyDisabledExtensions());
			})
			.then(done, done);
