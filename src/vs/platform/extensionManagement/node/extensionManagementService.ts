...
function parseManifest(
	raw: string
): TPromise<{ manifest: IExtensionManifest; metadata: IGalleryMetadata }> {
...
function readManifest(
	...
	const promises = [
		pfs
			.readFile(path.join(extensionPath, 'package.json'), 'utf8')
			.then(raw => parseManifest(raw)),
		pfs
			.readFile(path.join(extensionPath, 'package.nls.json'), 'utf8')
			.then<string>(
				null,
				err => (err.code !== 'ENOENT' ? TPromise.wrapError(err) : '{}')
			)
	...
	return TPromise.join<any>(
		promises
	).then(([{ manifest, metadata }, translations]) => {
		return {
...
export class ExtensionManagementService implements IExtensionManagementService {
	...
	onInstallExtension: Event<InstallExtensionEvent> = this._onInstallExtension
		.event;

	private _onDidInstallExtension = new Emitter<DidInstallExtensionEvent>();
	onDidInstallExtension: Event<DidInstallExtensionEvent> = this
	...
	onDidUninstallExtension: Event<DidUninstallExtensionEvent> = this
	...
	install(zipPath: string): TPromise<void> {
		...
		return validate(zipPath).then<void>(manifest => {
			...
			return this.isObsolete(id).then(isObsolete => {
				...
				return this.installExtension(zipPath, id).then(
					local => this._onDidInstallExtension.fire({ id, zipPath, local }),
					error => {
						this._onDidInstallExtension.fire({ id, zipPath, error });
	...
	installFromGallery(
		...
		promptToInstallDependencies: boolean = true
	): TPromise<void> {
		...
		return this.isObsolete(id).then(isObsolete => {
			...
			return this.installCompatibleVersion(
				extension,
				true,
				promptToInstallDependencies
			).then(
				local =>
					this._onDidInstallExtension.fire({ id, local, gallery: extension }),
				error => {
	...
	private installCompatibleVersion(
		extension: IGalleryExtension,
		installDependencies: boolean,
		promptToInstallDependencies: boolean
	): TPromise<ILocalExtension> {
		return this.galleryService
			.loadCompatibleVersion(extension)
				...
				this.getDependenciesToInstall(
					...
					if (promptToInstallDependencies) {
						...
						return this.choiceService
							.choose(Severity.Info, message, options, 1, true)
							.then(
								value => {
									if (value === 0) {
	...
	private getDependenciesToInstall(
		extension: IGalleryExtension,
		checkDependecies: boolean
	): TPromise<string[]> {
		if (!checkDependecies) {
			return TPromise.wrap([]);
		}
		// Filter out self
		const dependencies = extension.properties.dependencies
			? extension.properties.dependencies.filter(id => id !== extension.id)
			: [];
		if (!dependencies.length) {
			return TPromise.wrap([]);
		}
		...
		return this.getInstalled().then(installed => {
			return dependencies.filter(dep =>
				installed.every(
					i => `${i.manifest.publisher}.${i.manifest.name}` !== dep
				)
			);
		});
	}

	private installWithDependencies(
		extension: IGalleryExtension
	): TPromise<ILocalExtension> {
		return this.galleryService
			.getAllDependencies(extension)
			.then(allDependencies =>
				this.filterDependenciesToInstall(extension, allDependencies)
			)
			.then(toInstall =>
				this.filterObsolete(
					...toInstall.map(i => getLocalExtensionIdFromGallery(i, i.version))
				).then(obsolete => {
					if (obsolete.length) {
						return TPromise.wrapError<ILocalExtension>(
							new Error(
								nls.localize(
									'restartCode',
									'Please restart Code before reinstalling {0}.',
	...
	private bulkInstallWithDependencies(
		extension: IGalleryExtension,
		dependecies: IGalleryExtension[]
	): TPromise<ILocalExtension> {
		...
		return this.downloadAndInstall(extension)
			.then(localExtension => {
				return TPromise.join(
					dependecies.map(dep =>
						this.installCompatibleVersion(dep, false, false)
					)
				).then(
					installedLocalExtensions => {
						for (const installedLocalExtension of installedLocalExtensions) {
							const gallery = this.getGalleryExtensionForLocalExtension(
								dependecies,
								installedLocalExtension
							);
							this._onDidInstallExtension.fire({
								id: installedLocalExtension.id,
								local: installedLocalExtension,
								gallery
							});
						}
					...
					error => {
						return this.rollback(localExtension, dependecies).then(() => {
							return TPromise.wrapError<ILocalExtension>(
								Array.isArray(error) ? error[error.length - 1] : error
							);
						});
					}
				);
			...
			.then(
				...
				error => {
					for (const dependency of dependecies) {
						this._onDidInstallExtension.fire({
							id: getLocalExtensionIdFromGallery(
								dependency,
								dependency.version
							),
							gallery: dependency,
	...
	private rollback(
		...
		return this.doUninstall(localExtension.id)
			...
			.then(installed =>
				TPromise.join(installed.map(i => this.doUninstall(i.id)))
			)
			.then(() => null);
	}

	private filterDependenciesToInstall(
		extension: IGalleryExtension,
		dependencies: IGalleryExtension[]
	...
	private filterOutUninstalled(
		extensions: IGalleryExtension[]
	): TPromise<ILocalExtension[]> {
		return this.getInstalled().then(installed =>
			installed.filter(
				local => !!this.getGalleryExtensionForLocalExtension(extensions, local)
			)
		);
	}

	private getGalleryExtensionForLocalExtension(
		galleryExtensions: IGalleryExtension[],
		localExtension: ILocalExtension
	): IGalleryExtension {
		const filtered = galleryExtensions.filter(
			galleryExtension =>
				getLocalExtensionIdFromGallery(
					galleryExtension,
	...
	private installExtension(
		...
		return pfs.rimraf(extensionPath).then(() => {
			return extract(zipPath, extensionPath, {
				...
				.then(({ manifest }) => {
					return pfs.readdir(extensionPath).then(children => {
						...
						const readmeUrl = readme
							? URI.file(path.join(extensionPath, readme)).toString()
							: null;
						const changelog = children.filter(child =>
							/^changelog(\.txt|\.md|)$/i.test(child)
						)[0];
						const changelogUrl = changelog
							? URI.file(path.join(extensionPath, changelog)).toString()
							: null;
		...
		return this.removeOutdatedExtensions().then(() => {
			return this.scanUserExtensions().then<void>(installed => {
				const promises = installed
					.filter(
						e =>
							e.manifest.publisher === extension.manifest.publisher &&
							e.manifest.name === extension.manifest.name
					)
	...
	private checkForDependenciesAndUninstall(
		...
		return this.preUninstallExtension(extension.id)
			.then(
				() =>
					this.hasDependencies(extension, installed)
						? this.promptForDependenciesAndUninstall(
								extension,
								installed,
								force
							)
						: this.promptAndUninstall(extension, installed, force)
			)
			.then(
				() => this.postUninstallExtension(extension.id),
				error => {
	...
	private hasDependencies(
		...
		if (
			...
			extension.manifest.extensionDependencies.length
		) {
			return installed.some(
				i =>
					extension.manifest.extensionDependencies.indexOf(
						getGalleryExtensionIdFromLocal(i)
					) !== -1
			);
		}
		return false;
	...
	private promptForDependenciesAndUninstall(
		extension: ILocalExtension,
		installed: ILocalExtension[],
		force: boolean
	): TPromise<void> {
		if (force) {
			const dependencies = distinct(
				this.getDependenciesToUninstallRecursively(extension, installed, [])
		...
		return this.choiceService
			.choose(Severity.Info, message, options, 2, true)
			.then<void>(
				value => {
					if (value === 0) {
					...
					if (value === 1) {
						const dependencies = distinct(
							this.getDependenciesToUninstallRecursively(
								extension,
								installed,
								[]
							)
						).filter(e => e !== extension);
						return this.uninstallWithDependencies(
							extension,
							dependencies,
							installed
						);
	...
	private uninstallWithDependencies(
		extension: ILocalExtension,
		dependencies: ILocalExtension[],
		installed: ILocalExtension[]
	): TPromise<void> {
		const dependenciesToUninstall = this.filterDependents(
			extension,
			dependencies,
			installed
		);
		let dependents = this.getDependents(extension, installed).filter(
			dependent =>
				extension !== dependent &&
				dependenciesToUninstall.indexOf(dependent) === -1
		);
		if (dependents.length) {
			return TPromise.wrapError<void>(
				this.getDependentsErrorMessage(extension, dependents)
			);
		}
		return TPromise.join([
			this.uninstallExtension(extension.id),
			...dependenciesToUninstall.map(d => this.doUninstall(d.id))
		]).then(() => null);
	}
	...
	private getDependenciesToUninstallRecursively(
		extension: ILocalExtension,
		installed: ILocalExtension[],
		...
		const dependenciesToUninstall = installed.filter(
			i =>
				extension.manifest.extensionDependencies.indexOf(
					getGalleryExtensionIdFromLocal(i)
				) !== -1
		...
		for (const dep of dependenciesToUninstall) {
			depsOfDeps.push(
				...this.getDependenciesToUninstallRecursively(dep, installed, checked)
			);
		}
	...
	private filterDependents(
		...
		installed = installed.filter(
			i =>
				...
				i.manifest.extensionDependencies.length > 0
		);
		let result = dependencies.slice(0);
		for (let i = 0; i < dependencies.length; i++) {
			const dep = dependencies[i];
			const dependents = this.getDependents(dep, installed).filter(
				e => dependencies.indexOf(e) === -1
			);
			if (dependents.length) {
	...
	private getDependents(
		...
		return installed.filter(
			e =>
				...
				e.manifest.extensionDependencies.indexOf(
					getGalleryExtensionIdFromLocal(extension)
				) !== -1
		);
	}
	...
	private doUninstall(id: string): TPromise<void> {
		return this.preUninstallExtension(id)
			...
			.then(
				() => this.postUninstallExtension(id),
				error => {
			...
			.then(
				exists =>
					exists
						? null
						: Promise.wrapError(
								new Error(nls.localize('notExists', 'Could not find extension'))
							)
			)
	...
	private scanUserExtensions(): TPromise<ILocalExtension[]> {
		return this.scanExtensions(
			this.extensionsPath,
			LocalExtensionType.User
		).then(extensions => {
			const byId = values(
				groupBy(extensions, p => getGalleryExtensionIdFromLocal(p))
			);
			return byId.map(
				p =>
					p.sort((a, b) =>
						...
						const each = () =>
							pfs
								...
								.then(children => {
									...
									return readManifest(extensionPath).then<
										ILocalExtension
									>(({ manifest, metadata }) => {
										if (manifest.extensionDependencies) {
											manifest.extensionDependencies = manifest.extensionDependencies.map(
	...
	private scanExtensionFolders(root: string): TPromise<string[]> {
		return this.getObsoleteExtensions().then(obsolete =>
			pfs
				.readdir(root)
	...
	private removeOutdatedExtensions(): TPromise<any> {
		return this.getOutdatedExtensionIds().then(extensionIds =>
			this.removeExtensions(extensionIds)
		);
	}
	...
	private removeExtensions(extensionsIds: string[]): TPromise<any> {
		return TPromise.join(
			extensionsIds.map(id => {
				return pfs
					.rimraf(path.join(this.extensionsPath, id))
					.then(() =>
						this.withObsoleteExtensions(obsolete => delete obsolete[id])
					);
			})
		);
	}
	...
	private getOutdatedExtensionIds(): TPromise<string[]> {
		return this.scanExtensionFolders(this.extensionsPath).then(folders => {
			const galleryFolders = folders
				.map(folder =>
					assign({ folder }, getIdAndVersionFromLocalExtensionId(folder))
				)
			...
			return flatten(
				byId.map(p =>
					p.sort((a, b) => semver.rcompare(a.version, b.version)).slice(1)
				)
			).map(a => a.folder);
		});
	}
	...
	private setObsolete(id: string): TPromise<void> {
		return this.withObsoleteExtensions(obsolete =>
			assign(obsolete, { [id]: true })
		);
	}
	...
	private withObsoleteExtensions<T>(
		fn: (obsolete: { [id: string]: boolean }) => T
	): TPromise<T> {
		return this.obsoleteFileLimiter.queue(() => {
			let result: T = null;
			return pfs
				.readFile(this.obsoletePath, 'utf8')
				.then<string>(
					null,
					err =>
						err.code === 'ENOENT' ? TPromise.as('{}') : TPromise.wrapError(err)
				)
				.then<{ [id: string]: boolean }>(raw => {
					try {
