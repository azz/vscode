...
export class IdleMonitor extends Disposable {
	...
	get onStatusChange(): Event<UserStatus> {
	...
	constructor(idleTime) {
		...
		this._register(
			dom.addDisposableListener(document, 'mousemove', () =>
				this._onUserActive()
			)
		);
		this._register(
			dom.addDisposableListener(document, 'keydown', () => this._onUserActive())
		);
		this._onUserActive();
	}
	...
	private _scheduleIdleCheck(): void {
		const minimumTimeWhenUserCanBecomeIdle =
			this._lastActiveTime + this._idleTime;
