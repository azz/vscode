...
class ErrorTestingSettings {
	...
	constructor() {
		...
		this.dangerousPathWithoutImportantInfo =
			this.filePrefix + this.personalInfo;
		...
		this.noSuchFileMessage =
			this.noSuchFilePrefix + " '" + this.personalInfo + "'";

		this.stack = [
			'at e._modelEvents (a/path/that/doesnt/contain/code/names.js:11:7309)',
...
suite('TelemetryService', () => {
	...
	test('common properties added to *all* events, event with data', function() {
		...
		return service
			.publicLog('testEvent', { hightower: 'xl', price: 8000 })
			.then(_ => {
				let [first] = testAppender.events;
	...
	test(
		...
		sinon.test(function() {
			...
			assert.equal(errorStub.callCount, 1);
			assert.equal(
				testAppender.events[0].data.filename.indexOf(
			...
			assert.equal(
				testAppender.events[0].data.filename,
	...
	test(
		...
		sinon.test(function() {
			...
			try {
				...
				let dangerousPathWithoutImportantInfoError: any = new Error(
				...
				assert.equal(
					testAppender.events[0].data.message.indexOf(settings.personalInfo),
					-1
				);
				assert.equal(
					testAppender.events[0].data.message.indexOf(settings.filePrefix),
					-1
				);

				assert.equal(
					testAppender.events[0].data.stack.indexOf(settings.personalInfo),
					-1
				);
				assert.equal(
					testAppender.events[0].data.stack.indexOf(settings.filePrefix),
					-1
				);
				assert.notEqual(
					testAppender.events[0].data.stack.indexOf(settings.stack[4]),
					-1
				);
				assert.equal(
					testAppender.events[0].data.stack.split('\n').length,
	...
	test(
		...
		sinon.test(function() {
			...
			try {
				...
				let dangerousPathWithImportantInfoError: any = new Error(
					settings.dangerousPathWithImportantInfo
				);
				...
				assert.notEqual(
					testAppender.events[0].data.message.indexOf(settings.importantInfo),
					-1
				);
				assert.equal(
					testAppender.events[0].data.message.indexOf(settings.personalInfo),
				...
				assert.notEqual(
					testAppender.events[0].data.stack.indexOf(settings.importantInfo),
					-1
				);
				assert.equal(
					testAppender.events[0].data.stack.indexOf(settings.personalInfo),
	...
	test(
		...
		sinon.test(function() {
			...
			try {
				...
				assert.notEqual(
					testAppender.events[0].data.message.indexOf(
				...
				assert.notEqual(
					testAppender.events[0].data.stack.indexOf(
						settings.missingModelPrefix
					),
					-1
				);
	...
	test(
		...
		sinon.test(function() {
			...
			assert.notEqual(
				testAppender.events[0].data.stack.indexOf(settings.missingModelPrefix),
				-1
			);
			assert.equal(
				testAppender.events[0].data.stack.indexOf(settings.personalInfo),
	...
	test(
		...
		sinon.test(function() {
			...
			try {
				...
				assert.notEqual(
					testAppender.events[0].data.stack.indexOf(settings.noSuchFilePrefix),
					-1
				);
				assert.equal(
					testAppender.events[0].data.stack.indexOf(settings.personalInfo),
	...
	test(
		...
		sinon.test(function() {
			let testAppender = new TestTelemetryAppender();
			let service = new TelemetryService(
				{ userOptIn: false, appender: testAppender },
				undefined
			);
	...
	test(
		'Telemetry Service does not sent optInStatus when user opted out',
		sinon.test(function() {
			let testAppender = new TestTelemetryAppender();
			let service = new TelemetryService(
				{ userOptIn: false, appender: testAppender },
			...
			return service
				.publicLog(optInStatusEventName, { optIn: false })
				.then(() => {
					assert.equal(testAppender.getEventsCount(), 0);
					service.dispose();
				});
	...
	test(
		...
		sinon.test(function() {
			let testAppender = new TestTelemetryAppender();
			let service = new TelemetryService(
				{ userOptIn: true, appender: testAppender },
				undefined
			);
