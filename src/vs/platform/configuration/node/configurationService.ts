...
import {
	ConfigModel,
	DefaultConfigModel
} from 'vs/platform/configuration/common/model';
...
export class ConfigurationService<T> extends Disposable
	...
	private _onDidUpdateConfiguration: Emitter<
		IConfigurationServiceEvent
	> = this._register(new Emitter<IConfigurationServiceEvent>());
	public readonly onDidUpdateConfiguration: Event<
		IConfigurationServiceEvent
	...
	constructor(@IEnvironmentService environmentService: IEnvironmentService) {
		...
		this.userConfigModelWatcher = new ConfigWatcher(
			environmentService.appSettingsPath,
			{
				changeBufferDelay: 300,
				defaultConfig: new ConfigModel<T>(
					null,
					environmentService.appSettingsPath
				),
				parse: (content: string, parseErrors: any[]) => {
					const userConfigModel = new ConfigModel<T>(
						content,
						environmentService.appSettingsPath
					);
		...
		// Listeners
		this._register(
			this.userConfigModelWatcher.onDidUpdateConfiguration(() =>
				this.onConfigurationChange(ConfigurationSource.User)
			)
		);
		this._register(
			Registry.as<IConfigurationRegistry>(
				...
				this.onConfigurationChange(ConfigurationSource.Default)
			)
		);
	}
	...
	private onConfigurationChange(source: ConfigurationSource): void {
		...
		this._onDidUpdateConfiguration.fire({
			...
			source,
			sourceConfig: source === ConfigurationSource.Default
				? cache.defaults.contents
				: cache.user.contents
		});
	}
	...
	public getConfiguration<C>(arg?: any): C {
		...
		const configModel = options.overrideIdentifier
			? cache.consolidated.configWithOverrides<C>(options.overrideIdentifier)
			: cache.consolidated;
	...
	public lookup<C>(
		...
		return {
			default: objects.clone(
				getConfigurationValue<C>(
					overrideIdentifier
						? cache.defaults.configWithOverrides(overrideIdentifier).contents
						: cache.defaults.contents,
					key
				)
			),
			user: objects.clone(
				getConfigurationValue<C>(
					overrideIdentifier
						? cache.user.configWithOverrides(overrideIdentifier).contents
						: cache.user.contents,
					key
				)
			),
			value: objects.clone(
				getConfigurationValue<C>(
					overrideIdentifier
						? cache.consolidated.configWithOverrides(overrideIdentifier)
								.contents
						: cache.consolidated.contents,
					key
				)
			)
		};
