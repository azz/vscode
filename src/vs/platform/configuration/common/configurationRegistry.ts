...
class ConfigurationRegistry implements IConfigurationRegistry {
	...
	validateProperty(property: string): boolean {
		return (
			!OVERRIDE_PROPERTY_PATTERN.test(property) &&
			this.getConfigurationProperties()[property] !== void 0
		);
	}
	...
	private updateOverridePropertyPatternKey(): void {
		let patternProperties: IJSONSchema = this.configurationSchema
		...
		delete this.configurationSchema.patternProperties[
			this.overridePropertyPattern
		];
		this.computeOverridePropertyPattern();
		this.configurationSchema.patternProperties[
			this.overridePropertyPattern
		] = patternProperties;
		contributionRegistry.registerSchema(schemaId, this.configurationSchema);
	}
	...
	private update(
		configuration: IConfigurationNode,
		overridePropertiesSchema: IJSONSchema
	): void {
		let properties = configuration.properties;
		if (properties) {
			for (let key in properties) {
				if (properties[key].overridable) {
					overridePropertiesSchema.properties[
						key
		...
		if (subNodes) {
			subNodes.forEach(subNode =>
				this.update(subNode, overridePropertiesSchema)
			);
		}
	}
...
configurationExtPoint.setHandler(extensions => {
	...
	for (let i = 0; i < extensions.length; i++) {
		const configuration = <IConfigurationNode>objects.clone(
			extensions[i].value
		);
		...
		if (configuration.title && typeof configuration.title !== 'string') {
			collector.error(
				nls.localize('invalid.title', "'configuration.title' must be a string")
			);
		}
...
defaultConfigurationExtPoint.setHandler(extensions => {
	const defaultConfigurations: IDefaultConfigurationExtension[] = extensions.map(
		extension => {
