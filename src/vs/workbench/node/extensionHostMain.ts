...
export class ExtensionHostMain {
	...
	constructor(remoteCom: IRemoteCom, initData: IInitData) {
		...
		this._contextService = new WorkspaceContextService(
			initData.contextService.workspace
		);
		const threadService = new ExtHostThreadService(remoteCom);
		const telemetryService = new RemoteTelemetryService(
			'pluginHostTelemetry',
			threadService
		);
		...
		errors.setUnexpectedErrorHandler(err =>
			mainThreadErrors.onUnexpectedExtHostError(
	...
	public terminate(): void {
		...
		try {
			...
			let activatedExtensions = allExtensionsIds.filter(id =>
				this._extensionService.isActivated(id)
			);
		...
		let extensionsDeactivated = TPromise.join(allPromises).then<void>(
			() => void 0
		);

		// Give extensions 1 second to wrap up any async dispose, then exit
		setTimeout(() => {
			TPromise.any<void>([TPromise.timeout(4000), extensionsDeactivated]).then(
				() => exit(),
				() => exit()
			);
		}, 1000);
	...
	private handleWorkspaceContainsEagerExtensions(): TPromise<void> {
		...
		this._extensionService.getAllExtensionDescriptions().forEach(desc => {
			...
			for (let i = 0; i < activationEvents.length; i++) {
				if (/^workspaceContains:/.test(activationEvents[i])) {
					let fileName = activationEvents[i].substr(
		...
		const matchingPatterns = Object.keys(desiredFilesMap).map(p => {
			...
			if (p.indexOf('*') > -1 || p.indexOf('?') > -1) {
				...
				return this._diskSearch
					.search(query)
					.then(result => (result.results.length ? p : undefined));
			} else {
				return pfs
					.exists(join(folderPath, p))
					.then(exists => (exists ? p : undefined));
			}
		});

		return TPromise.join(matchingPatterns).then(patterns => {
			patterns.filter(p => p !== undefined).forEach(p => {
	...
	private handleExtensionTests(): TPromise<void> {
		if (
			...
			!this._environment.extensionDevelopmentPath
		) {
			return TPromise.as(null);
		}
		...
		try {
			testRunner = <any>require.__$__nodeRequire(
				this._environment.extensionTestsPath
			);
		} catch (error) {
			requireError = error;
			...
			return new TPromise<void>((c, e) => {
				testRunner.run(
					...
					(error, failures) => {
						...
						this.gracefulExit(
							failures && failures > 0 ? 1 /* ERROR */ : 0 /* OK */
						);
					}
				);
			});
