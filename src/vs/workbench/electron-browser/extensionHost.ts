...
export const EXTENSION_TERMINATE_BROADCAST_CHANNEL =
...
export class ExtensionHostProcessWorker {
	...
	constructor(
		...
		this.isExtensionDevelopmentDebug =
			typeof environmentService.debugExtensionHost.port === 'number';
		this.isExtensionDevelopmentDebugBrk = !!environmentService
			.debugExtensionHost.break;
		this.isExtensionDevelopmentTestFromCli =
			this.isExtensionDevelopmentHost &&
			!!environmentService.extensionTestsPath &&
			!environmentService.debugExtensionHost.break;
	...
	public start(extensionService: MainProcessExtensionService): void {
		...
		TPromise.join<any>([this.tryListenOnPipe(), this.tryFindDebugPort()])
			.then(data => {
				...
				const opts = {
					...
					execArgv: port
						? [
								'--nolazy',
								(this.isExtensionDevelopmentDebugBrk
									? '--debug-brk='
									: '--debug=') + port
							]
						: undefined
				...
				const crashReporterOptions = this.crashReporterService.getChildProcessStartOptions(
					'extensionHost'
				);
				if (crashReporterOptions) {
					opts.env.CRASH_REPORTER_START_OPTIONS = JSON.stringify(
						crashReporterOptions
					);
				}

				// Run Extension Host as fork of current process
				this.extensionHostProcess = fork(
				...
				this.extensionHostProcess.on('exit', (code: any, signal: any) =>
					this.onExit(code, signal, onExit)
				);
				...
				return this.tryExtHostHandshake(server).then(() =>
			...
			findFreePort(
				...
				port => {
					...
					if (port !== extensionHostPort) {
						console.warn(
							`%c[Extension Host] %cProvided debugging port ${extensionHostPort} is not free, using ${port} instead.`,
							'color: blue',
							'color: black'
						);
					}
		...
		return new TPromise<IMessagePassingProtocol>((resolve, reject) => {
			...
			protocol.onMessage(msg => {
				if (msg === 'ready') {
					...
					return this.createExtHostInitData().then(data =>
						protocol.send(stringify(data))
					);
		...
		return TPromise.join<any>([
			...
			return <IInitData>{
				...
				environment: {
					...
					extensionDevelopmentPath: this.environmentService
					...
					enableProposedApiForAll:
						!this.environmentService.isBuilt ||
							(!!this.environmentService.extensionDevelopmentPath &&
								product.nameLong.indexOf('Insiders') >= 0),
					enableProposedApiFor:
						this.environmentService.args['enable-proposed-api'] || []
				},
				contextService: {
	...
	private _onWillShutdown(event: ShutdownEvent): void {
		...
		if (
			...
			event.veto(
				TPromise.timeout(100 /* wait a bit for IPC to get delivered */).then(
					() => false
				)
			);
		}
