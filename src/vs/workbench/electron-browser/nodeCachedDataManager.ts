...
declare type OnNodeCachedDataArgs = [
	{ errorCode: string; path: string; detail?: string },
...
export class NodeCachedDataManager {
	...
	private _handleCachedDataInfo(): void {
		...
		this._telemetryService.publicLog('cachedDataInfo', {
			didRequestCachedData: Boolean(
				global.require.getConfig().nodeCachedDataDir
			),
			didRejectCachedData,
