...
class LanguageSurvey {
	constructor(
			...
			fileService.onFileChanges(e => {
				e.getUpdated().forEach(event => {
					if (event.type === FileChangeType.UPDATED) {
						...
						if (
							model &&
							model.getModeId() === data.languageId &&
							date !==
								storageService.get(
		...
		const sessionCount =
			storageService.getInteger(SESSION_COUNT_KEY, StorageScope.GLOBAL, 0) + 1;
		...
		const isCandidate =
			storageService.getBoolean(IS_CANDIDATE_KEY, StorageScope.GLOBAL, false) ||
		...
		const takeSurveyAction = new Action(
			'takeSurvey',
			nls.localize('takeShortSurvey', 'Take Short Survey'),
			'',
			true,
			() => {
				...
				return telemetryService.getTelemetryInfo().then(info => {
					window.open(
						`${data.surveyUrl}?o=${encodeURIComponent(
							process.platform
						)}&v=${encodeURIComponent(pkg.version)}&m=${encodeURIComponent(
							info.machineId
						)}`
					);
		...
		const remindMeLaterAction = new Action(
			'later',
			nls.localize('remindLater', 'Remind Me later'),
			'',
			true,
			() => {
		...
		const neverAgainAction = new Action(
			'never',
			nls.localize('neverAgain', "Don't Show Again"),
			'',
			true,
			() => {
