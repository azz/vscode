...
import {
	TerminalLinkHandler,
	LineColumnInfo
} from 'vs/workbench/parts/terminal/electron-browser/terminalLinkHandler';
import {
	IWorkspace,
...
suite('Workbench - TerminalLinkHandler', () => {
	suite('localLinkRegex', () => {
		test('Windows', () => {
			...
			function testLink(
				...
				if (lineNo) {
					const lineColumnInfo: LineColumnInfo = terminalLinkHandler.extractLineColumnInfo(
						link
					);
	...
	suite('preprocessPath', () => {
		test('Windows', () => {
			...
			assert.equal(
				linkHandler.preprocessPath('./src/file1'),
				'C:\\base\\./src/file1'
			);
			assert.equal(
				linkHandler.preprocessPath('src\\file2'),
				'C:\\base\\src\\file2'
			);
			assert.equal(
				linkHandler.preprocessPath('C:\\absolute\\path\\file3'),
				'C:\\absolute\\path\\file3'
			);
		...
		test('Linux', () => {
			...
			assert.equal(
				linkHandler.preprocessPath('/absolute/path/file3'),
				'/absolute/path/file3'
			);
			stub.restore();
		});
		...
		test('No Workspace', () => {
			const linkHandler = new TestTerminalLinkHandler(
				...
				null,
				null,
				new WorkspaceContextService(null)
			);
