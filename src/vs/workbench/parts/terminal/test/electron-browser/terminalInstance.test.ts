...
suite('Workbench - TerminalInstance', () => {
	...
	test('TerminalInstance - createTerminalEnv', function() {
		...
		const env1 = TerminalInstance.createTerminalEnv(
			parentEnv1,
			shell1,
			'/foo',
			'en-au'
		...
		const env2 = TerminalInstance.createTerminalEnv(
			parentEnv2,
			shell2,
			'/foo',
			'en-au'
		...
		const env3 = TerminalInstance.createTerminalEnv(
			parentEnv1,
			shell1,
			'/',
			null
		...
		const env4 = TerminalInstance.createTerminalEnv(
			parentEnv2,
			shell1,
			'/',
			null
	...
	suite('_getCwd', () => {
		...
		setup(() => {
			...
			instance = instantiationService.createInstance(
		...
		test('should default to os.homedir() for an empty workspace', () => {
			assertPathsMatch(
				instance._getCwd({ executable: null, args: [] }, null),
				os.homedir()
			);
		});
		...
		test('should use to the workspace if it exists', () => {
			assertPathsMatch(
				instance._getCwd(
					{ executable: null, args: [] },
					{ resource: Uri.file('/foo') }
				),
				'/foo'
			);
		});
		...
		test('should ignore custom cwd when told to ignore', () => {
			...
			assertPathsMatch(
				instance._getCwd(
					{ executable: null, args: [], ignoreConfigurationCwd: true },
					{ resource: Uri.file('/bar') }
				),
				'/bar'
			);
