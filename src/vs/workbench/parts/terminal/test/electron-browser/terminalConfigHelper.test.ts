...
suite('Workbench - TerminalConfigHelper', () => {
	...
	test('TerminalConfigHelper - getFont fontFamily', function() {
		...
		configHelper = new TerminalConfigHelper(
			Platform.Linux,
	...
	test('TerminalConfigHelper - getFont fontSize', function() {
		...
		assert.equal(
			configHelper.getFont().fontSize,
			`${EDITOR_FONT_DEFAULTS.fontSize}px`,
			'The default editor font size should be used when editor.fontSize is 0 and terminal.integrated.fontSize not set'
		);
	...
	test('TerminalConfigHelper - getFont lineHeight', function() {
		...
		assert.equal(
			configHelper.getFont().lineHeight,
			1.2,
			'editor.lineHeight should be 1.2 when terminal.integrated.lineHeight not set'
		);
	});
});
