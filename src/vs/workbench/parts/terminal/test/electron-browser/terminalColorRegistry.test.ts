...
import {
	...
	registerColors
} from 'vs/workbench/parts/terminal/electron-browser/terminalColorRegistry';
...
suite('Workbench - TerminalColorRegistry', () => {
	test('hc colors', function() {
		...
		let colors = ansiColorIdentifiers.map(colorId =>
			theme.getColor(colorId).toRGBAHex(true)
		);
