...
export class TerminalConfigHelper implements ITerminalConfigHelper {
	...
	public get config(): ITerminalConfiguration {
		return this._configurationService.getConfiguration<
	...
	public getFont(): ITerminalFont {
		...
		let lineHeight = terminalConfig.lineHeight <= 0
			? DEFAULT_LINE_HEIGHT
	...
	public mergeDefaultShellPathAndArgs(shell: IShellLaunchConfig): void {
		...
		const platformKey = platform.isWindows
			? 'windows'
			: platform.isMacintosh ? 'osx' : 'linux';
		const shellConfigValue = this._workspaceConfigurationService.lookup<string>(
			`terminal.integrated.shell.${platformKey}`
		);
		const shellArgsConfigValue = this._workspaceConfigurationService.lookup<
			string[]
		>(`terminal.integrated.shellArgs.${platformKey}`);

		// Check if workspace setting exists and whether it's whitelisted
		let isWorkspaceShellAllowed = false;
		if (
			shellConfigValue.workspace !== undefined ||
			shellArgsConfigValue.workspace !== undefined
		) {
			isWorkspaceShellAllowed = this._storageService.getBoolean(
				IS_WORKSPACE_SHELL_ALLOWED_STORAGE_KEY,
				StorageScope.WORKSPACE,
				undefined
			);
		...
		if (isWorkspaceShellAllowed === undefined) {
			...
			if (shellArgsConfigValue.workspace) {
				argsString = `[${shellArgsConfigValue.workspace
					.map(v => '"' + v + '"')
					.join(', ')}]`;
			}
			...
			const options = [
				nls.localize('allow', 'Allow'),
				nls.localize('disallow', 'Disallow')
			];
			this._choiceService
		...
		shell.executable =
			(isWorkspaceShellAllowed
				? shellConfigValue.value
				: shellConfigValue.user) || shellConfigValue.default;
		shell.args =
			(isWorkspaceShellAllowed
				? shellArgsConfigValue.value
				: shellArgsConfigValue.user) || shellArgsConfigValue.default;
	}
