...
export class TerminalService extends AbstractTerminalService
	implements ITerminalService {
	private _configHelper: TerminalConfigHelper;

	public get configHelper(): ITerminalConfigHelper {
	...
	constructor(
		...
		this._configHelper = this._instantiationService.createInstance(
	...
	public createInstance(
		...
		wasNewTerminalAction?: boolean
	): ITerminalInstance {
		let terminalInstance = this._instantiationService.createInstance(
			TerminalInstance,
		...
		terminalInstance.addDisposable(
			terminalInstance.onTitleChanged(
				this._onInstanceTitleChanged.fire,
				this._onInstanceTitleChanged
			)
		);
		terminalInstance.addDisposable(
			terminalInstance.onDisposed(
				this._onInstanceDisposed.fire,
				this._onInstanceDisposed
			)
		);
		terminalInstance.addDisposable(
			terminalInstance.onDataForApi(
				this._onInstanceData.fire,
				this._onInstanceData
			)
		);
		terminalInstance.addDisposable(
			terminalInstance.onProcessIdReady(
				this._onInstanceProcessIdReady.fire,
	...
	private _suggestShellChange(wasNewTerminalAction?: boolean): void {
		...
		const neverSuggest = this._storageService.getBoolean(
			NEVER_SUGGEST_SELECT_WINDOWS_SHELL_STORAGE_KEY,
			StorageScope.GLOBAL,
			false
		);
		...
		if (
			this._configHelper.config.shell.windows !== TERMINAL_DEFAULT_SHELL_WINDOWS
		) {
		...
		const options = [
			nls.localize('customize', 'Customize'),
			nls.localize('cancel', 'Cancel'),
			nls.localize('never again', 'OK, Never Show Again')
		];
		this._choiceService
			.choose(Severity.Info, message, options, 1)
			.then(choice => {
				switch (choice) {
					case 0:
	...
	public selectDefaultWindowsShell(): TPromise<string> {
		return this._detectWindowsShells().then(shells => {
			...
			return this._quickOpenService.pick(shells, options).then(value => {
				...
				const configChange = {
					key: 'terminal.integrated.shell.windows',
					value: shell
				};
				return this._configurationEditingService
					.writeConfiguration(ConfigurationTarget.USER, configChange)
					.then(() => shell);
			});
		});
	}
	...
	private _detectWindowsShells(): TPromise<IPickOpenEntry[]> {
		...
		Object.keys(expectedLocations).forEach(key =>
			promises.push(this._validateShellPaths(key, expectedLocations[key]))
		);
	...
	private _validateShellPaths(
		label: string,
		potentialPaths: string[]
	): TPromise<[string, string]> {
	...
	public getActiveOrCreateInstance(
		wasNewTerminalAction?: boolean
	): ITerminalInstance {
		const activeInstance = this.getActiveInstance();
		return activeInstance
			? activeInstance
			: this.createInstance(undefined, wasNewTerminalAction);
	}
