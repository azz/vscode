...
const newUserEntries = [
	...
	KeybindingsReferenceAction.AVAILABLE
...
export class WatermarkContribution implements IWorkbenchContribution {
	...
	constructor(
		...
		this.partService.joinCreation().then(() => {
			this.enabled = this.configurationService.lookup<boolean>(
		...
		this.configurationService.onDidUpdateConfiguration(e => {
			const enabled = this.configurationService.lookup<boolean>(
				'workbench.tips.enabled'
			).value;
	...
	private create(): void {
		...
		this.watermark = $().div({ class: 'watermark' });
		const box = $(this.watermark).div({ class: 'watermark-box' });
		const folder = this.contextService.hasWorkspace();
		const newUser = this.telemetryService.getExperiments().showNewUserWatermark;
		const selected = (newUser
			? newUserEntries
			: folder ? folderEntries : noFolderEntries).filter(
			entry => !('mac' in entry) || entry.mac === isMacintosh
		);
		const update = () => {
			const builder = $(box);
	...
	private destroy(): void {
		if (this.watermark) {
			...
			this.partService
				.getContainer(Parts.EDITOR_PART)
				.classList.remove('has-watermark');
			this.dispose();
		}
	}
