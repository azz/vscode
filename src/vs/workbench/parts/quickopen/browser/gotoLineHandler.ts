...
class GotoLineEntry extends EditorQuickOpenEntry {
	...
	public getLabel(): string {
		...
		return this.column
			...
			: nls.localize('gotoLineLabel', 'Go to line {0}', this.line);
	}

	private invalidRange(
		maxLineNumber: number = this.getMaxLineNumber()
	): boolean {
		return (
			!this.line ||
			!types.isNumber(this.line) ||
			(maxLineNumber > 0 &&
				types.isNumber(this.line) &&
				this.line > maxLineNumber)
		);
	}
	...
	private getMaxLineNumber(): number {
		...
		return model && types.isFunction((<ITextModel>model).getLineCount)
			? (<ITextModel>model).getLineCount()
			: -1;
	}
...
export class GotoLineHandler extends QuickOpenHandler {
	...
	public getResults(searchValue: string): TPromise<QuickOpenModel> {
		...
		return TPromise.as(
			new QuickOpenModel([
				new GotoLineEntry(searchValue, this.editorService, this)
			])
		);
	}
