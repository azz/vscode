...
export class HelpHandler extends QuickOpenHandler {
	...
	public getResults(searchValue: string): TPromise<QuickOpenModel> {
		...
		handlerDescriptors
			.sort((h1, h2) => h1.prefix.localeCompare(h2.prefix))
			.forEach(handlerDescriptor => {
				if (handlerDescriptor.prefix !== HELP_PREFIX) {
					...
					if (types.isArray(handlerDescriptor.helpEntries)) {
						...
						entry = new HelpEntry(
							handlerDescriptor.prefix,
							handlerDescriptor.description,
							this.quickOpenService,
							searchValue.length > 0
						);
						workbenchScoped.push(entry);
		...
		if (workbenchScoped.length > 0) {
			workbenchScoped[0].setGroupLabel(
				nls.localize('globalCommands', 'global commands')
			);
		}

		// Add separator for editor scoped handlers
		if (editorScoped.length > 0) {
			editorScoped[0].setGroupLabel(
				nls.localize('editorCommands', 'editor commands')
			);
			if (workbenchScoped.length > 0) {
		...
		return TPromise.as(
			new QuickOpenModel([...workbenchScoped, ...editorScoped])
		);
	}
