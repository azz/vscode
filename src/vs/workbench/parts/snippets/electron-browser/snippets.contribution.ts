...
class OpenSnippetsAction extends actions.Action {
	...
	private openFile(filePath: string): winjs.TPromise<void> {
		return this.windowsService.openWindow([filePath], {
	...
	public run(): winjs.Promise {
		...
		picks = picks.sort((e1, e2) => e1.label.localeCompare(e2.label));
			...
			.then(language => {
				if (language) {
					var snippetPath = join(
						this.environmentService.appSettingsHome,
						'snippets',
						language.id + '.json'
					);
...
var workbenchActionsRegistry = <workbenchActionRegistry.IWorkbenchActionRegistry>platform.Registry.as(
...
workbenchActionsRegistry.registerWorkbenchAction(
	new SyncActionDescriptor(
		OpenSnippetsAction,
		OpenSnippetsAction.ID,
		OpenSnippetsAction.LABEL
	),
	'Preferences: Open User Snippets',
	preferencesCategory
);

(<workbenchContributions.IWorkbenchContributionsRegistry>platform.Registry.as(
	workbenchContributions.Extensions.Workbench
...
let schemaRegistry = <JSONContributionRegistry.IJSONContributionRegistry>platform.Registry.as(
