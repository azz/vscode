...
import {
	ISnippetsService,
	ISnippet
} from 'vs/workbench/parts/snippets/electron-browser/snippetsService';
...
let snippetsExtensionPoint = ExtensionsRegistry.registerExtensionPoint<
	ISnippetsExtensionPoint[]
>('snippets', [languagesExtPoint], {
	...
	items: {
		type: 'object',
		defaultSnippets: [
			{ body: { language: '${1:id}', path: './snippets/${2:id}.json.' } }
		],
		properties: {
			language: {
				description: nls.localize(
...
export class MainProcessTextMateSnippet implements IWorkbenchContribution {
		...
		snippetsExtensionPoint.setHandler(extensions => {
			for (let i = 0; i < extensions.length; i++) {
				...
				for (let j = 0; j < tmSnippets.length; j++) {
					this._withSnippetContribution(
						extensions[i].description.name,
	...
	private _withSnippetContribution(
		...
		if (
			...
			!this._modeService.isRegisteredMode(snippet.language)
		) {
			collector.error(
				nls.localize(
					'invalid.language',
					'Unknown language in `contributes.{0}.language`. Provided value: {1}',
					snippetsExtensionPoint.name,
...
export function readAndRegisterSnippets(
	...
	return readFile(filePath).then(
		fileContents => {
			let snippets = parseSnippetFile(
				fileContents.toString(),
				extensionName,
				collector
			);
			snippetService.registerSnippets(
	...
	function fixBogousVariables(marker: Marker): string {
		if (marker instanceof Text) {
			...
			if (
				...
				let index = placeholders.has(marker.name)
