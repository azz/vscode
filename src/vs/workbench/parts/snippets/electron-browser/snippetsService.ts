...
export const ISnippetsService = createDecorator<ISnippetsService>(
	'snippetService'
);
...
export interface ISnippetsService {
	...
	visitSnippets(
		languageId: LanguageId,
		accept: (snippet: ISnippet) => boolean
	): void;
...
export class SnippetSuggestProvider implements ISuggestSupport {
	...
	provideCompletionItems(model: IModel, position: Position): ISuggestResult {
		...
		const lowWordUntil = model
			.getWordUntilPosition(position)
			.word.toLowerCase();
		const lowLineUntil = model
			.getLineContent(position.lineNumber)
			.substr(Math.max(0, position.column - 100), position.column - 1)
			.toLowerCase();
			...
			if (lowWordUntil.length > 0 && startsWith(lowPrefix, lowWordUntil)) {
				...
				accetSnippet =
					...
					!model.getWordAtPosition(
						new Position(position.lineNumber, position.column - overwriteBefore)
					);
			}
			...
			if (accetSnippet) {
				items.push({
					...
					suggestion: {
						...
						detail:
							snippet.extensionName ||
								localize('detail.userSnippet', 'User Snippet'),
		...
		for (const item of items.sort(
			SnippetSuggestProvider._compareSuggestionsByLabel
		)) {
			if (lastItem && lastItem.suggestion.label === item.suggestion.label) {
				// use the disambiguateLabel instead of the actual label
				lastItem.suggestion.label = localize(
					'snippetSuggest.longLabel',
					'{0}, {1}',
					lastItem.suggestion.label,
					lastItem.snippet.name
				);
				item.suggestion.label = localize(
					'snippetSuggest.longLabel',
					'{0}, {1}',
	...
	private static _compareSuggestionsByLabel(
		a: ISnippetSuggestion,
		b: ISnippetSuggestion
	): number {
