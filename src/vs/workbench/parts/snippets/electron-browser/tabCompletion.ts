...
export class TabCompletionController
	...
	static ContextKey = new RawContextKey<boolean>(
	...
	constructor(
		...
		const hasSnippets = TabCompletionController.ContextKey.bindTo(
			contextKeyService
		);
		this._cursorChangeSubscription = editor.onDidChangeCursorSelection(e => {
			...
			if (selectFn) {
				snippetService.visitSnippets(
					editor.getModel().getLanguageIdentifier().id,
					s => {
						if (selectFn(s)) {
	...
	performSnippetCompletions(): void {
		if (this._currentSnippets.length === 1) {
			...
			this._snippetController.insert(
				snippet.codeSnippet,
