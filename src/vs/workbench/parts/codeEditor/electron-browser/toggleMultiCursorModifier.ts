...
export class ToggleMultiCursorModifierAction extends Action {
	...
	constructor(
		...
		this.enabled =
			!!this.configurationService && !!this.configurationEditingService;
	}

	public run(): TPromise<any> {
		const editorConf = this.configurationService.getConfiguration<{
			multiCursorModifier: 'ctrlCmd' | 'alt';
		}>('editor');
		const newValue: 'ctrlCmd' | 'alt' = editorConf.multiCursorModifier ===
			'ctrlCmd'
			? 'alt'
			: 'ctrlCmd';
...
registry.registerWorkbenchAction(
	new SyncActionDescriptor(
		...
		ToggleMultiCursorModifierAction.ID,
		ToggleMultiCursorModifierAction.LABEL
	),
	'Toggle Multi-Cursor Modifier'
);
