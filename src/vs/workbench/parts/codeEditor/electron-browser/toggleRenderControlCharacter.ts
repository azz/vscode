...
export class ToggleRenderControlCharacterAction extends EditorAction {
	...
	public run(accessor: ServicesAccessor, editor: ICommonCodeEditor): void {
		const configurationEditingService = accessor.get(
			IConfigurationEditingService
		);

		let newRenderControlCharacters = !editor.getConfiguration().viewInfo
			.renderControlCharacters;

		configurationEditingService.writeConfiguration(ConfigurationTarget.USER, {
			key: 'editor.renderControlCharacters',
			value: newRenderControlCharacters
		});
	}
}
