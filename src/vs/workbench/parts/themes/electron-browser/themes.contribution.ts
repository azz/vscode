...
export class SelectColorThemeAction extends Action {
	...
	run(): TPromise<void> {
		return this.themeService.getColorThemes().then(themes => {
			...
			const pickInMarketPlace = findInMarketplacePick(
				this.viewletService,
				'category:themes',
				localize('installColorThemes', 'Install Additional Color Themes...')
			);
			...
			const selectTheme = (theme, applyTheme) => {
				...
				if (applyTheme) {
					...
					target = typeof confValue.workspace !== 'undefined'
						? ConfigurationTarget.WORKSPACE
						: ConfigurationTarget.USER;
				}

				this.themeService.setColorTheme(theme.id, target).done(null, err => {
			...
			return this.quickOpenService
				.pick(picks, { placeHolder, autoFocus: { autoFocusIndex } })
				.then(
					theme =>
						delayer.trigger(() => selectTheme(theme || currentTheme, true), 0),
					null,
	...
	run(): TPromise<void> {
		return this.themeService.getFileIconThemes().then(themes => {
			...
			const pickInMarketPlace = findInMarketplacePick(
				this.viewletService,
				'tag:icon-theme',
				localize('installIconThemes', 'Install Additional File Icon Themes...')
			);
			...
			const selectTheme = (theme, applyTheme) => {
				...
				this.themeService
					.setFileIconTheme(theme && theme.id, target)
					.done(null, err => {
...
function findInMarketplacePick(
	...
	return {
		...
		alwaysShow: true,
		run: () =>
			viewletService.openViewlet(VIEWLET_ID, true).then(viewlet => {
...
class GenerateColorThemeAction extends Action {
	...
	run(): TPromise<any> {
		...
		let colorRegistry = <IColorRegistry>Registry.as(
...
const colorThemeDescriptor = new SyncActionDescriptor(
...
const iconThemeDescriptor = new SyncActionDescriptor(
...
const generateColorThemeDescriptor = new SyncActionDescriptor(
	GenerateColorThemeAction,
	GenerateColorThemeAction.ID,
	GenerateColorThemeAction.LABEL
);
Registry.as<IWorkbenchActionRegistry>(
	Extensions.WorkbenchActions
).registerWorkbenchAction(
