...
export function registerContributions(): void {
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		handler: (accessor, args: any) => {
			const markersPanel = <MarkersPanel>accessor
				.get(IPanelService)
				.getActivePanel();
			markersPanel.openFileAtElement(
				markersPanel.getFocusElement(),
				false,
				true,
				true
			);
	...
	registry.registerWorkbenchAction(
		...
		'View: Show Problems',
		Messages.MARKERS_PANEL_VIEW_CATEGORY
	);
	...
	registry.registerWorkbenchAction(
		new SyncActionDescriptor(
			ToggleErrorsAndWarningsAction,
			ToggleErrorsAndWarningsAction.ID,
			ToggleErrorsAndWarningsAction.LABEL
		),
