...
} from 'vs/workbench/parts/debug/electron-browser/electronDebugActions';
...
export function renderExpressionValue(
	expressionOrValue: debug.IExpression | string,
	container: HTMLElement,
	options: IRenderValueOptions
): void {
	let value = typeof expressionOrValue === 'string'
		? expressionOrValue
	...
	if (
		options.showChanged &&
		(<any>expressionOrValue).valueChanged &&
	...
	const wrapUp = once((renamed: boolean) => {
		if (!disposed) {
			...
			if (element instanceof Expression && renamed && inputBox.value) {
				debugService
					.renameWatchExpression(element.getId(), inputBox.value)
				...
				debugService
					.renameFunctionBreakpoint(
						element.getId(),
						renamed ? inputBox.value : element.name
					)
					.done(null, errors.onUnexpectedError);
...
function getSourceName(
	...
	return getPathLabel(
		paths.basename(source.uri.fsPath),
		contextService,
		environmentService
	);
}
...
export class BaseDebugController extends DefaultController {
	...
	constructor(
		...
		this.contributedContextMenu = menuService.createMenu(
			menuId,
			contextKeyService
		);
	}
			...
			this.contextMenuService.showContextMenu({
				...
				getActions: () =>
					this.actionProvider
						...
						.then(actions => {
							fillInActions(
								this.contributedContextMenu,
								{ arg: this.getContext(element) },
								actions
							);
							return actions;
						}),
...
export class CallStackController extends BaseDebugController {
	...
	public showMoreStackFrames(
		tree: ITree,
		threadAndProcessIds: ThreadAndProcessIds
	): boolean {
		const process = this.debugService
			.getModel()
			.getProcesses()
			.filter(p => p.getId() === threadAndProcessIds.processId)
			.pop();
		...
		if (element instanceof Process) {
			...
			if (element.thread.process.session.capabilities.supportsRestartFrame) {
				actions.push(
					this.instantiationService.createInstance(
						RestartFrameAction,
						RestartFrameAction.ID,
						RestartFrameAction.LABEL
					)
				);
			}
			actions.push(
				new CopyStackTraceAction(
					CopyStackTraceAction.ID,
					CopyStackTraceAction.LABEL
				)
			);
		}
...
export class CallStackDataSource implements IDataSource {
	...
	public hasChildren(tree: ITree, element: any): boolean {
		return (
			...
			(element instanceof Thread && (<Thread>element).stopped)
		);
	}
	...
	private getThreadChildren(thread: Thread): TPromise<any> {
		...
		if (thread.stoppedDetails && thread.stoppedDetails.framesErrorMessage) {
			return TPromise.as(
				callStack.concat([thread.stoppedDetails.framesErrorMessage])
			);
		}
		if (
			thread.stoppedDetails &&
			thread.stoppedDetails.totalFrames > callStack.length &&
			callStack.length > 1
		) {
			return TPromise.as(
				callStack.concat([
					new ThreadAndProcessIds(thread.process.getId(), thread.threadId)
				])
			);
		}
...
export class CallStackRenderer implements IRenderer {
	...
	private renderProcess(
		...
		data: IProcessTemplateData
	): void {
		data.process.title = nls.localize(
			{ key: 'process', comment: ['Process is a noun'] },
			'Process'
		);
		data.name.textContent = process.name;
		const stoppedThread = process.getAllThreads().filter(t => t.stopped).pop();

		data.stateLabel.textContent = stoppedThread
	...
	private renderThread(thread: debug.IThread, data: IThreadTemplateData): void {
		...
		if (thread.stopped) {
			...
			data.stateLabel.textContent = nls.localize(
				{ key: 'running', comment: ['indicates state'] },
				'Running'
			);
		}
	...
	private renderLoadMore(element: any, data: ILoadMoreTemplateData): void {
		data.label.textContent = nls.localize(
			'loadMoreStackFrames',
			'Load More Stack Frames'
		);
	}

	private renderStackFrame(
		stackFrame: debug.IStackFrame,
		...
		dom.toggleClass(
			data.stackFrame,
			'disabled',
			stackFrame.source.presenationHint === 'deemphasize'
		);
		dom.toggleClass(
			data.stackFrame,
			'label',
			stackFrame.source.presenationHint === 'label'
		);
		dom.toggleClass(
			data.stackFrame,
			'subtle',
			stackFrame.source.presenationHint === 'subtle'
		);
		...
		data.fileName.textContent = getSourceName(
			stackFrame.source,
			this.contextService,
	...
	public getAriaLabel(tree: ITree, element: any): string {
		...
		if (element instanceof StackFrame) {
			return nls.localize(
				...
				(<StackFrame>element).name,
				(<StackFrame>element).range.startLineNumber,
				getSourceName((<StackFrame>element).source, this.contextService)
			);
		}
...
export class VariablesActionProvider implements IActionProvider {
	...
	public getSecondaryActions(tree: ITree, element: any): TPromise<IAction[]> {
		...
		actions.push(
			this.instantiationService.createInstance(
				AddToWatchExpressionsAction,
				AddToWatchExpressionsAction.ID,
				AddToWatchExpressionsAction.LABEL,
				variable
			)
		);
...
export class VariablesDataSource implements IDataSource {
	...
	public getChildren(tree: ITree, element: any): TPromise<any> {
		if (element instanceof ViewModel) {
			...
			return focusedStackFrame
				? focusedStackFrame.getScopes()
				: TPromise.as([]);
		}
...
export class VariablesRenderer implements IRenderer {
	...
	public renderElement(
		...
		if (templateId === VariablesRenderer.SCOPE_TEMPLATE_ID) {
			...
			const variable = <Variable>element;
			if (
				variable === this.debugService.getViewModel().getSelectedExpression() ||
				...
				renderRenameBox(
					...
					tree,
					variable,
					(<IVariableTemplateData>templateData).expression,
					{
	...
	public getSecondaryActions(tree: ITree, element: any): TPromise<IAction[]> {
		...
		if (element instanceof Expression) {
			...
			actions.push(
				this.instantiationService.createInstance(
					AddWatchExpressionAction,
					AddWatchExpressionAction.ID,
					AddWatchExpressionAction.LABEL
				)
			);
			actions.push(
				this.instantiationService.createInstance(
					EditWatchExpressionAction,
					EditWatchExpressionAction.ID,
					EditWatchExpressionAction.LABEL
				)
			);
			...
			actions.push(
				this.instantiationService.createInstance(
					RemoveWatchExpressionAction,
					RemoveWatchExpressionAction.ID,
					RemoveWatchExpressionAction.LABEL
				)
			);
			actions.push(
				this.instantiationService.createInstance(
					RemoveAllWatchExpressionsAction,
					RemoveAllWatchExpressionsAction.ID,
					RemoveAllWatchExpressionsAction.LABEL
				)
			);
		} else {
			actions.push(
...
export class WatchExpressionsDataSource implements IDataSource {
	...
	public hasChildren(tree: ITree, element: any): boolean {
		...
		return (
			watchExpression.hasChildren &&
			!equalsIgnoreCase(watchExpression.value, 'null')
		);
	}
...
export class WatchExpressionsRenderer implements IRenderer {
	...
	private renderWatchExpression(
		tree: ITree,
		watchExpression: debug.IExpression,
		data: IWatchExpressionTemplateData
	): void {
		let selectedExpression = this.debugService
			.getViewModel()
			.getSelectedExpression();
		if (
			(selectedExpression instanceof Expression &&
				selectedExpression.getId() === watchExpression.getId()) ||
			(watchExpression instanceof Expression && !watchExpression.name)
		) {
			renderRenameBox(
				this.debugService,
				this.contextViewService,
		...
		if (watchExpression.value) {
			...
			data.name.title = watchExpression.type
				? watchExpression.type
...
export class WatchExpressionsAccessibilityProvider
...
export class WatchExpressionsDragAndDrop extends DefaultDragAndDrop {
	...
	public onDragOver(
		...
		target: Expression | Model,
		originalEvent: DragMouseEvent
	): IDragOverReaction {
	...
	public drop(
		...
		if (Array.isArray(draggedData)) {
			...
			const position = target instanceof Model
				? watches.length - 1
				: watches.indexOf(target);
...
export class BreakpointsActionProvider implements IActionProvider {
	constructor(
		private instantiationService: IInstantiationService,
		private debugService: debug.IDebugService
	) {
		// noop
	}
	...
	public getSecondaryActions(tree: ITree, element: any): TPromise<IAction[]> {
		...
		if (
			...
			actions.push(
				this.instantiationService.createInstance(
					RemoveBreakpointAction,
					RemoveBreakpointAction.ID,
					RemoveBreakpointAction.LABEL
				)
			);
		}
		if (
			this.debugService.getModel().getBreakpoints().length +
				this.debugService.getModel().getFunctionBreakpoints().length >
			1
		) {
			actions.push(
				this.instantiationService.createInstance(
					RemoveAllBreakpointsAction,
					RemoveAllBreakpointsAction.ID,
					RemoveAllBreakpointsAction.LABEL
				)
			);
			actions.push(new Separator());
			...
			actions.push(
				this.instantiationService.createInstance(
					EnableAllBreakpointsAction,
					EnableAllBreakpointsAction.ID,
					EnableAllBreakpointsAction.LABEL
				)
			);
			actions.push(
				this.instantiationService.createInstance(
					DisableAllBreakpointsAction,
					DisableAllBreakpointsAction.ID,
					DisableAllBreakpointsAction.LABEL
				)
			);
		}
		...
		actions.push(
			this.instantiationService.createInstance(
				ReapplyBreakpointsAction,
				ReapplyBreakpointsAction.ID,
				ReapplyBreakpointsAction.LABEL
			)
		);
...
export class BreakpointsRenderer implements IRenderer {
	...
	public renderTemplate(
		...
		if (templateId === BreakpointsRenderer.BREAKPOINT_TEMPLATE_ID) {
			...
			const lineNumberContainer = dom.append(
				data.breakpoint,
	...
	public renderElement(
		...
		if (templateId === BreakpointsRenderer.EXCEPTION_BREAKPOINT_TEMPLATE_ID) {
			...
			templateId === BreakpointsRenderer.FUNCTION_BREAKPOINT_TEMPLATE_ID
		) {
	...
	private renderExceptionBreakpoint(
		exceptionBreakpoint: debug.IExceptionBreakpoint,
		data: IBaseBreakpointTemplateData
	): void {
		data.name.textContent =
	...
	private renderFunctionBreakpoint(
		tree: ITree,
		functionBreakpoint: debug.IFunctionBreakpoint,
		data: IBaseBreakpointTemplateData
	): void {
		const selected = this.debugService
			.getViewModel()
			.getSelectedFunctionBreakpoint();
		if (
			!functionBreakpoint.name ||
			(selected && selected.getId() === functionBreakpoint.getId())
		) {
			data.name.textContent = '';
			renderRenameBox(
				this.debugService,
			...
			if (
				...
				if (
					...
					data.breakpoint.title = nls.localize(
	...
	private renderBreakpoint(
		tree: ITree,
		breakpoint: debug.IBreakpoint,
		data: IBreakpointTemplateData
	): void {
		this.debugService.getModel().areBreakpointsActivated()
			? tree.removeTraits('disabled', [breakpoint])
			: tree.addTraits('disabled', [breakpoint]);

		data.name.textContent = getPathLabel(
			paths.basename(breakpoint.uri.fsPath),
			this.contextService
		);
		...
		data.filePath.textContent = getPathLabel(
			paths.dirname(breakpoint.uri.fsPath),
			this.contextService,
			this.environmentService
		);
		...
		const debugActive =
			this.debugService.state === debug.State.Running ||
			this.debugService.state === debug.State.Stopped ||
			this.debugService.state === debug.State.Initializing;
...
export class BreakpointsAccessibilityProvider
	implements IAccessibilityProvider {
	constructor(
		@IWorkspaceContextService private contextService: IWorkspaceContextService
	) {
	...
	public getAriaLabel(tree: ITree, element: any): string {
		if (element instanceof Breakpoint) {
			return nls.localize(
				...
				(<Breakpoint>element).lineNumber,
				getPathLabel(
					paths.basename((<Breakpoint>element).uri.fsPath),
					this.contextService
				),
				this.contextService
			);
		...
		if (element instanceof FunctionBreakpoint) {
			return nls.localize(
				...
				(<FunctionBreakpoint>element).name
			);
		}
		if (element instanceof ExceptionBreakpoint) {
			return nls.localize(
				...
				(<ExceptionBreakpoint>element).filter
			);
		}
