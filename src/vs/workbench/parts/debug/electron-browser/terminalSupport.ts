...
export class TerminalSupport {
	...
	public static runInTerminal(
		...
		nativeTerminalService: IExternalTerminalService,
		configurationService: IConfigurationService,
		args: DebugProtocol.RunInTerminalRequestArguments,
		response: DebugProtocol.RunInTerminalResponse
	): TPromise<void> {
		if (args.kind === 'external') {
			return nativeTerminalService.runInTerminal(
				args.title,
				args.cwd,
				args.args,
				args.env || {}
		...
		if (!TerminalSupport.integratedTerminalInstance) {
			TerminalSupport.integratedTerminalInstance = terminalService.createInstance(
		...
		if (!TerminalSupport.terminalDisposedListener) {
			...
			TerminalSupport.terminalDisposedListener = terminalService.onInstanceDisposed(
				terminal => {
					if (
						TerminalSupport.integratedTerminalInstance &&
						TerminalSupport.integratedTerminalInstance.id === terminal.id
					) {
		...
		terminalService.setActiveInstance(
			TerminalSupport.integratedTerminalInstance
		);
	...
	private static prepareCommand(
		...
		let shell: string;
		const shell_config = (<ITerminalConfiguration>configurationService.getConfiguration<
			any
