...
} from 'vs/workbench/parts/debug/electron-browser/replViewer';
...
const IPrivateReplService = createDecorator<IPrivateReplService>(
	'privateReplService'
);
...
export class Repl extends Panel implements IPrivateReplService {
		...
		this.toDispose.push(
			this.debugService.getModel().onDidChangeReplElements(() => {
				this.refreshReplElements(
					this.debugService.getModel().getReplElements().length === 0
				);
			})
		);
		this.toDispose.push(
			this.panelService.onDidPanelOpen(panel => this.refreshReplElements(true))
		);
	}
	...
	public create(parent: Builder): TPromise<void> {
		...
		const controller = this.instantiationService.createInstance(
			ReplExpressionsController,
			new ReplExpressionsActionProvider(this.instantiationService),
	...
	private createReplInput(container: HTMLElement): void {
		...
		const onFirstReplLine = debug.CONTEXT_ON_FIRST_DEBUG_REPL_LINE.bindTo(
			scopedContextKeyService
		);
		onFirstReplLine.set(true);
		const onLastReplLine = debug.CONTEXT_ON_LAST_DEBUG_REPL_LINE.bindTo(
			scopedContextKeyService
		);
		...
		const scopedInstantiationService = this.instantiationService.createChild(
			new ServiceCollection(
				...
				[IPrivateReplService, this]
			)
		);
		this.replInput = scopedInstantiationService.createInstance(
			ReplInputEditor,
			this.replInputContainer,
			this.getReplInputOptions()
		);
		const model = this.modelService.createModel(
			'',
			null,
			uri.parse(`${debug.DEBUG_SCHEME}:input`)
		);
		this.replInput.setModel(model);

		modes.SuggestRegistry.register(
			{ scheme: debug.DEBUG_SCHEME },
			{
				triggerCharacters: ['.'],
				provideCompletionItems: (
					...
					const text = this.replInput
						.getModel()
						.getLineContent(position.lineNumber);
					const focusedStackFrame = this.debugService.getViewModel()
						.focusedStackFrame;
					const frameId = focusedStackFrame
						? focusedStackFrame.frameId
						: undefined;
					const focusedProcess = this.debugService.getViewModel()
						.focusedProcess;
					const completions = focusedProcess
						? focusedProcess.completions(
								frameId,
								text,
								position,
								overwriteBefore
		...
		this.toDispose.push(
			this.replInput.onDidScrollChange(e => {
				if (!e.scrollHeightChanged) {
					return;
				}
				this.replInputHeight = Math.max(
					Repl.REPL_INPUT_INITIAL_HEIGHT,
					Math.min(
						Repl.REPL_INPUT_MAX_HEIGHT,
						e.scrollHeight,
		...
		this.toDispose.push(
			this.replInput.onDidChangeCursorPosition(e => {
				onFirstReplLine.set(e.position.lineNumber === 1);
				onLastReplLine.set(
					e.position.lineNumber === this.replInput.getModel().getLineCount()
				);
			})
		);
		...
		this.toDispose.push(
			dom.addStandardDisposableListener(
				this.replInputContainer,
				dom.EventType.FOCUS,
				() => dom.addClass(this.replInputContainer, 'synthetic-focus')
			)
		);
		this.toDispose.push(
			dom.addStandardDisposableListener(
				this.replInputContainer,
				dom.EventType.BLUR,
				() => dom.removeClass(this.replInputContainer, 'synthetic-focus')
			)
		);
	}
	...
	public layout(dimension: Dimension): void {
		...
		if (this.tree) {
			this.renderer.setWidth(
				dimension.width - 25,
				this.characterWidthSurveyor.clientWidth /
					this.characterWidthSurveyor.textContent.length
			);
		...
		this.replInput.layout({
			width: dimension.width - 20,
			height: this.replInputHeight
		});
	}
	...
	public shutdown(): void {
		this.storageService.store(
			HISTORY_STORAGE_KEY,
			JSON.stringify(Repl.HISTORY.save()),
			StorageScope.WORKSPACE
		);
	}
	...
	constructor() {
		super({
			...
			kbOpts: {
				kbExpr: ContextKeyExpr.and(
					EditorContextKeys.textFocus,
					debug.CONTEXT_ON_LAST_DEBUG_REPL_LINE
				),
