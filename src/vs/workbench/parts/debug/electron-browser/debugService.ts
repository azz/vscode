...
import {
	...
	TaskServiceEvents,
	ITaskSummary
} from 'vs/workbench/parts/tasks/common/taskService';
...
import {
	IWindowIPCService,
	IBroadcast
} from 'vs/workbench/services/window/electron-browser/windowService';
import {
	ILogEntry,
	EXTENSION_LOG_BROADCAST_CHANNEL,
	EXTENSION_ATTACH_BROADCAST_CHANNEL,
	EXTENSION_TERMINATE_BROADCAST_CHANNEL
} from 'vs/workbench/electron-browser/extensionHost';
...
export class DebugService implements debug.IDebugService {
		...
		this.callStackScheduler = new RunOnceScheduler(() => {
			...
			if (focusedThread) {
				...
				if (callStack.length <= 1) {
					this.model
						.fetchCallStack(focusedThread)
						.done(
							() => this.tryToAutoFocusStackFrame(focusedThread),
		...
		this.toDispose.push(
			this.configurationService.onDidUpdateConfiguration(event => {
				if (event.sourceConfig) {
					...
					if (
						names.every(
							name => name !== this.viewModel.selectedConfigurationName
						)
					) {
						// Current selected configuration no longer exists - take the first configuration instead.
						this.viewModel.setSelectedConfigurationName(
	...
	private onBroadcast(broadcast: IBroadcast): void {
		...
		const process = this.model
			.getProcesses()
			.filter(p =>
				strings.equalsIgnoreCase(p.configuration.type, 'extensionhost')
			)
			.pop();
		...
		if (broadcast.channel === EXTENSION_LOG_BROADCAST_CHANNEL) {
			...
			let sev = extensionOutput.severity === 'warn'
				? severity.Warning
				: extensionOutput.severity === 'error' ? severity.Error : severity.Info;
	...
	private tryToAutoFocusStackFrame(thread: debug.IThread): TPromise<any> {
		...
		const stackFrameToFocus = first(
			callStack,
			sf => sf.source && sf.source.available,
			undefined
		);
		...
		this.focusStackFrameAndEvaluate(stackFrameToFocus).done(
			null,
			errors.onUnexpectedError
		);
		if (thread.stoppedDetails) {
			...
			aria.alert(
				nls.localize(
					...
					stackFrameToFocus.source ? stackFrameToFocus.source.name : '',
					stackFrameToFocus.range.startLineNumber
				)
			);
		}
	...
	private registerSessionListeners(
		...
		this.toDisposeOnSessionEnd.get(session.getId()).push(
			session.onDidInitialize(event => {
				...
				this.sendAllBreakpoints(process)
					.then(sendConfigurationDone, sendConfigurationDone)
					.done(() => this.fetchThreads(session), errors.onUnexpectedError);
			})
		);

		this.toDisposeOnSessionEnd.get(session.getId()).push(
			session.onDidStop(event => {
				...
				session.threads().then(response => {
					...
					const rawThread = response.body.threads
		...
		this.toDisposeOnSessionEnd.get(session.getId()).push(
			session.onDidContinued(event => {
				const threadId = event.body.allThreadsContinued !== false
					? undefined
					: event.body.threadId;
				this.model.clearThreads(session.getId(), false, threadId);
				if (this.viewModel.focusedProcess.getId() === session.getId()) {
					this.focusStackFrameAndEvaluate(
						null,
						this.viewModel.focusedProcess
		...
		this.toDisposeOnSessionEnd.get(session.getId()).push(
			session.onDidBreakpoint(event => {
				const id = event.body && event.body.breakpoint
					? event.body.breakpoint.id
					: undefined;
				const breakpoint = this.model
					.getBreakpoints()
					.filter(bp => bp.idFromAdapter === id)
					.pop();
				if (breakpoint) {
		...
		this.toDisposeOnSessionEnd.get(session.getId()).push(
			session.onDidExitAdapter(event => {
				// 'Run without debugging' mode VSCode must terminate the extension host. More details: #3905
				const process = this.viewModel.focusedProcess;
				if (
					process &&
					session &&
					process.getId() === session.getId() &&
					strings.equalsIgnoreCase(
						process.configuration.type,
						'extensionhost'
					) &&
					this.sessionStates.get(session.getId()) === debug.State.Running &&
					process &&
					this.contextService.hasWorkspace() &&
					process.configuration.noDebug
				) {
					this.windowsService.closeExtensionHostWindow(
						this.contextService.getWorkspace().resource.fsPath
					);
				}
				if (session && session.getId() === event.body.sessionId) {
					this.onSessionEnd(session);
		...
		try {
			result = JSON.parse(
				...
				return new ExceptionBreakpoint(
					exBreakpoint.filter || exBreakpoint.name,
					exBreakpoint.label,
	...
	public focusStackFrameAndEvaluate(
		...
		if (!process) {
			...
			process = stackFrame
				...
				: processes.length ? processes[0] : null;
		}
		if (!stackFrame) {
			const threads = process ? process.getAllThreads() : null;
			const callStack = threads && threads.length
				? threads[0].getCallStack()
				: null;
	...
	public logToRepl(
		value: string | debug.IExpression,
		sev = severity.Info
	): void {
	...
	public addWatchExpression(name: string): TPromise<void> {
		return this.model.addWatchExpression(
			this.viewModel.focusedProcess,
			this.viewModel.focusedStackFrame,
			name
		);
	...
	public renameWatchExpression(id: string, newName: string): TPromise<void> {
		return this.model.renameWatchExpression(
			this.viewModel.focusedProcess,
			this.viewModel.focusedStackFrame,
			id,
			newName
				...
				this.extensionService.onReady().then(() => {
					...
					if (compound) {
						...
						return TPromise.join(
							compound.configurations.map(name => this.startDebugging(name))
						);
					}
					...
					return manager
						.getStartSessionCommand(config ? config.type : undefined)
						.then(commandAndType => {
							if (noDebug && config) {
								config.noDebug = true;
							}
							if (commandAndType && commandAndType.command) {
								...
								return this.commandService
									.executeCommand(
										commandAndType.command,
										config || defaultConfig
									)
									.then((result: StartSessionResult) => {
										if (this.contextService.hasWorkspace()) {
											if (result && result.status === 'initialConfiguration') {
												return manager.openConfigFile(
													false,
													commandAndType.type
												);
											}
											...
											if (result && result.status === 'saveConfiguration') {
												return this.fileService
													.updateContent(manager.configFileUri, result.content)
													.then(() => manager.openConfigFile(false));
											}
										}
										return undefined;
									});
	...
	public createProcess(config: debug.IConfig): TPromise<any> {
		return this.textFileService.saveAll().then(() =>
			this.configurationManager.resloveConfiguration(config).then(
				resolvedConfig => {
					if (!resolvedConfig) {
					...
					if (!this.configurationManager.getAdapter(resolvedConfig.type)) {
						const message = resolvedConfig.type
							? nls.localize(
							...
							errors.create(message, {
								actions: [
									this.instantiationService.createInstance(
										debugactions.ConfigureAction,
										debugactions.ConfigureAction.ID,
										debugactions.ConfigureAction.LABEL
									),
									CloseAction
								]
							})
					...
					return this.runPreLaunchTask(resolvedConfig.preLaunchTask).then(
						(taskSummary: ITaskSummary) => {
							const errorCount = resolvedConfig.preLaunchTask
								? this.markerService.getStatistics().errors
								: 0;
							const successExitCode = taskSummary && taskSummary.exitCode === 0;
							const failureExitCode =
								taskSummary &&
								taskSummary.exitCode !== undefined &&
								taskSummary.exitCode !== 0;
							...
							this.messageService.show(severity.Error, {
								...
								actions: [
									new Action(
										'debug.continue',
										nls.localize('debugAnyway', 'Debug Anyway'),
										null,
										true,
										() => {
									...
									this.instantiationService.createInstance(
										...
										ToggleMarkersPanelAction.LABEL
									),
									CloseAction
								]
							});
		...
		return this.telemetryService
			...
			.then(data => {
				...
				if (aiKey) {
					...
					this.customTelemetryService = new TelemetryService(
						{ appender },
				...
				const session = this.instantiationService.createInstance(
					RawDebugSession,
					sessionId,
					configuration.debugServer,
					adapter,
					this.customTelemetryService
				);
				...
				return session
					...
					.then((result: DebugProtocol.InitializeResponse) => {
						...
						return configuration.request === 'attach'
							? session.attach(configuration)
					...
					.then((result: DebugProtocol.Response) => {
						...
						if (
							...
							(!this.viewModel.changedWorkbenchViewState &&
								internalConsoleOptions === 'openOnFirstSessionStart')
						) {
							this.panelService
								.openPanel(debug.REPL_ID, false)
								.done(undefined, errors.onUnexpectedError);
						}

						if (
							!this.viewModel.changedWorkbenchViewState &&
							(this.partService.isVisible(Parts.SIDEBAR_PART) ||
								!this.contextService.hasWorkspace())
						) {
						...
						this.extensionService
							.activateByEvent(`onDebug:${configuration.type}`)
							.done(null, errors.onUnexpectedError);
						...
						return this.telemetryService.publicLog('debugSessionStart', {
							...
							extensionName: `${adapter.extensionDescription
								.publisher}.${adapter.extensionDescription.name}`,
							isBuiltin: adapter.extensionDescription.isBuiltin,
							launchJsonExists: !!this.configurationService.getConfiguration<
								debug.IGlobalConfig
							>('launch')
						});
					})
					.then(undefined, (error: any) => {
						...
						this.telemetryService.publicLog('debugMisconfiguration', {
							type: configuration ? configuration.type : undefined,
							error: errorMessage
						});
						...
						const configureAction = this.instantiationService.createInstance(
							debugactions.ConfigureAction,
							debugactions.ConfigureAction.ID,
							debugactions.ConfigureAction.LABEL
						);
						const actions = error.actions && error.actions.length
							? error.actions.concat([configureAction])
							: [CloseAction, configureAction];
						this.messageService.show(severity.Error, {
	...
	private runPreLaunchTask(taskName: string): TPromise<ITaskSummary> {
		...
		return this.taskService.tasks().then(descriptions => {
			...
			const taskPromise = this.taskService.run(filteredTasks[0]).then(
				result => {
					this.lastTaskEvent = null;
					return result;
				},
	...
	private rawAttach(session: RawDebugSession, port: number): TPromise<any> {
		...
		const config = this.configurationManager.getConfiguration(
			this.viewModel.selectedConfigurationName
		);
		return this.configurationManager
			.resloveConfiguration(config)
			.then(resolvedConfig => {
	...
	public restartProcess(
		...
		if (process.session.capabilities.supportsRestartRequest) {
			return this.textFileService
				.saveAll()
				.then(() => process.session.custom('restart', null));
		}
		const focusedProcess = this.viewModel.focusedProcess;
		const preserveFocus =
			focusedProcess && process.getId() === focusedProcess.getId();
		...
		return process.session
			...
			.then(() => {
				if (preserveFocus) {
					...
					const restartedProcess = this.model
						.getProcesses()
						.filter(p => p.configuration.name === process.configuration.name)
						.pop();
					if (
						restartedProcess &&
						restartedProcess !== this.viewModel.focusedProcess
					) {
	...
	public stopProcess(process: debug.IProcess): TPromise<any> {
		...
		if (processes.length) {
			return TPromise.join(
				processes.map(p => p.session.disconnect(false, true))
			);
		}
	...
	private onSessionEnd(session: RawDebugSession): void {
		...
		const process = this.model
			.getProcesses()
			.filter(p => p.getId() === session.getId())
			.pop();
		...
		this.toDisposeOnSessionEnd.set(
			session.getId(),
			lifecycle.dispose(this.toDisposeOnSessionEnd.get(session.getId()))
		);
		const focusedProcess = this.viewModel.focusedProcess;
		if (focusedProcess && focusedProcess.getId() === session.getId()) {
			this.focusStackFrameAndEvaluate(null).done(
				null,
				errors.onUnexpectedError
			);
		}
		...
		if (this.model.getProcesses().length === 0) {
			...
			if (
				this.partService.isVisible(Parts.SIDEBAR_PART) &&
				this.configurationService.getConfiguration<debug.IDebugConfiguration>(
					'debug'
				).openExplorerOnEnd
			) {
				this.viewletService
					.openViewlet(EXPLORER_VIEWLET_ID)
					.done(null, errors.onUnexpectedError);
			}
		}
	}
	...
	private sendAllBreakpoints(process?: debug.IProcess): TPromise<any> {
		return (
			TPromise.join(
				distinct(this.model.getBreakpoints(), bp => bp.uri.toString()).map(bp =>
					this.sendBreakpoints(bp.uri, false, process)
				)
			)
	...
	private sendBreakpoints(
		...
		targetProcess?: debug.IProcess
	): TPromise<void> {
		const sendBreakpointsToProcess = (
			process: debug.IProcess
		): TPromise<void> => {
			...
			const breakpointsToSend = this.model
				...
				.filter(
					bp =>
						this.model.areBreakpointsActivated() &&
						bp.enabled &&
						bp.uri.toString() === modelUri.toString()
				);
			...
			const rawSource = source
				...
				: {
						...
						name: paths.basename(modelUri.fsPath)
					};
		...
		return this.sendToOneOrAllProcesses(
			targetProcess,
			sendBreakpointsToProcess
		);
	}
	...
	private sendFunctionBreakpoints(
		...
		const sendFunctionBreakpointsToProcess = (
			process: debug.IProcess
		): TPromise<void> => {
			const session = <RawDebugSession>process.session;
			if (
				!session.readyForBreakpoints ||
			...
			return session
				.setFunctionBreakpoints({ breakpoints: breakpointsToSend })
				.then(response => {
					if (!response || !response.body) {
						return;
					}
	...
	private sendExceptionBreakpoints(
		...
		const sendExceptionBreakpointsToProcess = (
			process: debug.IProcess
		): TPromise<any> => {
			const session = <RawDebugSession>process.session;
			if (
				!session.readyForBreakpoints ||
				this.model.getExceptionBreakpoints().length === 0
			) {
				return TPromise.as(null);
			}
			...
			return session.setExceptionBreakpoints({
		...
		return this.sendToOneOrAllProcesses(
			targetProcess,
			sendExceptionBreakpointsToProcess
		);
	}

	private sendToOneOrAllProcesses(
		process: debug.IProcess,
		send: (process: debug.IProcess) => TPromise<void>
	): TPromise<void> {
		if (process) {
			return send(process);
		}

		return TPromise.join(this.model.getProcesses().map(p => send(p))).then(
			() => void 0
		);
	}
	...
	private onFileChanges(fileChangesEvent: FileChangesEvent): void {
		this.model.removeBreakpoints(
			this.model
				.getBreakpoints()
				.filter(bp => fileChangesEvent.contains(bp.uri, FileChangeType.DELETED))
		);

		fileChangesEvent.getUpdated().forEach(event => {
			if (
				this.breakpointsToSendOnResourceSaved.has(event.resource.toString())
			) {
				this.breakpointsToSendOnResourceSaved.delete(event.resource.toString());
				this.sendBreakpoints(event.resource, true).done(
					null,
					errors.onUnexpectedError
				);
			}
	...
	private store(): void {
		this.storageService.store(
			DEBUG_BREAKPOINTS_KEY,
			JSON.stringify(this.model.getBreakpoints()),
			StorageScope.WORKSPACE
		);
		this.storageService.store(
			DEBUG_BREAKPOINTS_ACTIVATED_KEY,
			this.model.areBreakpointsActivated() ? 'true' : 'false',
			StorageScope.WORKSPACE
		);
		this.storageService.store(
			DEBUG_FUNCTION_BREAKPOINTS_KEY,
			JSON.stringify(this.model.getFunctionBreakpoints()),
			StorageScope.WORKSPACE
		);
		this.storageService.store(
			DEBUG_EXCEPTION_BREAKPOINTS_KEY,
			JSON.stringify(this.model.getExceptionBreakpoints()),
			StorageScope.WORKSPACE
		);
		this.storageService.store(
			DEBUG_SELECTED_CONFIG_NAME_KEY,
		...
		this.storageService.store(
			...
			JSON.stringify(
				this.model
					.getWatchExpressions()
					.map(we => ({ name: we.name, id: we.getId() }))
			),
			StorageScope.WORKSPACE
		);
	}
	...
	public dispose(): void {
		this.toDisposeOnSessionEnd.forEach(toDispose =>
