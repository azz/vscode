...
export class VariablesView extends CollapsibleView {
	...
	constructor(
		...
		super(
			{
				...
				ariaHeaderLabel: nls.localize('variablesSection', 'Variables Section')
			},
			keybindingService,
			contextMenuService
		);
		...
		this.variablesFocusedContext = CONTEXT_VARIABLES_FOCUSED.bindTo(
			contextKeyService
		);
	...
	public renderBody(container: HTMLElement): void {
		...
		this.tree = new Tree(
			this.treeContainer,
			{
				...
				renderer: this.instantiationService.createInstance(
					viewer.VariablesRenderer
				),
				accessibilityProvider: new viewer.VariablesAccessibilityProvider(),
				controller: this.instantiationService.createInstance(
					viewer.VariablesController,
					new viewer.VariablesActionProvider(this.instantiationService),
		...
		this.toDispose.push(
			this.listService.register(this.tree, [this.variablesFocusedContext])
		);
		...
		const collapseAction = this.instantiationService.createInstance(
			CollapseAction,
			this.tree,
			false,
		...
		this.toDispose.push(
			viewModel.onDidFocusStackFrame(sf => {
		...
		this.toDispose.push(
			this.debugService.onDidChangeState(state => {
				collapseAction.enabled =
					state === State.Running || state === State.Stopped;
			})
		);

		this.toDispose.push(
			this.debugService.getViewModel().onDidSelectExpression(expression => {
				if (!expression || !(expression instanceof Variable)) {
					return;
				}

				this.tree
					.refresh(expression, false)
					.then(() => {
						this.tree.setHighlight(expression);
						this.tree.addOneTimeListener(
							EventType.HIGHLIGHT,
							(e: IHighlightEvent) => {
								if (!e.highlight) {
	...
	public shutdown(): void {
		this.settings[VariablesView.MEMENTO] =
			this.state === CollapsibleState.COLLAPSED;
		super.shutdown();
	}
}
...
export class WatchExpressionsView extends CollapsibleView {
	...
	constructor(
		...
		this.toDispose.push(
			this.debugService.getModel().onDidChangeWatchExpressions(we => {
		...
		this.watchExpressionsFocusedContext = CONTEXT_WATCH_EXPRESSIONS_FOCUSED.bindTo(
			contextKeyService
		);

		this.onWatchExpressionsUpdatedScheduler = new RunOnceScheduler(() => {
			this.tree.refresh().done(() => {
				return this.toReveal instanceof Expression
					? this.tree.reveal(this.toReveal)
					: TPromise.as(true);
	...
	public renderBody(container: HTMLElement): void {
		...
		const actionProvider = new viewer.WatchExpressionsActionProvider(
		...
		this.toDispose.push(attachListStyler(this.tree, this.themeService));
		this.toDispose.push(
			this.listService.register(this.tree, [
				this.watchExpressionsFocusedContext
			])
		);

		this.tree.setInput(this.debugService.getModel());

		const addWatchExpressionAction = this.instantiationService.createInstance(
			AddWatchExpressionAction,
		...
		const removeAllWatchExpressionsAction = this.instantiationService.createInstance(
			RemoveAllWatchExpressionsAction,
	...
	public shutdown(): void {
		this.settings[WatchExpressionsView.MEMENTO] =
			this.state === CollapsibleState.COLLAPSED;
		super.shutdown();
	}
}
...
export class CallStackView extends CollapsibleView {
	...
	constructor(
		...
		super(
			{
				...options,
				ariaHeaderLabel: nls.localize('callstackSection', 'Call Stack Section'),
				sizing: ViewSizing.Flexible
			},
			keybindingService,
			contextMenuService
		...
		this.onCallStackChangeScheduler = new RunOnceScheduler(() => {
			...
			if (
				!this.debugService.getViewModel().isMultiProcessView() &&
				processes.length
			) {
			...
			if (newTreeInput instanceof Thread && newTreeInput.stoppedDetails) {
				this.pauseMessageLabel.text(
					newTreeInput.stoppedDetails.description ||
						nls.localize(
							'debugStopped',
							'Paused on {0}',
							newTreeInput.stoppedDetails.reason
						)
				);
				if (newTreeInput.stoppedDetails.text) {
					this.pauseMessageLabel.title(newTreeInput.stoppedDetails.text);
				}
				newTreeInput.stoppedDetails.reason === 'exception'
					? this.pauseMessageLabel.addClass('exception')
					: this.pauseMessageLabel.removeClass('exception');
				this.pauseMessage.show();
			} else {
				this.pauseMessage.hide();
			}

			(this.tree.getInput() === newTreeInput
				? this.tree.refresh()
				: this.tree.setInput(newTreeInput)).done(
	...
	public renderHeader(container: HTMLElement): void {
		...
		$('span.title')
			.text(nls.localize('callStack', 'Call Stack'))
			.appendTo(title);
	...
	public renderBody(container: HTMLElement): void {
		...
		this.treeContainer = renderViewTree(container);
		const actionProvider = this.instantiationService.createInstance(
			viewer.CallStackActionProvider
		);
		const controller = this.instantiationService.createInstance(
			viewer.CallStackController,
			actionProvider,
		...
		this.tree = new Tree(
			this.treeContainer,
			{
				dataSource: this.instantiationService.createInstance(
					viewer.CallStackDataSource
				),
				renderer: this.instantiationService.createInstance(
					viewer.CallStackRenderer
				),
				accessibilityProvider: this.instantiationService.createInstance(
					viewer.CallstackAccessibilityProvider
				),
				controller
			},
			{
		...
		this.toDispose.push(
			this.tree.addListener('selection', event => {
	...
	public shutdown(): void {
		this.settings[CallStackView.MEMENTO] =
			this.state === CollapsibleState.COLLAPSED;
		super.shutdown();
	}
}
...
export class BreakpointsView extends CollapsibleView {
	...
	constructor(
		...
		super(
			{
				...
				initialBodySize: BreakpointsView.getExpandedBodySize(
					debugService.getModel().getBreakpoints().length +
						debugService.getModel().getFunctionBreakpoints().length +
						debugService.getModel().getExceptionBreakpoints().length
				)
			},
			keybindingService,
			contextMenuService
		...
		this.settings = options.viewletSettings;
		this.breakpointsFocusedContext = CONTEXT_BREAKPOINTS_FOCUSED.bindTo(
			contextKeyService
		);
		this.toDispose.push(
			this.debugService
	...
	public renderHeader(container: HTMLElement): void {
		const titleDiv = $('div.title').appendTo(container);
		$('span')
			.text(nls.localize('breakpoints', 'Breakpoints'))
			.appendTo(titleDiv);
	...
	public renderBody(container: HTMLElement): void {
		...
		this.treeContainer = renderViewTree(container);
		const actionProvider = new viewer.BreakpointsActionProvider(
			this.instantiationService,
			this.debugService
		);
		const controller = this.instantiationService.createInstance(
			viewer.BreakpointsController,
			actionProvider,
		...
		this.tree = new Tree(
			this.treeContainer,
			{
				...
				renderer: this.instantiationService.createInstance(
					viewer.BreakpointsRenderer,
					actionProvider,
					this.actionRunner
				),
				accessibilityProvider: this.instantiationService.createInstance(
					viewer.BreakpointsAccessibilityProvider
				),
				controller,
				sorter: {
					compare(tree: ITree, element: any, otherElement: any): number {
						...
						if (first.uri.toString() !== second.uri.toString()) {
							return paths
								.basename(first.uri.fsPath)
								.localeCompare(paths.basename(second.uri.fsPath));
						}
		...
		this.toDispose.push(attachListStyler(this.tree, this.themeService));
		this.toDispose.push(
			this.listService.register(this.tree, [this.breakpointsFocusedContext])
		);
		...
		this.toDispose.push(
			this.debugService.getViewModel().onDidSelectFunctionBreakpoint(fbp => {
						...
						this.tree.addOneTimeListener(
							...
							(e: IHighlightEvent) => {
								if (!e.highlight) {
									this.debugService
										.getViewModel()
										.setSelectedFunctionBreakpoint(null);
								}
							}
						);
					})
	...
	public getActions(): IAction[] {
		return [
			this.instantiationService.createInstance(
				AddFunctionBreakpointAction,
				AddFunctionBreakpointAction.ID,
				AddFunctionBreakpointAction.LABEL
			),
			this.instantiationService.createInstance(
				ToggleBreakpointsActivatedAction,
				ToggleBreakpointsActivatedAction.ID,
				ToggleBreakpointsActivatedAction.ACTIVATE_LABEL
			),
			this.instantiationService.createInstance(
				RemoveAllBreakpointsAction,
	...
	private onBreakpointsChange(): void {
		...
		this.setBodySize(
			BreakpointsView.getExpandedBodySize(
				model.getBreakpoints().length +
					model.getExceptionBreakpoints().length +
	...
	public shutdown(): void {
		this.settings[BreakpointsView.MEMENTO] =
			this.state === CollapsibleState.COLLAPSED;
		super.shutdown();
	}
}
