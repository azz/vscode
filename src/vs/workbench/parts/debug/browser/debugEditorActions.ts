...
class ToggleBreakpointAction extends EditorAction {
	...
	public run(
		...
		const bps = debugService
			...
			.filter(
				bp =>
					bp.lineNumber === position.lineNumber &&
					bp.uri.toString() === modelUri.toString()
			);

		if (bps.length) {
			return TPromise.join(
				bps.map(bp => debugService.removeBreakpoints(bp.getId()))
			);
		}
		if (
			debugService
				.getConfigurationManager()
				.canSetBreakpointsIn(editor.getModel())
		) {
			return debugService.addBreakpoints(modelUri, [
...
function addColumnBreakpoint(
	...
	const position = editor.getPosition();
	const modelUri = editor.getModel().uri;
	const bp = debugService
		.getModel()
		.getBreakpoints()
		.filter(
			bp =>
				bp.lineNumber === position.lineNumber &&
				bp.column === position.column &&
				bp.uri.toString() === modelUri.toString()
		)
		.pop();
	...
	public run(accessor: ServicesAccessor, editor: ICommonCodeEditor): void {
		...
		if (
			...
			editor
				...
				.showBreakpointWidget(lineNumber, column);
		}
	}
}
...
class RunToCursorAction extends EditorAction {
	...
	public run(
		...
		const oneTimeListener = debugService
			.getViewModel()
			.focusedProcess.session.onDidEvent(event => {
				if (event.event === 'stopped' || event.event === 'exit') {
					const toRemove = debugService
						.getModel()
						.getBreakpoints()
						.filter(
							bp =>
		...
		return (bpExists
			? TPromise.as(null)
			: debugService.addBreakpoints(uri, [
					{ lineNumber: position.lineNumber, column: position.column }
				])).then(() => {
