...
export abstract class AbstractDebugAction extends Action {
	...
	constructor(
		...
		this.toDispose = [];
		this.toDispose.push(
			this.debugService.onDidChangeState(state => this.updateEnablement(state))
		);
...
export class ConfigureAction extends AbstractDebugAction {
	...
	constructor(
		...
		this.toDispose.push(
			debugService
				.getViewModel()
				.onDidSelectConfiguration(configurationName => this.updateClass())
		);
		this.updateClass();
	}
	...
	private updateClass(): void {
		this.class = this.debugService.getViewModel().selectedConfigurationName
			? 'debug-action configure'
			: 'debug-action configure notification';
	}
...
export class StartAction extends AbstractDebugAction {
	...
	protected isEnabled(state: State): boolean {
		...
		const compound = this.debugService
			.getConfigurationManager()
			.getCompound(this.debugService.getViewModel().selectedConfigurationName);
		return (
			state !== State.Initializing &&
			processes.every(
				p =>
					p.name !== this.debugService.getViewModel().selectedConfigurationName
			) &&
			(!compound ||
				!compound.configurations ||
				processes.every(p => compound.configurations.indexOf(p.name) === -1)) &&
			(!this.contextService ||
...
export class RestartAction extends AbstractDebugAction {
	...
	private setLabel(process: IProcess): void {
		this.updateLabel(
			process && process.isAttach()
				? RestartAction.RECONNECT_LABEL
				: RestartAction.LABEL
		);
	}
...
export class RemoveAllBreakpointsAction extends AbstractDebugAction {
	...
	constructor(
		...
		this.toDispose.push(
			this.debugService
				.getModel()
				.onDidChangeBreakpoints(() => this.updateEnablement())
		);
	}
	...
	public run(): TPromise<any> {
		return TPromise.join([
			...
			this.debugService.removeFunctionBreakpoints()
		]);
	}
	...
	protected isEnabled(state: State): boolean {
		...
		return (
			...
			(model.getBreakpoints().length > 0 ||
				model.getFunctionBreakpoints().length > 0)
		);
	}
}
...
export class EnableAllBreakpointsAction extends AbstractDebugAction {
	...
	constructor(
		...
		super(
			id,
			label,
			'debug-action enable-all-breakpoints',
			debugService,
			keybindingService
		);
		this.toDispose.push(
	...
	protected isEnabled(state: State): boolean {
		...
		return (
			super.isEnabled(state) &&
			(<IEnablement[]>model.getBreakpoints())
				.concat(model.getFunctionBreakpoints())
				.concat(model.getExceptionBreakpoints())
				.some(bp => !bp.enabled)
		);
	}
}
...
export class DisableAllBreakpointsAction extends AbstractDebugAction {
	...
	constructor(
		...
		super(
			id,
			label,
			'debug-action disable-all-breakpoints',
			debugService,
			keybindingService
		);
		this.toDispose.push(
...
export class ToggleBreakpointsActivatedAction extends AbstractDebugAction {
	...
	static ACTIVATE_LABEL = nls.localize(
		'activateBreakpoints',
		'Activate Breakpoints'
	);
	static DEACTIVATE_LABEL = nls.localize(
	...
	constructor(
		...
		super(
			id,
			label,
			'debug-action breakpoints-activate',
			debugService,
			keybindingService
		);
		this.updateLabel(
			this.debugService.getModel().areBreakpointsActivated()
				? ToggleBreakpointsActivatedAction.DEACTIVATE_LABEL
	...
	public run(): TPromise<any> {
		return this.debugService.setBreakpointsActivated(
			!this.debugService.getModel().areBreakpointsActivated()
		);
	}
...
export class AddFunctionBreakpointAction extends AbstractDebugAction {
	...
	constructor(
		...
		super(
			id,
			label,
			'debug-action add-function-breakpoint',
			debugService,
			keybindingService
		);
	}
...
export class AddConditionalBreakpointAction extends AbstractDebugAction {
	...
	public run(): TPromise<any> {
		this.editor
			...
			.showBreakpointWidget(this.lineNumber, undefined);
		return TPromise.as(null);
	}
}
...
export class SetValueAction extends AbstractDebugAction {
	...
	protected isEnabled(state: State): boolean {
		...
		return (
			...
			state === State.Stopped &&
			process &&
			process.session.capabilities.supportsSetVariable
		);
	}
}
...
export class AddWatchExpressionAction extends AbstractDebugAction {
	...
	constructor(
		...
		super(
			id,
			label,
			'debug-action add-watch-expression',
			debugService,
			keybindingService
		);
		this.toDispose.push(
			this.debugService
				.getModel()
				.onDidChangeWatchExpressions(() => this.updateEnablement())
		);
	}
...
export class AddToWatchExpressionsAction extends AbstractDebugAction {
	...
	public run(): TPromise<any> {
		const name = this.expression instanceof Variable
			? this.expression.evaluateName
...
export class ToggleReplAction extends TogglePanelAction {
	...
	private registerListeners(): void {
		this.toDispose.push(
			this.debugService.getModel().onDidChangeReplElements(() => {
...
export class FocusProcessAction extends AbstractDebugAction {
	...
	public run(processName: string): TPromise<any> {
		const process = this.debugService
			.getModel()
			.getProcesses()
			.filter(p => p.name === processName)
			.pop();
		return this.debugService
			.focusStackFrameAndEvaluate(null, process)
			.then(() => {
...
export class StepBackAction extends AbstractDebugAction {
	...
	protected isEnabled(state: State): boolean {
		...
		return (
			...
			state === State.Stopped &&
			process &&
			process.session.capabilities.supportsStepBack
		);
	}
}
