...
export class BreakpointWidget extends ZoneWidget {
	...
	private get placeholder(): string {
		return this.hitCountContext
			? HIT_COUNT_PLACEHOLDER
	...
	protected _fillContainer(container: HTMLElement): void {
		...
		const breakpoint = this.debugService
			.getModel()
			.getBreakpoints()
			.filter(
				bp =>
					bp.lineNumber === this.lineNumber &&
					bp.column === this.column &&
					bp.uri.toString() === uri.toString()
			)
			.pop();

		this.hitCountContext =
			breakpoint && breakpoint.hitCondition && !breakpoint.condition;
		const selected = this.hitCountContext ? 1 : 0;
		const selectBox = new SelectBox(
			[
				nls.localize('expression', 'Expression'),
				nls.localize('hitCount', 'Hit Count')
			],
			selected
		);
		...
		dom.addClass(
			...
			isWindows ? 'windows' : isMacintosh ? 'mac' : 'linux'
		);
		this.inputBox.value = this.getInputBoxValue(breakpoint);
		...
		const wrapUp = once((success: boolean) => {
			if (!disposed) {
				...
				if (success) {
					...
					const oldBreakpoint = this.debugService
						.getModel()
						.getBreakpoints()
						.filter(
							bp =>
					...
					if (oldBreakpoint) {
						this.debugService
							.removeBreakpoints(oldBreakpoint.getId())
							.done(null, errors.onUnexpectedError);
					}

					this.debugService
						.addBreakpoints(uri, [raw])
						.done(null, errors.onUnexpectedError);
				}
