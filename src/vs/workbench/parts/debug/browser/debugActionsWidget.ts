...
export class DebugActionsWidget extends Themable
	...
	constructor(
		...
		this.$el = $()
			.div()
			.addClass('debug-actions-widget')
			.style('top', `${partService.getTitleBarOffset()}px`);
	...
	private registerListeners(): void {
		this.toDispose.push(
			this.debugService.onDidChangeState(state => this.update(state))
		);
		this.toDispose.push(
		...
		this.toDispose.push(
			this.actionBar.actionRunner.addListener(EventType.RUN, (e: any) => {
				// check for error
	...
	private storePosition(): void {
		const position =
			parseFloat(this.$el.getComputedStyle().left) / window.innerWidth;
		this.storageService.store(
			DEBUG_ACTIONS_WIDGET_POSITION_KEY,
			position,
			StorageScope.WORKSPACE
		);
		this.telemetryService.publicLog(DEBUG_ACTIONS_WIDGET_POSITION_KEY, {
			position
		});
	}
	...
	protected updateStyles(): void {
		...
		if (this.$el) {
			...
			this.$el.style(
				'box-shadow',
				widgetShadowColor ? `0 5px 8px ${widgetShadowColor}` : null
			);
	...
	private setXCoordinate(x?: number): void {
		...
		const widgetWidth = this.$el.getHTMLElement().clientWidth;
		if (x === undefined) {
			const positionPercentage = this.storageService.get(
				DEBUG_ACTIONS_WIDGET_POSITION_KEY,
				StorageScope.WORKSPACE
			);
			x = positionPercentage !== undefined
				? parseFloat(positionPercentage) * window.innerWidth
				: 0.5 * window.innerWidth - 0.5 * widgetWidth;
		}
	...
	private getActions(): AbstractDebugAction[] {
		if (!this.allActions) {
			...
			this.allActions.push(
				this.instantiationService.createInstance(
					StepBackAction,
					StepBackAction.ID,
					StepBackAction.LABEL
				)
			);
			this.allActions.push(
				this.instantiationService.createInstance(
					ReverseContinueAction,
					ReverseContinueAction.ID,
					ReverseContinueAction.LABEL
				)
			);
			this.allActions.push(
				this.instantiationService.createInstance(
					FocusProcessAction,
					FocusProcessAction.ID,
					FocusProcessAction.LABEL
				)
			);
		...
		const attached =
			...
			!strings.equalsIgnoreCase(process.configuration.type, 'extensionHost');
