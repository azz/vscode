...
export class StartDebugActionItem extends EventEmitter implements IActionItem {
	private static ADD_CONFIGURATION = nls.localize(
		'addConfiguration',
		'Add Configuration...'
	);
	...
	constructor(
		...
		this.toDispose.push(
			attachSelectBoxStyler(this.selectBox, themeService, {
	...
	private registerListeners(): void {
		...
		this.toDispose.push(
			this.selectBox.onDidSelect(configurationName => {
				if (configurationName === StartDebugActionItem.ADD_CONFIGURATION) {
					this.selectBox.select(
						this.debugService
							...
							.indexOf(
								this.debugService.getViewModel().selectedConfigurationName
							)
					);
					this.commandService
						.executeCommand('debug.addConfiguration')
						.done(undefined, errors.onUnexpectedError);
				} else {
					this.debugService
						.getViewModel()
						.setSelectedConfigurationName(configurationName);
				}
			})
		);
		this.toDispose.push(
			this.debugService
				.getViewModel()
				.onDidSelectConfiguration(configurationName => {
					const manager = this.debugService.getConfigurationManager();
					this.selectBox.select(
						manager.getConfigurationNames().indexOf(configurationName)
					);
				})
		);
	}
	...
	public render(container: HTMLElement): void {
		...
		this.toDispose.push(
			attachStylerCallback(this.themeService, { selectBorder }, colors => {
				this.container.style.border = colors.selectBorder
					? `1px solid ${colors.selectBorder}`
					: null;
				selectBoxContainer.style.borderLeft = colors.selectBorder
					? `1px solid ${colors.selectBorder}`
					: null;
			})
		);
	...
	private updateOptions(): void {
		const options = this.debugService
			.getConfigurationManager()
			.getConfigurationNames();
		if (options.length === 0) {
