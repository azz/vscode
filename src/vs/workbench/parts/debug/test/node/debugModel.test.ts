...
suite('Debug - Model', () => {
	...
	test('breakpoints two files', () => {
		...
		model.addBreakpoints(modelUri1, [
			{ lineNumber: 5, enabled: true },
			{ lineNumber: 10, enabled: false }
		]);
		model.addBreakpoints(modelUri2, [
		...
		model.removeBreakpoints(
			model
				.getBreakpoints()
				.filter(bp => bp.uri.toString() === modelUri1.toString())
		);
		assert.equal(model.getBreakpoints().length, 3);
	});
	...
	test('threads simple', () => {
		...
		const process = model
			.getProcesses()
			.filter(p => p.getId() === rawSession.getId())
			.pop();
	...
	test('repl output', () => {
		...
		model.appendToRepl(
			new OutputNameValueElement('fake', keyValueObject),
			null
		);
