...
const extensionPacks: ExtensionSuggestion[] = [
	{
		name: localize('welcomePage.javaScript', 'JavaScript'),
	...
	{
		name: localize('welcomePage.docker', 'Docker'),
...
const keymapExtensions: ExtensionSuggestion[] = [
	{
		name: localize('welcomePage.vim', 'Vim'),
		id: 'vscodevim.vim',
		isKeymap: true
	},
	{
		name: localize('welcomePage.sublime', 'Sublime'),
		id: 'ms-vscode.sublime-keybindings',
		isKeymap: true
	},
	{
		name: localize('welcomePage.atom', 'Atom'),
...
class WelcomePage {
	...
	private create() {
		...
		const uri = URI.parse(require.toUrl('./vs_code_welcome_page')).with({
		...
		const input = this.instantiationService.createInstance(
			WalkThroughInput,
			localize('welcome.title', 'Welcome'),
			'',
			uri,
			telemetryFrom,
			container => this.onReady(container, recentlyOpened, installedExtensions)
		);
		this.editorService
			.openEditor(input, { pinned: true }, Position.ONE)
			.then(null, onUnexpectedError);
	}

	private onReady(
		container: HTMLElement,
		recentlyOpened: TPromise<{ files: string[]; folders: string[] }>,
		installedExtensions: TPromise<IExtensionStatus[]>
	): void {
		const enabled = this.configurationService.lookup<boolean>(enabledKey).value;
		const showOnStartup = <HTMLInputElement>container.querySelector(
			'#showOnStartup'
		);
		if (enabled) {
		...
		showOnStartup.addEventListener('click', e => {
			this.configurationEditingService.writeConfiguration(
				...
				{ key: enabledKey, value: showOnStartup.checked }
			);
		});
		...
		recentlyOpened
			.then(({ folders }) => {
				...
				folders.slice(0, 5).forEach(folder => {
					...
					const tildifiedParentFolder = tildify(
						parentFolder,
		...
		this.addExtensionList(
			container,
			'.extensionPackList',
			extensionPacks,
			extensionPackStrings
		);
		this.addExtensionList(
			container,
			'.keymapList',
		...
		this.disposables.push(
			this.instantiationService.invokeFunction(onExtensionChanged)(ids => {
				for (const id of ids) {
					if (
						container.querySelector(
							`.installExtension[data-extension="${id}"], .enabledExtension[data-extension="${id}"]`
						)
					) {
						const installedExtensions = this.instantiationService.invokeFunction(
							getInstalledExtensions
				...
				if (i) {
					list.appendChild(
						document.createTextNode(
							localize('welcomePage.extensionListSeparator', ', ')
						)
					);
				}
	...
	private installExtension(
		...
		this.instantiationService
			...
			.then(extensions => {
				const installedExtension = arrays.first(
					extensions,
					extension => extension.identifier === extensionSuggestion.id
				);
				if (installedExtension && installedExtension.globallyEnabled) {
					...
					this.messageService.show(
						Severity.Info,
						strings.alreadyInstalled.replace('{0}', extensionSuggestion.name)
					);
					return;
				}
				const foundAndInstalled = installedExtension
					? TPromise.as(true)
					: this.extensionGalleryService
							.query({ names: [extensionSuggestion.id] })
							.then(result => {
				...
				this.messageService.show(Severity.Info, {
					message: strings.reloadAfterInstall.replace(
						'{0}',
						extensionSuggestion.name
					),
					actions: [
						new Action('ok', localize('ok', 'OK'), null, true, () => {
							...
							messageDelay.then(() => {
								this.messageService.show(Severity.Info, {
									message: strings.installing.replace(
										'{0}',
										extensionSuggestion.name
									),
									actions: [CloseAction]
									...
									return foundAndInstalled.then(found => {
										...
										if (found) {
											return this.extensionEnablementService
												.setEnablement(extensionSuggestion.id, true)
												.then(() => {
													this.telemetryService.publicLog(
														strings.installedEvent,
														{
															from: telemetryFrom,
															extensionId: extensionSuggestion.id,
															outcome: installedExtension
																? 'enabled'
																: 'installed'
														}
													);
											...
											this.messageService.show(
												Severity.Error,
												strings.extensionNotFound
													.replace('{0}', extensionSuggestion.name)
													.replace('{1}', extensionSuggestion.id)
											);
											return undefined;
										}
									});
						...
						new Action(
							...
							() => {
								...
								this.extensionsWorkbenchService
									.queryGallery({ names: [extensionSuggestion.id] })
									.then(result =>
										this.extensionsWorkbenchService.open(result.firstPage[0])
									)
									.then(null, onUnexpectedError);
	...
	private updateInstalledExtensions(
		container: HTMLElement,
		installedExtensions: TPromise<IExtensionStatus[]>
	) {
		installedExtensions
			.then(extensions => {
				const elements = container.querySelectorAll(
					'.installExtension, .enabledExtension'
				);
				for (let i = 0; i < elements.length; i++) {
					elements[i].classList.remove('installed');
				}
				extensions
					.filter(ext => ext.globallyEnabled)
					.map(ext => ext.identifier)
					.forEach(id => {
						const install = container.querySelectorAll(
							`.installExtension[data-extension="${id}"]`
						);
						for (let i = 0; i < install.length; i++) {
							install[i].classList.add('installed');
						}
						const enabled = container.querySelectorAll(
							`.enabledExtension[data-extension="${id}"]`
						);
...
registerThemingParticipant((theme, collector) => {
	...
	if (foregroundColor) {
		collector.addRule(
			`.monaco-workbench > .part.editor > .content .welcomePage .caption { color: ${foregroundColor}; }`
		);
	}
	const descriptionColor = theme.getColor(descriptionForeground);
	if (descriptionColor) {
		collector.addRule(
			`.monaco-workbench > .part.editor > .content .welcomePage .detail { color: ${descriptionColor}; }`
		);
	}
	const buttonColor = getExtraColor(theme, buttonBackground, {
		dark: 'rgba(0, 0, 0, .2)',
		extra_dark: 'rgba(200, 235, 255, .042)',
		light: 'rgba(0,0,0,.04)',
		hc: 'black'
	});
	if (buttonColor) {
		collector.addRule(
			`.monaco-workbench > .part.editor > .content .welcomePage .commands li button { background: ${buttonColor}; }`
		);
	}
	const buttonHoverColor = getExtraColor(theme, buttonHoverBackground, {
		dark: 'rgba(200, 235, 255, .072)',
		extra_dark: 'rgba(200, 235, 255, .072)',
		light: 'rgba(0,0,0,.10)',
		hc: null
	});
	if (buttonHoverColor) {
		collector.addRule(
			`.monaco-workbench > .part.editor > .content .welcomePage .commands li button:hover { background: ${buttonHoverColor}; }`
		);
	}
	const link = theme.getColor(textLinkForeground);
	if (link) {
		collector.addRule(
			`.monaco-workbench > .part.editor > .content .welcomePage a { color: ${link}; }`
		);
	}
	...
	if (focusColor) {
		collector.addRule(
			`.monaco-workbench > .part.editor > .content .welcomePage a:focus { outline-color: ${focusColor}; }`
		);
	}
	const border = theme.getColor(contrastBorder);
	if (border) {
		collector.addRule(
			`.monaco-workbench > .part.editor > .content .welcomePage .commands li button { border-color: ${border}; }`
		);
	}
	const activeBorder = theme.getColor(activeContrastBorder);
	if (activeBorder) {
		collector.addRule(
			`.monaco-workbench > .part.editor > .content .welcomePage .commands li button:hover { outline-color: ${activeBorder}; }`
		);
	}
});
