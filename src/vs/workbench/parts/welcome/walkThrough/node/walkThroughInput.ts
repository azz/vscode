...
export class WalkThroughInput extends EditorInput {
	...
	constructor(
		...
		this.disposables.push(
			lifecycleService.onShutdown(e => this.disposeTelemetry(e))
		);
	}
	...
	resolve(refresh?: boolean): TPromise<WalkThroughModel> {
		if (!this.promise) {
			...
			this.promise = this.textModelResolverService
				.createModelReference(this.resource)
				.then(ref => {
					...
					renderer.code = (code, lang) => {
						const resource = this.resource.with({
					...
					const markdown = ref.object.textEditorModel
						.getLinesContent()
						.join('\n');
					marked(markdown, { renderer });

					return TPromise.join(snippets).then(
						refs => new WalkThroughModel(ref, refs)
					);
				});
		}
