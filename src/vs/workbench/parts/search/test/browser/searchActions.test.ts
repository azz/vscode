...
import {
	...
	stubFunction
} from 'vs/platform/instantiation/test/common/instantiationServiceMock';
import {
	Match,
	FileMatch,
	FileMatchOrMatch
...
suite('Search Actions', () => {
	...
	test('get next element to focus after removing a match when it has next sibling match', function() {
		...
		let tree = aTree(data);
		let target = data[2];
		let testObject: ReplaceAction = instantiationService.createInstance(
			ReplaceAction,
			tree,
			target,
			null
	...
	function aTree(elements: FileMatchOrMatch[]): any {
		return stubFunction(Tree, 'getNavigator', () => {
			return new ArrayNavigator(elements);
		});
	}
