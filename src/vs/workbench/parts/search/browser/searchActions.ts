...
export function isSearchViewletFocussed(
	...
	return (
		activeViewlet &&
		activeViewlet.getId() === Constants.VIEWLET_ID &&
		activeElement &&
		DOM.isAncestor(
			activeElement,
			(<SearchViewlet>activeViewlet).getContainer().getHTMLElement()
		)
	);
}
...
export function appendKeyBindingLabel(
	label: string,
	keyBinding: number | ResolvedKeybinding,
	keyBindingService2: IKeybindingService
): string {
	if (typeof keyBinding === 'number') {
		const resolvedKeybindings = keyBindingService2.resolveKeybinding(
			createKeybinding(keyBinding, OS)
		);
		return doAppendKeyBindingLabel(
			label,
			resolvedKeybindings.length > 0 ? resolvedKeybindings[0] : null
		);
	} else {
...
export class ShowNextSearchTermAction extends Action {
	...
	public run(): TPromise<any> {
		let searchAndReplaceWidget = (<SearchViewlet>this.viewletService.getActiveViewlet())
...
export class OpenSearchViewletAction extends ToggleViewletAction {
	...
	public run(): TPromise<any> {
		...
		const searchViewletWasOpen =
			activeViewlet && activeViewlet.getId() === Constants.VIEWLET_ID;
		...
		return super.run().then(() => {
			if (!searchViewletWasOpen) {
				...
				if (searchViewlet && searchViewlet.getId() === Constants.VIEWLET_ID) {
					const searchAndReplaceWidget = (<SearchViewlet>searchViewlet)
						.searchAndReplaceWidget;
...
export class FindInFolderAction extends Action {
	...
	public run(event?: any): TPromise<any> {
		return this.instantiationService.invokeFunction.apply(
			...
			[findInFolderCommand, this.resource]
		);
	}
}

export const findInFolderCommand = (
	...
	if (!URI.isUri(resource)) {
		const focused = listService.getFocused()
			? listService.getFocused().getFocus()
			: void 0;
		if (focused) {
			const file = explorerItemToFileResource(focused);
			if (file) {
				resource = file.isDirectory
					? file.resource
					: URI.file(paths.dirname(file.resource.fsPath));
			}
		}
	}
...
export class FocusNextSearchResultAction extends Action {
	...
	public run(): TPromise<any> {
		return this.viewletService
			.openViewlet(Constants.VIEWLET_ID)
			.then((searchViewlet: SearchViewlet) => {
...
export abstract class AbstractSearchAndReplaceAction extends Action {
	 ...
	 */
	public getElementToFocusAfterRemoved(
		viewer: ITree,
		elementToBeRemoved: FileMatchOrMatch
	): FileMatchOrMatch {
		let elementToFocus = this.getNextElementAfterRemoved(
			viewer,
			elementToBeRemoved
		);
		if (!elementToFocus) {
			elementToFocus = this.getPreviousElementAfterRemoved(
				viewer,
				elementToBeRemoved
			);
		}
	...
	public getNextElementAfterRemoved(
		viewer: ITree,
...
export class RemoveAction extends AbstractSearchAndReplaceAction {
	constructor(private viewer: ITree, private element: FileMatchOrMatch) {
		super(
			'remove',
			nls.localize('RemoveAction.label', 'Remove'),
			'action-remove'
		);
	}

	public run(): TPromise<any> {
		let nextFocusElement = this.getElementToFocusAfterRemoved(
			this.viewer,
			this.element
		);
		if (nextFocusElement) {
...
export class ReplaceAllAction extends AbstractSearchAndReplaceAction {
	constructor(
		...
		super(
			Constants.ReplaceAllInFileActionId,
			appendKeyBindingLabel(
				nls.localize('file.replaceAll.label', 'Replace All'),
				keyBindingService.lookupKeybinding(Constants.ReplaceAllInFileActionId),
				keyBindingService
			),
			'action-replace-all'
		);
...
export class ReplaceAction extends AbstractSearchAndReplaceAction {
	constructor(
		...
		super(
			Constants.ReplaceActionId,
			appendKeyBindingLabel(
				nls.localize('match.replace.label', 'Replace'),
				keyBindingService.lookupKeybinding(Constants.ReplaceActionId),
				keyBindingService
			),
			'action-replace'
		);
	...
	public run(): TPromise<any> {
		...
		return this.element.parent().replace(this.element).then(() => {
			...
			let elementToShowReplacePreview = this.getElementToShowReplacePreview(
				elementToFocus
			);
			this.viewer.DOMFocus();
				...
				if (elementToFocus.parent().id() === this.element.parent().id()) {
					...
					if (
						this.element
							...
							.getStartPosition()
							.isBeforeOrEqual(
								(<Match>elementToFocus).range().getStartPosition()
							)
					) {
	...
	private getElementToShowReplacePreview(
		elementToFocus: FileMatchOrMatch
	): Match {
		if (this.hasSameParent(elementToFocus)) {
			return <Match>elementToFocus;
		}
		let previousElement = this.getPreviousElementAfterRemoved(
			this.viewer,
			this.element
		);
	...
	private hasSameParent(element: FileMatchOrMatch): boolean {
		return (
			...
			element.parent().resource() === this.element.parent().resource()
		);
	}
