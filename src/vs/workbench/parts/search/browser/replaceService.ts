...
import {
	...
	IResourceEdit,
	createBulkEdit
} from 'vs/editor/common/services/bulkEdit';
...
class ReplacePreviewModel extends Disposable {
	...
	resolve(replacePreviewUri: URI): TPromise<IModel> {
		...
		const fileMatch = <FileMatch>this.searchWorkbenchService.searchModel.searchResult
			.matches()
			.filter(
				match => match.resource().toString() === fileResource.toString()
			)[0];
		return this.textModelResolverService
			.createModelReference(fileResource)
			.then(ref => {
				...
				const replacePreviewModel = this.modelService.createModel(
					...
					this.modeService.getOrCreateMode(sourceModelModeId),
					replacePreviewUri
				);
				this._register(
					fileMatch.onChange(modelChange =>
						this.update(
							sourceModel,
							replacePreviewModel,
							fileMatch,
...
export class ReplaceService implements IReplaceService {
	...
	public replace(
		...
		let bulkEdit: BulkEdit = createBulkEdit(
