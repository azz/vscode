...
suite('Keybindings Editor Model test', () => {
	...
	setup(() => {
		...
		instantiationService.stub(IExtensionService, {}, 'onReady', () =>
			TPromise.as(null)
		);
	...
	test('fetch returns default keybindings', () => {
		const expected = prepareKeybindingService(
			aResolvedKeybindingItem({
				...
				firstPart: { keyCode: KeyCode.Escape }
			}),
			aResolvedKeybindingItem({
				command: 'b' + uuid.generateUuid(),
				firstPart: { keyCode: KeyCode.Escape },
				chordPart: { keyCode: KeyCode.Escape }
			})
		);
	...
	test('fetch returns default keybindings sorted by command id', () => {
		const keybindings = prepareKeybindingService(
			...
			aResolvedKeybindingItem({
				command: 'a' + uuid.generateUuid(),
				firstPart: { keyCode: KeyCode.Backspace }
			})
		);
	...
	test('convert without title and binding to entry', () => {
		...
		return testObject.resolve().then(() => {
			const actual = testObject
				...
				.filter(
					element =>
						element.keybindingItem.command === 'command_without_keybinding'
				)[0];
	...
	test('filter by cmd key', () => {
		...
		const expected = aResolvedKeybindingItem({
			command,
			firstPart: { keyCode: KeyCode.Escape, modifiers: { metaKey: true } },
			when: 'whenContext1 && whenContext2',
			isDefault: false
		});
		prepareKeybindingService(expected);
		...
		return testObject.resolve().then(() => {
			...
			assert.deepEqual(actual[0].keybindingMatches.firstPart, {
				metaKey: true
			});
	...
	test('filter by meta key', () => {
		...
		prepareKeybindingService(
			...
			aResolvedKeybindingItem({
				command,
				firstPart: { keyCode: KeyCode.Escape, modifiers: { shiftKey: true } },
				when: 'whenContext1 && whenContext2',
				isDefault: false
			})
		);
	...
	test('filter by command key', () => {
		...
		prepareKeybindingService(
			...
			aResolvedKeybindingItem({
				command,
				firstPart: { keyCode: KeyCode.Escape, modifiers: { altKey: true } },
				when: 'whenContext1 && whenContext2',
				isDefault: false
			})
		);
	...
	test('filter by windows key', () => {
		...
		prepareKeybindingService(
			...
			aResolvedKeybindingItem({
				command,
				firstPart: { keyCode: KeyCode.Escape, modifiers: { ctrlKey: true } },
				when: 'whenContext1 && whenContext2',
				isDefault: false
			})
		);
	...
	test('filter by arrow', () => {
		...
		const expected = aResolvedKeybindingItem({
			command,
			firstPart: { keyCode: KeyCode.RightArrow, modifiers: { shiftKey: true } },
			when: 'whenContext1 && whenContext2',
			isDefault: false
		});
		prepareKeybindingService(
	...
	test('filter by modifier and key', () => {
		...
		const expected = aResolvedKeybindingItem({
			command,
			firstPart: { keyCode: KeyCode.RightArrow, modifiers: { altKey: true } },
			when: 'whenContext1 && whenContext2',
			isDefault: false
		});
		prepareKeybindingService(
	...
	test('filter by first part', () => {
		...
		const expected = aResolvedKeybindingItem({
			...
			chordPart: { keyCode: KeyCode.Delete },
			when: 'whenContext1 && whenContext2',
			isDefault: false
		});
		prepareKeybindingService(
	...
	test('filter matches first part and in chord part', () => {
		...
		prepareKeybindingService(
			...
			aResolvedKeybindingItem({
				...
				chordPart: { keyCode: KeyCode.UpArrow },
				when: 'whenContext1 && whenContext2',
				isDefault: false
			})
		);
	...
	test('filter exact matches', () => {
		...
		const expected = aResolvedKeybindingItem({
			command,
			firstPart: { keyCode: KeyCode.KEY_C, modifiers: { ctrlKey: true } },
			when: 'whenContext1 && whenContext2',
			isDefault: false
		});
		prepareKeybindingService(
			...
			aResolvedKeybindingItem({
				...
				chordPart: { keyCode: KeyCode.KEY_C, modifiers: { ctrlKey: true } },
				when: 'whenContext1 && whenContext2',
				isDefault: false
			})
		);
	...
	test('filter exact matches with first and chord part no results', () => {
		...
		const expected = aResolvedKeybindingItem({
			...
			chordPart: { keyCode: KeyCode.Delete, modifiers: { metaKey: true } },
			when: 'whenContext1 && whenContext2',
			isDefault: false
		});
		prepareKeybindingService(
	...
	function prepareKeybindingService(
		...keybindingItems: ResolvedKeybindingItem[]
	...
	function registerCommandWithTitle(command: string, title: string): void {
		...
		registry.registerWorkbenchAction(
			new SyncActionDescriptor(AnAction, command, title, { primary: null }),
			''
		);
	}

	function assertKeybindingItems(
		actual: ResolvedKeybindingItem[],
		expected: ResolvedKeybindingItem[]
	) {
	...
	function aResolvedKeybindingItem({
		...
		when?: string;
		isDefault?: boolean;
		firstPart?: { keyCode: KeyCode; modifiers?: Modifiers };
		chordPart?: { keyCode: KeyCode; modifiers?: Modifiers };
	}): ResolvedKeybindingItem {
		const aSimpleKeybinding = function(part: {
			keyCode: KeyCode;
			modifiers?: Modifiers;
		}): SimpleKeybinding {
			const { ctrlKey, shiftKey, altKey, metaKey } = part.modifiers || {
				ctrlKey: false,
				shiftKey: false,
				altKey: false,
				metaKey: false
		...
		const keybinding = firstPart
			? chordPart
				? new ChordKeybinding(
						aSimpleKeybinding(firstPart),
						aSimpleKeybinding(chordPart)
					)
				: aSimpleKeybinding(firstPart)
			: null;
		return new ResolvedKeybindingItem(
			keybinding ? new USLayoutResolvedKeybinding(keybinding, OS) : null,
	...
	function asResolvedKeybindingItems(
		keybindingEntries: IKeybindingItemEntry[],
		keepUnassigned: boolean = false
	): ResolvedKeybindingItem[] {
		if (!keepUnassigned) {
