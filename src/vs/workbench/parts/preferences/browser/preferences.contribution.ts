...
} from 'vs/workbench/parts/preferences/browser/keybindingsEditor';
import {
...
} from 'vs/workbench/parts/preferences/browser/preferencesActions';
import {
	IPreferencesService,
	IKeybindingsEditor,
...
class PreferencesEditorInputFactory implements IEditorInputFactory {
			...
			if (detailsInputFactory && masterInputFactory) {
				...
				if (detailsSerialized && masterSerialized) {
					return JSON.stringify(
						<ISerializedPreferencesEditorInput>{
							name: input.getName(),
	...
	public deserialize(
		...
		const deserialized: ISerializedPreferencesEditorInput = JSON.parse(
		...
		if (detailsInputFactory && masterInputFactory) {
			...
			if (detailsInput && masterInput) {
				return new PreferencesEditorInput(
					deserialized.name,
...
class DefaultPreferencesEditorInputFactory implements IEditorInputFactory {
	public serialize(editorInput: EditorInput): string {
		...
		const serialized: ISerializedDefaultPreferencesEditorInput = {
	...
	public deserialize(
		...
		const deserialized: ISerializedDefaultPreferencesEditorInput = JSON.parse(
			serializedEditorInput
		);
...
Registry.as<IEditorRegistry>(
	...
	PreferencesEditorInput.ID,
	PreferencesEditorInputFactory
);
Registry.as<IEditorRegistry>(
	EditorExtensions.Editors
).registerEditorInputFactory(
	DefaultPreferencesEditorInput.ID,
	DefaultPreferencesEditorInputFactory
);
Registry.as<IEditorRegistry>(
	EditorExtensions.Editors
).registerEditorInputFactory(
	KeybindingsEditorInput.ID,
	KeybindingsEditorInputFactory
);
...
registry.registerWorkbenchAction(
	new SyncActionDescriptor(
		OpenWorkspaceSettingsAction,
		OpenWorkspaceSettingsAction.ID,
		OpenWorkspaceSettingsAction.LABEL
	),
...
registry.registerWorkbenchAction(
	new SyncActionDescriptor(
		OpenGlobalKeybindingsAction,
		OpenGlobalKeybindingsAction.ID,
		OpenGlobalKeybindingsAction.LABEL,
		{
			primary: KeyChord(
...
registry.registerWorkbenchAction(
	new SyncActionDescriptor(
		OpenGlobalKeybindingsFileAction,
		OpenGlobalKeybindingsFileAction.ID,
		OpenGlobalKeybindingsFileAction.LABEL,
		{ primary: null }
	),
...
registry.registerWorkbenchAction(
	new SyncActionDescriptor(
		...
		ConfigureLanguageBasedSettingsAction.LABEL
	),
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
	...
	handler: (accessor, args: any) => {
		const editor = accessor
			...
			.getActiveEditor() as IKeybindingsEditor;
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
	...
	handler: (accessor, args: any) =>
		(accessor
			.get(IWorkbenchEditorService)
			.getActiveEditor() as IKeybindingsEditor).search('')
});
...
).registerWorkbenchContribution(PreferencesContentProvider);
