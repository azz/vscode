...
} from 'vs/workbench/parts/preferences/browser/preferencesEditor';
...
export class PreferencesService extends Disposable
	implements IPreferencesService {
	_serviceBrand: any;

	// TODO:@sandy merge these models into editor inputs by extending resource editor model
	private defaultPreferencesEditorModels: Map<
		URI,
		TPromise<IPreferencesEditorModel<any>>
	>;
	...
	constructor(
		...
		super();
		this.defaultPreferencesEditorModels = new Map<
			URI,
			TPromise<IPreferencesEditorModel<any>>
		>();
	...
	readonly defaultSettingsResource = URI.from({
	...
	createPreferencesEditorModel(
		...
		if (this.defaultSettingsResource.fsPath === uri.fsPath) {
			promise = TPromise.join<any>([
				this.extensionService.onReady(),
				this.fetchMostCommonlyUsedSettings()
			]).then(result => {
		...
		if (
			this.getEditableSettingsURI(ConfigurationTarget.USER).fsPath ===
			uri.fsPath
		) {
	...
	switchSettings(): TPromise<void> {
		...
		if (activeEditorInput instanceof PreferencesEditorInput) {
			...
			const toTarget = ConfigurationTarget.USER === fromTarget
				? ConfigurationTarget.WORKSPACE
				: ConfigurationTarget.USER;
			return this.getOrCreateEditableSettingsEditorInput(
				toTarget
			).then(toInput => {
				const replaceWith = new PreferencesEditorInput(
					toInput.getName(),
					toInput.getDescription(),
					this.instantiationService.createInstance(
						DefaultPreferencesEditorInput,
						this.defaultSettingsResource
					),
					toInput
				);
	...
	openGlobalKeybindingSettings(textual: boolean): TPromise<void> {
		...
		if (textual) {
			const emptyContents =
				...
				'\n[\n]';
			const editableKeybindings = URI.file(
	...
	configureSettingsForLanguage(language: string): void {
		this.openGlobalSettings().then(editor => {
			const codeEditor = getCodeEditor(editor);
			this.getPosition(language, codeEditor).then(position => {
	...
	private doOpenSettings(
		...
		checkToOpenDefaultSettings: boolean = true
	): TPromise<IEditor> {
		const openDefaultSettings =
			!checkToOpenDefaultSettings ||
		...
		return this.getOrCreateEditableSettingsEditorInput(
			...
			if (openDefaultSettings) {
				const defaultPreferencesEditorInput = this.instantiationService.createInstance(
					DefaultPreferencesEditorInput,
					this.defaultSettingsResource
				);
				const preferencesEditorInput = new PreferencesEditorInput(
					editableSettingsEditorInput.getName(),
					editableSettingsEditorInput.getDescription(),
					defaultPreferencesEditorInput,
					<EditorInput>editableSettingsEditorInput
				);
				this.lastOpenedSettingsInput = preferencesEditorInput;
				return this.editorService.openEditor(preferencesEditorInput, {
					pinned: true
				});
			}
	...
	private getEmptyEditableSettingsContent(
		configurationTarget: ConfigurationTarget
	): string {
		switch (configurationTarget) {
			case ConfigurationTarget.USER:
				const emptySettingsHeader = nls.localize(
					'emptySettingsHeader',
	...
	private getEditableSettingsURI(
		configurationTarget: ConfigurationTarget
	): URI {
		switch (configurationTarget) {
			case ConfigurationTarget.USER:
		...
		return this.fileService
			...
			.then(null, error => {
				if (
					...
					return this.fileService
						.updateContent(resource, contents)
						.then(null, error => {
	...
	private getSettingsConfigurationTarget(
		...
		if (preferencesEditorInput) {
			...
			return resource.toString() === this.userSettingsResource.toString()
				? ConfigurationTarget.USER
				: ConfigurationTarget.WORKSPACE;
		}
	...
	private getPosition(
		...
		return this.createPreferencesEditorModel(
			...
			const { eol } = this.configurationService.getConfiguration<{
				eol: string;
			}>('files');
			if (setting) {
				if (setting.overrides.length) {
					...
					let content;
					if (
						lastSetting.valueRange.endLineNumber === setting.range.endLineNumber
					) {
						content =
							',' +
							eol +
							this.spaces(2, configuration) +
							eol +
							this.spaces(1, configuration);
					} else {
						content = ',' + eol + this.spaces(2, configuration);
					}
					const editOperation = EditOperation.insert(
						new Position(
							lastSetting.valueRange.endLineNumber,
							lastSetting.valueRange.endColumn
						),
						content
					);
					model.pushEditOperations([], [editOperation], () => []);
					return {
						lineNumber: lastSetting.valueRange.endLineNumber + 1,
						column: model.getLineMaxColumn(
							lastSetting.valueRange.endLineNumber + 1
						)
					};
				}
				return {
			...
			return this.configurationEditingService
				...
				.then(() => {
					...
					let content =
						...
						eol +
						this.spaces(1, configuration);
					let editOperation = EditOperation.insert(
						new Position(
							setting.valueRange.endLineNumber,
							setting.valueRange.endColumn - 1
						),
						content
					);
					model.pushEditOperations([], [editOperation], () => []);
