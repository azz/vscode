...
const AUTO_SAVE_AFTER_DELAY_DISABLED_TIME =
	CONTENT_CHANGE_EVENT_BUFFER_DELAY + 500;
...
export class BackupModelTracker implements IWorkbenchContribution {
	...
	private registerListeners() {
		...
		this.toDispose.push(
			this.textFileService.models.onModelContentChanged(e =>
				this.onTextFileModelChanged(e)
			)
		);
		this.toDispose.push(
			this.textFileService.models.onModelSaved(e =>
				this.discardBackup(e.resource)
			)
		);
		this.toDispose.push(
			this.textFileService.models.onModelDisposed(e => this.discardBackup(e))
		);

		// Listen for untitled model changes
		this.toDispose.push(
			this.untitledEditorService.onDidChangeContent(e =>
				this.onUntitledModelChanged(e)
			)
		);
		this.toDispose.push(
			this.untitledEditorService.onDidDisposeModel(e => this.discardBackup(e))
		);
