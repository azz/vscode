...
export class BackupRestorer implements IWorkbenchContribution {
		...
		return this.backupFileService.getWorkspaceFileBackups().then(backups => {
			...
			return this.doResolveOpenedBackups(backups).then(unresolved => {
				...
				if (unresolved.length > 0) {
					return this.doOpenEditors(unresolved).then(() =>
						this.doResolveOpenedBackups(unresolved)
					);
				}
				return undefined;
			});
			...
			if (stacks.isOpen(backup)) {
				if (backup.scheme === Schemas.file) {
					restorePromises.push(
						this.textFileService.models
							.loadOrCreate(backup)
							.then(null, () => unresolved.push(backup))
					);
				} else if (backup.scheme === UNTITLED_SCHEMA) {
					restorePromises.push(
						this.untitledEditorService
							.loadOrCreate({ resource: backup })
							.then(null, () => unresolved.push(backup))
					);
				}
			} else {
	...
	private doOpenEditors(resources: URI[]): TPromise<void> {
		...
		const inputs = resources.map((resource, index) =>
			this.resolveInput(resource, index, hasOpenedEditors)
		);
		...
		return this.editorService
			.openEditors(
				inputs.map(input => {
					return { input, position: Position.ONE };
				})
			)
			.then(() => void 0);
	}
	...
	private resolveInput(
		resource: URI,
		index: number,
		hasOpenedEditors: boolean
	): IResourceInput | IUntitledResourceInput {
		const options = {
			pinned: true,
			preserveFocus: true,
			inactive: index > 0 || hasOpenedEditors
		};
