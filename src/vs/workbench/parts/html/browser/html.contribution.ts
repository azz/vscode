...
CommandsRegistry.registerCommand('_workbench.htmlZone', function(
	accessor: ServicesAccessor,
	params: HtmlZoneParams
) {
	if (warn) {
		console.warn(
			`'_workbench.htmlZone' is an EXPERIMENTAL feature and therefore subject to CHANGE and REMOVAL without notice.`
		);
		warn = false;
	}

	let codeEditor: ICommonCodeEditor;
	for (const editor of accessor
	...
	return textModelResolverService
		.createModelReference(params.resource)
		.then(ref => {
			const model = ref.object;
			const contents = model.textEditorModel.getValue();
			ref.dispose();

			HtmlZoneController.getInstance(codeEditor).addZone(
				params.lineNumber,
				contents
			);
		});
...
CommandsRegistry.registerCommand('_workbench.previewHtml', function(
	...
	if (!input) {
		input = accessor
			...
			.createInstance(HtmlInput, label, '', uri);
	} else {
...
CommandsRegistry.registerCommand(
	...
	(accessor: ServicesAccessor, resource: URI | string, message: any) => {
		const uri = resource instanceof URI ? resource : URI.parse(resource);
		const activePreviews = accessor
			.get(IWorkbenchEditorService)
			.getVisibleEditors()
			...
			.filter(
				e =>
					e.model.uri.scheme === uri.scheme && e.model.uri.fsPath === uri.fsPath
			);
