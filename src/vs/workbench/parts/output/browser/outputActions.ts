	...
	public static LABEL = nls.localize(
		{
			...
			comment: ['Turn on / off automatic output scrolling']
		},
	...
	constructor(
		...
		this.toDispose.push(
			this.outputService.onActiveOutputChannel(channel =>
				this.setClass(this.outputService.getActiveChannel().scrollLock)
			)
		);
	}
...
export class SwitchOutputAction extends Action {
	...
	constructor(@IOutputService private outputService: IOutputService) {
		super(
			SwitchOutputAction.ID,
			nls.localize('switchToOutput.label', 'Switch to Output')
		);
...
export class SwitchOutputActionItem extends SelectActionItem {
	constructor(
		...
		this.toDispose.push(
			this.outputService.onOutputChannel(() => {
				const activeChannelIndex = this.getSelected(
					this.outputService.getActiveChannel().id
				);
				this.setOptions(this.getOptions(), activeChannelIndex);
			})
		);
		this.toDispose.push(
			this.outputService.onActiveOutputChannel(activeChannelId =>
				this.setOptions(this.getOptions(), this.getSelected(activeChannelId))
			)
		);
		...
		this.setOptions(
			this.getOptions(),
			this.getSelected(this.outputService.getActiveChannel().id)
		);
	}

	protected getActionContext(option: string): string {
		const channel = this.outputService
			.getChannels()
			.filter(channelData => channelData.label === option)
			.pop();
	...
	private getSelected(outputId: string): number {
		...
		return Math.max(
			0,
			this.outputService.getChannels().map(c => c.id).indexOf(outputId)
		);
	}
}
