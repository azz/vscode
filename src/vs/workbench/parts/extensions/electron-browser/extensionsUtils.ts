...
export class KeymapExtensions implements IWorkbenchContribution {
	...
	constructor(
		...
		this.disposables.push(
			...
			instantiationService.invokeFunction(onExtensionChanged)(ids => {
				TPromise.join(ids.map(id => this.checkForOtherKeymaps(id))).then(
					null,
					onUnexpectedError
				);
			})
			...
			.then(extensions => {
				...
				if (extension && extension.globallyEnabled) {
					const otherKeymaps = keymaps.filter(
						extension =>
							extension.identifier !== extensionId && extension.globallyEnabled
					);
	...
	private promptForDisablingOtherKeymaps(
		...
		const options = [localize('yes', 'Yes'), localize('no', 'No')];
		return this.choiceService
			.choose(Severity.Info, message, options, 1, false)
			.then<void>(
				value => {
...
export function isKeymapExtension(
	tipsService: IExtensionTipsService,
	extension: IExtensionStatus
): boolean {
	const cats = extension.local.manifest.categories;
	return (
		(cats && cats.indexOf('Keymaps') !== -1) ||
		tipsService.getKeymapRecommendations().indexOf(extension.identifier) !== -1
	);
}
							...
							() => {
								...
								return extensionManagementService
									...
									.then(extensions => {
										return Promise.all(
											extensions
												.filter(e => stripVersion(e.id) === BetterMergeId)
