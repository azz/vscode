...
suite('ExtensionsActions Test', () => {
	...
	suiteSetup(() => {
		...
		instantiationService.set(
			IWorkspaceContextService,
			new WorkspaceContextService(TestWorkspace)
		);
	...
	setup(() => {
		...
		instantiationService.stub(IExtensionService, {
			getExtensions: () => TPromise.wrap([])
		});
		(<TestExtensionEnablementService>instantiationService.get(
			IExtensionEnablementService
		)).reset();
	...
	teardown(() => {
		(<ExtensionsWorkbenchService>instantiationService.get(
			IExtensionsWorkbenchService
		)).dispose();
	});

	test('Install action is disabled when there is no extension', () => {
		const testObject: ExtensionsActions.InstallAction = instantiationService.createInstance(
	...
	test('Test Install action when state is installed', done => {
		...
		workbenchService.queryLocal().done(() => {
			instantiationService.stubPromise(
				IExtensionGalleryService,
				'query',
				aPage(aGalleryExtension('a', { id: local.id }))
			);
	...
	test('Uninstall action is disabled when there is no extension', () => {
		const testObject: ExtensionsActions.UninstallAction = instantiationService.createInstance(
	...
	test('Test Uninstall action when state is uninstalling', done => {
		...
		instantiationService
			...
			.done(extensions => {
				...
				assert.equal(
					'extension-action uninstall uninstalling',
					testObject.class
				);
				done();
			});
	...
	test('Test CombinedInstallAction when there is no extension', () => {
		const testObject: ExtensionsActions.CombinedInstallAction = instantiationService.createInstance(
			ExtensionsActions.CombinedInstallAction
		);
	...
	test('Test UpdateAction when there is no extension', () => {
		const testObject: ExtensionsActions.UpdateAction = instantiationService.createInstance(
		...
		instantiationService
			...
			.done(extensions => {
				...
				instantiationService.stubPromise(
					IExtensionGalleryService,
					'query',
					aPage(aGalleryExtension('a', { id: local.id, version: '1.0.1' }))
				);
				instantiationService
					.get(IExtensionsWorkbenchService)
					.queryGallery()
	...
	test('Test ManageExtensionAction when there is no extension', () => {
		const testObject: ExtensionsActions.ManageExtensionAction = instantiationService.createInstance(
	...
	test('Test EnableForWorkspaceAction when there is no extension', () => {
		const testObject: ExtensionsActions.EnableForWorkspaceAction = instantiationService.createInstance(
	...
	test('Test EnableForWorkspaceAction when there extension is disabled globally', done => {
		instantiationService
			.get(IExtensionEnablementService)
			.setEnablement('pub.a', false);
		const testObject: ExtensionsActions.EnableForWorkspaceAction = instantiationService.createInstance(
			ExtensionsActions.EnableForWorkspaceAction,
			'id'
		);
	...
	test('Test EnableForWorkspaceAction when extension is disabled for workspace', done => {
		instantiationService
			.get(IExtensionEnablementService)
			.setEnablement('pub.a', false, true);
		const testObject: ExtensionsActions.EnableForWorkspaceAction = instantiationService.createInstance(
			ExtensionsActions.EnableForWorkspaceAction,
			'id'
		);
	...
	test('Test EnableGloballyAction when there is no extension', () => {
		const testObject: ExtensionsActions.EnableGloballyAction = instantiationService.createInstance(
	...
	test('Test EnableAction when there is no extension', () => {
		const testObject: ExtensionsActions.EnableAction = instantiationService.createInstance(
			ExtensionsActions.EnableAction
		);
	...
	test('Test DisableForWorkspaceAction when there is no extension', () => {
		const testObject: ExtensionsActions.DisableForWorkspaceAction = instantiationService.createInstance(
	...
	test('Test DisableGloballyAction when there is no extension', () => {
		const testObject: ExtensionsActions.DisableGloballyAction = instantiationService.createInstance(
	...
	test('Test DisableAction when there is no extension', () => {
		const testObject: ExtensionsActions.DisableAction = instantiationService.createInstance(
			ExtensionsActions.DisableAction
		);
	...
	test('Test UpdateAllAction when no installed extensions', () => {
		const testObject: ExtensionsActions.UpdateAllAction = instantiationService.createInstance(
			ExtensionsActions.UpdateAllAction,
			'id',
			'label'
		);
	...
	test('Test UpdateAllAction when installed extensions are not outdated', done => {
		...
		instantiationService.stubPromise(
			IExtensionManagementService,
			'getInstalled',
			[aLocalExtension('a'), aLocalExtension('b')]
		);
		instantiationService
			.get(IExtensionsWorkbenchService)
			.queryLocal()
	...
	test('Test UpdateAllAction when some installed extensions are outdated', done => {
		...
		const local = [
			...
			aLocalExtension('b', { version: '1.0.1' }),
			aLocalExtension('c', { version: '1.0.1' })
		];
		const workbenchService = instantiationService.get(
			IExtensionsWorkbenchService
		);
		...
		workbenchService.queryLocal().done(() => {
			instantiationService.stubPromise(
				...
				'query',
				aPage(
					aGalleryExtension('a', { id: local[0].id, version: '1.0.2' }),
					aGalleryExtension('b', { id: local[1].id, version: '1.0.2' }),
					aGalleryExtension('c', local[2].manifest)
				)
			);
	...
	test('Test ReloadAction when there is no extension', () => {
		const testObject: ExtensionsActions.ReloadAction = instantiationService.createInstance(
			ExtensionsActions.ReloadAction
		);
	...
	test('Test ReloadAction when extension is newly installed', done => {
		instantiationService.stubPromise(IExtensionService, 'getExtensions', [
			{ id: 'pub.b' }
		]);
		const testObject: ExtensionsActions.ReloadAction = instantiationService.createInstance(
			ExtensionsActions.ReloadAction
		...
		instantiationService
			...
			.done(paged => {
				...
				assert.equal(
					`Reload this window to activate the extension 'a'?`,
					testObject.reloadMessaage
				);
				done();
			});
	});
		...
		instantiationService
			...
			.done(paged => {
				...
				didInstallEvent.fire({
					id,
					gallery,
					local: aLocalExtension('a', gallery, { id })
				});
	...
	test('Test ReloadAction when extension is uninstalled', done => {
		...
		instantiationService
			...
			.done(extensions => {
				...
				assert.equal(
					`Reload this window to deactivate the uninstalled extension 'a'?`,
					testObject.reloadMessaage
				);
				done();
			});
	...
	test('Test ReloadAction when extension is updated while running', done => {
		...
		workbenchService.queryLocal().done(extensions => {
			...
			assert.equal(
				`Reload this window to activate the updated extension 'a'?`,
				testObject.reloadMessaage
			);
			done();
		});
	...
	test('Test ReloadAction when extension is disabled when running', done => {
		...
		workbenchService.queryLocal().done(extensions => {
			...
			assert.equal('Reload to deactivate', testObject.tooltip);
			assert.equal(
				`Reload this window to deactivate the extension 'a'?`,
				testObject.reloadMessaage
			);
			done();
		});
	...
	function aLocalExtension(
		...
		assign(
			localExtension,
			{ type: LocalExtensionType.User, manifest: {} },
			properties
		);
		assign(
			localExtension.manifest,
			{ name, publisher: 'pub', version: '1.0.0' },
			manifest
		);
		localExtension.metadata = {
			id: localExtension.id,
			publisherId: localExtension.manifest.publisher,
			publisherDisplayName: 'somename'
		};
		localExtension.id = getLocalExtensionIdFromManifest(
			localExtension.manifest
		);
	...
	function aGalleryExtension(
		name: string,
		properties: any = {},
		galleryExtensionProperties: any = {},
		assets: any = {}
	): IGalleryExtension {
		...
		galleryExtension.id = getGalleryExtensionId(
