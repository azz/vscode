...
export class InstallAction extends Action {
	...
	constructor(
		...
		super(
			'extensions.install',
			InstallAction.InstallLabel,
			InstallAction.Class,
			false
		);

		this.disposables.push(
			this.extensionsWorkbenchService.onChange(() => this.update())
		);
		this.update();
	}
...
export class UninstallAction extends Action {
	...
	private update(): void {
		...
		const installedExtensions = this.extensionsWorkbenchService.local.filter(
			e => e.id === this.extension.id
		);
...
export class UpdateAction extends Action {
	...
	private update(): void {
		...
		const canInstall = this.extensionsWorkbenchService.canInstall(
			this.extension
		);
...
export class DropDownMenuActionItem extends ActionItem {
	...
	constructor(
		action: IAction,
		private menuActionGroups: IExtensionAction[][],
	...
	public showMenu(): void {
		...
		let elementPosition = DOM.getDomNodePagePosition(
	...
	private getActions(): IAction[] {
		let actions: IAction[] = [];
		const menuActionGroups = this.menuActionGroups.filter(group =>
			group.some(action => action.enabled)
		);
		for (const menuActions of menuActionGroups) {
...
export class ManageExtensionAction extends Action {
	...
	set extension(extension: IExtension) {
		this._extension = extension;
		this._actionItem.extension = extension;
		this.update();
	}
	...
	constructor(
		...
		private extensionsWorkbenchService: IExtensionsWorkbenchService,
		@IExtensionEnablementService
		private extensionEnablementService: IExtensionEnablementService,
		@IInstantiationService private instantiationService: IInstantiationService
	) {
		super(ManageExtensionAction.ID);

		this._actionItem = this.instantiationService.createInstance(
			...
			[
				[
					instantiationService.createInstance(
						...
						localize('enableForWorkspaceAction.label', 'Enable (Workspace)')
					),
					instantiationService.createInstance(
						EnableGloballyAction,
						localize('enableAlwaysAction.label', 'Enable (Always)')
					)
				],
				[
					instantiationService.createInstance(
						DisableForWorkspaceAction,
						localize('disableForWorkspaceAction.label', 'Disable (Workspace)')
					),
					instantiationService.createInstance(
						DisableGloballyAction,
						localize('disableAlwaysAction.label', 'Disable (Always)')
					)
				],
	...
	private update(): void {
		...
		if (this.extension && this.extension.type !== LocalExtensionType.System) {
			...
			this.class = this.enabled || state === ExtensionState.Uninstalling
				? ManageExtensionAction.Class
				: ManageExtensionAction.HideManageExtensionClass;
			this.tooltip = state === ExtensionState.Uninstalling
				? localize('ManageExtensionAction.uninstallingTooltip', 'Uninstalling')
				: '';
		}
	}
...
export class EnableForWorkspaceAction extends Action
	...
	private update(): void {
		...
		if (this.extension) {
			this.enabled =
				!this.extension.disabledGlobally &&
				this.extension.disabledForWorkspace &&
				this.extensionEnablementService.canEnable(this.extension.id);
		}
	}
...
export class EnableAction extends Action {
	...
	constructor(
		...
		super(
			EnableAction.ID,
			localize('enableAction', 'Enable'),
			EnableAction.DisabledClass,
			false
		);
...
export class DisableForWorkspaceAction extends Action
	...
	private update(): void {
		...
		if (this.extension && this.workspaceContextService.hasWorkspace()) {
			this.enabled =
				this.extension.type !== LocalExtensionType.System &&
				!this.extension.disabledGlobally &&
				!this.extension.disabledForWorkspace;
		}
	}
...
export class DisableAction extends Action {
	...
	constructor(
		...
		super(
			DisableAction.ID,
			localize('disableAction', 'Disable'),
			DisableAction.DisabledClass,
			false
		);
		this._actionItem = this.instantiationService.createInstance(
			DropDownMenuActionItem,
...
export class ToggleAutoUpdateAction extends Action {
	...
	private updateEnablement(): void {
		this.enabled =
			this.extensionsWorkbenchService.isAutoUpdateEnabled !==
...
export class UpdateAllAction extends Action {
	...
	private get outdated(): IExtension[] {
		return this.extensionsWorkbenchService.local.filter(
			e => e.outdated && e.state !== ExtensionState.Installing
		);
	}
	...
	run(): TPromise<any> {
		return TPromise.join(
			this.outdated.map(e => this.extensionsWorkbenchService.install(e))
		);
	}
...
export class ReloadAction extends Action {
	...
	constructor(
		...
		super(
			'extensions.reload',
			localize('reloadAction', 'Reload'),
			ReloadAction.DisabledClass,
			false
		);
			...
			.queue(() => {
				...
				return this.extensionService
					...
					.then(runningExtensions =>
						this.computeReloadState(runningExtensions)
					);
			})
			.done(() => {
				this.class = this.enabled
					? ReloadAction.EnabledClass
					: ReloadAction.DisabledClass;
			});
	}
	...
	private computeReloadState(runningExtensions: IExtensionDescription[]): void {
		...
		const isDisabled =
			this.extension.disabledForWorkspace || this.extension.disabledGlobally;

		const filteredExtensions = runningExtensions.filter(e =>
			areSameExtensions(e, this.extension)
		);
		const isExtensionRunning = filteredExtensions.length > 0;
		const isDifferentVersionRunning =
			filteredExtensions.length > 0 &&
			this.extension.version !== filteredExtensions[0].version;
	...
	run(): TPromise<any> {
		if (
			this.messageService.confirm({
				message: this.reloadMessaage,
				type: 'question',
				primaryButton: localize('reload', '&&Reload Window')
			})
		) {
...
export class ConfigureWorkspaceRecommendedExtensionsAction extends Action {
	...
	constructor(
		...
		@IExtensionsWorkbenchService
		private extensionsService: IExtensionsWorkbenchService,
	...
	private openExtensionsFile(): TPromise<any> {
		...
		return this.getOrCreateExtensionsFile().then(
			value => {
	...
	private getOrCreateExtensionsFile(): TPromise<{
		created: boolean;
		extensionsFileResource: URI;
	}> {
		const extensionsFileResource = URI.file(
			paths.join(
		...
		return this.fileService.resolveContent(extensionsFileResource).then(
			content => {
...
export class DisableAllAction extends Action {
	...
	constructor(
		id: string = DisableAllAction.ID,
		label: string = DisableAllAction.LABEL,
		@IExtensionsWorkbenchService
		private extensionsWorkbenchService: IExtensionsWorkbenchService,
		@IExtensionEnablementService
		private extensionEnablementService: IExtensionEnablementService
	...
	private update(): void {
		this.enabled = this.extensionsWorkbenchService.local.some(
			e =>
				e.type === LocalExtensionType.User &&
				!e.disabledForWorkspace &&
				!e.disabledGlobally
		);
	}
	...
	run(): TPromise<any> {
		return TPromise.join(
			this.extensionsWorkbenchService.local.map(e =>
				this.extensionsWorkbenchService.setEnablement(e, false)
			)
		);
	}
...
export class DisableAllWorkpsaceAction extends Action {
	...
	constructor(
		id: string = DisableAllWorkpsaceAction.ID,
		label: string = DisableAllWorkpsaceAction.LABEL,
		@IWorkspaceContextService
		private workspaceContextService: IWorkspaceContextService,
		@IExtensionsWorkbenchService
		private extensionsWorkbenchService: IExtensionsWorkbenchService,
	...
	private update(): void {
		this.enabled =
			this.workspaceContextService.hasWorkspace() &&
			this.extensionsWorkbenchService.local.some(
				e =>
					e.type === LocalExtensionType.User &&
					!e.disabledForWorkspace &&
					!e.disabledGlobally
	...
	run(): TPromise<any> {
		return TPromise.join(
			this.extensionsWorkbenchService.local.map(e =>
				this.extensionsWorkbenchService.setEnablement(e, false, true)
			)
		);
	}
...
export class EnableAllAction extends Action {
	...
	constructor(
		id: string = EnableAllAction.ID,
		label: string = EnableAllAction.LABEL,
		@IExtensionsWorkbenchService
		private extensionsWorkbenchService: IExtensionsWorkbenchService,
		@IExtensionEnablementService
		private extensionEnablementService: IExtensionEnablementService
	...
	private update(): void {
		this.enabled = this.extensionsWorkbenchService.local.some(
			e => this.extensionEnablementService.canEnable(e.id) && e.disabledGlobally
		);
	}
	...
	run(): TPromise<any> {
		return TPromise.join(
			this.extensionsWorkbenchService.local.map(e =>
				this.extensionsWorkbenchService.setEnablement(e, true)
			)
		);
	}
...
export class EnableAllWorkpsaceAction extends Action {
	...
	constructor(
		id: string = EnableAllWorkpsaceAction.ID,
		label: string = EnableAllWorkpsaceAction.LABEL,
		@IWorkspaceContextService
		private workspaceContextService: IWorkspaceContextService,
		@IExtensionsWorkbenchService
		private extensionsWorkbenchService: IExtensionsWorkbenchService,
	...
	private update(): void {
		this.enabled =
			...
			this.extensionsWorkbenchService.local.some(
				e =>
					this.extensionEnablementService.canEnable(e.id) &&
					!e.disabledGlobally &&
					e.disabledForWorkspace
			);
	}
	...
	run(): TPromise<any> {
		return TPromise.join(
			this.extensionsWorkbenchService.local.map(e =>
				this.extensionsWorkbenchService.setEnablement(e, true, true)
			)
		);
	}
...
CommandsRegistry.registerCommand(
	...
	function(accessor: ServicesAccessor, fileExtension: string) {
...
export const extensionButtonProminentForeground = registerColor(
...
export const extensionButtonProminentHoverBackground = registerColor(
...
registerThemingParticipant((theme: ITheme, collector: ICssStyleCollector) => {
	...
	if (foregroundColor) {
		collector.addRule(
			`.monaco-action-bar .action-item .action-label.extension-action.built-in-status { border-color: ${foregroundColor}; }`
		);
	}
	...
	if (buttonBackgroundColor) {
		collector.addRule(
			`.monaco-action-bar .action-item .action-label.extension-action { background-color: ${buttonBackgroundColor}; }`
		);
	}
	...
	if (buttonHoverBackgroundColor) {
		collector.addRule(
			`.monaco-action-bar .action-item:hover .action-label.extension-action { background-color: ${buttonHoverBackgroundColor}; }`
		);
	}
	...
	if (contrastBorderColor) {
		collector.addRule(
			`.monaco-action-bar .action-item .action-label.extension-action { border: 1px solid ${contrastBorderColor}; }`
		);
	}

	const extensionButtonProminentBackgroundColor = theme.getColor(
		extensionButtonProminentBackground
	);
	if (extensionButtonProminentBackground) {
		collector.addRule(
			`.monaco-action-bar .action-item .action-label.extension-action.prominent { background-color: ${extensionButtonProminentBackgroundColor}; }`
		);
	}

	const extensionButtonProminentForegroundColor = theme.getColor(
		extensionButtonProminentForeground
	);
	if (extensionButtonProminentForeground) {
		collector.addRule(
			`.monaco-action-bar .action-item .action-label.extension-action.prominent { color: ${extensionButtonProminentForegroundColor}; }`
		);
	}

	const extensionButtonProminentHoverBackgroundColor = theme.getColor(
		extensionButtonProminentHoverBackground
	);
	if (extensionButtonProminentHoverBackground) {
		collector.addRule(
			`.monaco-action-bar .action-item:hover .action-label.extension-action.prominent { background-color: ${extensionButtonProminentHoverBackgroundColor}; }`
		);
	}
});
