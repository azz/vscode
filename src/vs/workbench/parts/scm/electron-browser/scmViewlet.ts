...
class ResourceGroupRenderer
	implements IRenderer<ISCMResourceGroup, ResourceGroupTemplate> {
...
class MultipleSelectionActionRunner extends ActionRunner {
	...
	runAction(action: IAction, context: ISCMResource): TPromise<any> {
		if (action instanceof MenuItemAction) {
			...
			if (
				selection.length === filteredSelection.length ||
				selection.length === 1
			) {
...
class ResourceRenderer implements IRenderer<ISCMResource, ResourceTemplate> {
	...
	renderTemplate(container: HTMLElement): ResourceTemplate {
		...
		const name = append(element, $('.name'));
		const fileLabel = this.instantiationService.createInstance(
			FileLabel,
			name,
			void 0
		);
	...
	renderElement(
		...
		toggleClass(
			template.name,
			'strike-through',
			resource.decorations.strikeThrough
		);
		toggleClass(template.element, 'faded', resource.decorations.faded);

		const theme = this.themeService.getTheme();
		const icon = theme.type === LIGHT
			? resource.decorations.icon
			: resource.decorations.iconDark;
...
class Delegate implements IDelegate<ISCMResourceGroup | ISCMResource> {
	...
	getTemplateId(element: ISCMResourceGroup | ISCMResource) {
		return isSCMResource(element)
			? ResourceRenderer.TEMPLATE_ID
			: ResourceGroupRenderer.TEMPLATE_ID;
	}
}
...
export class SCMViewlet extends Viewlet {
	...
	private setActiveProvider(activeProvider: ISCMProvider | undefined): void {
		...
		if (activeProvider) {
			...
			if (activeProvider.onDidChangeCommitTemplate) {
				disposables.push(
					activeProvider.onDidChangeCommitTemplate(this.updateInputBox, this)
				);
			}
	...
	create(parent: Builder): TPromise<void> {
		...
		this.inputBox = new InputBox(
			...
			this.contextViewService,
			{
				placeholder: localize(
					...
					platform.isMacintosh ? 'Cmd+Enter' : 'Ctrl+Enter'
				),
				flexibleHeight: true
			}
		);
		this.disposables.push(
			attachInputBoxStyler(this.inputBox, this.themeService)
		);
		this.disposables.push(this.inputBox);

		this.inputBox.value = this.scmService.input.value;
		this.inputBox.onDidChange(
			value => (this.scmService.input.value = value),
			null,
			this.disposables
		);
		this.scmService.input.onDidChange(
			value => (this.inputBox.value = value),
			null,
			this.disposables
		);
		...
		chain(domEvent(this.inputBox.inputElement, 'keydown'))
			...
			.filter(
				e =>
					e.equals(KeyMod.CtrlCmd | KeyCode.Enter) ||
					e.equals(KeyMod.CtrlCmd | KeyCode.KEY_S)
			)
	...
	private update(): void {
		...
		const elements = provider.resources.reduce<
			(ISCMResourceGroup | ISCMResource)[]
		>((r, g) => [...r, g, ...g.resources.sort(resourceSorter)], []);
	...
	private open(e: ISCMResource): void {
		...
		this.commandService
			.executeCommand(e.command.id, ...e.command.arguments)
			.done(undefined, onUnexpectedError);
	}
	...
	getTitle(): string {
		...
		const providerLabel =
			this.scmService.activeProvider && this.scmService.activeProvider.label;
	...
	getActionItem(action: IAction): IActionItem {
		...
		return new SCMMenuItemActionItem(
			action,
	...
	private onListContextMenu(
		e: IListContextMenuEvent<ISCMResourceGroup | ISCMResource>
	): void {
		...
		this.contextMenuService.showContextMenu({
			...
			actionRunner: new MultipleSelectionActionRunner(() =>
				this.getSelectedResources()
			)
		});
	}
