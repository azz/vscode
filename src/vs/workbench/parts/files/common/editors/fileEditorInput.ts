...
export class FileEditorInput extends EditorInput implements IFileEditorInput {
	...
	private registerListeners(): void {
		// Model changes
		this.toUnbind.push(
			this.textFileService.models.onModelDirty(e => this.onDirtyStateChange(e))
		);
		this.toUnbind.push(
			this.textFileService.models.onModelSaveError(e =>
				this.onDirtyStateChange(e)
			)
		);
		this.toUnbind.push(
			this.textFileService.models.onModelSaved(e => this.onDirtyStateChange(e))
		);
		this.toUnbind.push(
		...
		this.toUnbind.push(
			this.textFileService.models.onModelOrphanedChanged(e =>
				this.onModelOrphanedChanged(e)
			)
		);
	}
		...
		switch (verbosity) {
			case Verbosity.SHORT:
				title = this.shortTitle
					? this.shortTitle
					: (this.shortTitle = this.getName());
				break;
			case Verbosity.MEDIUM:
				title = this.mediumTitle
					? this.mediumTitle
					: (this.mediumTitle = labels.getPathLabel(
							this.resource,
							this.contextService,
							this.environmentService
						));
			...
			case Verbosity.LONG:
				title = this.longTitle
					? this.longTitle
					: (this.longTitle = labels.getPathLabel(
							this.resource,
							void 0,
							this.environmentService
						));
	...
	public isDirty(): boolean {
		...
		if (
			...
			model.hasState(ModelState.ERROR)
		) {
	...
	public resolve(
		refresh?: boolean
	): TPromise<TextFileEditorModel | BinaryEditorModel> {
		// Resolve as binary
		...
		// Resolve as text
		return this.textFileService.models
			.loadOrCreate(this.resource, {
			...
			.then(
				model => {
					...
					if (!this.textModelReference) {
						this.textModelReference = this.textModelResolverService.createModelReference(
							this.resource
						);
					}
	...
	private resolveAsBinary(): TPromise<BinaryEditorModel> {
		return this.instantiationService
			.createInstance(BinaryEditorModel, this.resource, this.getName())
			.load()
	...
	public matches(otherInput: any): boolean {
		...
		if (otherInput) {
			return (
				otherInput instanceof FileEditorInput &&
