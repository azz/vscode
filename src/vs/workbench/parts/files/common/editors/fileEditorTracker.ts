...
export class FileEditorTracker implements IWorkbenchContribution {
	...
	private handleDeletes(
		...
		const nonDirtyFileEditors = this.getOpenedFileEditors(
			false /* non-dirty only */
		);
		nonDirtyFileEditors.forEach(editor => {
			...
			if (this.closeOnFileDelete || !isExternal || !editor.isResolved()) {
				...
				if (
					...
					resource.fsPath.indexOf(movedTo.fsPath) === 0
				) {
					return;
				}
				...
				if (isExternal) {
					checkExists = TPromise.timeout(100).then(() =>
						this.fileService.existsFile(resource)
					);
				} else {
				...
				checkExists.done(exists => {
					if (!exists && !editor.isDisposed()) {
						...
						console.warn(
							`File exists even though we received a delete event: ${resource.toString()}`
						);
					}
				});
			}
	...
	private handleMovedFileInOpenedEditors(
		oldResource: URI,
		newResource: URI
	): void {
		const stacks = this.editorGroupService.getStacksModel();
				...
				if (input instanceof FileEditorInput) {
					...
					if (
						...
						if (oldResource.toString() === resource.toString()) {
							...
							reopenFileResource = URI.file(
								paths.join(
									newResource.fsPath,
									resource.fsPath.substr(index + oldResource.fsPath.length + 1)
								)
	...
	private handleUpdates(e: FileChangesEvent): void {
		...
		const modelsToUpdate = distinct(
			[...e.getUpdated(), ...e.getAdded()]
	...
	private handleUpdatesToVisibleEditors(
		e: FileChangesEvent
	): ITextFileEditorModel[] {
		...
		editors.forEach(editor => {
			const fileResource = toResource(editor.input, {
				filter: 'file',
			...
			if (fileResource) {
				...
				// and updated right after.
				if (
					e.contains(fileResource, FileChangeType.UPDATED) ||
					e.contains(fileResource, FileChangeType.ADDED)
				) {
					...
					if (textModel) {
						...
						if (!textModel.isDirty()) {
							...
							textModel.load().done(() => {
								...
								if (
									textModel.getETag() !== lastKnownEtag &&
									codeEditor.getModel() === textModel.textEditorModel
								) {
						...
						this.editorService
							.openEditor(
								editor.input,
								{ forceOpen: true, preserveFocus: true },
								editor.position
							)
							.done(null, errors.onUnexpectedError);
					}
