...
suite('Files - FileEditorTracker', () => {
	...
	test('disposes input when resource gets deleted - local file changes', function() {
		...
		accessor.fileService.fireAfterOperation(
			new FileOperationEvent(other, FileOperation.DELETE)
		);
		assert.ok(!input.isDisposed());

		accessor.fileService.fireAfterOperation(
			new FileOperationEvent(parent, FileOperation.DELETE)
		);
		assert.ok(input.isDisposed());
		...
		accessor.fileService.fireAfterOperation(
			new FileOperationEvent(resource, FileOperation.MOVE, to)
		);
		assert.ok(input.isDisposed());
	...
	test('disposes when resource gets deleted - remote file changes', function(
		...
		accessor.fileService.fireFileChanges(
			new FileChangesEvent([{ resource, type: FileChangeType.DELETED }])
		);
		...
		once(input.onDispose)(() => {
			...
			accessor.fileService.fireFileChanges(
				new FileChangesEvent([
					{ resource: other, type: FileChangeType.DELETED }
				])
			);
			assert.ok(!input.isDisposed());
	...
	test('file change event updates model', function(done) {
		...
		accessor.textFileService.models
			.loadOrCreate(resource)
			.then((model: TextFileEditorModel) => {
				...
				model.save().then(() => {
					...
					accessor.fileService.fireFileChanges(
						new FileChangesEvent([{ resource, type: FileChangeType.UPDATED }])
					);
