...
export class SaveErrorHandler
	...
	constructor(
		...
		this.conflictResolutionContext = new RawContextKey<boolean>(
	...
	public provideTextContent(resource: URI): TPromise<IModel> {
		...
		return this.textFileService
			.resolveTextContent(URI.file(resource.fsPath))
			.then(content => {
				let codeEditorModel = this.modelService.getModel(resource);
				if (!codeEditorModel) {
					codeEditorModel = this.modelService.createModel(
						content.value,
						this.modeService.getOrCreateModeByFilenameOrFirstLine(
							resource.fsPath
						),
						resource
					);
	...
	private registerListeners(): void {
		this.toUnbind.push(
			this.textFileService.models.onModelSaved(e =>
				this.onFileSavedOrReverted(e.resource)
			)
		);
		this.toUnbind.push(
			this.textFileService.models.onModelReverted(e =>
				this.onFileSavedOrReverted(e.resource)
			)
		);
		this.toUnbind.push(
	...
	private onEditorsChanged(): void {
		...
		if (
			activeEditor &&
			activeEditor.input instanceof DiffEditorInput &&
			activeEditor.input.originalInput instanceof ResourceEditorInput
		) {
			const resource = activeEditor.input.originalInput.getResource();
			isActiveEditorSaveConflictResolution =
				resource && resource.scheme === CONFLICT_RESOLUTION_SCHEME;
		}
	...
	public onSaveError(error: any, model: ITextFileEditorModel): void {
		...
		if (
			...
			message = this.instantiationService.createInstance(
			...
			actions.push(
				new Action(
					'workbench.files.action.saveAs',
					SaveFileAsAction.LABEL,
					null,
					true,
					() => {
						const saveAsAction = this.instantiationService.createInstance(
							SaveFileAsAction,
							SaveFileAsAction.ID,
							SaveFileAsAction.LABEL
						);
						...
						saveAsAction
							.run()
							.done(() => saveAsAction.dispose(), errors.onUnexpectedError);
			...
			actions.push(
				new Action(
					'workbench.files.action.discard',
					nls.localize('discard', 'Discard'),
					null,
					true,
					() => {
						const revertFileAction = this.instantiationService.createInstance(
							RevertFileAction,
							RevertFileAction.ID,
						...
						revertFileAction
							.run()
							.done(() => revertFileAction.dispose(), errors.onUnexpectedError);
			...
			if (isReadonly) {
				actions.push(
					new Action(
						'workbench.files.action.overwrite',
						nls.localize('overwrite', 'Overwrite'),
						null,
						true,
						() => {
							if (!model.isDisposed()) {
								model
									.save({ overwriteReadonly: true })
				...
				actions.push(
					new Action(
						'workbench.files.action.retry',
						nls.localize('retry', 'Retry'),
						null,
						true,
						() => {
							const saveFileAction = this.instantiationService.createInstance(
								SaveFileAction,
								SaveFileAction.ID,
							...
							saveFileAction
								.run()
								.done(() => saveFileAction.dispose(), errors.onUnexpectedError);
	...
	constructor(
		...
		this.actions = [
			new Action(
				...
				nls.localize('compareChanges', 'Compare'),
				null,
				true,
				() => {
					if (!this.model.isDisposed()) {
						...
						const editorLabel = nls.localize(
							...
							name,
							name,
							this.environmentService.appNameLong
						);

						return this.editorService
							.openEditor({
								leftResource: URI.from({
									scheme: CONFLICT_RESOLUTION_SCHEME,
							...
							.then(() => {
								// Inform user
								pendingResolveSaveConflictMessages.push(
		...
		return model.revert().then(() => {
			...
			return model.save().then(() => {
				...
				return editorService
					.openEditor({ resource: model.getResource() }, position)
					.then(() => {
						// Clean up
						input.dispose();
						reference.dispose();
