...
import {
	IAction,
	ActionRunner as BaseActionRunner,
	IActionRunner
...
} from 'vs/base/parts/tree/browser/treeDnd';
import {
	ClickBehavior,
	DefaultController
...
export class FileDataSource implements IDataSource {
	...
	public getChildren(tree: ITree, stat: FileStat): TPromise<FileStat[]> {
		...
		if (stat.isDirectoryResolved) {
			...
			const promise = this.fileService
				.resolveFile(stat.resource, { resolveSingleChildDescendants: true })
				.then(
					dirStat => {
	...
	public getParent(tree: ITree, stat: FileStat): TPromise<FileStat> {
		...
		if (
			workspace &&
			stat.resource.toString() === workspace.resource.toString()
		) {
...
export class FileActionProvider extends ContributableActionProvider {
	...
	public runAction(
		...
		return promise.then((actions: IAction[]) => {
			...
			promise = this.hasSecondaryActions(tree, stat)
...
export class FileRenderer implements IRenderer {
	...
	private renderInputBox(
		...
		const isFolder =
			stat.isDirectory ||
			(stat instanceof NewStatPlaceholder && stat.isDirectoryPlaceholder());
		...
		const done = once(commit => {
			...
			if (commit && inputBox.value) {
				this.state.actionProvider.runAction(tree, stat, editableData.action, {
...
export class FileController extends DefaultController {
	...
	constructor(
		...
		super({
			clickBehavior:
				ClickBehavior.ON_MOUSE_UP /* do not change to not break DND */,
			keyboardSupport: false /* handled via IListService */
		});
	...
	public onLeftClick(
		...
		// Cancel Event
		const isMouseDown =
			event && event.browserEvent && event.browserEvent.type === 'mousedown';
		if (!isMouseDown) {
		...
		if (event.shiftKey && !(stat instanceof NewStatPlaceholder)) {
			...
			if (!stat.isDirectory) {
				this.openEditor(stat, {
					preserveFocus,
					sideBySide: event && (event.ctrlKey || event.metaKey),
	...
	public onContextMenu(
		...
		this.contextMenuService.showContextMenu({
			getAnchor: () => anchor,
			getActions: () => {
				return this.state.actionProvider
					.getSecondaryActions(tree, stat)
					.then(actions => {
						fillInActions(
			...
			getActionItem: this.state.actionProvider.getActionItem.bind(
				this.state.actionProvider,
				tree,
				stat
			),
	...
	public openEditor(
		stat: FileStat,
		options: { preserveFocus: boolean; sideBySide: boolean; pinned: boolean }
	): void {
		if (stat && !stat.isDirectory) {
			...
			this.editorService
				.openEditor({ resource: stat.resource, options }, options.sideBySide)
...
export class FileFilter implements IFilter {
	...
	private doIsVisible(stat: FileStat): boolean {
		...
		if (
			siblings &&
			siblings.length > FileFilter.MAX_SIBLINGS_FILTER_THRESHOLD
		) {
			siblings = void 0;
		}
		...
		if (
			glob.match(
				...
				this.contextService.toWorkspaceRelativePath(stat.resource, true),
				siblingsFn
			)
		) {
...
export class FileDragAndDrop implements IDragAndDrop {
	...
	constructor(
		...
		this.onConfigurationUpdated(
			configurationService.getConfiguration<IFilesConfiguration>()
		);
	...
	private onConfigurationUpdated(config: IFilesConfiguration): void {
		this.dropEnabled =
			config && config.explorer && config.explorer.enableDragAndDrop;
	}
	...
	public getDragURI(tree: ITree, stat: FileStat): string {
		if (stat.isDirectory) {
			...
			}).toString(); // indicates that we are dragging a folder
		}
		...
		if (!(data instanceof DesktopDragAndDropData)) {
			if (source && !source.isDirectory) {
				originalEvent.dataTransfer.setData(
					'DownloadURL',
					[MIME_BINARY, source.name, source.resource.toString()].join(':')
				);
			}
		}
	}
	...
	public onDragOver(
			...
			if (
				sources.some(source => {
					...
					if (
						!isCopy &&
						isEqual(
							paths.dirname(source.resource.fsPath),
		...
		if (target.isDirectory) {
			return fromDesktop || isCopy
				? DRAG_OVER_ACCEPT_BUBBLE_DOWN_COPY(true)
				: DRAG_OVER_ACCEPT_BUBBLE_DOWN(true);
		}

		const workspace = this.contextService.getWorkspace();
		if (
			workspace &&
			target.resource.toString() !== workspace.resource.toString()
		) {
			return fromDesktop || isCopy
				? DRAG_OVER_ACCEPT_BUBBLE_UP_COPY
				: DRAG_OVER_ACCEPT_BUBBLE_UP;
	...
	public drop(
		...
		if (data instanceof DesktopDragAndDropData) {
			const importAction = this.instantiationService.createInstance(
				ImportFileAction,
				tree,
				target,
				null
			...
			promise = tree.expand(target).then(() => {
				...
				if (isCopy) {
					return this.instantiationService
						.createInstance(DuplicateFileAction, tree, source, target)
						.run();
				}
				...
				const onSuccess = () =>
					TPromise.join(
						dirtyMoved.map(t => this.textFileService.models.loadOrCreate(t))
					);
				const onError = (error?: Error, showError?: boolean) => {
					...
					return TPromise.join(
						dirtyMoved.map(d => this.backupFileService.discardResourceBackup(d))
					);
				};
					...
					TPromise.join(
						dirty.map(d => {
							...
							if (isEqual(source.resource.fsPath, d.fsPath)) {
								moved = URI.file(
									paths.join(target.resource.fsPath, source.name)
								);
							} else {
								...
								moved = URI.file(
									paths.join(
										target.resource.fsPath,
										d.fsPath.substr(source.parent.resource.fsPath.length + 1)
									)
								);
							}
						...
						.then(() => {
							...
							return this.fileService
								.moveFile(source.resource, targetResource)
								.then(null, error => {
									// Conflict
									if (
										...
										if (this.messageService.confirm(confirm)) {
											const targetDirty = this.textFileService
												.getDirty()
												.filter(d =>
													isEqualOrParent(
														d.fsPath,
											...
											return this.textFileService
												.revertAll(targetDirty, {
													soft: true /* do not attempt to load content from disk */
												})
												.then(() => {
													// Then continue to do the move operation
													return this.fileService
														.moveFile(source.resource, targetResource, true)
														.then(onSuccess, error => onError(error, true));
												});
										}
