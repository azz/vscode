...
import {
	...
	ActionProvider,
	DragAndDrop
} from 'vs/workbench/parts/files/browser/views/openEditorsViewer';
...
export class OpenEditorsView extends CollapsibleView {
	...
	constructor(
		...
		super(
			{
				...
				ariaHeaderLabel: nls.localize(
					{ key: 'openEditosrSection', comment: ['Open is an adjective'] },
					'Open Editors Section'
				),
				sizing: ViewSizing.Fixed,
				initialBodySize: OpenEditorsView.computeExpandedBodySize(
					editorGroupService.getStacksModel()
				)
			},
			keybindingService,
			contextMenuService
		...
		this.openEditorsFocussedContext = OpenEditorsFocussedContext.bindTo(
			contextKeyService
		);
	...
	public renderHeader(container: HTMLElement): void {
		...
		titleSpan.textContent = nls.localize(
		...
		this.toDispose.push(
			attachStylerCallback(
				this.themeService,
				{ badgeBackground, badgeForeground, contrastBorder },
				colors => {
					const background = colors.badgeBackground
						? colors.badgeBackground.toString()
						: null;
					const foreground = colors.badgeForeground
						? colors.badgeForeground.toString()
						: null;
					const border = colors.contrastBorder
						? colors.contrastBorder.toString()
						: null;
	...
	public getActions(): IAction[] {
		return [
			this.instantiationService.createInstance(
				...
				ToggleEditorLayoutAction.ID,
				ToggleEditorLayoutAction.LABEL
			),
			this.instantiationService.createInstance(
				SaveAllAction,
				SaveAllAction.ID,
	...
	public renderBody(container: HTMLElement): void {
		...
		this.tree = new Tree(
			...
			},
			{
				...
				ariaLabel: nls.localize(
					{ key: 'treeAriaLabel', comment: ['Open is an adjective'] },
		...
		this.toDispose.push(
			this.listService.register(this.tree, [
				...
				this.openEditorsFocussedContext
			])
		);
		...
		this.toDispose.push(
			this.tree.addListener('selection', event => {
				if (event && event.payload && event.payload.origin === 'keyboard') {
					controller.openEditor(this.tree.getFocus(), {
						pinned: false,
						sideBySide: false,
						preserveFocus: false
					});
		...
		this.toDispose.push(
			this.tree.addListener('item:collapsed', (event: IItemCollapseEvent) => {
	...
	private registerListeners(): void {
		...
		this.toDispose.push(
			this.model.onModelChanged(e => this.onEditorStacksModelChanged(e))
		);
	...
	private onEditorStacksModelChanged(e: IStacksModelChangeEvent): void {
		...
		if (e.structural) {
			...
			this.tree
				.refresh(toRefresh, false)
				.done(() => this.highlightActiveEditor(), errors.onUnexpectedError);
		}
	}
	...
	private structuralTreeUpdate(): void {
		...
		const treeInput = this.model.groups.length === 1
			? this.model.groups[0]
			: this.model;
		...
		(treeInput !== this.tree.getInput()
			? this.tree.setInput(treeInput)
			: this.tree.refresh(toRefresh)).done(() => {
			this.fullRefreshNeeded = false;
			this.groupToRefresh = null;

			// Always expand all the groups as they are unclickable
			return this.tree
				.expandAll(this.model.groups)
				.then(() => this.highlightActiveEditor());
		}, errors.onUnexpectedError);
	}

	private highlightActiveEditor(): void {
		if (
			this.model.activeGroup &&
			this.model.activeGroup.activeEditor /* could be empty */
		) {
			const openEditor = new OpenEditor(
				this.model.activeGroup.activeEditor,
				this.model.activeGroup
			);
	...
	private onConfigurationUpdated(configuration: IFilesConfiguration): void {
		...
		let visibleOpenEditors =
			configuration &&
			configuration.explorer &&
			configuration.explorer.openEditors &&
			configuration.explorer.openEditors.visible;
		...
		let dynamicHeight =
			...
			configuration.explorer &&
			configuration.explorer.openEditors &&
			configuration.explorer.openEditors.dynamicHeight;
	...
	private updateDirtyIndicator(): void {
		let dirty = this.textFileService.getAutoSaveMode() !==
			AutoSaveMode.AFTER_SHORT_DELAY
			? this.textFileService.getDirty().length
	...
	private getExpandedBodySize(model: IEditorStacksModel): number {
		return OpenEditorsView.computeExpandedBodySize(
			model,
			this.visibleOpenEditors,
			this.dynamicHeight
		);
	}

	private static computeExpandedBodySize(
		model: IEditorStacksModel,
