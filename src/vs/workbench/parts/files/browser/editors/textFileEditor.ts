...
export class TextFileEditor extends BaseTextEditor {
	...
	constructor(
		...
		this.toUnbind.push(
			editorGroupService
				.getStacksModel()
				.onWillCloseEditor(e => this.onWillCloseEditor(e))
		);
	}
	...
	private onWillCloseEditor(e: IEditorCloseEvent): void {
		if (
			e.editor === this.input &&
			this.position ===
				this.editorGroupService.getStacksModel().positionOfGroup(e.group)
		) {
	...
	public setInput(
		...
		if (oldInput) {
			...
			if (selection) {
				this.historyService.add(oldInput, {
					selection: {
		...
		return input.resolve(true).then(
			resolvedModel => {
				...
				const inputChanged =
					hasInput &&
					this.input.getResource().toString() !==
						textFileModel.getResource().toString();
				if (
				...
				const editorViewState = this.loadTextEditorViewState(
					this.input.getResource().toString()
				);
				if (editorViewState) {
			...
			error => {
				...
				if (
					...
					paths.isValidBasename(paths.basename(input.getResource().fsPath))
				) {
					return TPromise.wrapError<void>(
						errors.create(toErrorMessage(error), {
							actions: [
								new Action(
									'workbench.files.action.createMissingFile',
									nls.localize('createFile', 'Create File'),
									null,
									true,
									() => {
										return this.fileService
											.updateContent(input.getResource(), '')
											.then(() => {
												// Open
	...
	private openAsBinary(input: FileEditorInput, options: EditorOptions): void {
		...
		this.editorService
			.openEditor(input, options, this.position)
