...
Registry.as<ViewletRegistry>(ViewletExtensions.Viewlets).setDefaultViewletId(
	VIEWLET_ID
);
...
registry.registerWorkbenchAction(
	new SyncActionDescriptor(
		...
		OpenExplorerViewletAction.ID,
		OpenExplorerViewletAction.LABEL,
		openViewletKb
	),
	'View: Show Explorer',
...
class FileEditorInputFactory implements IEditorInputFactory {
	...
	public deserialize(
		...
		return instantiationService.invokeFunction<FileEditorInput>(accessor => {
			...
			const resource = !!fileInput.resourceJSON
				? URI.revive(fileInput.resourceJSON)
				: URI.parse(fileInput.resource);
			const encoding = fileInput.encoding;

			return accessor
				.get(IWorkbenchEditorService)
				.createInput({ resource, encoding }) as FileEditorInput;
		});
	}
}
...
Registry.as<IEditorRegistry>(
	EditorExtensions.Editors
).registerEditorInputFactory(FILE_EDITOR_INPUT_ID, FileEditorInputFactory);

// Register File Editor Tracker
Registry.as<IWorkbenchContributionsRegistry>(
	WorkbenchExtensions.Workbench
).registerWorkbenchContribution(FileEditorTracker);

// Register Save Error Handler
Registry.as<IWorkbenchContributionsRegistry>(
	WorkbenchExtensions.Workbench
).registerWorkbenchContribution(SaveErrorHandler);

// Register Dirty Files Tracker
Registry.as<IWorkbenchContributionsRegistry>(
	WorkbenchExtensions.Workbench
).registerWorkbenchContribution(DirtyFilesTracker);
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		'files.watcherExclude': {
			type: 'object',
			default: platform.isWindows /* https://github.com/Microsoft/vscode/issues/23954 */
				? {
						'**/.git/objects/**': true,
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		'explorer.openEditors.dynamicHeight': {
			type: 'boolean',
			description: nls.localize(
				{ key: 'dynamicHeight', comment: ['Open is an adjective'] },
