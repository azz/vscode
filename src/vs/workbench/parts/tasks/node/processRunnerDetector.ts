...
export class ProcessRunnerDetector {
	...
	private static TaskMatchers: Collections.IStringDictionary<DetectorConfig> = {
		...
		jake: {
			matcher: new RegexpTaskMatcher(/^jake\s+([^\s]+)\s/),
			arg: '--tasks'
		},
	...
	private static DefaultProblemMatchers: string[] = [
		'$lessCompile',
		'$tsc',
		'$jshint'
	];
	...
	constructor(
		...
		this._stderr = [];
		this._stdout = [];
		this._cwd = this.contextService.hasWorkspace()
			? Paths.normalize(
	...
	public detect(
		list: boolean = false,
		detectSpecific?: string
	): TPromise<DetectorResult> {
		if (
			this.taskConfiguration &&
			this.taskConfiguration.command &&
			ProcessRunnerDetector.supports(this.taskConfiguration.command)
		) {
			let config = ProcessRunnerDetector.detectorConfig(
				this.taskConfiguration.command
			);
			let args = (this.taskConfiguration.args || []).concat(config.arg);
			let options: CommandOptions = this.taskConfiguration.options
				? this.resolveCommandOptions(this.taskConfiguration.options)
				: { cwd: this._cwd };
			let isShellCommand = !!this.taskConfiguration.isShellCommand;
			return this.runDetection(
				new LineProcess(
					this.taskConfiguration.command,
					this.configurationResolverService.resolve(args),
					isShellCommand,
					options
				),
				this.taskConfiguration.command,
				isShellCommand,
				config.matcher,
				ProcessRunnerDetector.DefaultProblemMatchers,
				list
			);
		} else {
			if (detectSpecific) {
	...
	private tryDetectGulp(list: boolean): TPromise<DetectorResult> {
		return this.fileService
			.resolveFile(this.contextService.toResource('gulpfile.js'))
			.then(
				stat => {
					let config = ProcessRunnerDetector.detectorConfig('gulp');
					let process = new LineProcess(
						'gulp',
						[config.arg, '--no-color'],
						true,
						{ cwd: this._cwd }
	...
	private tryDetectGrunt(list: boolean): TPromise<DetectorResult> {
		return this.fileService
			.resolveFile(this.contextService.toResource('Gruntfile.js'))
			.then(
				stat => {
	...
	private tryDetectJake(list: boolean): TPromise<DetectorResult> {
		let run = () => {
			...
			let process = new LineProcess('jake', [config.arg], true, {
				cwd: this._cwd
			});
			return this.runDetection(
				process,
		...
		return this.fileService
			.resolveFile(this.contextService.toResource('Jakefile'))
			.then(
				stat => {
					return run();
				},
				(err: any) => {
					return this.fileService
						.resolveFile(this.contextService.toResource('Jakefile.js'))
						.then(
							stat => {
								return run();
							},
	...
	private runDetection(
		...
		return process.start().then(
			success => {
				...
				result.tasks = this.createTaskDescriptions(
					tasks,
					problemMatchers,
					list
				);
	...
	private createTaskDescriptions(
		tasks: string[],
		problemMatchers: string[],
		list: boolean
	...
	private testBuild(taskInfo: TaskInfo, taskName: string, index: number): void {
		if (taskName === build) {
			...
			(Strings.startsWith(taskName, build) ||
				Strings.endsWith(taskName, build)) &&
			taskInfo.exact < 4
		) {
			taskInfo.index = index;
			taskInfo.exact = 3;
	...
	private testTest(taskInfo: TaskInfo, taskName: string, index: number): void {
		if (taskName === test) {
			...
			(Strings.startsWith(taskName, test) ||
				Strings.endsWith(taskName, test)) &&
			taskInfo.exact < 3
		) {
			taskInfo.index = index;
			taskInfo.exact = 2;
