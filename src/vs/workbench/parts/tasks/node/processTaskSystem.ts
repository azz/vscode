...
export class ProcessTaskSystem extends EventEmitter implements ITaskSystem {
	...
	public run(task: Task): ITaskExecuteResult {
		if (this.activeTask) {
			return {
				...
				active: {
					same: this.activeTask._id === task._id,
					background: this.activeTask.isBackground
				},
				promise: this.activeTaskPromise
			};
		}
			...
			result.promise = result.promise.then(
				success => {
					this.telemetryService.publicLog(
						ProcessTaskSystem.TelemetryEventName,
						telemetryEvent
					);
					return success;
				},
	...
	private doExecuteTask(
		...
		this.childProcess = new LineProcess(
			command,
			args,
			commandConfig.type === CommandType.Shell,
			this.resolveOptions(commandConfig.options)
		);
		...
		if (
			...
			(reveal === RevealKind.Silent && task.problemMatchers.length === 0)
		) {
			this.showOutput();
		}
		...
		if (task.isBackground) {
			let watchingProblemMatcher = new WatchingProblemCollector(
				this.resolveMatchers(task.problemMatchers),
				this.markerService,
				this.modelService
			);
			...
			this.activeTaskPromise = this.childProcess.start().then(
				(success): ITaskSummary => {
			...
			let result: ITaskExecuteResult = (<any>task).tscWatch
				? {
						...
						started: { restartOnFileChanges: '**/*.ts' },
						promise: this.activeTaskPromise
					}
				: {
						kind: TaskExecuteKind.Started,
