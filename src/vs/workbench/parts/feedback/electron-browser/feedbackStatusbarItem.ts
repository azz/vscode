...
class TwitterFeedbackService implements IFeedbackService {
	...
	public submitFeedback(feedback: IFeedback): void {
		const queryString = `?${feedback.sentiment === 1
			? `hashtags=${this.combineHashTagsAsString()}&`
			: null}ref_src=twsrc%5Etfw&related=twitterapi%2Ctwitter&text=${feedback.feedback}&tw_p=tweetbutton&via=${TwitterFeedbackService.VIA_NAME}`;
		...
		if (product.sendASmile) {
			this.dropdown = this.instantiationService.createInstance(
				...
				element,
				{
					...
					feedbackService: this.instantiationService.createInstance(
