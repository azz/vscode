...
function configurationsEqual(
	...
	return a.tabSize === b.tabSize && a.insertSpaces === b.insertSpaces;
}
...
export class MainThreadTextEditor {
	...
	constructor(
		...
		this._onConfigurationChanged = new Emitter<
			IResolvedTextEditorConfiguration
		>();
		...
		this._modelListeners.push(
			this._model.onDidChangeOptions(e => {
				this._setConfiguration(
					this._readConfiguration(this._model, this._codeEditor)
				);
			})
		);
	...
	public setCodeEditor(codeEditor: EditorCommon.ICommonCodeEditor): void {
		...
		if (this._codeEditor) {
			...
			this._codeEditorListeners.push(
			...
			this._codeEditorListeners.push(
				this._codeEditor.onDidChangeCursorSelection(forwardSelection)
			);
			if (
			...
			this._codeEditorListeners.push(
				this._codeEditor.onDidBlurEditor(() => {
	...
	private _readConfiguration(
		...
		let cursorStyle = this._configuration
			? this._configuration.cursorStyle
			: TextEditorCursorStyle.Line;
		let lineNumbers: TextEditorLineNumbersStyle = this._configuration
			? this._configuration.lineNumbers
	...
	private _setConfiguration(
		newConfiguration: IResolvedTextEditorConfiguration
	): void {
	...
	public applyEdits(
		...
		let transformedEdits = edits.map(
			(edit): EditorCommon.IIdentifiedSingleEditOperation => {
				return {
					identifier: null,
