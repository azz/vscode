...
export class MainThreadDocuments extends MainThreadDocumentsShape {
	...
	constructor(
		...
		this._toDispose.push(
			documentsAndEditors.onDocumentAdd(models =>
				models.forEach(this._onModelAdded, this)
			)
		);
		this._toDispose.push(
			documentsAndEditors.onDocumentRemove(urls =>
				urls.forEach(this._onModelRemoved, this)
			)
		);
		modelService.onModelModeChanged(
		...
		this._toDispose.push(
			textFileService.models.onModelSaved(e => {
		...
		this._toDispose.push(
			textFileService.models.onModelReverted(e => {
				if (this._shouldHandleFileEvent(e)) {
		...
		this._toDispose.push(
			textFileService.models.onModelDirty(e => {
				if (this._shouldHandleFileEvent(e)) {
	...
	private _onModelAdded(model: editorCommon.IModel): void {
		...
		this._modelToDisposeMap[
			modelUrl.toString()
		] = model.onDidChangeContent(e => {
			this._proxy.$acceptModelChanged(
				modelUrl.toString(),
				e,
				this._textFileService.isDirty(modelUrl)
			);
		});
	}
	...
	private _onModelModeChanged(event: {
		...
		this._proxy.$acceptModelModeChanged(
			model.uri.toString(),
			oldModeId,
	...
	$tryOpenDocument(uri: URI): TPromise<any> {
		...
		return promise.then(
			...
			err => {
				return TPromise.wrapError(
					'cannot open ' + uri.toString() + '. Detail: ' + toErrorMessage(err)
				);
			}
		);
	}
	...
	private _handleAsResourceInput(uri: URI): TPromise<boolean> {
		return this._textModelResolverService
	...
	private _handleUnititledScheme(uri: URI): TPromise<boolean> {
		...
		return this._fileService.resolveFile(asFileUri).then(
			stats => {
	...
	private _doCreateUntitled(
		...
		return this._untitledEditorService
			.loadOrCreate({ resource, modeId, initialValue })
			.then(model => {
					...
					return this._proxy
						...
						.then(value => {
							if (typeof value === 'string') {
								...
								const mode = this._modeService.getOrCreateModeByFilenameOrFirstLine(
									uri.fsPath,
									firstLineText
								);
