...
export class MainThreadStatusBar extends MainThreadStatusBarShape {
	...
	constructor(@IStatusbarService private statusbarService: IStatusbarService) {
		super();
	...
	$setEntry(
		...
		// Add new
		let disposeable = this.statusbarService.addEntry(
			{ text, tooltip, command, color, extensionId },
			alignment,
			priority
		);
