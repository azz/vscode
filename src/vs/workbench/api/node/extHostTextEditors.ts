...
export class ExtHostEditors extends ExtHostEditorsShape {
	private readonly _onDidChangeTextEditorSelection = new Emitter<
		vscode.TextEditorSelectionChangeEvent
	>();
	private readonly _onDidChangeTextEditorOptions = new Emitter<
		vscode.TextEditorOptionsChangeEvent
	>();
	private readonly _onDidChangeTextEditorViewColumn = new Emitter<
	...
	readonly onDidChangeTextEditorOptions: Event<
		vscode.TextEditorOptionsChangeEvent
	...
	readonly onDidChangeActiveTextEditor: Event<vscode.TextEditor> = this
		._onDidChangeActiveTextEditor.event;
	readonly onDidChangeVisibleTextEditors: Event<vscode.TextEditor[]> = this
	...
	showTextDocument(
		...
		if (typeof columnOrOptions === 'number') {
			...
			options = {
				...
				pinned: columnOrOptions.preview === undefined
					? true
		...
		return this._proxy
			.$tryShowTextDocument(<URI>document.uri, options)
			.then(id => {
				...
				if (editor) {
					...
					throw new Error(
						`Failed to show text document ${document.uri.toString()}, should show in editor #${id}`
					);
				}
			});
	}
