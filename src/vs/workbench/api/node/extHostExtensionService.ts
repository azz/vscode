...
} from 'vs/workbench/api/node/extHost.api.impl';
...
class ExtensionMemento implements IExtensionMemento {
	...
	constructor(id: string, global: boolean, storage: ExtHostStorage) {
		...
		this._init = this._storage
			.getValue(this._shared, this._id, Object.create(null))
			.then(value => {
				this._value = value;
				return this;
			});
...
class ExtensionStoragePath {
	...
	constructor(
		...
		this._ready = this._getOrCreateWorkspaceStoragePath().then(
	...
	private _getOrCreateWorkspaceStoragePath(): TPromise<string> {
		...
		const storagePath = join(
			this._environment.appSettingsHome,
			'workspaceStorage',
			storageName
		);
		...
		return dirExists(storagePath).then(exists => {
			...
			return mkdirp(storagePath).then(
				success => {
					return storagePath;
				},
				err => {
...
export class ExtHostExtensionService extends AbstractExtensionService<
	ExtHostExtension
> {
	...
	constructor(
		...
		this._storagePath = new ExtensionStoragePath(
			contextService,
			initData.environment
		);
		this._proxy = this._threadService.get(
			MainContext.MainProcessExtensionService
		);
	...
	public get(extensionId: string): IExtensionAPI {
		if (!hasOwnProperty.call(this._activatedExtensions, extensionId)) {
			throw new Error(
				'Extension `' + extensionId + '` is not known or not activated'
			);
		}
	...
	): TPromise<IExtensionContext> {
		let globalState = new ExtensionMemento(
			extensionDescription.id,
			true,
			this._storage
		);
		let workspaceState = new ExtensionMemento(
			extensionDescription.id,
			false,
			this._storage
		);
		...
		return TPromise.join([
			...
			return Object.freeze(
				<IExtensionContext>{
					...
					subscriptions: [],
					get extensionPath() {
						return extensionDescription.extensionFolderPath;
					},
					storagePath: this._storagePath.value(extensionDescription),
					asAbsolutePath: (relativePath: string) => {
						return join(extensionDescription.extensionFolderPath, relativePath);
					}
				}
			);
		});
	...
	protected _actualActivateExtension(
		extensionDescription: IExtensionDescription
	): TPromise<ExtHostExtension> {
		return this._doActualActivateExtension(extensionDescription).then(
			activatedExtension => {
	...
	private _doActualActivateExtension(
		extensionDescription: IExtensionDescription
	): TPromise<ExtHostExtension> {
			...
			return TPromise.join<any>([
				...
				values => {
					return ExtHostExtensionService._callActivate(
						<IExtensionModule>values[0],
						<IExtensionContext>values[1]
					);
				},
				(errors: any[]) => {
...
function getTelemetryActivationEvent(
	...
	for (let contribution in extensionDescription.contributes) {
		...
		switch (contribution) {
			case 'debuggers':
				let types = contributionDetails.reduce(
					(p, c) => (p ? p + ',' + c['type'] : c['type']),
					''
				);
				event['contribution.debuggers'] = types;
				break;
			case 'grammars':
				let grammers = contributionDetails.reduce(
					(p, c) => (p ? p + ',' + c['language'] : c['language']),
					''
				);
				event['contribution.grammars'] = grammers;
				break;
			case 'languages':
				let languages = contributionDetails.reduce(
					(p, c) => (p ? p + ',' + c['id'] : c['id']),
					''
				);
				...
				break;
			case 'tmSnippets':
				let tmSnippets = contributionDetails.reduce(
					(p, c) => (p ? p + ',' + c['languageId'] : c['languageId']),
					''
				);
