...
export class ExtHostDocumentSaveParticipant extends ExtHostDocumentSaveParticipantShape {
	...
	constructor(
		...
		workspace: MainThreadWorkspaceShape,
		thresholds: { timeout: number; errors: number } = {
			timeout: 1500,
			errors: 3
		}
	) {
	...
	$participateInSave(resource: URI, reason: SaveReason): TPromise<boolean[]> {
		...
		let didTimeoutHandle = setTimeout(
			() => (didTimeout = true),
			this._thresholds.timeout
		);

		const promise = sequence(
			entries.map(([fn, thisArg]) => {
				return () => {
					if (didTimeout) {
					...
					return this._deliverEventAsyncAndBlameBadListeners(
						fn,
						thisArg,
						<any>{ document, reason: TextDocumentSaveReason.to(reason) }
					);
				};
			})
		);
	...
	private _deliverEventAsync(
		...
		return new TPromise<vscode.TextEdit[][]>((resolve, reject) => {
			...
			return always(TPromise.join(promises), () => clearTimeout(handle)).then(
				resolve,
				reject
			);
		}).then(values => {
			...
			for (const value of values) {
				if (
					Array.isArray(value) &&
					(<vscode.TextEdit[]>value).every(e => e instanceof TextEdit)
				) {
