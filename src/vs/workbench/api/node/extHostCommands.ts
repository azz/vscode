...
export class ExtHostCommands extends ExtHostCommandsShape {
	...
	constructor(threadService: IThreadService, heapService: ExtHostHeapService) {
		super();
	...
	$executeContributedCommand<T>(id: string, ...args: any[]): Thenable<T> {
		...
		if (!command) {
			return TPromise.wrapError<T>(
				`Contributed command '${id}' does not exist.`
			);
		}
		...
		if (description) {
			for (let i = 0; i < description.args.length; i++) {
				try {
					...
					return TPromise.wrapError<T>(
						`Running the contributed command:'${id}' failed. Illegal argument '${description
							.args[i].name}' - ${description.args[i].description}`
					);
				}
			}
		}
		...
		args = args.map(arg =>
			this._argumentProcessors.reduce((r, p) => p.processArgument(r), arg)
		);
		...
		try {
			...
			// }
			return TPromise.wrapError<T>(
				`Running the contributed command:'${id}' failed.`
			);
		}
	}
