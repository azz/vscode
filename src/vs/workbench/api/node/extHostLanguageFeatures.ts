...
class OutlineAdapter {
	...
	provideDocumentSymbols(resource: URI): TPromise<modes.SymbolInformation[]> {
		...
		return asWinJsPromise(token =>
			this._provider.provideDocumentSymbols(doc, token)
		).then(value => {
...
class CodeLensAdapter {
	private static _badCmd: vscode.Command = {
		command: 'missing',
	...
	provideCodeLenses(resource: URI): TPromise<modes.ICodeLensSymbol[]> {
		...
		return asWinJsPromise(token =>
			this._provider.provideCodeLenses(doc, token)
		).then(lenses => {
	...
	resolveCodeLens(
	...
	): TPromise<modes.ICodeLensSymbol> {
		const lens = this._heapService.get<vscode.CodeLens>(
			ObjectIdentifier.of(symbol)
		);
		if (!lens) {
			return undefined;
		}
		...
		if (
			...
			resolve = asWinJsPromise(token =>
				this._provider.resolveCodeLens(lens, token)
			);
		}
		...
		return resolve.then(newLens => {
			...
			symbol.command = this._commands.toInternal(
				newLens.command || CodeLensAdapter._badCmd
			);
			return symbol;
		});
	}
...
class DefinitionAdapter {
	...
	provideDefinition(
		resource: URI,
		position: IPosition
	): TPromise<modes.Definition> {
		let doc = this._documents.getDocumentData(resource).document;
		let pos = TypeConverters.toPosition(position);
		return asWinJsPromise(token =>
			this._provider.provideDefinition(doc, pos, token)
		).then(value => {
			if (Array.isArray(value)) {
...
class ImplementationAdapter {
	...
	provideImplementation(
		...
		let pos = TypeConverters.toPosition(position);
		return asWinJsPromise(token =>
			this._provider.provideImplementation(doc, pos, token)
		).then(value => {
			if (Array.isArray(value)) {
				return value.map(TypeConverters.location.from);
			} else if (value) {
...
class TypeDefinitionAdapter {
	...
	provideTypeDefinition(
		...
		return asWinJsPromise(token =>
			this._provider.provideTypeDefinition(doc, pos, token)
		).then(value => {
			if (Array.isArray(value)) {
				return value.map(TypeConverters.location.from);
			} else if (value) {
...
class HoverAdapter {
	...
	public provideHover(
		resource: URI,
		position: IPosition
	): TPromise<modes.Hover> {
		let doc = this._documents.getDocumentData(resource).document;
		let pos = TypeConverters.toPosition(position);

		return asWinJsPromise(token =>
			this._provider.provideHover(doc, pos, token)
		).then(value => {
			if (!value) {
				return undefined;
			}
...
class DocumentHighlightAdapter {
	...
	provideDocumentHighlights(
		resource: URI,
		position: IPosition
	): TPromise<modes.DocumentHighlight[]> {
		let doc = this._documents.getDocumentData(resource).document;
		let pos = TypeConverters.toPosition(position);

		return asWinJsPromise(token =>
			this._provider.provideDocumentHighlights(doc, pos, token)
		).then(value => {
			if (Array.isArray(value)) {
...
class ReferenceAdapter {
	...
	): TPromise<modes.Location[]> {
		let doc = this._documents.getDocumentData(resource).document;
		let pos = TypeConverters.toPosition(position);

		return asWinJsPromise(token =>
			this._provider.provideReferences(doc, pos, context, token)
		).then(value => {
			if (Array.isArray(value)) {
				return value.map(TypeConverters.location.from);
			}
...
class QuickFixAdapter {
	...
	provideCodeActions(
		...
		return asWinJsPromise(token =>
			this._provider.provideCodeActions(
				doc,
				ran,
...
class DocumentFormattingAdapter {
	...
	): TPromise<ISingleEditOperation[]> {
		const { document } = this._documents.getDocumentData(resource);

		return asWinJsPromise(token =>
			this._provider.provideDocumentFormattingEdits(
				document,
				<any>options,
				token
			)
...
class OnTypeFormattingAdapter {
	...
	provideOnTypeFormattingEdits(
		...
		return asWinJsPromise(token =>
			this._provider.provideOnTypeFormattingEdits(
				document,
				pos,
				ch,
				<any>options,
...
class NavigateTypeAdapter implements IWorkspaceSymbolProvider {
	...
	provideWorkspaceSymbols(search: string): TPromise<modes.SymbolInformation[]> {
		return asWinJsPromise(token =>
			this._provider.provideWorkspaceSymbols(search, token)
		).then(value => {
			if (Array.isArray(value)) {
	...
	resolveWorkspaceSymbol(
		item: modes.SymbolInformation
	): TPromise<modes.SymbolInformation> {
		if (typeof this._provider.resolveWorkspaceSymbol !== 'function') {
			return TPromise.as(item);
		}

		const symbolInfo = this._heapService.get<vscode.SymbolInformation>(
			ObjectIdentifier.of(item)
		);
		if (symbolInfo) {
			return asWinJsPromise(token =>
				this._provider.resolveWorkspaceSymbol(symbolInfo, token)
			).then(value => {
...
class RenameAdapter {
	...
	provideRenameEdits(
		...
		position: IPosition,
		newName: string
	): TPromise<modes.WorkspaceEdit> {
		let doc = this._documents.getDocumentData(resource).document;
		let pos = TypeConverters.toPosition(position);

		return asWinJsPromise(token =>
			this._provider.provideRenameEdits(doc, pos, newName, token)
		).then(
			value => {
				if (!value) {
					return undefined;
...
class SuggestAdapter {
	...
	provideCompletionItems(
		...
		>(token =>
			this._provider.provideCompletionItems(doc, pos, token)
		).then(value => {
			...
			for (const item of list.items) {
				const suggestion = this._convertCompletionItem(
					item,
					pos,
					wordRangeBeforePos
				);
	...
	resolveCompletionItem(
		...
		return asWinJsPromise(token =>
			this._provider.resolveCompletionItem(item, token)
		).then(resolvedItem => {
			if (!resolvedItem) {
				return suggestion;
			}
			...
			const wordRangeBeforePos = (doc.getWordRangeAtPosition(pos) ||
				new Range(pos, pos))
				.with({ end: pos });
			const newSuggestion = this._convertCompletionItem(
				resolvedItem,
				pos,
				wordRangeBeforePos
			);
		...
		const result: modes.ISuggestion = {
			...
			additionalTextEdits:
				item.additionalTextEdits &&
					item.additionalTextEdits.map(TypeConverters.TextEdit.from),
...
class SignatureHelpAdapter {
	...
	provideSignatureHelp(
		resource: URI,
		position: IPosition
	): TPromise<modes.SignatureHelp> {
		const doc = this._documents.getDocumentData(resource).document;
		const pos = TypeConverters.toPosition(position);

		return asWinJsPromise(token =>
			this._provider.provideSignatureHelp(doc, pos, token)
		).then(value => {
			if (value) {
...
class LinkProviderAdapter {
	...
	provideLinks(resource: URI): TPromise<modes.ILink[]> {
		...
		return asWinJsPromise(token =>
			this._provider.provideDocumentLinks(doc, token)
		).then(links => {
	...
	resolveLink(link: modes.ILink): TPromise<modes.ILink> {
		if (typeof this._provider.resolveDocumentLink === 'function') {
			return asWinJsPromise(token =>
				this._provider.resolveDocumentLink(
					TypeConverters.DocumentLink.to(link),
					token
				)
			).then(value => {
				if (value) {
...
export class ExtHostLanguageFeatures extends ExtHostLanguageFeaturesShape {
	...
	private _withAdapter<A, R>(
		handle: number,
		ctor: { new (...args: any[]): A },
		callback: (adapter: A) => TPromise<R>
	): TPromise<R> {
	...
	$provideDocumentSymbols(
		...
		return this._withAdapter(handle, OutlineAdapter, adapter =>
			adapter.provideDocumentSymbols(resource)
		);
	}
	...
	registerCodeLensProvider(
		...
		const handle = this._nextHandle();
		const eventHandle = typeof provider.onDidChangeCodeLenses === 'function'
			? this._nextHandle()
			: undefined;
	...
	$provideCodeLenses(
		...
		return this._withAdapter(handle, CodeLensAdapter, adapter =>
	...
	$provideDefinition(
		...
		return this._withAdapter(handle, DefinitionAdapter, adapter =>
			adapter.provideDefinition(resource, position)
		);
	}
	...
	$provideImplementation(
		...
		return this._withAdapter(handle, ImplementationAdapter, adapter =>
	...
	$provideTypeDefinition(
		...
		return this._withAdapter(handle, TypeDefinitionAdapter, adapter =>
			adapter.provideTypeDefinition(resource, position)
		);
	}
	...
	$provideHover(
		...
		return this._withAdapter(handle, HoverAdapter, adpater =>
			adpater.provideHover(resource, position)
		);
	}
	...
	$provideDocumentHighlights(
		...
		return this._withAdapter(handle, DocumentHighlightAdapter, adapter =>
			adapter.provideDocumentHighlights(resource, position)
		);
	}
	...
	$provideReferences(
		...
		return this._withAdapter(handle, ReferenceAdapter, adapter =>
			adapter.provideReferences(resource, position, context)
		);
	}
	...
	$provideCodeActions(
		...
		return this._withAdapter(handle, QuickFixAdapter, adapter =>
			adapter.provideCodeActions(resource, range)
		);
	}
	...
	$provideDocumentFormattingEdits(
		...
		return this._withAdapter(handle, DocumentFormattingAdapter, adapter =>
	...
	$provideDocumentRangeFormattingEdits(
		...
		return this._withAdapter(handle, RangeFormattingAdapter, adapter =>
	...
	$provideOnTypeFormattingEdits(
		...
		return this._withAdapter(handle, OnTypeFormattingAdapter, adapter =>
	...
	registerWorkspaceSymbolProvider(
		...
		this._adapter.set(
			handle,
			new NavigateTypeAdapter(provider, this._heapService)
		);
	...
	$provideWorkspaceSymbols(
		...
		return this._withAdapter(handle, NavigateTypeAdapter, adapter =>
			adapter.provideWorkspaceSymbols(search)
		);
	}

	$resolveWorkspaceSymbol(
		handle: number,
		symbol: modes.SymbolInformation
	): TPromise<modes.SymbolInformation> {
		return this._withAdapter(handle, NavigateTypeAdapter, adapter =>
			adapter.resolveWorkspaceSymbol(symbol)
		);
	}
	...
	$provideRenameEdits(
		...
		return this._withAdapter(handle, RenameAdapter, adapter =>
			adapter.provideRenameEdits(resource, position, newName)
		);
	}
	...
	$provideCompletionItems(
		...
		return this._withAdapter(handle, SuggestAdapter, adapter =>
			adapter.provideCompletionItems(resource, position)
		);
	}
	...
	$resolveCompletionItem(
		...
		return this._withAdapter(handle, SuggestAdapter, adapter =>
			adapter.resolveCompletionItem(resource, position, suggestion)
		);
	}
	...
	$provideSignatureHelp(
		...
		return this._withAdapter(handle, SignatureHelpAdapter, adapter =>
			adapter.provideSignatureHelp(resource, position)
		);
	}
	...
	$provideDocumentLinks(
		handle: number,
		resource: URI
	): TPromise<modes.ILink[]> {
		return this._withAdapter(handle, LinkProviderAdapter, adapter =>
			adapter.provideLinks(resource)
		);
	}
