...
suite('ConfigurationEditingService', () => {
	...
	setup(() => {
		return setUpWorkspace().then(() => setUpServices());
	});
	...
	function setUpServices(noWorkspace: boolean = false): TPromise<void> {
		...
		instantiationService.stub(
			IEnvironmentService,
			new SettingsTestEnvironmentService(
		...
		instantiationService.stub(
			...
			new WorkspaceContextService(
				noWorkspace ? null : { resource: URI.file(workspaceDir) }
			)
		);
		...
		instantiationService.stub(
			IFileService,
			new FileService(workspaceDir, { disableWatcher: true })
		);
		instantiationService.stub(
			IUntitledEditorService,
			instantiationService.createInstance(UntitledEditorService)
		);
		...
		instantiationService.stub(
			...
			<ITextModelResolverService>instantiationService.createInstance(
	...
	function clearServices(): void {
		if (instantiationService) {
			const configuraitonService = <WorkspaceConfigurationService>instantiationService.get(
	...
	test('errors cases - invalid target', () => {
		return testObject
			.writeConfiguration(ConfigurationTarget.USER, {
				key: 'tasks.something',
				value: 'value'
			})
			.then(
				() => assert.fail('Should fail with ERROR_INVALID_TARGET'),
				(error: IConfigurationEditingError) =>
					assert.equal(
						error.code,
						ConfigurationEditingErrorCode.ERROR_INVALID_TARGET
					)
			);
	});
	...
	test('errors cases - no workspace', () => {
		return setUpServices(true)
			.then(() =>
				testObject.writeConfiguration(ConfigurationTarget.WORKSPACE, {
			...
			.then(() => {
				...
				assert.equal(
					instantiationService
						...
						.lookup('configurationEditing.service.testSetting').value,
					'value'
				);
			});
	});
	...
	test('write one setting - existing file', () => {
		...
		return testObject
			...
			.then(() => {
				...
				assert.equal(
					configurationService.lookup(
						'configurationEditing.service.testSetting'
					).value,
					'value'
				);
				assert.equal(
					configurationService.lookup('my.super.setting').value,
					'my.super.value'
				);
			});
	});
	...
	test('write workspace standalone setting - empty file', () => {
		return testObject
			...
			.then(() => {
				...
				assert.equal(
					configurationService.lookup('tasks.service.testSetting').value,
					'value'
				);
			});
	});
	...
	test('write workspace standalone setting - existing file', () => {
		...
		return testObject
			.writeConfiguration(ConfigurationTarget.WORKSPACE, {
				key: 'launch.service.testSetting',
				value: 'value'
			})
			.then(() => {
				const contents = fs.readFileSync(target).toString('utf8');
				...
				assert.equal(
					configurationService.lookup('launch.service.testSetting').value,
					'value'
				);
				assert.equal(
					configurationService.lookup('launch.my.super.setting').value,
					'my.super.value'
				);
			});
	});
