...
suite('ConfigurationService - Model', () => {
	test('Test scoped configs are undefined', () => {
		const settingsConfig = new WorkspaceSettingsConfigModel(
			JSON.stringify({
				awesome: true
			})
		);
	...
	test('Test consolidate (settings and tasks)', () => {
		...
		const tasksConfig = new ScopedConfigModel(
			JSON.stringify({
				awesome: false
			}),
			'',
		...
		assert.deepEqual(
			new WorkspaceConfigModel(settingsConfig, [tasksConfig]).contents,
			expected
		);
	});
	...
	test('Test consolidate (settings and launch)', () => {
		...
		const launchConfig = new ScopedConfigModel(
			JSON.stringify({
				awesome: false
			}),
			'',
		...
		assert.deepEqual(
			new WorkspaceConfigModel(settingsConfig, [launchConfig]).contents,
			expected
		);
	});
	...
	test('Test consolidate (settings and launch and tasks) - launch/tasks wins over settings file', () => {
		...
		assert.deepEqual(
			new WorkspaceConfigModel(settingsConfig, [launchConfig, tasksConfig])
				.contents,
			expected
		);
		assert.deepEqual(
			new WorkspaceConfigModel(settingsConfig, [tasksConfig, launchConfig])
				.contents,
			expected
		);
	});
