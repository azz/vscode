...
} from 'vs/workbench/services/configuration/common/configurationModels';
import {
 ...
 */
export class WorkspaceConfigurationService extends Disposable
	implements IWorkspaceConfigurationService, IDisposable {
	...
	private workspaceFilePathToConfiguration: {
		[relativeWorkspacePath: string]: TPromise<IConfigModel<any>>;
	};
	...
	constructor(
		...
		this.cachedWorkspaceConfig = new WorkspaceConfigModel(
			new WorkspaceSettingsConfigModel(null),
			[]
		);

		this._onDidUpdateConfiguration = this._register(
			new Emitter<IConfigurationServiceEvent>()
		);

		this.baseConfigurationService = this._register(
			new BaseConfigurationService(environmentService)
		);
		...
		this.reloadConfigurationScheduler = this._register(
			new RunOnceScheduler(
				...
				WorkspaceConfigurationService.RELOAD_CONFIGURATION_DELAY
			)
		);

		this._register(
			this.baseConfigurationService.onDidUpdateConfiguration(e =>
				this.onBaseConfigurationChanged(e)
			)
		);
	}
	...
	private onBaseConfigurationChanged(event: IConfigurationServiceEvent): void {
		...
		const configModel = <ConfigModel<any>>this.baseConfigurationService
			.getCache()
	...
	public getConfiguration<C>(arg?: any): C {
		...
		const configModel = options.overrideIdentifier
			? this.cachedConfig.configWithOverrides<C>(options.overrideIdentifier)
			: this.cachedConfig;
		return options.section
			? configModel.getContentsFor<C>(options.section)
			: configModel.contents;
	}

	public lookup<C>(
		key: string,
		overrideIdentifier?: string
	): IWorkspaceConfigurationValue<C> {
		const configurationValue = this.baseConfigurationService.lookup<C>(
			key,
			overrideIdentifier
		);
		return {
			...
			workspace: objects.clone(
				getConfigurationValue<C>(
					overrideIdentifier
						? this.cachedWorkspaceConfig.configWithOverrides(overrideIdentifier)
								.contents
						: this.cachedWorkspaceConfig.contents,
					key
				)
			),
			value: objects.clone(
				getConfigurationValue<C>(
					overrideIdentifier
						? this.cachedConfig.configWithOverrides(overrideIdentifier).contents
						: this.cachedConfig.contents,
					key
				)
			)
		};
	...
	public reloadConfiguration(section?: string): TPromise<any> {
		...
		return this.baseConfigurationService.reloadConfiguration().then(() => {
			...
			return this.doLoadConfiguration().then(configuration => {
				...
				return section
					? configuration.consolidated[section]
	...
	private doLoadConfiguration<T>(): TPromise<IWorkspaceConfiguration<T>> {
		...
		return this.loadWorkspaceConfigFiles().then(workspaceConfigFiles => {
			...
			const workspaceSettingsConfig =
				(<WorkspaceSettingsConfigModel<T>>workspaceConfigFiles[
					WORKSPACE_CONFIG_DEFAULT_PATH
				]) || new WorkspaceSettingsConfigModel<T>(null);
			const otherConfigModels = Object.keys(workspaceConfigFiles)
				.filter(key => key !== WORKSPACE_CONFIG_DEFAULT_PATH)
				.map(key => <ScopedConfigModel<T>>workspaceConfigFiles[key]);
			this.cachedWorkspaceConfig = new WorkspaceConfigModel<T>(
				workspaceSettingsConfig,
				otherConfigModels
			);

			// Override base (global < user) with workspace locals (global < user < workspace)
			this.cachedConfig = <ConfigModel<any>>this.baseConfigurationService
				.getCache()
				.consolidated // global/default values (do NOT modify)
				.merge(this.cachedWorkspaceConfig); // workspace configured values
	...
	private loadWorkspaceConfigFiles<T>(): TPromise<{
		[relativeWorkspacePath: string]: IConfigModel<T>;
	}> {
		...
		if (!this.bulkFetchFromWorkspacePromise) {
			this.bulkFetchFromWorkspacePromise = resolveStat(
				this.contextService.toResource(this.workspaceSettingsRootFolder)
			)
				.then(
					stat => {
						if (!stat.isDirectory) {
						...
						return resolveContents(
							stat.children
								.filter(stat => {
									const isJson =
										paths.extname(stat.resource.fsPath) === '.json';
									if (!isJson) {
										return false; // only JSON files
									}

									return this.isWorkspaceConfigurationFile(
										this.contextService.toWorkspaceRelativePath(stat.resource)
				...
				.then((contents: IContent[]) => {
					contents.forEach(
						content =>
							(this.workspaceFilePathToConfiguration[
								this.contextService.toWorkspaceRelativePath(content.resource)
							] = TPromise.as(this.createConfigModel(content)))
					);
		...
		return this.bulkFetchFromWorkspacePromise.then(() =>
			TPromise.join(this.workspaceFilePathToConfiguration)
		);
	}
	...
	public handleWorkspaceFileEvents(event: FileChangesEvent): void {
		...
		for (let i = 0, len = events.length; i < len; i++) {
			...
			const isDeletedSettingsFolder =
				events[i].type === FileChangeType.DELETED &&
				isEqual(
					paths.basename(resource.fsPath),
					this.workspaceSettingsRootFolder
				);
			if (!isJson && !isDeletedSettingsFolder) {
				continue; // only JSON files or the actual settings folder
			}

			const workspacePath = this.contextService.toWorkspaceRelativePath(
				resource
			);
			if (!workspacePath) {
			...
			if (
				workspacePath === this.workspaceSettingsRootFolder &&
				events[i].type === FileChangeType.DELETED
			) {
				this.workspaceFilePathToConfiguration = Object.create(null);
			...
			switch (events[i].type) {
				case FileChangeType.DELETED:
					affectedByChanges = collections.remove(
						this.workspaceFilePathToConfiguration,
						workspacePath
					);
					break;
				...
				case FileChangeType.ADDED:
					this.workspaceFilePathToConfiguration[workspacePath] = resolveContent(
						resource
					).then(
						content => this.createConfigModel(content),
						errors.onUnexpectedError
					);
					affectedByChanges = true;
			}
	...
	private createConfigModel<T>(content: IContent): IConfigModel<T> {
		...
		if (path === WORKSPACE_CONFIG_DEFAULT_PATH) {
			return new WorkspaceSettingsConfigModel<T>(
				content.value,
			...
			if (matches && matches[1]) {
				return new ScopedConfigModel<T>(
					content.value,
					content.resource.toString(),
					matches[1]
				);
	...
	private isWorkspaceConfigurationFile(workspaceRelativePath: string): boolean {
		return [
			WORKSPACE_CONFIG_DEFAULT_PATH,
			WORKSPACE_STANDALONE_CONFIGURATIONS.launch,
			WORKSPACE_STANDALONE_CONFIGURATIONS.tasks
		].some(p => p === workspaceRelativePath);
	}
...
function resolveContent(resource: uri): TPromise<IContent> {
	return readFile(resource.fsPath).then(contents => ({
		resource,
			...
			if (error) {
				...
				c({
					...
					children: children.map(child => {
						return { resource: uri.file(paths.join(resource.fsPath, child)) };
					})
				});
			}
		});
