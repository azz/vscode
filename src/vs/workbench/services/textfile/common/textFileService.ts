...
export abstract class TextFileService implements ITextFileService {
	...
	constructor(
		...
		this._models = this.instantiationService.createInstance(
		...
		const configuration = this.configurationService.getConfiguration<
			IFilesConfiguration
		>();
		this.currentFilesAssociationConfig =
			configuration && configuration.files && configuration.files.associations;

		this.onConfigurationChange(configuration);

		this.telemetryService.publicLog(
			'autoSave',
			this.getAutoSaveConfiguration()
		);
	...
	private registerListeners(): void {
		...
		this.lifecycleService.onWillShutdown(event =>
			event.veto(this.beforeShutdown(event.reason))
		);
		...
		this.toUnbind.push(
			this.configurationService.onDidUpdateConfiguration(e =>
				this.onConfigurationChange(e.config)
			)
		);
	}
	...
	private beforeShutdown(reason: ShutdownReason): boolean | TPromise<boolean> {
		...
		if (dirty.length) {
			...
			if (this.getAutoSaveMode() !== AutoSaveMode.OFF) {
				handleAutoSave = this.saveAll(false /* files only */).then(() =>
					this.getDirty()
				);
			} else {
			...
			return handleAutoSave.then(dirty => {
				...
				if (dirty.length) {
					...
					if (this.isHotExitEnabled) {
						return this.backupBeforeShutdown(dirty, this.models, reason).then(
							result => {
								if (result.didBackup) {
	...
	private backupBeforeShutdown(
		dirtyToBackup: URI[],
		textFileEditorModelManager: ITextFileEditorModelManager,
		reason: ShutdownReason
	): TPromise<IBackupResult> {
		return this.windowsService.getWindowCount().then(windowCount => {
			...
			// Telemetry
			this.telemetryService.publicLog('hotExit:triggered', {
				reason,
				windowCount,
				fileCount: dirtyToBackup.length
			});
	...
	private doBackupAll(
		dirtyFileModels: ITextFileEditorModel[],
		untitledResources: URI[]
	): TPromise<void> {
		// Handle file resources first
		return TPromise.join(
			dirtyFileModels.map(model =>
				this.backupFileService.backupResource(
					model.getResource(),
					model.getValue(),
					model.getVersionId()
				)
			...
			const untitledModelPromises = untitledResources
				...
				.map(untitled =>
					this.untitledEditorService.loadOrCreate({ resource: untitled })
				);
	...
	protected onConfigurationChange(configuration: IFilesConfiguration): void {
		...
		const autoSaveMode =
			(configuration && configuration.files && configuration.files.autoSave) ||
		...
		// Hot exit
		const hotExitMode = configuration && configuration.files
			? configuration.files.hotExit
	...
	public isDirty(resource?: URI): boolean {
		...
		return this.untitledEditorService
			.getDirty()
			.some(dirty => !resource || dirty.toString() === resource.toString());
	}
	...
	public save(resource: URI, options?: ISaveOptions): TPromise<boolean> {
		...
		if (
			...
			!this.isDirty(resource)
		) {
			const model = this._models.get(resource);
			if (model) {
				model
					.save({ force: true, reason: SaveReason.EXPLICIT })
					.then(() => !model.isDirty());
			}
		}

		return this.saveAll([resource]).then(
			result => result.results.length === 1 && result.results[0].success
		);
	}
	...
	public saveAll(
		...
		toSave.forEach(s => {
			if (s.scheme === Schemas.file) {
				...
				(Array.isArray(arg1) || arg1 === true) /* includeUntitled */ &&
				s.scheme === UNTITLED_SCHEMA
			) {
	...
	private doSaveAll(
		...
		return this.doSaveAllFiles(fileResources, reason).then(result => {
			...
			targetsForUntitled.forEach((target, index) => {
				const untitledSaveAsPromise = this.saveAs(
					untitledResources[index],
					target
				).then(uri => {
					result.results.push({
	...
	private doSaveAllFiles(
		arg1?: any /* URI[] */,
		reason?: SaveReason
	): TPromise<ITextFileOperationResult> {
		const dirtyFileModels = this.getDirtyFileModels(
			Array.isArray(arg1) ? arg1 : void 0 /* Save All */
		).filter(model => {
			if (
				model.hasState(ModelState.CONFLICT) &&
	...
	private doSaveAs(resource: URI, target?: URI): TPromise<URI> {
		...
		let modelPromise: TPromise<
			ITextFileEditorModel | UntitledEditorModel
		> = TPromise.as(null);
		if (resource.scheme === Schemas.file) {
			modelPromise = TPromise.as(this._models.get(resource));
		} else if (
			resource.scheme === UNTITLED_SCHEMA &&
			this.untitledEditorService.exists(resource)
		) {
	...
	private doSaveTextFileAs(
		sourceModel: ITextFileEditorModel | UntitledEditorModel,
		resource: URI,
		target: URI
	): TPromise<void> {
		...
		if (targetModel && targetModel.isResolved()) {
			...
			targetModelResolver = this.fileService
				.resolveFile(target)
				.then(stat => stat, () => null)
				.then(stat => stat || this.fileService.updateContent(target, ''))
				.then(stat => {
					return this.models.loadOrCreate(target);
				});
		}

		return targetModelResolver.then(
			targetModel => {
			...
			error => {
				...
				if (
					...
					return this.fileService
						.del(target)
						.then(() => this.doSaveTextFileAs(sourceModel, resource, target));
				}
		...
		if (workspace) {
			return URI.file(
				paths.join(
					...
					this.untitledEditorService.suggestFileName(untitledResource)
				)
			).fsPath;
		}
	...
	public revert(resource: URI, options?: IRevertOptions): TPromise<boolean> {
		return this.revertAll([resource], options).then(
			result => result.results.length === 1 && result.results[0].success
		);
	}
	...
	public revertAll(
		...
		return this.doRevertAllFiles(resources, options).then(operation => {
			...
			reverted.forEach(res =>
				operation.results.push({ source: res, success: true })
			);
	...
	private doRevertAllFiles(
		resources?: URI[],
		options?: IRevertOptions
	): TPromise<ITextFileOperationResult> {
		const fileModels = options && options.force
			? this.getFileModels(resources)
			: this.getDirtyFileModels(resources);
	...
	public getAutoSaveMode(): AutoSaveMode {
		...
		if (this.configuredAutoSaveDelay && this.configuredAutoSaveDelay > 0) {
			return this.configuredAutoSaveDelay <= 1000
	...
	public getAutoSaveConfiguration(): IAutoSaveConfiguration {
		return {
			autoSaveDelay: this.configuredAutoSaveDelay &&
				this.configuredAutoSaveDelay > 0
				? this.configuredAutoSaveDelay
				: void 0,
