...
export class TextFileEditorModelManager implements ITextFileEditorModelManager {
	...
	private mapResourceToPendingModelLoaders: ResourceMap<
		TPromise<ITextFileEditorModel>
	>;
	...
	constructor(
		...
		this.mapResourceToModelContentChangeListener = new ResourceMap<
			IDisposable
		>();
		this.mapResourceToPendingModelLoaders = new ResourceMap<
			TPromise<ITextFileEditorModel>
		>();
	...
	private debounce(
		event: Event<TextFileModelChangeEvent>
	): Event<TextFileModelChangeEvent[]> {
		return debounceEvent(
			event,
			(prev: TextFileModelChangeEvent[], cur: TextFileModelChangeEvent) => {
				if (!prev) {
					prev = [cur];
				} else {
					prev.push(cur);
	...
	public loadOrCreate(
		...
		if (model) {
			...
			model = this.instantiationService.createInstance(
				TextFileEditorModel,
				resource,
			...
			this.mapResourceToStateChangeListener.set(
				resource,
				model.onDidStateChange(state => {
			...
			this.mapResourceToModelContentChangeListener.set(
				resource,
				model.onDidContentChange(e => {
					this._onModelContentChanged.fire(
						new TextFileModelChangeEvent(model, e)
					);
				})
			);
		}
		...
		return modelPromise.then(
			model => {
				...
				if (model.isDirty()) {
					this._onModelDirty.fire(
						new TextFileModelChangeEvent(model, StateChange.DIRTY)
					);
				}
	...
	public getAll(
		resource?: URI,
		filter?: (model: ITextFileEditorModel) => boolean
	): ITextFileEditorModel[] {
		if (resource) {
	...
	public remove(resource: URI): void {
		...
		const stateChangeListener = this.mapResourceToStateChangeListener.get(
			resource
		);
		...
		const modelContentChangeListener = this.mapResourceToModelContentChangeListener.get(
			resource
		);
