...
suite('Files - TextFileEditorModelManager', () => {
	...
	test('add, remove, clear, get, getAll', function() {
		const manager: TestTextFileEditorModelManager = instantiationService.createInstance(
			TestTextFileEditorModelManager
		);

		const model1: TextFileEditorModel = instantiationService.createInstance(
			TextFileEditorModel,
			toResource('/path/random1.txt'),
			'utf8'
		);
		const model2: TextFileEditorModel = instantiationService.createInstance(
			TextFileEditorModel,
			toResource('/path/random2.txt'),
			'utf8'
		);
		const model3: TextFileEditorModel = instantiationService.createInstance(
			TextFileEditorModel,
			toResource('/path/random3.txt'),
			'utf8'
		);
	...
	test('loadOrCreate', function(done) {
		...
		manager.loadOrCreate(resource, { encoding, reload: true }).done(
			model => {
				assert.ok(model);
	...
	test('events', function(done) {
		...
		manager.loadOrCreate(resource1, { encoding: 'utf8' }).done(
			model1 => {
				...
				return manager
					.loadOrCreate(resource2, { encoding: 'utf8' })
					.then(model2 => {
	...
	test('disposing model takes it out of the manager', function(done) {
		...
		manager.loadOrCreate(resource, { encoding: 'utf8' }).done(
			model => {
				model.dispose();
