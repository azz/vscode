...
class ServiceAccessor {
	constructor(
		...
		public untitledEditorService: IUntitledEditorService,
...
suite('Files - TextFileService', () => {
	...
	test('confirm onWillShutdown - no veto and backups cleaned up if user does not want to save', function(
		done
	) {
		model = instantiationService.createInstance(
			TextFileEditorModel,
	...
	test('isDirty/getDirty - files and untitled', function(done) {
		...
		model.load().done(
			() => {
				...
				assert.equal(service.getDirty().length, 1);
				assert.equal(
					service.getDirty([model.getResource()])[0].toString(),
					model.getResource().toString()
				);
				...
				return untitled.resolve().then((model: UntitledEditorModel) => {
					...
					assert.equal(
						service.getDirty([untitled.getResource()])[0].toString(),
