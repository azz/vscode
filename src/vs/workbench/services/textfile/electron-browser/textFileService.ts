...
export class TextFileService extends AbstractTextFileService {
	...
	public confirmSave(resources?: URI[]): ConfirmResult {
		...
		const message = [
			resourcesToConfirm.length === 1
				? nls.localize(
						'saveChangesMessage',
						'Do you want to save the changes you made to {0}?',
						paths.basename(resourcesToConfirm[0].fsPath)
					)
				: nls.localize(
						'saveChangesMessages',
						'Do you want to save the changes to the following {0} files?',
						resourcesToConfirm.length
					)
		];
		...
		if (resourcesToConfirm.length > 1) {
			...
			message.push(
				...resourcesToConfirm
					.slice(0, TextFileService.MAX_CONFIRM_FILES)
					.map(r => paths.basename(r.fsPath))
			);

			if (resourcesToConfirm.length > TextFileService.MAX_CONFIRM_FILES) {
				if (
					resourcesToConfirm.length - TextFileService.MAX_CONFIRM_FILES ===
					1
				) {
					message.push(
						nls.localize('moreFile', '...1 additional file not shown')
					);
				} else {
					message.push(
						nls.localize(
		...
		const save = {
			label: resourcesToConfirm.length > 1
				? this.mnemonicLabel(
						nls.localize(
		...
		return this.windowService
			...
			.showSaveDialog(
				this.getSaveDialogOptions(
					defaultPath ? paths.normalize(defaultPath, true) : void 0
				)
			);
	}

	private getSaveDialogOptions(
		defaultPath?: string
		...
		const filters: IFilter[] = this.modeService
			...
			.map(languageName => {
				...
				const filter: IFilter = {
					name: languageName,
					extensions: extensions.slice(0, 10).map(e => strings.trim(e, '.'))
				};
		...
		const allFilesFilter = {
			name: nls.localize('allFiles', 'All Files'),
			extensions: ['*']
		};
		if (matchingFilter) {
		...
		filters.push({
			name: nls.localize('noExt', 'No Extension'),
			extensions: ['']
		});
