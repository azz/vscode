...
suite('Search', () => {
	...
	test('Files: *.* exclude folder without wildcard', function(
		done: () => void
	) {
		let engine = new FileSearchEngine({
			rootFolders: rootfolders(),
	...
	test('Files: *.* exclude folder with leading wildcard', function(
		done: () => void
	) {
		let engine = new FileSearchEngine({
			rootFolders: rootfolders(),
	...
	test('Files: *.* exclude folder with trailing wildcard', function(
		done: () => void
	) {
		let engine = new FileSearchEngine({
			rootFolders: rootfolders(),
	...
	test('Files: absolute path to file ignores excludes', function(
		done: () => void
	) {
		let engine = new FileSearchEngine({
			rootFolders: rootfolders(),
			filePattern: path.normalize(
				path.join(require.toUrl('./fixtures'), 'site.css')
			),
	...
	test('Files: relative path to file ignores excludes', function(
		done: () => void
	) {
		let engine = new FileSearchEngine({
			rootFolders: rootfolders(),
	...
	test('Files: extraFiles only', function(done: () => void) {
		let engine = new FileSearchEngine({
			...
			extraFiles: [
				...
				path.normalize(
					path.join(require.toUrl('./fixtures'), 'examples', 'company.js')
				),
	...
	test('Find: exclude subfolder', function(done: () => void) {
		...
		const cmd1 = walker.spawnFindCmd(
			rootfolders()[0],
			glob.parse({ '**/something': true })
		);
		walker.readStdout(cmd1, 'utf8', (err1, stdout1) => {
			...
			const cmd2 = walker.spawnFindCmd(
				rootfolders()[0],
				glob.parse({ '**/subfolder': true })
			);
	...
	test('Find: exclude folder path suffix', function(done: () => void) {
		...
		const cmd1 = walker.spawnFindCmd(
			rootfolders()[0],
			glob.parse({ '**/examples/something': true })
		);
		walker.readStdout(cmd1, 'utf8', (err1, stdout1) => {
			assert.equal(err1, null);
			assert.notStrictEqual(stdout1.split('\n').indexOf(file0), -1, stdout1);
			...
			const cmd2 = walker.spawnFindCmd(
				rootfolders()[0],
				glob.parse({ '**/examples/subfolder': true })
			);
			walker.readStdout(cmd2, 'utf8', (err2, stdout2) => {
				assert.equal(err2, null);
				assert.notStrictEqual(stdout1.split('\n').indexOf(file0), -1, stdout1);
	...
	test('Find: exclude subfolder path suffix', function(done: () => void) {
		...
		const cmd1 = walker.spawnFindCmd(
			rootfolders()[0],
			glob.parse({ '**/subfolder/something': true })
		);
		walker.readStdout(cmd1, 'utf8', (err1, stdout1) => {
			assert.equal(err1, null);
			assert.notStrictEqual(stdout1.split('\n').indexOf(file0), -1, stdout1);
			...
			const cmd2 = walker.spawnFindCmd(
				rootfolders()[0],
				glob.parse({ '**/subfolder/anotherfolder': true })
			);
			walker.readStdout(cmd2, 'utf8', (err2, stdout2) => {
				assert.equal(err2, null);
				assert.notStrictEqual(stdout1.split('\n').indexOf(file0), -1, stdout1);
