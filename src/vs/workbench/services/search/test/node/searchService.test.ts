...
suite('SearchService', () => {
	...
	test('Individual results', function() {
		...
		let results = 0;
		return service.doFileSearch(Engine, rawSearch).then(() => {
	...
	test('Batch results', function() {
		...
		const results = [];
		return service.doFileSearch(Engine, rawSearch, 10).then(() => {
	...
	test('Collect batched results', function() {
		...
		return DiskSearch.collectResults(
			service.doFileSearch(Engine, rawSearch, 10)
		).then(
			result => {
			...
			.then(
				() => {
					...
					assert.deepStrictEqual(results, [
						normalize('/some/where/bbc'),
						normalize('/some/where/bab')
					]);
				},
				null,
				value => {
		...
		return service
			...
			.then(
				complete => {
					...
					assert.deepStrictEqual(results, [
						normalize('/some/where/bcb'),
						normalize('/some/where/bbc'),
						normalize('/some/where/aab')
					]);
				},
				null,
				value => {
