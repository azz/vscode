...
export class Engine implements ISearchEngine<ISerializedFileMatch[]> {
	...
	constructor(
		config: IRawSearch,
		walker: FileWalker,
		workerProvider: ITextSearchWorkerProvider
	) {
		this.config = config;
		this.walker = walker;
	...
	cancel(): void {
		...
		this.workers.forEach(w => {
			w.cancel().then(null, onUnexpectedError);
		});
	}

	initializeWorkers(): void {
		this.workers.forEach(w => {
			w.initialize().then(null, onUnexpectedError);
		});
	}
	...
	search(
		...
		const run = (batch: string[], batchBytes: number): void => {
			...
			const maxResults =
				this.config.maxResults && this.config.maxResults - this.numResults;
			const searchArgs = {
				absolutePaths: batch,
				maxResults,
				pattern: this.config.contentPattern,
				fileEncoding: this.config.fileEncoding
			};
			worker.search(searchArgs).then(
				result => {
					...
					if (
						...
						this.numResults >= this.config.maxResults
					) {
		...
		this.walker.walk(
			...
			result => {
				...
				const absolutePath = result.base
					? [result.base, result.relativePath].join(path.sep)
					: result.relativePath;
