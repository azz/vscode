...
export class FileWalker {
	...
	constructor(config: IRawSearch) {
		...
		if (this.filePattern) {
			this.normalizedFilePatternLowercase = strings
				.stripWildcards(this.filePattern)
				.toLowerCase();
		}
	}
	...
	public walk(
		...
		done: (error: Error, isLimitHit: boolean) => void
	): void {
		...
		this.checkFilePatternAbsoluteMatch((exists, size) => {
			...
			if (extraFiles) {
				extraFiles.forEach(extraFilePath => {
					...
					this.matchFile(onResult, {
						relativePath: extraFilePath /* no workspace relative path */,
						basename
					});
				});
			}
			...
			flow.parallel<string, void>(
				rootFolders,
				(rootFolder: string, rootFolderDone: (err?: Error) => void) => {
	...
	private findTraversal(
		...
		this.collectStdout(
			cmd,
			'utf8',
			(err: Error, stdout?: string, last?: boolean) => {
				if (err) {
					done(err);
					return;
				}
				...
				const normalized =
					leftover + (isMac ? strings.normalizeNFC(stdout) : stdout);
	...
	public spawnFindCmd(
		rootFolder: string,
		excludePattern: glob.ParsedExpression
	) {
	...
	private collectStdout(
		cmd: childProcess.ChildProcess,
		encoding: string,
		cb: (err: Error, stdout?: string, last?: boolean) => void
	): void {
		...
		cmd.on('close', (code: number) => {
			if (code !== 0) {
					...
					new Error(
						`find failed with error code ${code}: ${this.decodeData(
							stderr,
							encoding
						)}`
					)
	...
	private checkFilePatternAbsoluteMatch(
		clb: (exists: boolean, size?: number) => void
	): void {
	...
	private checkFilePatternRelativeMatch(
		basePath: string,
		clb: (matchPath: string, size?: number) => void
	): void {
		if (!this.filePattern || paths.isAbsolute(this.filePattern)) {
			return clb(null);
		}
		...
		return fs.stat(absolutePath, (error, stat) => {
			return clb(
				!error && !stat.isDirectory() ? absolutePath : null,
				stat && stat.size
			); // only existing files
		});
	}
	...
	private doWalk(
		...
		flow.parallel(
			files,
			(file: string, clb: (error: Error) => void): void => {
				// Check canceled
				if (this.isCanceled || this.isLimitHit) {
					return clb(null);
				}
				...
				// Check exclude pattern
				let currentRelativePath = relativeParentPath
					? [relativeParentPath, file].join(paths.sep)
					: file;
				...
				let currentAbsolutePath = [rootFolder, currentRelativePath].join(
					paths.sep
				);
						...
						if (stat.isDirectory()) {
							...
							return this.realPathIfNeeded(
								...
								(error, realpath) => {
									...
									// Continue walking
									return extfs.readdir(
										currentAbsolutePath,
										(error: Error, children: string[]): void => {
											if (error || this.isCanceled || this.isLimitHit) {
												return clb(null);
											}
	...
	private matchFile(
		...
		if (
			this.isFilePatternMatch(candidate.relativePath) &&
			(!this.includePattern ||
	...
	private statLinkIfNeeded(
		path: string,
		lstat: fs.Stats,
		clb: (error: Error, stat: fs.Stats) => void
	): void {
	...
	private realPathIfNeeded(
		path: string,
		lstat: fs.Stats,
		clb: (error: Error, realpath?: string) => void
	): void {
		if (lstat.isSymbolicLink()) {
