...
import {
	languagesExtPoint,
	ModeServiceImpl
} from 'vs/editor/common/services/modeServiceImpl';
...
export class WorkbenchModeServiceImpl extends ModeServiceImpl {
	...
	constructor(
		...
		languagesExtPoint.setHandler(
			(extensions: IExtensionPointUser<ILanguageExtensionPoint[]>[]) => {
		...
		this._configurationService.onDidUpdateConfiguration(e =>
			this.onConfigurationChange(e.config)
		);

		this.onDidCreateMode(mode => {
			this._extensionService
				.activateByEvent(`onLanguage:${mode.getId()}`)
