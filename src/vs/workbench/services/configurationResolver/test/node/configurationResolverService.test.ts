...
suite('Configuration Resolver Service', () => {
	...
	let envVariables: { [key: string]: string } = {
		key1: 'Value for Key1',
		key2: 'Value for Key2'
	};
	...
	test('substitute one', () => {
		if (platform.isWindows) {
			assert.strictEqual(
				configurationResolverService.resolve('abc ${workspaceRoot} xyz'),
	...
	test('workspace root folder name', () => {
		assert.strictEqual(
			configurationResolverService.resolve(
	...
	test('current selected line number', () => {
		assert.strictEqual(
			configurationResolverService.resolve('abc ${lineNumber} xyz'),
			`abc ${editorService.mockLineNumber} xyz`
		);
	});
	...
	test('substitute one env variable', () => {
		if (platform.isWindows) {
			assert.strictEqual(
				configurationResolverService.resolve(
					'abc ${workspaceRoot} ${env:key1} xyz'
				),
	...
	test('substitute many env variable', () => {
		if (platform.isWindows) {
			assert.strictEqual(
				configurationResolverService.resolve(
					'${workspaceRoot} - ${workspaceRoot} ${env:key1} - ${env:key2}'
				),
	...
	test('substitute one configuration variable', () => {
		...
		assert.strictEqual(
			service.resolve('abc ${config:editor.fontFamily} xyz'),
			'abc foo xyz'
		);
	});
	...
	test('substitute nested configuration variables', () => {
		...
		configurationService = new MockConfigurationService({
			editor: {
				fontFamily:
					'foo ${workspaceRoot} ${config:terminal.integrated.fontFamily}'
			},
			terminal: {
				integrated: {
					fontFamily: 'bar'
		...
		if (platform.isWindows) {
			assert.strictEqual(
				...
				'abc foo \\VSCode\\workspaceLocation bar bar xyz'
			);
		} else {
			assert.strictEqual(
				service.resolve(
	...
	test('substitute accidental self referenced configuration variables', () => {
		...
		configurationService = new MockConfigurationService({
			editor: {
				fontFamily:
					'foo ${workspaceRoot} ${config:terminal.integrated.fontFamily} ${config:editor.fontFamily}'
			},
			terminal: {
				integrated: {
					fontFamily: 'bar'
	...
	test('substitute one env variable and a configuration variable', () => {
		...
		if (platform.isWindows) {
			assert.strictEqual(
				service.resolve(
					'abc ${config:editor.fontFamily} ${workspaceRoot} ${env:key1} xyz'
				),
	...
	test('substitute many env variable and a configuration variable', () => {
		...
		if (platform.isWindows) {
			assert.strictEqual(
				service.resolve(
					'${config:editor.fontFamily} ${config:terminal.integrated.fontFamily} ${workspaceRoot} - ${workspaceRoot} ${env:key1} - ${env:key2}'
				),
	...
	test('configuration should not evaluate Javascript', () => {
		...
		assert.strictEqual(
			service.resolve("abc ${config:editor['abc'.substr(0)]} xyz"),
			'abc  xyz'
		);
	});
	...
	test('uses empty string as fallback', () => {
		...
		assert.strictEqual(
			service.resolve('abc ${config:editor.abc} xyz'),
			'abc  xyz'
		);
		assert.strictEqual(
			service.resolve('abc ${config:editor.abc.def} xyz'),
			'abc  xyz'
		);
		assert.strictEqual(service.resolve('abc ${config:panel} xyz'), 'abc  xyz');
		assert.strictEqual(
			service.resolve('abc ${config:panel.abc} xyz'),
			'abc  xyz'
		);
	});
	...
	test('is restricted to own properties', () => {
		...
		assert.strictEqual(
			service.resolve('abc ${config:editor.__proto__} xyz'),
			'abc  xyz'
		);
		assert.strictEqual(
			service.resolve('abc ${config:editor.toString} xyz'),
			'abc  xyz'
		);
	});
	...
	test('configuration variables with invalid accessor', () => {
		...
		assert.strictEqual(
			service.resolve('abc ${config:} xyz'),
			'abc ${config:} xyz'
		);
		assert.strictEqual(
			service.resolve('abc ${config:editor..fontFamily} xyz'),
		...
		assert.strictEqual(
			service.resolve('abc ${config:editor.none.none2} xyz'),
			'abc  xyz'
		);
	});
	...
	test('interactive variable simple', () => {
		...
		configurationResolverService
			.resolveInteractiveVariables(configuration, interactiveVariables)
			.then(resolved => {
