...
export class BackupFilesModel implements IBackupFilesModel {
	private cache: {
		[resource: string]: number /* version ID */;
	} = Object.create(null);

	public resolve(backupRoot: string): TPromise<IBackupFilesModel> {
		return pfs
			.readDirsInDir(backupRoot)
			.then(backupSchemas => {
				// For all supported schemas
				return TPromise.join(
					backupSchemas.map(backupSchema => {
						...
						return pfs.readdir(backupSchemaPath).then(backupHashes => {
							...
							backupHashes.forEach(backupHash => {
								const backupResource = Uri.file(
									path.join(backupSchemaPath, backupHash)
								);
...
export class BackupFileService implements IBackupFileService {
	...
	public loadBackupResource(resource: Uri): TPromise<Uri> {
		return this.ready.then(model => {
			...
			if (platform.isWindows || platform.isMacintosh) {
				const legacyBackupResource = this.getBackupResource(
					resource,
					true /* legacyMacWindowsFormat */
				);
		...
		return this.ready.then(model => {
			...
			return this.getResourceIOQueue(backupResource).queue(() => {
				return this.fileService
					.updateContent(backupResource, content, BACKUP_FILE_UPDATE_OPTIONS)
					.then(() => model.add(backupResource, versionId));
			});
		});
	}
	...
	public discardResourceBackup(resource: Uri): TPromise<void> {
		return this.ready.then(model => {
			...
			return this.getResourceIOQueue(backupResource)
				.queue(() => {
					return pfs
						.del(backupResource.fsPath)
						.then(() => model.remove(backupResource));
				})
				.then(() => {
					...
					if (platform.isWindows || platform.isMacintosh) {
						...
						if (model.has(legacyBackupResource)) {
							return this.getResourceIOQueue(legacyBackupResource).queue(() => {
								return pfs
									.del(legacyBackupResource.fsPath)
									.then(() => model.remove(legacyBackupResource));
							});
						}
					}
	...
	protected getBackupResource(
		...
		return Uri.file(
			path.join(
				...
				resource.scheme,
				this.hashPath(resource, legacyMacWindowsFormat)
			)
		);
	}

	private hashPath(resource: Uri, legacyMacWindowsFormat?: boolean): string {
		const caseAwarePath = legacyMacWindowsFormat
			? resource.fsPath.toLowerCase()
			: resource.fsPath;
