...
suite('Telemetry - WorkspaceStats', () => {
	...
	test('SCP-like remotes', function() {
		...
		assert.deepStrictEqual(
			getDomainsOfRemotes(remote('user@git.server.org:project.git'), whitelist),
			['server.org']
		);
		assert.deepStrictEqual(
			getDomainsOfRemotes(remote('git.server2.org:project.git'), whitelist),
			['server2.org']
		);
	});
	...
	test('Local remotes', function() {
		assert.deepStrictEqual(
			getDomainsOfRemotes(remote('/opt/git/project.git'), whitelist),
			[]
		);
		assert.deepStrictEqual(
			getDomainsOfRemotes(remote('file:///opt/git/project.git'), whitelist),
			[]
		);
	});
	...
	test('Multiple remotes', function() {
		const config = [
			...
			.map(remote)
			.join('');
		assert.deepStrictEqual(getDomainsOfRemotes(config, whitelist).sort(), [
			'example.com',
			'github.com'
		]);
	});
