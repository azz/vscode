...
import {
	...
	normalize,
	IRawFileChange
} from 'vs/workbench/services/files/node/watcher/common';
...
function etag(arg1: any, arg2?: any): string {
	...
	return `"${crypto
		.createHash('sha1')
		.update(String(size) + String(mtime))
		.digest('hex')}"`;
}
...
export class FileService implements IFileService {
	...
	constructor(basePath: string, options: IFileServiceOptions) {
		...
		if (
			this.basePath &&
			this.basePath.indexOf('\\\\') === 0 &&
			strings.endsWith(this.basePath, paths.sep)
		) {
		...
		this.fileChangesWatchDelayer = new ThrottledDelayer<void>(
			FileService.FS_EVENT_DELAY
		);
	...
	private setupWin32WorkspaceWatching(): void {
		this.toDispose.push(
			toDisposable(
				new WindowsWatcherService(
					this.basePath,
					this.options.watcherIgnoredPatterns,
					e => this._onFileChanges.fire(e),
					this.options.errorLogger,
					this.options.verboseLogging
				).startWatching()
			)
		);
	}
	...
	public resolveContent(
		resource: uri,
		options?: IResolveContentOptions
	): TPromise<IContent> {
		return this.doResolveContent(resource, options, (stat, enc) =>
			this.resolveFileContent(stat, enc)
		);
	}

	public resolveStreamContent(
		resource: uri,
		options?: IResolveContentOptions
	): TPromise<IStreamContent> {
		return this.doResolveContent(resource, options, (stat, enc) =>
			this.resolveFileStreamContent(stat, enc)
		);
	}

	private doResolveContent<IStreamContent>(
		resource: uri,
		options: IResolveContentOptions,
		contentResolver: (stat: IFileStat, enc?: string) => TPromise<IStreamContent>
	): TPromise<IStreamContent> {
		const absolutePath = this.toAbsolutePath(resource);

		// Guard early against attempts to resolve an invalid file path
		if (resource.scheme !== 'file' || !resource.fsPath) {
			return TPromise.wrapError<IStreamContent>(
				<IFileOperationResult>{
		...
		// 1.) resolve resource
		return this.resolve(resource).then(
			(model): TPromise<IStreamContent> => {
				...
				const autoGuessEncoding =
					...
					(this.options && this.options.autoGuessEncoding);
				return detectMimesFromFile(absolutePath, {
					autoGuessEncoding
				}).then((detected: IMimeAndEncoding) => {
			...
			contentPromises.push(
				limiter.queue(() =>
					this.resolve(resource)
						.then(model => this.resolveFileContent(model))
						.then(
							content => content,
							error => TPromise.as(null /* ignore errors gracefully */)
						)
				)
			);
		});
	...
	public updateContent(
		resource: uri,
		value: string,
		options: IUpdateContentOptions = Object.create(null)
	): TPromise<IFileStat> {
		const absolutePath = this.toAbsolutePath(resource);
		...
		return this.checkFile(absolutePath, options).then(exists => {
			...
			return createParentsPromise.then(() => {
				...
				if (
					encodingToWrite === encoding.UTF16be ||
					encodingToWrite === encoding.UTF16le ||
					encodingToWrite === encoding.UTF8_with_bom
				) {
					...
					if (options.overwriteEncoding) {
						...
						addBomPromise = encoding
							.detectEncodingByBOM(absolutePath)
							.then(enc => enc === encoding.UTF8); // otherwise preserve it if found
					}
				}
				...
				return addBomPromise.then(addBom => {
					...
					if (!addBom && encodingToWrite === encoding.UTF8) {
						writeFilePromise = pfs.writeFile(
							absolutePath,
							value,
							encoding.UTF8
						);
						...
						const encoded = encoding.encode(value, encodingToWrite, {
							addBOM: addBom
						});
	...
	public createFile(resource: uri, content: string = ''): TPromise<IFileStat> {
		...
		return this.updateContent(resource, content).then(result => {
			...
			this._onAfterOperation.fire(
				new FileOperationEvent(resource, FileOperation.CREATE, result)
			);
			...
			return this.resolve(target).then(result => {
				...
				this._onAfterOperation.fire(
					new FileOperationEvent(
						source,
						keepCopy ? FileOperation.COPY : FileOperation.MOVE,
						result
					)
				);
	...
	private doMoveOrCopyFile(
		sourcePath: string,
		targetPath: string,
		keepCopy: boolean,
		overwrite: boolean
	): TPromise<boolean> /* exists */ {
		...
		return pfs.exists(targetPath).then(exists => {
			...
			if (exists && !isCaseRename) {
				if (
					isEqualOrParent(sourcePath, targetPath, !isLinux /* ignorecase */)
				) {
					return TPromise.wrapError<boolean>(
						nls.localize(
			...
			return deleteTargetPromise.then(() => {
				...
				return pfs
					.mkdirp(paths.dirname(targetPath))
					.then(() => {
						// 4.) copy/move
						if (isSameFile) {
	...
	public importFile(source: uri, targetFolder: uri): TPromise<IImportResult> {
		const sourcePath = this.toAbsolutePath(source);
		const targetResource = uri.file(
			paths.join(targetFolder.fsPath, paths.basename(source.fsPath))
		);
		...
		return pfs.stat(sourcePath).then(stat => {
			if (stat.isDirectory()) {
				return TPromise.wrapError<IImportResult>(
					nls.localize(
						'foldersCopyError',
						'Folders cannot be copied into the workspace. Please select individual files to copy them.'
					)
				); // for now we do not allow to import a folder into a workspace
			}
			...
			return this.doMoveOrCopyFile(
				...
				return this.resolve(targetResource).then(stat => {
					// Events
					this._onAfterOperation.fire(
						new FileOperationEvent(source, FileOperation.IMPORT, stat)
					);
	...
	private resolve(
		resource: uri,
		options: IResolveFileOptions = Object.create(null)
	): TPromise<IFileStat> {
	...
	private resolveFileStreamContent(
		model: IFileStat,
		enc?: string
	): TPromise<IStreamContent> {
		// Return early if file is too large to load
		...
		const reader = fs
			...
			.pipe(encoding.decodeStream(fileEncoding)); // decode takes care of stripping any BOMs from the file content
	...
	private getEncodingOverride(resource: uri): string {
		if (
			resource &&
			this.options.encodingOverride &&
			this.options.encodingOverride.length
		) {
		...
		return pfs.exists(absolutePath).then(exists => {
			if (exists) {
				return pfs.stat(absolutePath).then(stat => {
					if (stat.isDirectory()) {
						return TPromise.wrapError<boolean>(
							new Error('Expected file is actually a directory')
						);
					}
					...
					if (
						...
						options.mtime < stat.mtime.getTime()
					) {
	...
	public watchFileChanges(resource: uri): void {
		assert.ok(
			resource && resource.scheme === 'file',
			`Invalid resource for watching: ${resource}`
		);
		...
		if (!watcher) {
			...
			watcher.on('change', (eventType: string, filename: string) => {
				const renamedOrDeleted =
					(filename && filename !== fsName) || eventType === 'rename';
		...
		if (this.options.verboseLogging) {
			console.log(
				...
				event.type === FileChangeType.ADDED
					? '[ADDED]'
					: event.type === FileChangeType.DELETED ? '[DELETED]' : '[CHANGED]',
				event.path
			);
		}
		...
		this.fileChangesWatchDelayer.trigger(() => {
			...
			if (this.options.verboseLogging) {
				normalizedEvents.forEach(r => {
					console.log(
						'%c[node.js Watcher]%c >> normalized',
						'color: green',
						'color: black',
						r.type === FileChangeType.ADDED
							? '[ADDED]'
							: r.type === FileChangeType.DELETED ? '[DELETED]' : '[CHANGED]',
						r.path
					);
				});
			}
...
export class StatResolver {
		...
		if (!this.isDirectory) {
			...
			return new TPromise<IFileStat>((c, e) => {
				...
				this.resolveChildren(
					...
					absoluteTargetPaths,
					options && options.resolveSingleChildDescendants,
					children => {
	...
	private resolveChildren(
		...
		callback: (children: IFileStat[]) => void
	): void {
		extfs.readdir(absolutePath, (error: Error, files: string[]) => {
			...
			flow.parallel(
				files,
				(file: string, clb: (error: Error, children: IFileStat) => void) => {
					...
					flow.sequence(
						...
						function resolve(childCount: number): void {
							...
							if (files.length === 1 && resolveSingleChildDescendants) {
								...
								childCount > 0 &&
								absoluteTargetPaths &&
								absoluteTargetPaths.some(targetPath =>
									isEqualOrParent(
										targetPath,
										fileResource.fsPath,
										!isLinux /* ignorecase */
