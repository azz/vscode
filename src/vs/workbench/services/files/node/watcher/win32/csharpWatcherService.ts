...
export class OutOfProcessWin32FolderWatcher {
	...
	private static changeTypeMap: FileChangeType[] = [
	...
	private startWatcher(): void {
		...
		this.handle = cp.spawn(
			uri.parse(
				require.toUrl(
					'vs/workbench/services/files/node/watcher/win32/CodeHelper.exe'
				)
			).fsPath,
			args
		);
			...
			stdoutLineDecoder.write(data).forEach(line => {
				...
				if (eventParts.length === 2) {
					...
					if (changeType >= 0 && changeType < 3) {
						...
						if (
							this.ignored &&
							this.ignored.some(ignore => glob.match(ignore, absolutePath))
						) {
							return;
						}
		...
		// Exit
		this.handle.on('exit', (code: any, signal: any) =>
	...
	private onExit(code: any, signal: any): void {
		if (this.handle) {
			...
			this.errorCallback(
				`[FileWatcher] terminated unexpectedly (code: ${code}, signal: ${signal})`
			);
			...
			if (this.restartCounter <= OutOfProcessWin32FolderWatcher.MAX_RESTARTS) {
				...
				this.errorCallback(
					'[FileWatcher] Watcher failed to start after retrying for some time, giving up. Please report this as a bug report!'
				);
			}
		}
	}
