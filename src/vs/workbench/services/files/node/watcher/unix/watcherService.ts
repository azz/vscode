...
} from 'vs/workbench/services/files/node/watcher/unix/watcherIpc';
...
export class FileWatcher {
	...
	public startWatching(): () => void {
		...
		const channel = getNextTickChannel(
			client.getChannel<IWatcherChannel>('watcher')
		);
			...
			.done(() => {
				...
				if (!this.isDisposed) {
					if (this.restartCounter <= FileWatcher.MAX_RESTARTS) {
						this.errorLogger(
							'[FileWatcher] terminated unexpectedly and is restarted again...'
						);
						this.restartCounter++;
						this.startWatching();
					} else {
