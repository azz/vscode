...
suite('Stat Resolver', () => {
		...
		resolver
			...
			.then(result => {
				...
				result.children.forEach(value => {
					...
					if (
						['examples', 'other'].indexOf(
							path.basename(value.resource.fsPath)
						) >= 0
					) {
						...
						assert.ok(
							!'Unexpected value ' + path.basename(value.resource.fsPath)
						);
					}
				});
			})
	...
	test('resolve directory - resolveTo single directory', function(
		...
		resolver
			.resolve({ resolveTo: [toResource('other/deep')] })
			.then(result => {
				assert.ok(result);
				assert.ok(result.children);
				assert.ok(result.hasChildren);
	...
	test('resolve directory - resolveTo multiple directories', function(
		...
		resolver
			.resolve({
				resolveTo: [toResource('other/deep'), toResource('examples')]
			})
			.then(result => {
				assert.ok(result);
				assert.ok(result.children);
	...
	test('resolve directory - resolveSingleChildFolders', function(
		...
		resolver
			.resolve({ resolveSingleChildDescendants: true })
			.then(result => {
				assert.ok(result);
				assert.ok(result.children);
				assert.ok(result.hasChildren);
