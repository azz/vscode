...
suite('Watcher', () => {
	...
	pathSpecs.forEach(p => {
		test(
			'watching - delete only reported for top level folder (' + p + ')',
			function(done: () => void) {
				const watch = new TestFileWatcher();
	...
	test('watching - event normalization: ignore CREATE followed by DELETE', function(
		done: () => void
	) {
		const watch = new TestFileWatcher();
	...
	test('watching - event normalization: flatten DELETE followed by CREATE into CHANGE', function(
		done: () => void
	) {
		const watch = new TestFileWatcher();
	...
	test('watching - event normalization: ignore UPDATE when CREATE received', function(
		done: () => void
	) {
		const watch = new TestFileWatcher();
	...
	test('watching - event normalization: apply DELETE', function(
		done: () => void
	) {
		const watch = new TestFileWatcher();
