...
import {
	FileService,
	IEncodingOverride
} from 'vs/workbench/services/files/node/fileService';
import {
	FileOperation,
	FileOperationEvent,
	FileChangesEvent,
...
suite('FileService', () => {
	...
	test('createFile', function(done: () => void) {
		...
		service.createFile(resource, contents).done(
			s => {
	...
	test('createFolder', function(done: () => void) {
		...
		service.resolveFile(uri.file(testDir)).done(
			parent => {
				const resource = uri.file(
					path.join(parent.resource.fsPath, 'newFolder')
				);
	...
	test('touchFile', function(done: () => void) {
		service.touchFile(uri.file(path.join(testDir, 'test.txt'))).done(
			s => {
				assert.equal(s.name, 'test.txt');
				assert.equal(fs.existsSync(s.resource.fsPath), true);
	...
	test('renameFile', function(done: () => void) {
		...
		service.resolveFile(resource).done(
			source => {
	...
	test('moveFile', function(done: () => void) {
		...
		service.resolveFile(resource).done(
			source => {
				return service
					.moveFile(source.resource, uri.file(path.join(testDir, 'other.html')))
					.then(renamed => {
						assert.equal(fs.existsSync(renamed.resource.fsPath), true);
						assert.equal(fs.existsSync(source.resource.fsPath), false);
	...
	test('move - FILE_MOVE_CONFLICT', function(done: () => void) {
		...
		service.resolveFile(uri.file(path.join(testDir, 'index.html'))).done(
			source => {
				return service
					.moveFile(source.resource, uri.file(path.join(testDir, 'binary.txt')))
					.then(null, (e: IFileOperationResult) => {
						assert.equal(
	...
	test('moveFile - MIX CASE', function(done: () => void) {
		...
		service.resolveFile(resource).done(
			source => {
				return service
					.moveFile(source.resource, uri.file(path.join(testDir, 'INDEX.html')))
					.then(renamed => {
						assert.equal(fs.existsSync(renamed.resource.fsPath), true);
						assert.equal(path.basename(renamed.resource.fsPath), 'INDEX.html');
		...
		service.resolveFile(uri.file(testDir)).done(
			parent => {
				const folderResource = uri.file(
					path.join(parent.resource.fsPath, 'conway.js')
				);
	...
	test('importFile', function(done: () => void) {
		...
		service.resolveFile(uri.file(path.join(testDir, 'deep'))).done(
			target => {
				const resource = uri.file(
					require.toUrl('./fixtures/service/index.html')
				);
	...
	test('importFile - MIX CASE', function(done: () => void) {
		service.resolveFile(uri.file(path.join(testDir, 'index.html'))).done(
			source => {
				return service.rename(source.resource, 'CONWAY.js').then(renamed => {
					// index.html => CONWAY.js
					assert.equal(fs.existsSync(renamed.resource.fsPath), true);
					assert.ok(fs.readdirSync(testDir).some(f => f === 'CONWAY.js'));

					return service
						.resolveFile(uri.file(path.join(testDir, 'deep', 'conway.js')))
						.done(source => {
							return service
								.importFile(source.resource, uri.file(testDir))
								.then(res => {
									// CONWAY.js => conway.js
									assert.equal(fs.existsSync(res.stat.resource.fsPath), true);
									assert.ok(
										fs.readdirSync(testDir).some(f => f === 'conway.js')
									);
			...
			parent => {
				...
				return service.createFolder(folderResource).then(f => {
					...
					return service.importFile(resource, uri.file(testDir)).then(res => {
						...
						assert.equal(
							importEvent.target.resource.fsPath,
							res.stat.resource.fsPath
						);
		...
		service.resolveFile(uri.file(path.join(testDir, 'index.html'))).done(
			source => {
				return service
					.importFile(
						source.resource,
						uri.file(path.dirname(source.resource.fsPath))
					)
					.then(imported => {
	...
	test('resolveFile', function(done: () => void) {
		service
			.resolveFile(uri.file(testDir), {
				resolveTo: [uri.file(path.join(testDir, 'deep'))]
			})
			.done(
				r => {
	...
	test('existsFile', function(done: () => void) {
		service.existsFile(uri.file(testDir)).then(
			exists => {
	...
	test('updateContent', function(done: () => void) {
		...
		service.resolveContent(resource).done(
			c => {
				...
				return service.updateContent(c.resource, c.value).then(c => {
					assert.equal(
						fs.readFileSync(resource.fsPath),
	...
	test('updateContent - use encoding (UTF 16 BE)', function(done: () => void) {
		...
		service.resolveContent(resource).done(
			c => {
				...
				return service
					.updateContent(c.resource, c.value, { encoding: encoding })
					.then(c => {
						return encodingLib
							.detectEncodingByBOM(c.resource.fsPath)
							.then(enc => {
	...
	test('resolveContent - FILE_IS_BINARY', function(done: () => void) {
		...
		service.resolveContent(resource, { acceptTextOnly: true }).done(
			null,
			(e: IFileOperationResult) => {
				assert.equal(e.fileOperationResult, FileOperationResult.FILE_IS_BINARY);

				return service
					.resolveContent(uri.file(path.join(testDir, 'small.txt')), {
						acceptTextOnly: true
					})
					.then(r => {
	...
	test('resolveContent - FILE_NOT_MODIFIED_SINCE', function(done: () => void) {
		...
		service.resolveContent(resource).done(
			c => {
				return service
					.resolveContent(resource, { etag: c.etag })
					.then(null, (e: IFileOperationResult) => {
						assert.equal(
							e.fileOperationResult,
	...
	test('resolveContent - FILE_MODIFIED_SINCE', function(done: () => void) {
		...
		service.resolveContent(resource).done(
			c => {
				...
				return service
					.updateContent(resource, c.value, {
						etag: c.etag,
						mtime: c.mtime - 1000
					})
					.then(null, (e: IFileOperationResult) => {
	...
	test('resolveContent - encoding picked up', function(done: () => void) {
		...
		service.resolveContent(resource, { encoding: encoding }).done(
			c => {
				assert.equal(c.encoding, encoding);
	...
	test('resolveContent - user overrides BOM', function(done: () => void) {
		...
		service.resolveContent(resource, { encoding: 'windows1252' }).done(
			c => {
	...
	test('resolveContent - BOM removed', function(done: () => void) {
		...
		service.resolveContent(resource).done(
			c => {
				assert.equal(
					encodingLib.detectEncodingByBOMFromBuffer(new Buffer(c.value), 512),
					null
				);
	...
	test('resolveContent - invalid encoding', function(done: () => void) {
		...
		service.resolveContent(resource, { encoding: 'superduper' }).done(
			c => {
	...
	test('options - encoding', function(done: () => void) {
		...
		extfs.copy(_sourceDir, _testDir, () => {
			...
			_service
				.resolveContent(uri.file(path.join(testDir, 'index.html')))
				.done(c => {
					assert.equal(c.encoding, 'windows1252');

					return _service
						.resolveContent(uri.file(path.join(testDir, 'deep', 'conway.js')))
						.done(c => {
	...
	test('UTF 8 BOMs', function(done: () => void) {
		...
		extfs.copy(_sourceDir, _testDir, () => {
			fs.readFile(resource.fsPath, (error, data) => {
				assert.equal(
					encodingLib.detectEncodingByBOMFromBuffer(data, 512),
					null
				);

				// Update content: UTF_8 => UTF_8_BOM
				_service
					.updateContent(resource, 'Hello Bom', {
						encoding: encodingLib.UTF8_with_bom
					})
					.done(() => {
						fs.readFile(resource.fsPath, (error, data) => {
							assert.equal(
							...
							_service
								.updateContent(resource, 'Please stay Bom', {
									encoding: encodingLib.UTF8
								})
								.done(() => {
									fs.readFile(resource.fsPath, (error, data) => {
										assert.equal(
										...
										_service
											.updateContent(resource, 'Go away Bom', {
												encoding: encodingLib.UTF8,
											...
											.done(() => {
												fs.readFile(resource.fsPath, (error, data) => {
													assert.equal(
														encodingLib.detectEncodingByBOMFromBuffer(
															data,
															512
														),
														null
													...
													_service
														.updateContent(resource, 'Do not come back Bom', {
															encoding: encodingLib.UTF8
														})
														.done(() => {
															fs.readFile(resource.fsPath, (error, data) => {
