...
export const IKeybindingEditingService = createDecorator<
...
export class KeybindingsEditingService extends Disposable
	...
	private updateUserKeybinding(
		...
		const userKeybindingEntries = <IUserFriendlyKeybinding[]>json.parse(
			model.getValue()
		);
	...
	private updateDefaultKeybinding(
		newKey: string,
		keybindingItem: ResolvedKeybindingItem,
		model: editorCommon.IModel
	): void {
	...
	private removeUserKeybinding(
		...
		if (userKeybindingEntryIndex !== -1) {
			this.applyEditsToBuffer(
				setProperty(model.getValue(), [userKeybindingEntryIndex], void 0, {
					tabSize,
					insertSpaces,
					eol
				})[0],
	...
	private removeUnassignedDefaultKeybinding(
		...
		const index = this.findUnassignedDefaultKeybindingEntryIndex(
			keybindingItem,
			userKeybindingEntries
		);
		if (index !== -1) {
	...
	private findUserKeybindingEntryIndex(
		...
		for (let index = 0; index < userKeybindingEntries.length; index++) {
			...
			if (keybinding.command === keybindingItem.command) {
				...
				if (keybinding.when && keybindingItem.when) {
					if (
						ContextKeyExpr.deserialize(keybinding.when).serialize() ===
	...
	private findUnassignedDefaultKeybindingEntryIndex(
		...
		for (let index = 0; index < userKeybindingEntries.length; index++) {
			if (
				userKeybindingEntries[index].command === `-${keybindingItem.command}`
			) {
				return index;
			}
		}
	...
	private resolveModelReference(): TPromise<IReference<ITextEditorModel>> {
		return this.fileService.existsFile(this.resource).then(exists => {
			...
			const result = exists
				...
				: this.fileService.updateContent(
						this.resource,
						this.getEmptyContent(EOL),
						{ encoding: 'utf8' }
					);
			return result.then(() =>
				this.textModelResolverService.createModelReference(this.resource)
			);
		});
	}
	...
	private resolveAndValidate(): TPromise<IReference<ITextEditorModel>> {
		...
		if (this.textFileService.isDirty(this.resource)) {
			return TPromise.wrapError<IReference<ITextEditorModel>>(
				localize(
	...
	private parse(
		model: editorCommon.IModel
	): { result: IUserFriendlyKeybinding[]; parseErrors: json.ParseError[] } {
