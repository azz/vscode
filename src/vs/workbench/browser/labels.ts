...
export class ResourceLabel extends IconLabel {
	...
	private registerListeners(): void {
		this.extensionService
			.onReady()
			.then(() => this.render(true /* clear cache */)); // update when extensions are loaded with potentially new languages
		this.toDispose.push(
			this.configurationService.onDidUpdateConfiguration(() =>
				this.render(true /* clear cache */)
			)
		); // update when file.associations change
	}
	...
	private render(clearIconCache: boolean): void {
		if (this.label) {
			const configuredLangId = getConfiguredLangId(
				this.modelService,
				this.label.resource
			);
		...
		if (!this.computedIconClasses) {
			this.computedIconClasses = getIconClasses(
				...
				this.modeService,
				resource,
				this.options && this.options.isFolder
			);
		}
...
export class FileLabel extends ResourceLabel {
	public setFile(
		resource: uri,
		options: IFileLabelOptions = Object.create(null)
	): void {
		this.setLabel(
			{
				resource,
				name: !options.hideLabel ? paths.basename(resource.fsPath) : void 0,
				description: !options.hidePath
					? getPathLabel(
...
export function getIconClasses(
	...
	if (path) {
		...
		if (isFolder) {
			...
			configuredLangId =
				configuredLangId || modeService.getModeIdByFilenameOrFirstLine(path);
			if (configuredLangId) {
