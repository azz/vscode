...
class ConfigureLocaleAction extends Action {
	...
	public run(event?: any): TPromise<IEditor> {
		const file = URI.file(
			Path.join(this.environmentService.appSettingsHome, 'locale.json')
		);
				...
				error => {
					throw new Error(
						nls.localize(
							...
							Labels.getPathLabel(file, this.contextService),
							error
						)
					);
				}
...
registry.registerWorkbenchAction(
	new SyncActionDescriptor(
		...
		ConfigureLocaleAction.ID,
		ConfigureLocaleAction.LABEL
	),
	'Configure Language'
);
