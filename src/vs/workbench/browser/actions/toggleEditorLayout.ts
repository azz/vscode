...
export class ToggleEditorLayoutAction extends Action {
	...
	private registerListeners(): void {
		this.toDispose.push(
			this.editorGroupService.onEditorsChanged(() => this.updateEnablement())
		);
		this.toDispose.push(
			this.editorGroupService.onGroupOrientationChanged(() =>
				this.updateLabel()
			)
		);
	}
	...
	private updateLabel(): void {
		...
		this.label = editorGroupLayoutVertical
			? nls.localize('horizontalLayout', 'Horizontal Editor Group Layout')
			: nls.localize('verticalLayout', 'Vertical Editor Group Layout');
	}
	...
	public run(): TPromise<any> {
		...
		const newGroupOrientation: GroupOrientation = groupOrientiation ===
			'vertical'
			? 'horizontal'
			: 'vertical';
...
CommandsRegistry.registerCommand(
	...
	function(accessor: ServicesAccessor, args: [GroupOrientation]) {
...
registry.registerWorkbenchAction(
	new SyncActionDescriptor(
		...
		{
			primary: KeyMod.Shift | KeyMod.Alt | KeyCode.KEY_1,
			mac: { primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyCode.KEY_1 }
		}
	),
	'View: Toggle Editor Group Vertical/Horizontal Layout',
	group
);
