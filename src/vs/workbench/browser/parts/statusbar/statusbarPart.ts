...
export class StatusbarPart extends Part implements IStatusbarService {
	...
	private getEntries(alignment: StatusbarAlignment): HTMLElement[] {
		...
		for (let i = 0; i < children.length; i++) {
			...
			if (
				$(childElement).getProperty(StatusbarPart.ALIGNMENT_PROP) === alignment
			) {
	...
	public createContentArea(parent: Builder): Builder {
		...
		const leftDescriptors = registry.items
			.filter(d => d.alignment === StatusbarAlignment.LEFT)
			.sort((a, b) => b.priority - a.priority);
		const rightDescriptors = registry.items
			.filter(d => d.alignment === StatusbarAlignment.RIGHT)
		...
		this.toDispose.push(
			...descriptors.map(descriptor => {
				...
				const el = this.doCreateStatusItem(
	...
	protected updateStyles(): void {
		...
		const borderColor =
			this.getColor(STATUS_BAR_BORDER) || this.getColor(contrastBorder);
...
class StatusBarEntryItem implements IStatusbarItem {
	...
	constructor(
		...
		if (!manageExtensionAction) {
			manageExtensionAction = this.instantiationService.createInstance(
	...
	public render(el: HTMLElement): IDisposable {
		...
		if (this.entry.command) {
			...
			$(textContainer).on(
				'click',
				() => this.executeCommand(this.entry.command),
				toDispose
			);
		} else {
		...
		if (color) {
			if (isThemeColor(color)) {
				...
				color = (this.themeService.getTheme().getColor(colorId) ||
					Color.transparent)
					.toString();
				toDispose.push(
					this.themeService.onThemeChange(theme => {
						let colorValue = (this.themeService.getTheme().getColor(colorId) ||
							Color.transparent)
							.toString();
	...
	private executeCommand(id: string) {
		...
		const builtInActionDescriptor = Registry.as<IWorkbenchActionRegistry>(
			ActionExtensions.WorkbenchActions
		).getWorkbenchAction(id);
		if (builtInActionDescriptor) {
			const action = this.instantiationService.createInstance(
				builtInActionDescriptor.syncDescriptor
			);
...
registerThemingParticipant((theme: ITheme, collector: ICssStyleCollector) => {
	const statusBarItemHoverBackground = theme.getColor(
		STATUS_BAR_ITEM_HOVER_BACKGROUND
	);
	if (statusBarItemHoverBackground) {
		collector.addRule(
			`.monaco-workbench > .part.statusbar > .statusbar-item a:hover:not([disabled]):not(.disabled) { background-color: ${statusBarItemHoverBackground}; }`
		);
	}

	const statusBarItemActiveBackground = theme.getColor(
		STATUS_BAR_ITEM_ACTIVE_BACKGROUND
	);
	if (statusBarItemActiveBackground) {
		collector.addRule(
			`.monaco-workbench > .part.statusbar > .statusbar-item a:active:not([disabled]):not(.disabled) { background-color: ${statusBarItemActiveBackground}; }`
		);
	}

	const statusBarProminentItemBackground = theme.getColor(
		STATUS_BAR_PROMINENT_ITEM_BACKGROUND
	);
	if (statusBarProminentItemBackground) {
		collector.addRule(
			`.monaco-workbench > .part.statusbar > .statusbar-item .status-bar-info { background-color: ${statusBarProminentItemBackground}; }`
		);
	}

	const statusBarProminentItemHoverBackground = theme.getColor(
		STATUS_BAR_PROMINENT_ITEM_HOVER_BACKGROUND
	);
	if (statusBarProminentItemHoverBackground) {
		collector.addRule(
			`.monaco-workbench > .part.statusbar > .statusbar-item a.status-bar-info:hover:not([disabled]):not(.disabled) { background-color: ${statusBarProminentItemHoverBackground}; }`
		);
	}
});
