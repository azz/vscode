...
function toEditorWithEncodingSupport(input: IEditorInput): IEncodingSupport {
	...
	if (
		types.areFunctions(encodingSupport.setEncoding, encodingSupport.getEncoding)
	) {
...
class State {
	...
	public get selectionStatus(): string {
	...
	public get tabFocusMode(): boolean {
		return this._tabFocusMode;
	}

	private _screenReaderMode: boolean;
	public get screenReaderMode(): boolean {
...
const nlsScreenReaderDetected = nls.localize(
	'screenReaderDetected',
	'Screen Reader Detected'
);
const nlsScreenReaderDetectedTitle = nls.localize(
...
export class EditorStatus implements IStatusbarItem {
	...
	public render(container: HTMLElement): IDisposable {
		...
		this.tabFocusModeElement = append(
			this.element,
			$('a.editor-status-tabfocusmode.status-bar-info')
		);
		this.tabFocusModeElement.title = nls.localize(
			'disableTabMode',
		...
		this.screenRedearModeElement = append(
			this.element,
		...
		this.indentationElement = append(
			this.element,
		...
		this.encodingElement.title = nls.localize(
			'selectEncoding',
			'Select Encoding'
		);
		...
		this.eolElement.title = nls.localize(
			'selectEOL',
		...
		this.modeElement.title = nls.localize(
			'selectLanguageMode',
		...
		this.toDispose.push(
			...
			this.untitledEditorService.onDidChangeEncoding(r =>
				this.onResourceEncodingChange(r)
			),
			this.textFileService.models.onModelEncodingChanged(e =>
				this.onResourceEncodingChange(e.resource)
			),
	...
	private onIndentationClick(): void {
		const action = this.instantiationService.createInstance(
			...
			ChangeIndentationAction.ID,
			ChangeIndentationAction.LABEL
		);
		action.run().done(null, errors.onUnexpectedError);
		action.dispose();
	}
	...
	private onEditorsChanged(): void {
		...
		if (control) {
			...
			this.activeEditorListeners.push(
				control.onDidChangeConfiguration(
					(event: IConfigurationChangedEvent) => {
			...
			this.activeEditorListeners.push(
				control.onDidChangeModelLanguage(
			...
			this.activeEditorListeners.push(
				control.onDidChangeModelContent(e => {
					this.onEOLChange(control);
				})
			);
			...
			this.activeEditorListeners.push(
				control.onDidChangeModelOptions((event: IModelOptionsChangedEvent) => {
					this.onIndentationChange(control);
				})
			);
		} else if (
			activeEditor instanceof BaseBinaryResourceEditor ||
			activeEditor instanceof BinaryResourceDiffEditor
		) {
			...
			binaryEditors.forEach(editor => {
				this.activeEditorListeners.push(
					editor.onMetadataChanged(metadata => {
						this.onMetadataChange(activeEditor);
					})
				);
			});
	...
	private onScreenReaderModeChange(editorWidget: ICommonCodeEditor): void {
		...
		if (editorWidget) {
			screenReaderMode =
				editorWidget.getConfiguration().accessibilitySupport ===
	...
	private onSelectionChange(editorWidget: ICommonCodeEditor): void {
		...
		if (editorWidget) {
			...
			if (info.selections.length === 1) {
				const visibleColumn = editorWidget.getVisibleColumnFromPosition(
	...
	private onEncodingChange(e: IBaseEditor): void {
		...
		if (getEditorWidget(e)) {
			const encodingSupport: IEncodingSupport = toEditorWithEncodingSupport(
				e.input
			);
			if (encodingSupport) {
...
export class ShowLanguageExtensionsAction extends Action {
	...
	constructor(
		...
		super(
			ShowLanguageExtensionsAction.ID,
			nls.localize(
...
export class ChangeModeAction extends Action {
	...
	constructor(
		...
		@ICommandService private commandService: ICommandService,
		@IConfigurationEditingService
		private configurationEditService: IConfigurationEditingService
	) {
	...
	public run(): TPromise<any> {
		...
		if (!editorWidget) {
			return this.quickOpenService.pick([
				{
					label: nls.localize('noEditor', 'No text editor active at this time')
				}
			]);
		}

		const textModel = editorWidget.getModel();
		const fileResource = toResource(activeEditor.input, {
			supportSideBySide: true,
			filter: 'file'
		});
		...
		const picks: IPickOpenEntry[] = languages.sort().map((lang, index) => {
			...
			if (currentModeId === lang) {
				description = nls.localize(
					...
					this.modeService.getModeIdForLanguageName(lang.toLowerCase())
				);
			} else {
				description = nls.localize(
		...
		if (fileResource) {
			picks[0].separator = {
				border: true,
				label: nls.localize('languagesPicks', 'languages (identifier)')
			};
		}
		...
		if (fileResource) {
			const ext =
				paths.extname(fileResource.fsPath) ||
				paths.basename(fileResource.fsPath);

			galleryAction = this.instantiationService.createInstance(
		...
		return this.quickOpenService
			.pick(picks, {
				placeHolder: nls.localize('pickLanguage', 'Select Language Mode')
			})
			.then(pick => {
				if (!pick) {
					return;
				...
				if (editorWidget) {
					...
					if (pick === autoDetectMode) {
						mode = this.modeService.getOrCreateModeByFilenameOrFirstLine(
							toResource(activeEditor.input, {
								supportSideBySide: true,
								filter: ['file', 'untitled']
							}).fsPath,
	...
	private configureFileAssociation(resource: uri): void {
		...
		const currentAssociation = this.modeService.getModeIdByFilenameOrFirstLine(
			basename
		);
		...
		const picks: IPickOpenEntry[] = languages.sort().map((lang, index) => {
			...
			return <IPickOpenEntry>{
				id,
				label: lang,
				description: id === currentAssociation
					? nls.localize('currentAssociation', 'Current Association')
					: void 0
			};
		});
		...
		TPromise.timeout(
			...
			this.quickOpenService
				...
				.done(language => {
					if (language) {
						const fileAssociationsConfig = this.configurationService.lookup(
							ChangeModeAction.FILE_ASSOCIATION_KEY
						);
						...
						if (
							fileAssociationsConfig.workspace &&
							!!fileAssociationsConfig.workspace[associationKey]
						) {
							target = ConfigurationTarget.WORKSPACE;
						}

						// Make sure to write into the value of the target and not the merged value from USER and WORKSPACE config
						let currentAssociations = target === ConfigurationTarget.WORKSPACE
							? fileAssociationsConfig.workspace
							: fileAssociationsConfig.user;
						...
						// Write config
						this.configurationEditingService.writeConfiguration(target, {
							key: ChangeModeAction.FILE_ASSOCIATION_KEY,
							value: currentAssociations
						});
					}
				});
		});
...
class ChangeIndentationAction extends Action {
	...
	public run(): TPromise<any> {
		...
		(<IPickOpenEntry>picks[0]).separator = {
			label: nls.localize('indentView', 'change view')
		};
		(<IPickOpenEntry>picks[3]).separator = {
			label: nls.localize('indentConvert', 'convert file'),
			border: true
		};
...
export class ChangeEncodingAction extends Action {
	...
	public run(): TPromise<any> {
		...
		let encodingSupport: IEncodingSupport = toEditorWithEncodingSupport(
			activeEditor.input
		);
		if (!encodingSupport) {
			return this.quickOpenService.pick([
				{ label: nls.localize('noFileEditor', 'No file active at this time') }
			]);
		}
		...
		if (language === LANGUAGE_DEFAULT) {
			saveWithEncodingPick = {
				label: nls.localize('saveWithEncoding', 'Save with Encoding')
			};
			reopenWithEncodingPick = {
				label: nls.localize('reopenWithEncoding', 'Reopen with Encoding')
			};
		} else {
			saveWithEncodingPick = {
				label: nls.localize('saveWithEncoding', 'Save with Encoding'),
				detail: 'Save with Encoding'
			};
			reopenWithEncodingPick = {
				label: nls.localize('reopenWithEncoding', 'Reopen with Encoding'),
				detail: 'Reopen with Encoding'
			};
		}
				...
				.then(guessedEncoding => {
					...
					const picks: IPickOpenEntry[] = Object.keys(SUPPORTED_ENCODINGS)
						.sort((k1, k2) => {
							...
							return (
								SUPPORTED_ENCODINGS[k1].order - SUPPORTED_ENCODINGS[k2].order
							);
						})
						.filter(k => {
							if (
						...
						.map((key, index) => {
							if (key === encodingSupport.getEncoding()) {
								...
								SUPPORTED_ENCODINGS[key].alias === encodingSupport.getEncoding()
							) {
					...
					return this.quickOpenService
						.pick(picks, {
							...
							autoFocus: {
								autoFocusIndex: typeof directMatchIndex === 'number'
									? directMatchIndex
						...
						.then(encoding => {
							if (encoding) {
								activeEditor = this.editorService.getActiveEditor();
								encodingSupport = toEditorWithEncodingSupport(
									activeEditor.input
								);
								if (
									encodingSupport &&
									encodingSupport.getEncoding() !== encoding.id
								) {
									encodingSupport.setEncoding(
										encoding.id,
