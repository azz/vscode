...
export class TextDiffEditor extends BaseTextEditor {
	...
	public createEditorControl(
		...
		const diffEditorInstantiator = this.instantiationService.createChild(
			new ServiceCollection([IWorkbenchEditorService, delegatingEditorService])
		);

		return diffEditorInstantiator.createInstance(
			DiffEditorWidget,
	...
	public setInput(input: EditorInput, options?: EditorOptions): TPromise<void> {
		...
		return input.resolve(true).then(
			resolvedModel => {
				// Assert Model Instance
				if (
					!(resolvedModel instanceof TextDiffEditorModel) &&
				...
				diffEditor.setModel(
					(<TextDiffEditorModel>resolvedModel).textDiffEditorModel
				);
				...
				if (options instanceof TextDiffEditorOptions) {
					...
					autoRevealFirstChange = !types.isUndefinedOrNull(
	...
	private openAsBinary(input: EditorInput, options: EditorOptions): boolean {
		if (input instanceof DiffEditorInput) {
			...
			const binaryDiffInput = new DiffEditorInput(
				input.getName(),
			...
			if (
				types.isFunction(
					((originalInput as IEditorInput) as IFileEditorInput)
			...
			if (
				types.isFunction(
					((modifiedInput as IEditorInput) as IFileEditorInput)
						.setForceOpenAsBinary
				)
			) {
			...
			this.editorService
				.openEditor(binaryDiffInput, options, this.position)
	...
	protected computeConfiguration(
		...
		if (language) {
			objects.assign(
				editorConfiguration,
				this.configurationService.getConfiguration<IEditorConfiguration>({
	...
	protected getAriaLabel(): string {
		...
		if (this.isReadOnly()) {
			...
			ariaLabel = inputName
				...
				: nls.localize('editableEditorAriaLabel', 'Text file compare editor.');
		}
	...
	public getActions(): IAction[] {
		return [this.previousDiffAction, this.nextDiffAction];
	}
	...
	public getSecondaryActions(): IAction[] {
		...
		actions.push(...[toggleEditorModeAction]);
...
class NavigateAction extends Action {
	...
	constructor(editor: TextDiffEditor, next: boolean) {
		...
		this.label = this.next
			? nls.localize('navigate.next.label', 'Next Change')
			: nls.localize('navigate.prev.label', 'Previous Change');
		this.class = this.next
...
class ToggleEditorModeAction extends Action {
	...
	private static INLINE_LABEL = nls.localize(
		'inlineDiffLabel',
		'Switch to Inline View'
	);
	private static SIDEBYSIDE_LABEL = nls.localize(
		'sideBySideDiffLabel',
	...
	public get label(): string {
		return ToggleEditorModeAction.isInlineMode(this.editor)
			? ToggleEditorModeAction.SIDEBYSIDE_LABEL
			: ToggleEditorModeAction.INLINE_LABEL;
	}
