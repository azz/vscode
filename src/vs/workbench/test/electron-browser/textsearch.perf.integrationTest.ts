...
suite('TextSearch performance (integration)', () => {
	test('Measure', () => {
		...
		const searchModel: SearchModel = instantiationService.createInstance(
			SearchModel
		);
		function runSearch(): TPromise<any> {
			const queryBuilder: QueryBuilder = instantiationService.createInstance(
				QueryBuilder
			);
			...
			const onSearchResultsFinished = event.filterEvent(
				telemetryService.eventLogged,
				e => e.name === 'searchResultsFinished'
			);
			event.once(onSearchResultsFinished)(onComplete);

			function onComplete(): void {
				try {
					const allEvents = telemetryService.events
						.map(e => JSON.stringify(e))
						.join('\n');
					assert.equal(
						telemetryService.events.length,
						3,
			...
			.then(() => {
				if (testWorkspaceArg) {
					...
					return (function iterate() {
						...
						return runSearch().then((resultsFinishedEvent: any) => {
							console.log(
								`Iteration ${n - i}: ${resultsFinishedEvent.data.duration /
									1000}s`
							);
						...
						const totalTime = finishedEvents.reduce(
							(sum, e) => sum + e.data.duration,
							0
						);
