...
} from 'vs/editor/contrib/goToDeclaration/browser/goToDeclaration';
...
} from 'vs/editor/contrib/format/common/format';
...
suite('ExtHostLanguageFeatures', function() {
	suiteSetup(() => {
		...
		threadService.setTestInstance(
			MainContext.MainThreadCommands,
			instantiationService.createInstance(MainThreadCommands)
		);
		...
		mainThread = <MainThreadLanguageFeatures>threadService.setTestInstance(
			MainContext.MainThreadLanguageFeatures,
			instantiationService.createInstance(MainThreadLanguageFeatures)
		);
	});
	...
	test('DocumentSymbols, register/deregister', function() {
		...
		let d1 = extHost.registerDocumentSymbolProvider(
			defaultSelector,
	...
	test('DocumentSymbols, data conversion', function() {
		...
		return threadService.sync().then(() => {
			return getDocumentSymbols(model).then(value => {
				...
				assert.deepEqual(entry.location.range, {
					startLineNumber: 1,
					startColumn: 1,
					endLineNumber: 1,
					endColumn: 1
	...
	test('Rename, evil provider 1/2', function() {
		...
		return threadService.sync().then(() => {
			return rename(model, new EditorPosition(1, 1), 'newName').then(
				value => {
					throw new Error('');
				},
				err => {
				...
				<vscode.CompletionItemProvider>{
					provideCompletionItems(): any {
						return new types.CompletionList(
							[<any>new types.CompletionItem('hello')],
							true
						);
					}
				},
	...
	test('Format Doc, data conversion', function() {
		...
		return threadService.sync().then(() => {
			return getDocumentFormattingEdits(model, {
				insertSpaces: true,
				tabSize: 4
			}).then(value => {
				assert.equal(value.length, 2);
	...
	test('Format Range, data conversion', function() {
		...
		return threadService.sync().then(() => {
			return getDocumentRangeFormattingEdits(
				model,
	...
	test('Format on Type, data conversion', function() {
			...
			extHost.registerOnTypeFormattingEditProvider(
				...
				<vscode.OnTypeFormattingEditProvider>{
					provideOnTypeFormattingEdits(): any {
						return [
							new types.TextEdit(new types.Range(0, 0, 0, 0), arguments[2])
						];
					}
				},
				[';']
		...
		return threadService.sync().then(() => {
			return getOnTypeFormattingEdits(model, new EditorPosition(1, 1), ';', {
				insertSpaces: true,
				tabSize: 2
			}).then(value => {
				assert.equal(value.length, 1);
