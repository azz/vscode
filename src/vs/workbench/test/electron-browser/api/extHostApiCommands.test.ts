...
suite('ExtHostLanguageFeatureCommands', function() {
	suiteSetup(done => {
		...
		mainThread = threadService.setTestInstance(
	...
	teardown(function(done) {
		...
		threadService.sync().then(() => done(), err => done(err));
	});
		...
		return threadService.sync().then(() => {
			return commands
				...
				.then(list => {
					...
					assert.equal(
						(<types.SnippetString>fourth.insertText).value,
						'foo$0bar'
					);
				});
		});
			...
			extHost.registerCodeActionProvider(
				...
				<vscode.CodeActionProvider>{
					provideCodeActions(): any {
						return [
							{ command: 'testing', title: 'Title', arguments: [1, 2, true] }
						];
					}
				}
			)
	...
	test('CodeLens, back and forth', function() {
		...
		return threadService.sync().then(() => {
			return commands
				.executeCommand<vscode.CodeLens[]>(
	...
	test('Links, back and forth', function() {
		...
		return threadService.sync().then(() => {
			return commands
				.executeCommand<vscode.DocumentLink[]>(
