...
suite('ExtHostConfiguration', function() {
	...
	function createExtHostConfiguration(
		data: IWorkspaceConfigurationValues = Object.create(null),
		shape?: MainThreadConfigurationShape
	) {
		if (!shape) {
	...
	function createConfigurationValue<
		T
	>(value: T): IWorkspaceConfigurationValue<T> {
		return {
			value,
			default: value,
			user: undefined,
	...
	test('getConfiguration fails regression test 1.7.1 -> 1.8 #15552', function() {
		...
		assert.equal(
			extHostConfig.getConfiguration('search').get('exclude')[
				'**/node_modules'
			],
			true
		);
	...
	test('has/get', function() {
		const all = createExtHostConfiguration({
			...
			['farboo.nested.config2']: createConfigurationValue(
		...
		assert.deepEqual(config.get('nested'), {
			config1: 42,
	...
	test('update, what is #15834', function() {
		...
		allConfig
			...
			.update('formatOnSave', { extensions: ['ts'] });
	...
	test('bogous data, #15834', function() {
		...
		assert.ok(
			!allConfig.getConfiguration('').has('editor.formatOnSave.extensions')
		);
	});
		...
		const shape = new class extends MainThreadConfigurationShape {
			$updateConfigurationOption(
				...
				return TPromise.wrapError(
					<IConfigurationEditingError>{
						code: ConfigurationEditingErrorCode.ERROR_UNKNOWN_KEY,
						message: 'Unknown Key'
					}
				); // something !== OK
			}
