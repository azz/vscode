...
suite('Workbench - Editor', () => {
	...
	test('toResource', function() {
		...
		assert.equal(
			toResource(untitled).toString(),
			untitled.getResource().toString()
		);
		assert.equal(
			toResource(untitled, { supportSideBySide: true }).toString(),
			untitled.getResource().toString()
		);
		assert.equal(
			toResource(untitled, { filter: 'untitled' }).toString(),
			untitled.getResource().toString()
		);
		assert.equal(
			toResource(untitled, { filter: ['file', 'untitled'] }).toString(),
			untitled.getResource().toString()
		);
		...
		assert.equal(
			toResource(file, { supportSideBySide: true }).toString(),
			file.getResource().toString()
		);
		assert.equal(
			toResource(file, { filter: 'file' }).toString(),
			file.getResource().toString()
		);
		assert.equal(
			toResource(file, { filter: ['file', 'untitled'] }).toString(),
			file.getResource().toString()
		);
		assert.ok(!toResource(file, { filter: 'untitled' }));

		const diffEditorInput = new DiffEditorInput(
			'name',
			'description',
			untitled,
			file
		...
		assert.equal(
			toResource(file, { supportSideBySide: true, filter: 'file' }).toString(),
			file.getResource().toString()
		);
		assert.equal(
			toResource(file, {
