...
export class Rule extends Lint.Rules.AbstractRule {
	public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
		return this.applyWithWalker(
			new NoUnexternalizedStringsRuleWalker(sourceFile, this.getOptions())
		);
	}
}
...
class NoUnexternalizedStringsRuleWalker extends Lint.RuleWalker {
	...
	constructor(file: ts.SourceFile, opts: Lint.IOptions) {
		...
		let first: UnexternalizedStringsOptions = options && options.length > 0
			? options[0]
			: null;
		if (first) {
			if (Array.isArray(first.signatures)) {
				first.signatures.forEach(
					(signature: string) => (this.signatures[signature] = true)
				);
			}
			if (Array.isArray(first.ignores)) {
				first.ignores.forEach(
					(ignore: string) => (this.ignores[ignore] = true)
				);
			}
		...
		Object.keys(this.usedKeys).forEach(key => {
			...
			if (occurences.length > 1) {
				occurences.forEach(occurence => {
					this.addFailure(
						this.createFailure(
							occurence.key.getStart(),
							occurence.key.getWidth(),
							`Duplicate key ${occurence.key.getText()} with different message value.`
						)
					);
				});
			}
	...
	private checkStringLiteral(node: ts.StringLiteral): void {
		...
		let doubleQuoted =
			text.length >= 2 &&
			text[0] === NoUnexternalizedStringsRuleWalker.DOUBLE_QUOTE &&
			text[text.length - 1] === NoUnexternalizedStringsRuleWalker.DOUBLE_QUOTE;
		...
		let functionName = callInfo
			? callInfo.callExpression.expression.getText()
			: null;
		...
		if (
			doubleQuoted &&
			(!callInfo || callInfo.argIndex === -1 || !this.signatures[functionName])
		) {
			const s = node.getText();
			const replacement = new Lint.Replacement(
				node.getStart(),
				node.getWidth(),
				`nls.localize('KEY-${s.substring(1, s.length - 1)}', ${s})`
			);
			...
			this.addFailure(
				this.createFailure(
					node.getStart(),
					node.getWidth(),
					`Unexternalized string found: ${node.getText()}`,
					fix
				)
			);
			return;
		...
		if (messageNode) {
			if (
				occurences.some(
					pair =>
						pair.message
							? pair.message.getText() === messageNode.getText()
							: false
				)
			) {
				return;
	...
	private findDescribingParent(
		...
		callInfo?: { callExpression: ts.CallExpression; argIndex: number };
		ignoreUsage?: boolean;
	} {
		let parent: ts.Node;
			...
			if (kind === ts.SyntaxKind.CallExpression) {
				...
				return {
					callInfo: {
						...
						argIndex: callExpression.arguments.indexOf(<any>node)
					}
				};
			} else if (
