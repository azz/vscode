...
export function activate(context: vscode.ExtensionContext) {
	...
	const contentProvider = new MDDocumentContentProvider(
		engine,
		context,
		cspArbiter,
		logger
	...
	if (
		vscode.workspace
			...
			.get('enableExperimentalExtensionApi', false)
	) {
		for (const extension of vscode.extensions.all) {
			...
			if (styles) {
				...
				for (const style of styles) {
					try {
						contentProvider.addStyle(
							resolveExtensionResources(extension, style)
						);
					} catch (e) {
						// noop
					}
			...
			if (scripts) {
				...
				for (const script of scripts) {
					try {
						contentProvider.addScript(
							resolveExtensionResources(extension, script)
						);
					} catch (e) {
						// noop
					}
	...
	const symbolsProviderRegistration = vscode.languages.registerDocumentSymbolProvider(
		{ language: 'markdown' },
		symbolsProvider
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('markdown.showPreview', showPreview)
	);
	context.subscriptions.push(
		vscode.commands.registerCommand('markdown.showPreviewToSide', uri =>
			showPreview(uri, true)
		)
	);
	context.subscriptions.push(
		vscode.commands.registerCommand('markdown.showSource', showSource)
	);

	context.subscriptions.push(
		vscode.commands.registerCommand('_markdown.revealLine', (uri, line) => {
		...
		vscode.commands.registerCommand(
			...
			(args: OpenDocumentLinkArgs) => {
				...
				if (
					...
					vscode.window.activeTextEditor.document.uri.fsPath === args.path
				) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			(resource: string | undefined) => {
				previewSecuritySelector.showSecutitySelectorForWorkspace(
					resource ? vscode.Uri.parse(resource).query : undefined
				);
			}
		)
	);
	...
	context.subscriptions.push(
		vscode.window.onDidChangeTextEditorSelection(event => {
			if (isMarkdownFile(event.textEditor.document)) {
				...
				logger.log('updatePreviewForSelection', {
...
function showSource(mdUri: vscode.Uri) {
	...
	for (const editor of vscode.window.visibleTextEditors) {
		if (
			editor.document.uri.scheme === docUri.scheme &&
			editor.document.uri.fsPath === docUri.fsPath
		) {
