...
export abstract class EmmetCompletionItemProviderBase
	...
	public provideCompletionItems(
		...
		let abbreviationSuggestions = this.getAbbreviationSuggestions(
			getSyntax(document),
			currentWord,
	...
	protected getExpandedAbbreviation(
		document: vscode.TextDocument,
		position: vscode.Position
	): vscode.CompletionItem {
		if (
			!vscode.workspace.getConfiguration('emmet')['showExpandedAbbreviation']
		) {
			return;
		}
	...
	abstract getAbbreviationSuggestions(
		syntax: string,
		prefix: string,
...
export class EmmetCompletionItemProviderHtml extends EmmetCompletionItemProviderBase {
	...
	getAbbreviationSuggestions(
		...
		snippetCompletions = snippetCompletions.filter(
			x => x.label.startsWith(prefix) && (!skipExactMatch || x.label !== prefix)
		);
