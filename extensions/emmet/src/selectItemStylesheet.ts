...
export function nextItemStylesheet(
	startOffset: number,
	endOffset: number,
	...
	if (
		...
		let singlePropertyValue = getSelectionFromProperty(
			currentNode,
			editor.document,
			startOffset,
			endOffset,
	...
	if (
		(currentNode.type === 'rule' &&
			endOffset < currentNode.selectorToken.end) ||
...
function getSelectionFromNode(
	...
	return new vscode.Selection(
		document.positionAt(nodeToSelect.start),
		document.positionAt(nodeToSelect.end)
	);
}

function getSelectionFromProperty(
	node: Node,
	document: vscode.TextDocument,
	...
	let propertyValue = node.valueToken.stream.substring(
	...
	selectFullValue =
		selectFullValue ||
		(direction === 'prev' &&
			selectionStart === node.valueToken.start &&
			selectionEnd < node.valueToken.end);

	if (selectFullValue) {
		return new vscode.Selection(
			document.positionAt(node.valueToken.start),
			document.positionAt(node.valueToken.end)
		);
	}
	...
	if (direction === 'prev') {
		...
		pos = selectionStart > node.valueToken.end
	...
	if (direction === 'next') {
		...
		pos = selectionEnd === node.valueToken.end
			? -1
			: selectionEnd - node.valueToken.start - 1;
	}

	let [newSelectionStart, newSelectionEnd] = direction === 'prev'
