...
export function fetchSelectItem(direction: string): void {
	...
	editor.selections.forEach(selection => {
		const selectionStart = editor.document.offsetAt(
			selection.isReversed ? selection.active : selection.anchor
		);
		const selectionEnd = editor.document.offsetAt(
			selection.isReversed ? selection.anchor : selection.active
		);

		let updatedSelection = direction === 'next'
			? nextItem(selectionStart, selectionEnd, editor, rootNode)
			: prevItem(selectionStart, selectionEnd, editor, rootNode);
