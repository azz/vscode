...
export default class TypingsStatus extends Disposable {
	private _acquiringTypings: {
		[eventId: string]: NodeJS.Timer;
	} = Object.create({});
	...
	private onTypesInstallerInitializationFailed() {
		...
		if (
			workspace
				...
				.get<boolean>('check.npmIsInstalled', true)
		) {
			window
				.showWarningMessage<MyMessageItem>(
					localize(
					...
					{
						title: localize(
							'typesInstallerInitializationFailed.moreInformation',
							'More Information'
						),
						id: 1
					},
					...
					{
						title: localize(
							'typesInstallerInitializationFailed.close',
							'Close'
						),
						id: 3,
					...
					switch (selected.id) {
						case 1:
							commands.executeCommand(
								'vscode.open',
								Uri.parse('https://go.microsoft.com/fwlink/?linkid=847635')
							);
							break;
						case 2:
