...
export default class TypeScriptDocumentSymbolProvider
	...
	public provideDocumentSymbols(
		...
		if (this.client.apiVersion.has206Features()) {
			return this.client.execute('navtree', args, token).then(
				response => {
			...
			return this.client.execute('navbar', args, token).then(
				response => {
					const result: SymbolInformation[] = [];
					if (response.body) {
						let foldingMap: ObjectMap<SymbolInformation> = Object.create(null);
						response.body.forEach(item =>
							TypeScriptDocumentSymbolProvider.convertNavBar(
								resource.uri,
								0,
								foldingMap,
								result,
	...
	private static convertNavBar(
		resource: Uri,
		indent: number,
		foldingMap: ObjectMap<SymbolInformation>,
		bucket: SymbolInformation[],
		item: Proto.NavigationBarItem,
		...
		if (
			realIndent !== 0 &&
			!foldingMap[key] &&
			TypeScriptDocumentSymbolProvider.shouldInclueEntry(item.text)
		) {
			let result = new SymbolInformation(
				item.text,
	...
	private static convertNavTree(
		...
		const result = new SymbolInformation(
			item.text,
			outlineTypeTable[item.kind] || SymbolKind.Variable,
			containerLabel ? containerLabel : '',
			new Location(resource, textSpan2Range(item.spans[0]))
