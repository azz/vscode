...
export default class TypeScriptCodeActionProvider
	implements CodeActionProvider {
	private commandId: string;

	private _supportedCodeActions?: Thenable<NumberSet>;

	constructor(private readonly client: ITypescriptServiceClient, mode: string) {
			...
			.then(supportedActions => {
				...
				return this.client
					.execute(
						...
						} as Proto.CodeFixRequestArgs,
						token
					)
					.then(response => response.body || []);
			})
			.then(codeActions =>
				codeActions.map(action => this.actionToEdit(source, action))
			);
	}
	...
	private getSupportedActionsForContext(
		context: CodeActionContext
	): Thenable<Set<number>> {
		return this.supportedCodeActions.then(
			supportedActions =>
				new Set(
					context.diagnostics
	...
	private onCodeAction(source: Source, workspaceEdit: WorkspaceEdit) {
		workspace.applyEdit(workspaceEdit).then(success => {
			...
			const editedRange = new Range(
				...
				0,
				firstEdit.range.end.line + 1 + (newLines ? newLines.length : 0),
				0
			);
