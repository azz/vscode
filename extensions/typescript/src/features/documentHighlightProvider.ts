...
export default class TypeScriptDocumentHighlightProvider
	...
	public provideDocumentHighlights(
		...
		return this.client.execute('occurrences', args, token).then(
			(response): DocumentHighlight[] => {
				...
				if (data && data.length) {
					...
					if (
						this.client.apiVersion.has213Features() &&
						firstOccurrence.start.offset > 1
					) {
						// Check to see if contents around first occurrence are string delimiters
						const contents = resource.getText(
							new Range(
								firstOccurrence.start.line - 1,
								firstOccurrence.start.offset - 1 - 1,
								firstOccurrence.end.line - 1,
								firstOccurrence.end.offset - 1 + 1
							)
						);
						...
						if (
							contents &&
							contents.length > 2 &&
							stringDelimiters.indexOf(contents[0]) >= 0 &&
