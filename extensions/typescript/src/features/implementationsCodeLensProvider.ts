...
export default class TypeScriptImplementationsCodeLensProvider extends TypeScriptBaseCodeLensProvider {
	...
	resolveCodeLens(
		...
		return this.client
			.execute('implementation', args, token)
			.then(response => {
				...
				const locations = response.body
					.map(
						reference =>
							...
							new Location(
								this.client.asUrl(reference.file),
								reference.start.line === reference.end.line
									? new Range(
											reference.start.line - 1,
											reference.start.offset - 1,
											reference.end.line - 1,
											reference.end.offset - 1
										)
									: new Range(
											reference.start.line - 1,
											reference.start.offset - 1,
			...
			.catch(() => {
				codeLens.command = {
					title: localize(
						'implementationsErrorLabel',
						'Could not determine implementations'
					),
					command: ''
				};
				return codeLens;
