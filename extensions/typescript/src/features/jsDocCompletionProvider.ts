...
export class JsDocCompletionProvider implements CompletionItemProvider {
	...
	public updateConfiguration(): void {
		const jsDocCompletionConfig = workspace.getConfiguration(
			configurationNamespace
		);
		this.config.enabled = jsDocCompletionConfig.get(
...
export class TryCompleteJsDocCommand {
	...
	public tryCompleteJsDoc(
		...
		return this.tryInsertJsDocFromTemplate(
			editor,
			file,
			start
	...
	private tryInsertJsDocFromTemplate(
		editor: TextEditor,
		file: string,
		position: Position
	): Promise<boolean> {
