...
export class SettingsDocument {
	...
	public provideCompletionItems(
		...
		const range =
			this.document.getWordRangeAtPosition(position) ||
		...
		if (
			location.path[0] === 'files.exclude' ||
			location.path[0] === 'search.exclude'
		) {
	...
	private provideWindowTitleCompletionItems(
		location: Location,
		range: vscode.Range
	): vscode.ProviderResult<vscode.CompletionItem[]> {
		...
		completions.push(
			this.newSimpleCompletionItem(
				'${activeEditorShort}',
				range,
				localize('activeEditorShort', 'e.g. myFile.txt')
			)
		);
		completions.push(
			this.newSimpleCompletionItem(
				'${activeEditorMedium}',
				range,
				localize('activeEditorMedium', 'e.g. myFolder/myFile.txt')
			)
		);
		completions.push(
			this.newSimpleCompletionItem(
				...
				localize(
					'activeEditorLong',
					'e.g. /Users/Development/myProject/myFolder/myFile.txt'
				)
			)
		);
		completions.push(
			this.newSimpleCompletionItem(
				'${rootName}',
				range,
				localize('rootName', 'e.g. myProject')
			)
		);
		completions.push(
			this.newSimpleCompletionItem(
				'${rootPath}',
				range,
				localize('rootPath', 'e.g. /Users/Development/myProject')
			)
		);
		completions.push(
			this.newSimpleCompletionItem(
		...
		completions.push(
			this.newSimpleCompletionItem(
				'${dirty}',
				range,
				localize('dirty', 'a dirty indicator if the active editor is dirty')
			)
		);
		completions.push(
			this.newSimpleCompletionItem(
	...
	private provideFilesAssociationsCompletionItems(
		location: Location,
		range: vscode.Range
	): vscode.ProviderResult<vscode.CompletionItem[]> {
		const completions: vscode.CompletionItem[] = [];
		...
		if (location.path.length === 1) {
			completions.push(
				this.newSnippetCompletionItem({
					...
					documentation: localize(
						'assocDescriptionFile',
						'Map all files matching the glob pattern in their filename to the language with the given identifier.'
					),
					snippet: location.isAtPropertyKey
			...
			completions.push(
				this.newSnippetCompletionItem({
					...
					documentation: localize(
						'assocDescriptionPath',
						'Map all files matching the absolute path glob pattern in their path to the language with the given identifier.'
					),
					snippet: location.isAtPropertyKey
	...
	private provideExcludeCompletionItems(
		location: Location,
		range: vscode.Range
	): vscode.ProviderResult<vscode.CompletionItem[]> {
		const completions: vscode.CompletionItem[] = [];
		...
		if (location.path.length === 1) {
			...
			completions.push(
				this.newSnippetCompletionItem({
					...
					snippet: location.isAtPropertyKey
						? '"**/*.${1:source-extension}": { "when": "$(basename).${2:target-extension}" }'
						: '{ "**/*.${1:source-extension}": { "when": "$(basename).${2:target-extension}" } }',
					range
				})
			);
			...
			completions.push(
				this.newSimpleCompletionItem(
					'false',
					range,
					localize('falseDescription', 'Disable the pattern.')
				)
			);
			completions.push(
				this.newSimpleCompletionItem(
					'true',
					range,
					localize('trueDescription', 'Enable the pattern.')
				)
			);
	...
	private provideLanguageCompletionItems(
		location: Location,
		range: vscode.Range,
		formatFunc: (string) => string = l => JSON.stringify(l)
	): vscode.ProviderResult<vscode.CompletionItem[]> {
	...
	private provideLanguageOverridesCompletionItems(
		...
		if (
			...
			return this.provideLanguageCompletionItems(
				location,
				range,
				language => `"[${language}]"`
			);
