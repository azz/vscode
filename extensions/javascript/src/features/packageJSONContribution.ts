...
export class PackageJSONContribution implements IJSONContribution {
	...
	public collectDefaultSuggestions(
		...
		let proposal = new CompletionItem(
			localize('json.package.default', 'Default package.json')
		);
								...
								if (obj && Array.isArray(obj.rows)) {
									...
									for (let i = 0; i < results.length; i++) {
										...
										if (Array.isArray(keys) && keys.length > 0) {
											let name = keys[0];
											let insertText = new SnippetString().appendText(
												JSON.stringify(name)
											);
											if (addValue) {
												insertText
		...
		if (
			...
			if (typeof currentKey === 'string') {
				let queryUrl =
					...
					encodeURIComponent(currentKey).replace('%40', '@');
				return this.xhr({
					url: queryUrl
				}).then(
					success => {
						try {
							let obj = JSON.parse(success.responseText);
							let latest =
								obj && obj['dist-tags'] && obj['dist-tags']['latest'];
							if (latest) {
								let name = JSON.stringify(latest);
								let proposal = new CompletionItem(name);
								proposal.kind = CompletionItemKind.Property;
								proposal.insertText = name;
								proposal.documentation = localize(
	...
	private getInfo(pack: string): Thenable<string[]> {
		let queryUrl =
			'http://registry.npmjs.org/' +
			encodeURIComponent(pack).replace('%40', '@');
		return this.xhr({
			url: queryUrl
		}).then(
			success => {
