...
export function addJSONProviders(xhr: XHRRequest): Disposable {
	let contributions = [
		new PackageJSONContribution(xhr),
		new BowerJSONContribution(xhr)
	];
...
export class JSONHoverProvider implements HoverProvider {
	...
	public provideHover(
		...
		if (node && node.offset <= offset && offset <= node.offset + node.length) {
			let promise = this.jsonContribution.getInfoContribution(
				fileName,
				location
			);
			if (promise) {
...
export class JSONCompletionItemProvider implements CompletionItemProvider {
	...
	public provideCompletionItems(
		...
		if (location.isAtPropertyKey) {
			let addValue =
				!location.previousNode || !location.previousNode.columnOffset;
	...
	private isLast(document: TextDocument, position: Position): boolean {
		...
		if (
			nextToken === SyntaxKind.StringLiteral &&
			scanner.getTokenError() === ScanError.UnexpectedEndOfString
		) {
			nextToken = scanner.scan();
		}
		return (
			nextToken === SyntaxKind.CloseBraceToken || nextToken === SyntaxKind.EOF
		);
	}
}
