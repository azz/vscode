...
suite('commands namespace tests', () => {
	...
	test('api-command: vscode.diff', function() {
		...
		let b = commands
			.executeCommand('vscode.diff', Uri.parse('sc:a'), Uri.parse('sc:b'))
			.then(value => {
				assert.ok(value === void 0);
				registration.dispose();
			});
		...
		let c = commands
			...
			.then(() => assert.ok(false), () => assert.ok(true));
		let d = commands
			.executeCommand('vscode.diff', 1, 2, 3)
			.then(() => assert.ok(false), () => assert.ok(true));
	...
	test('api-command: vscode.open', function() {
		...
		let a = commands
			...
			.then(() => assert.ok(true), () => assert.ok(false));
		let b = commands
			.executeCommand('vscode.open', uri, ViewColumn.Two)
			.then(() => assert.ok(true), () => assert.ok(false));
		let c = commands
			.executeCommand('vscode.open')
			.then(() => assert.ok(false), () => assert.ok(true));
		let d = commands
			.executeCommand('vscode.open', uri, true)
			.then(() => assert.ok(false), () => assert.ok(true));
